...
public class DiplomaEstudianteRestClient implements Serializable {
    ...
    public SgDiplomaEstudiante guardar(SgDiplomaEstudiante diplomaEstudiante) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (diplomaEstudiante == null ) {
            return null;
        }
        if (diplomaEstudiante.getDiePk() == null) {
            ...
            return restClient.invokePost(webTarget, diplomaEstudiante, SgDiplomaEstudiante.class);
        } else {
            WebTarget webTarget = RestClient.getWebTarget(client,ConstantesServiciosRest.SERVICIO_CENTROS_EDUCATIVOS, "v1/diplomaestudiante");
            webTarget = webTarget.path(diplomaEstudiante.getDiePk().toString());
            return restClient.invokePut(webTarget, diplomaEstudiante, SgDiplomaEstudiante.class);
        }
    }
    ...
    public void eliminar(Long diplomaEstudiantePk) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (diplomaEstudiantePk == null ) {
            return;
        }
        WebTarget webTarget = RestClient.getWebTarget(client,ConstantesServiciosRest.SERVICIO_CENTROS_EDUCATIVOS, "v1/diplomaestudiante");
	...
	@Retry(maxRetries = 2, retryOn = {HttpServerUnavailableException.class, HttpServerException.class, TimeoutException.class}, delay = 250)
    public List<RevHistorico> obtenerHistorialPorId(Long diplomaEstudiantePk) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (diplomaEstudiantePk == null ) {
            return new ArrayList<>();
        }
        WebTarget webTarget = RestClient.getWebTarget(client,ConstantesServiciosRest.SERVICIO_CENTROS_EDUCATIVOS, "v1/diplomaestudiante/historial");
        webTarget = webTarget.path(diplomaEstudiantePk.toString());
        RevHistorico[] diplomaEstudiante = restClient.invokeGet(webTarget, RevHistorico[].class);
        return Arrays.asList(diplomaEstudiante);
    }
