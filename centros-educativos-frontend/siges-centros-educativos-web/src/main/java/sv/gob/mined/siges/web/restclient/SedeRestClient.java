...
public class SedeRestClient implements Serializable {
    ...
    private static final Logger LOGGER = Logger.getLogger(SedeRestClient.class.getName());
    ...
    @Retry(maxRetries = 1, retryOn = {HttpServerUnavailableException.class, HttpServerException.class, TimeoutException.class}, delay = 1000)
    public List<SgSedeLite> buscarLite(FiltroSedes filtro) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (filtro == null) {
            return new ArrayList<>();
        }
        WebTarget webTarget = RestClient.getWebTarget(client, ConstantesServiciosRest.SERVICIO_CENTROS_EDUCATIVOS, "v1/sedes/buscar");
        SgSedeLite[] sede = restClient.invokePost(webTarget, filtro, SgSedeLite[].class);
        return new ArrayList<>(Arrays.asList(sede));
    }
    ...
    @Retry(maxRetries = 1, retryOn = {HttpServerUnavailableException.class, HttpServerException.class, TimeoutException.class}, delay = 1000)
    public void activar(Long sedeId) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (sedeId == null) {
            return;
        }
        WebTarget webTarget = RestClient.getWebTarget(client, ConstantesServiciosRest.SERVICIO_CENTROS_EDUCATIVOS, "v1/sedes/activar");
        webTarget = webTarget.path(sedeId.toString());
        restClient.invokePost(webTarget, null, null);
    }
    
    @Retry(maxRetries = 1, retryOn = {HttpServerUnavailableException.class, HttpServerException.class, TimeoutException.class}, delay = 1000)
    public void cerrar(SgSede sede) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (sede == null) {
            return;
        }
        WebTarget webTarget = RestClient.getWebTarget(client, ConstantesServiciosRest.SERVICIO_CENTROS_EDUCATIVOS, "v1/sedes/cerrar");
 
        restClient.invokePost(webTarget, sede, null);
    }
