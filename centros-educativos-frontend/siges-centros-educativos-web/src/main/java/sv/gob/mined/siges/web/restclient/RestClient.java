...
import sv.gob.mined.siges.web.beans.RestClientManager;
...
public class RestClient implements Serializable {
    ...
    public <S, T> T invokePostReturnFile(WebTarget webTarget, S request, Class<T> responseType) throws HttpClientException, HttpServerException, HttpServerUnavailableException, BusinessException {
        ...
        Response response = webTarget.request(new String[]{MediaType.APPLICATION_OCTET_STREAM, MediaType.APPLICATION_JSON})
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + userToken)
                .post(requestEntity);
        return respond(response, responseType);
    }
...
//            LOGGER.log(Level.SEVERE, contextResolver.getDefaultMapper().writeValueAsString(request));
//       } catch (Exception ex){}
        Entity<S> requestEntity = Entity.entity(request, MediaType.APPLICATION_JSON_TYPE);
