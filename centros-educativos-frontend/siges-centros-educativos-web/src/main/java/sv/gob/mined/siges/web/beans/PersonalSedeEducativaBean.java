...
@Named
@ViewScoped
public class PersonalSedeEducativaBean implements Serializable {
    ...
    @Inject
    private EstudioRealizadoBean estudioRealizadoBean;

    @Inject
    private DocenteDocumentoBean docenteDocumentoBean;

    private SgPersonalSedeEducativa entidadEnEdicion;
    private SgDatoContratacion datoContratacionEnEdicion;
    ...
    public PersonalSedeEducativaBean() {
    }
    ...
    public void aceptarDatoContratacionInicial() {
        docenteContratosBean.setearCombosEnEntidadContrato();
        if (docenteContratosBean.validarDatoContratacion()) {
            datoContratacionEnEdicion = docenteContratosBean.getEntidadEnEdicionContratos();
            if (TipoPersonalSedeEducativa.DOC.equals(datoContratacionEnEdicion.getDcoTipo())) {
                continuarExistePersona = Boolean.TRUE;
            }
            agregar();
        }
    }
    ...
    public void guardar() {
        try {
            ...
            if (entidadEnEdicion.getPsePersona().getPerIngresoAlgunaIdentificacion()) {
                entidadEnEdicion.setPseCrearConDatoContratacion(datoContratacionEnEdicion);
    ...
    public String getTituloPagina() {
        if (this.pseId == null) {
            return Etiquetas.getValue("agregarPersonal");
        } else if (this.pseRev != null) {
            return Etiquetas.getValue("historialPersonal") + " (" + entidadEnEdicion.getPseUltModUsuario() + (entidadEnEdicion.getPseUltModFecha() != null ? (" " + this.appBean.getDateTimeFormater().format(entidadEnEdicion.getPseUltModFecha())) : "") + ")";
        } else if (this.soloLectura) {
            return Etiquetas.getValue("verPersonal");
        } else {
            return Etiquetas.getValue("edicionPersonal");
        }
    }
    ...
    public void changeTab(TabChangeEvent event) {
        ...
        } else if (tabActiveId.equals("tab_estudios_realizados")) {
            if (estudioRealizadoBean.getPersonalSede() == null) {
                estudioRealizadoBean.personalSedeEducativa(entidadEnEdicion);
            }
