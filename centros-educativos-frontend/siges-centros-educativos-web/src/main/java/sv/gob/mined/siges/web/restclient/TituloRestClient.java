...
public class TituloRestClient implements Serializable {
    ...
    public SgTitulo guardar(SgTitulo titulo) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (titulo == null ) {
            return null;
        }
        if (titulo.getTitPk() == null) {
            WebTarget webTarget = RestClient.getWebTarget(client,ConstantesServiciosRest.SERVICIO_CENTROS_EDUCATIVOS, "v1/titulo");
            return restClient.invokePost(webTarget, titulo, SgTitulo.class);
        } else {
            WebTarget webTarget = RestClient.getWebTarget(client,ConstantesServiciosRest.SERVICIO_CENTROS_EDUCATIVOS, "v1/titulo");
            webTarget = webTarget.path(titulo.getTitPk().toString());
            return restClient.invokePut(webTarget, titulo, SgTitulo.class);
        }
    }
    ...
    public void eliminar(Long tituloPk) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (tituloPk == null ) {
            return;
        }
        WebTarget webTarget = RestClient.getWebTarget(client,ConstantesServiciosRest.SERVICIO_CENTROS_EDUCATIVOS, "v1/titulo");
	...
	@Retry(maxRetries = 1, retryOn = {HttpServerUnavailableException.class, HttpServerException.class, TimeoutException.class}, delay = 1000)
    public List<RevHistorico> obtenerHistorialPorId(Long tituloPk) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (tituloPk == null ) {
            return new ArrayList<>();
        }
        WebTarget webTarget = RestClient.getWebTarget(client, ConstantesServiciosRest.SERVICIO_CENTROS_EDUCATIVOS, "v1/titulo/historial");
        webTarget = webTarget.path(tituloPk.toString());
