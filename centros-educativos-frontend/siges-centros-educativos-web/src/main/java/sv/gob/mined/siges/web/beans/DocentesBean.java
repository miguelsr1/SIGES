...
public class DocentesBean implements Serializable {
    ...
    private List<EnumModeloContrato> modelos = new ArrayList();
    ...
    public String buscar() {
        try {
            ...
            filtro.setPerTipoEmpleado(TipoPersonalSedeEducativa.DOC);
            filtro.setModeloContrato(modelos);
            filtro.setTieneContratos(filtro.getPersonalActivo());
    ...
    public List<EnumModeloContrato> completeTipoContrato(String query) {
        try {
            List<EnumModeloContrato> contratos = new ArrayList(Arrays.asList(EnumModeloContrato.values()));
            return contratos.stream().filter(x -> x.getText().toLowerCase().contains(query.toLowerCase())).collect(Collectors.toList());
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, LoggingUtils.getMessage(this.sessionBean.getUser(), ex), ex);
            JSFUtils.agregarError(ConstantesComponentesId.ID_MSG_TEMPLATE, Mensajes.obtenerMensaje(Mensajes.ERROR_GENERAL), "");
        }
    ...
    public List<EnumModeloContrato> getModelos() {
        return modelos;
    }

    public void setModelos(List<EnumModeloContrato> modelos) {
