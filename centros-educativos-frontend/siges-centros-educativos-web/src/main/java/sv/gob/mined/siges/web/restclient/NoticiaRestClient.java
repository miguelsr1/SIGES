...
public class NoticiaRestClient implements Serializable {
    ...
    public SgOpcion obtenerEnRevision(Long noticiaPk, Long noticiaRev) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (noticiaPk == null || noticiaRev == null) {
            return null;
        }
        WebTarget webTarget = RestClient.getWebTarget(client, ConstantesServiciosRest.SERVICIO_CENTROS_EDUCATIVOS, "v1/noticias/revision");
        webTarget = webTarget.path(noticiaPk.toString());
        webTarget = webTarget.path(noticiaRev.toString());
