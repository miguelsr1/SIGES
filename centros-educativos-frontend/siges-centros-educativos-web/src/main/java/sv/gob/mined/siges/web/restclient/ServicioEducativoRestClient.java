...
public class ServicioEducativoRestClient implements Serializable {
    ...
    public Boolean copiarServicios(Long sedeHija, Long sedePadre) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (sedeHija == null || sedePadre == null) {
            return null;
        }
        
        WebTarget webTarget = RestClient.getWebTarget(client, ConstantesServiciosRest.SERVICIO_CENTROS_EDUCATIVOS, "v1/servicioseducativos/copiarservicios");
        webTarget = webTarget.path(sedeHija.toString());
        webTarget = webTarget.path(sedePadre.toString());
        return restClient.invokeGet(webTarget, Boolean.class);
    ...
    public Boolean copiarServiciosSelectos(List<SgServicioEducativo> servicios) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (servicios == null ) {
            return Boolean.FALSE;
        }
        
        WebTarget webTarget = RestClient.getWebTarget(client, ConstantesServiciosRest.SERVICIO_CENTROS_EDUCATIVOS, "v1/servicioseducativos/copiarserviciosselectos");
        return restClient.invokePost(webTarget, servicios, Boolean.class);
        
    }
    
    @Timeout(value = 60000L)
    public List<SgServicioEducativo> buscarServicioPadre(Long sedeHija, Long sedePadre) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (sedeHija == null || sedePadre == null) {
             return new ArrayList<>();
        }
        
        WebTarget webTarget = RestClient.getWebTarget(client, ConstantesServiciosRest.SERVICIO_CENTROS_EDUCATIVOS, "v1/servicioseducativos/buscarserviciospadre");
        webTarget = webTarget.path(sedeHija.toString());
        webTarget = webTarget.path(sedePadre.toString());
        SgServicioEducativo[] organizacionesCurricular =  restClient.invokeGet(webTarget, SgServicioEducativo[].class);
        return Arrays.asList(organizacionesCurricular);
