...
public class CentroEducativoPrivadoRestClient implements Serializable {
    ...
    public CentroEducativoPrivadoRestClient() {
    }
    ...
    public SgSede guardar(SgSede sede) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        ...
        if (sede.getSedPk() == null) {
            ...
            return restClient.invokePost(webTarget, sede, SgCentroEducativoPrivado.class);
        } else {
            WebTarget webTarget = RestClient.getWebTarget(client, ConstantesServiciosRest.SERVICIO_CENTROS_EDUCATIVOS, "v1/centroseducativoprivado");
            webTarget = webTarget.path(sede.getSedPk().toString());
            return restClient.invokePut(webTarget, sede, SgCentroEducativoPrivado.class);
        }
    }

    @Retry(maxRetries = 1, retryOn = {HttpServerUnavailableException.class, HttpServerException.class, TimeoutException.class}, delay = 1000)
    public SgCentroEducativoPrivado obtenerPorId(Long sedeId) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (sedeId == null) {
            return null;
        }
        WebTarget webTarget = RestClient.getWebTarget(client, ConstantesServiciosRest.SERVICIO_CENTROS_EDUCATIVOS, "v1/centroseducativoprivado");
        webTarget = webTarget.path(sedeId.toString());
        return restClient.invokeGet(webTarget, SgCentroEducativoPrivado.class);
    }
    ...
    public List<RevHistorico> obtenerHistorialPorId(Long sedeId) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        ...
        WebTarget webTarget = RestClient.getWebTarget(client, ConstantesServiciosRest.SERVICIO_CENTROS_EDUCATIVOS, "v1/centroseducativoprivado/historial");
        webTarget = webTarget.path(sedeId.toString());
