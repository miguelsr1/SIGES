    ...
    public void init() {
        try {
            ...
            if (sessionBean.getOperaciones().contains(ConstantesOperaciones.MENU_PAGOS)) {
                ...
                if (sessionBean.getOperaciones().contains(ConstantesOperaciones.MENU_CUENTAS_BANCARIAS_DDE)) {
            ...
            if (sessionBean.getOperaciones().contains(ConstantesOperaciones.MENU_CONCILIACION_BANCARIA)) {
                item2 = new DefaultMenuItem(Etiquetas.getValue("gestionConciliacionBancaria", locale), null, ConstantesPaginas.CONCILIACIONES_BANCARIAS_DETALLE);
                submenu6.addElement(item2);
            }
