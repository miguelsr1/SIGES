...
public class MovimientoLiquidacionOtroRestClient implements Serializable {
    ...
    public SgMovimientoLiquidacionOtro guardar(SgMovimientoLiquidacionOtro movimientoLiquidacionOtro) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (movimientoLiquidacionOtro == null || userToken == null) {
            return null;
        }
        if (movimientoLiquidacionOtro.getMloPk() == null) {
            ...
            return restClient.invokePost(webTarget, movimientoLiquidacionOtro, SgMovimientoLiquidacionOtro.class);
        } else {
            WebTarget webTarget = RestClient.getWebTarget(client,ConstantesServiciosRest.SERVICIO_FINANZAS, "v1/movimientosliquidacionotros");
            webTarget = webTarget.path(movimientoLiquidacionOtro.getMloPk().toString());
            return restClient.invokePut(webTarget, movimientoLiquidacionOtro, SgMovimientoLiquidacionOtro.class);
        }
    }
    ...
    public void eliminar(Long movimientoLiquidacionOtroPk) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (movimientoLiquidacionOtroPk == null || userToken == null) {
            return;
        }
        WebTarget webTarget = RestClient.getWebTarget(client,ConstantesServiciosRest.SERVICIO_FINANZAS, "v1/movimientosliquidacionotros");
	...
	@Retry(maxRetries = 2, retryOn = {HttpServerUnavailableException.class, HttpServerException.class, TimeoutException.class}, delay = 250)
    public List<SgMovimientoLiquidacionOtro> obtenerHistorialPorId(Long movimientoLiquidacionOtroPk) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (movimientoLiquidacionOtroPk == null || userToken == null) {
            return new ArrayList<>();
        }
        WebTarget webTarget = RestClient.getWebTarget(client,ConstantesServiciosRest.SERVICIO_FINANZAS, "v1/movimientosliquidacionotros/historial");
        webTarget = webTarget.path(movimientoLiquidacionOtroPk.toString());
        SgMovimientoLiquidacionOtro[] movimientosLiquidacionOtros = restClient.invokeGet(webTarget, SgMovimientoLiquidacionOtro[].class);
        return Arrays.asList(movimientosLiquidacionOtros);
    }
