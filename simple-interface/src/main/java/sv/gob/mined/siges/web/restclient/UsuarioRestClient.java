...
@Bulkhead
@CircuitBreaker
@Retry(maxRetries = 1, retryOn = {HttpServerUnavailableException.class, TimeoutException.class}, delay = 500)
@Timeout(value = 4000L)
public class UsuarioRestClient implements Serializable {
    ...
    @Retry(maxRetries = 3, retryOn = {HttpServerUnavailableException.class, HttpServerException.class, TimeoutException.class, Exception.class}, delay = 250)
    public SgUsuario obtenerPorCodigo(String codigo, String userToken) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException, Exception {
        if (codigo == null) {
            return null;
        }
