...
import sv.gob.mined.siges.negocio.servicios.GradoAfectacionBean;
 ...
 */
@RequestScoped
@Path("/v1/gradoafectacion")
@Tag(name = "Grado Afectacion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_GRADO_AFECTACION,
    ConstantesOperaciones.ACTUALIZAR_GRADO_AFECTACION,
    ConstantesOperaciones.ELIMINAR_GRADO_AFECTACION})
public class GradoAfectacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(GradoAfectacionRecurso.class.getName());

    @Inject
    private GradoAfectacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgGradoAfectacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgGradoAfectacion.class)))
        ,
    ...
    @Path("/{gradoAfectacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("gradoAfectacionId") Long id) {
        try {
            SgGradoAfectacion gaf = servicio.obtenerPorId(id);
            if (gaf == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(gaf).build(); 
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgGradoAfectacion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgGradoAfectacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_GRADO_AFECTACION})
    public Response crear(SgGradoAfectacion gaf) {
        try {
            gaf.setGafPk(null);
            gaf.setGafVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(gaf)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_GRADO_AFECTACION})
    public Response actualizar(@PathParam("gradoAfectacionId") Long id, SgGradoAfectacion gaf) {
        try {
            if (servicio.objetoExistePorId(id)) {
                gaf.setGafPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(gaf)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_GRADO_AFECTACION})
    public Response eliminarPorId(@PathParam("gradoAfectacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{gradoAfectacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("gradoAfectacionId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgGradoAfectacion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgGradoAfectacion.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
