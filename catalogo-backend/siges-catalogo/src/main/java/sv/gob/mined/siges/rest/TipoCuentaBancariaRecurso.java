...
import sv.gob.mined.siges.negocio.servicios.TipoCuentaBancariaBean;
 ...
 */
@RequestScoped
@Path("/v1/tiposcuentasbancarias")
@Tag(name = "Tipos de Cuentas Bancarias API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPOS_CUENTAS_BANCARIAS,
    ConstantesOperaciones.ACTUALIZAR_TIPOS_CUENTAS_BANCARIAS,
    ConstantesOperaciones.ELIMINAR_TIPOS_CUENTAS_BANCARIAS})
public class TipoCuentaBancariaRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoCuentaBancariaRecurso.class.getName());

    @Inject
    private TipoCuentaBancariaBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTipoCuentaBancaria
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoCuentaBancaria.class)))
        ,
    ...
    @Path("/{tipoCuentaBancariaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoCuentaBancariaId") Long id) {
        try {
            SgTipoCuentaBancaria tcb = servicio.obtenerPorId(id);
            if (tcb == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tcb).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTipoCuentaBancaria
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgTipoCuentaBancaria.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPOS_CUENTAS_BANCARIAS})
    public Response crear(SgTipoCuentaBancaria tcb) {
        try {
            tcb.setTcbPk(null);
            tcb.setTcbVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tcb)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPOS_CUENTAS_BANCARIAS})
    public Response actualizar(@PathParam("tipoCuentaBancariaId") Long id, SgTipoCuentaBancaria tcb) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tcb.setTcbPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tcb)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPOS_CUENTAS_BANCARIAS})
    public Response eliminarPorId(@PathParam("tipoCuentaBancariaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tipoCuentaBancariaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoCuentaBancariaId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoCuentaBancaria.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgTipoCuentaBancaria.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
