...
import sv.gob.mined.siges.negocio.servicios.InfTipoImagenBean;
...
public class InfTipoImagenRecurso {

    private static final Logger LOGGER = Logger.getLogger(InfTipoImagenRecurso.class.getName());

    @Inject
    private InfTipoImagenBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgInfTipoImagen
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgInfTipoImagen.class)))
        ,
    ...
    @Path("/{infTipoImagenId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("infTipoImagenId") Long id) {
        try {
            SgInfTipoImagen tii = servicio.obtenerPorId(id);
            if (tii == null) {
     ...
     * @return Response Lista de SgInfTipoImagen
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgInfTipoImagen.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPO_IMAGEN})
    public Response crear(SgInfTipoImagen tii) {
        try {
            tii.setTiiPk(null);
            tii.setTiiVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tii)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPO_IMAGEN})
    public Response actualizar(@PathParam("infTipoImagenId") Long id, SgInfTipoImagen tii) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tii.setTiiPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tii)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPO_IMAGEN})
    public Response eliminarPorId(@PathParam("infTipoImagenId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{infTipoImagenId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("infTipoImagenId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgInfTipoImagen.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgInfTipoImagen.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
