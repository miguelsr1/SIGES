...
import sv.gob.mined.siges.negocio.servicios.InfObraExteriorBean;
 ...
 */
@RequestScoped
@Path("/v1/obraexterior")
@Tag(name = "Obras exteriores API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_OBRA_EXTERIOR,
    ConstantesOperaciones.ACTUALIZAR_OBRA_EXTERIOR,
    ConstantesOperaciones.ELIMINAR_OBRA_EXTERIOR})
public class InfObraExteriorRecurso {

    private static final Logger LOGGER = Logger.getLogger(InfObraExteriorRecurso.class.getName());

    @Inject
    private InfObraExteriorBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgInfObraExterior
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgInfObraExterior.class)))
        ,
    ...
    @Path("/{infObraExteriorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("infObraExteriorId") Long id) {
        try {
            SgInfObraExterior oex = servicio.obtenerPorId(id);
            if (oex == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(oex).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgInfObraExterior
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgInfObraExterior.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_OBRA_EXTERIOR})
    public Response crear(SgInfObraExterior oex) {
        try {
            oex.setOexPk(null);
            oex.setOexVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(oex)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_OBRA_EXTERIOR})
    public Response actualizar(@PathParam("infObraExteriorId") Long id, SgInfObraExterior oex) {
        try {
            if (servicio.objetoExistePorId(id)) {
                oex.setOexPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(oex)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_OBRA_EXTERIOR})
    public Response eliminarPorId(@PathParam("infObraExteriorId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{infObraExteriorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("infObraExteriorId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgInfObraExterior.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgInfObraExterior.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
