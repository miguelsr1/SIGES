...
import sv.gob.mined.siges.negocio.servicios.ZonaBean;
...
@RequestScoped
@Path("/v1/zonas")
@Tag(name = "Zona API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ZONA,
    ConstantesOperaciones.ACTUALIZAR_ZONA,
    ConstantesOperaciones.ELIMINAR_ZONA,
...
public class ZonaRecurso {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgZona.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("zonaId") Long id) {
        try {
            SgZona zon = servicio.obtenerPorId(id);
            if (zon == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(zon).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgZona 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgZona.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ZONA})
    public Response crear(SgZona zon) {
        try {
            zon.setZonPk(null);
            zon.setZonVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(zon)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ZONA})
    public Response actualizar(@PathParam("zonaId") Long id, SgZona zon) {
        try {
            if (servicio.objetoExistePorId(id)) {
                zon.setZonPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(zon)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ZONA})
    public Response eliminarPorId(@PathParam("zonaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{zonaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("zonaId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgZona.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
