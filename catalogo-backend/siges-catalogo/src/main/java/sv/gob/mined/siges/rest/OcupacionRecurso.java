...
import sv.gob.mined.siges.negocio.servicios.OcupacionBean;
...
@RequestScoped
@Path("/v1/ocupaciones")
@Tag(name = "Ocupaciones API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_OCUPACIONES,
    ConstantesOperaciones.ACTUALIZAR_OCUPACIONES,
    ConstantesOperaciones.ELIMINAR_OCUPACIONES,
    ConstantesOperaciones.AUTENTICADO})
public class OcupacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(OcupacionRecurso.class.getName());

    @Inject
    private OcupacionBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgOcupacion.class)))
        ,
    ...
    @Path("/{ocupacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("ocupacionId") Long id) {
        try {
            SgOcupacion ocu = servicio.obtenerPorId(id);
            if (ocu == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(ocu).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgOcupacion 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgOcupacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_OCUPACIONES})
    public Response crear(SgOcupacion ocu) {
        try {
            ocu.setOcuPk(null);
            ocu.setOcuVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ocu)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_OCUPACIONES})
    public Response actualizar(@PathParam("ocupacionId") Long id, SgOcupacion ocu) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ocu.setOcuPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ocu)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_OCUPACIONES})
    public Response eliminarPorId(@PathParam("ocupacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{ocupacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("ocupacionId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgOcupacion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
