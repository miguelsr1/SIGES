...
import sv.gob.mined.siges.negocio.servicios.VelocidadInternetBean;
 ...
 */
@RequestScoped
@Path("/v1/velocidadesinternet")
@Tag(name = "Velocidades de Internet API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_VELOCIDAD_INTERNET,
    ConstantesOperaciones.ACTUALIZAR_VELOCIDAD_INTERNET,
    ConstantesOperaciones.ELIMINAR_VELOCIDAD_INTERNET})
public class VelocidadInternetRecurso {

    private static final Logger LOGGER = Logger.getLogger(VelocidadInternetRecurso.class.getName());

    @Inject
    private VelocidadInternetBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgVelocidadInternet
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgVelocidadInternet.class)))
        ,
    ...
    @Path("/{velocidadInternetId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("velocidadInternetId") Long id) {
        try {
            SgVelocidadInternet vin = servicio.obtenerPorId(id);
            if (vin == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(vin).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgVelocidadInternet
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgVelocidadInternet.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_VELOCIDAD_INTERNET})
    public Response crear(SgVelocidadInternet vin) {
        try {
            vin.setVinPk(null);
            vin.setVinVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(vin)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_VELOCIDAD_INTERNET})
    public Response actualizar(@PathParam("velocidadInternetId") Long id, SgVelocidadInternet vin) {
        try {
            if (servicio.objetoExistePorId(id)) {
                vin.setVinPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(vin)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_VELOCIDAD_INTERNET})
    public Response eliminarPorId(@PathParam("velocidadInternetId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{velocidadInternetId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("velocidadInternetId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgVelocidadInternet.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgVelocidadInternet.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
