...
import sv.gob.mined.siges.negocio.servicios.GradoRiesgoBean;
 ...
 */
@RequestScoped
@Path("/v1/gradosriesgo")
@Tag(name = "Grados de Riesgo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_GRADO_RIESGO,
    ConstantesOperaciones.ACTUALIZAR_GRADO_RIESGO,
    ConstantesOperaciones.ELIMINAR_GRADO_RIESGO})
public class GradoRiesgoRecurso {

    private static final Logger LOGGER = Logger.getLogger(GradoRiesgoRecurso.class.getName());

    @Inject
    private GradoRiesgoBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgGradoRiesgo.class)))
        ,
    ...
    @Path("/{gradoRiesgoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("gradoRiesgoId") Long id) {
        try {
            SgGradoRiesgo gre = servicio.obtenerPorId(id);
            if (gre == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(gre).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgGradoRiesgo
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgGradoRiesgo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_GRADO_RIESGO})
    public Response crear(SgGradoRiesgo gre) {
        try {
            gre.setGrePk(null);
            gre.setGreVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(gre)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_GRADO_RIESGO})
    public Response actualizar(@PathParam("gradoRiesgoId") Long id, SgGradoRiesgo gre) {
        try {
            if (servicio.objetoExistePorId(id)) {
                gre.setGrePk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(gre)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_GRADO_RIESGO})
    public Response eliminarPorId(@PathParam("gradoRiesgoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{gradoRiesgoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("gradoRiesgoId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgGradoRiesgo.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgGradoRiesgo.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
