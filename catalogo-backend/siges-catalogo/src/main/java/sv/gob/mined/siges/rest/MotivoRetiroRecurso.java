...
import sv.gob.mined.siges.negocio.servicios.MotivoRetiroBean;
...
@RequestScoped
@Path("/v1/motivosretiro")
@Tag(name = "MotivoRetiro API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MOTIVO_RETIRO,
    ConstantesOperaciones.ACTUALIZAR_MOTIVO_RETIRO,
    ConstantesOperaciones.ELIMINAR_MOTIVO_RETIRO,
    ConstantesOperaciones.AUTENTICADO})
public class MotivoRetiroRecurso {

    private static final Logger LOGGER = Logger.getLogger(MotivoRetiroRecurso.class.getName());

    @Inject
    private MotivoRetiroBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMotivoRetiro
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMotivoRetiro.class)))
        ,
    ...
    @Path("/{motivoRetiroId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("motivoRetiroId") Long id) {
        try {
            SgMotivoRetiro mre = servicio.obtenerPorId(id);
            if (mre == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(mre).build();
            }
        } catch (Exception ex) {
     ...
     *
     * @param filtro FiltroMotivoRetiro
     * @return Response Lista de SgMotivoRetiro 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgMotivoRetiro.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroMotivoRetiro filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroMotivoRetiro filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MOTIVO_RETIRO})
    public Response crear(SgMotivoRetiro mre) {
        try {
            mre.setMrePk(null);
            mre.setMreVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(mre)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MOTIVO_RETIRO})
    public Response actualizar(@PathParam("motivoRetiroId") Long id, SgMotivoRetiro mre) {
        try {
            if (servicio.objetoExistePorId(id)) {
                mre.setMrePk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(mre)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MOTIVO_RETIRO})
    public Response eliminarPorId(@PathParam("motivoRetiroId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{motivoRetiroId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("motivoRetiroId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMotivoRetiro.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
