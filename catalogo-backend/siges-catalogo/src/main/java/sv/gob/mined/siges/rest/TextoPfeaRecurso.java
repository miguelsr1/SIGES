...
import sv.gob.mined.siges.negocio.servicios.TextoPfeaBean;
...
public class TextoPfeaRecurso {

    private static final Logger LOGGER = Logger.getLogger(TextoPfeaRecurso.class.getName());

    @Inject
    private TextoPfeaBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTextoPfea.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("configuracionId") Long id) {
        try {
            SgTextoPfea con = servicio.obtenerPorId(id);
            if (con == null) {
     ...
     * @return Response Lista de SgTextoPfea
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgTextoPfea.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    public Response crear(SgTextoPfea con) {
        try {
            con.setTexPk(null);
            con.setTexVersion(0);
    ...
    public Response actualizar(@PathParam("configuracionId") Long id, SgTextoPfea con) {
        try {
    ...
    public Response obtenerHistorialPorId(@PathParam("configuracionId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTextoPfea.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
