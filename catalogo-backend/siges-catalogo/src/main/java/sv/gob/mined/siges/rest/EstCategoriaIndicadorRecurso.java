...
import sv.gob.mined.siges.negocio.servicios.EstCategoriaIndicadorBean;
 ...
 */
@RequestScoped
@Path("/v1/categoriasindicador")
@Tag(name = "Categorias indicador API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CATEGORIA_INDICADOR,
    ConstantesOperaciones.ACTUALIZAR_CATEGORIA_INDICADOR,
    ConstantesOperaciones.ELIMINAR_CATEGORIA_INDICADOR})
public class EstCategoriaIndicadorRecurso {

    private static final Logger LOGGER = Logger.getLogger(EstCategoriaIndicadorRecurso.class.getName());

    @Inject
    private EstCategoriaIndicadorBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgEstCategoriaIndicador
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEstCategoriaIndicador.class)))
        ,
    ...
    @Path("/{estCategoriaIndicadorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("estCategoriaIndicadorId") Long id) {
        try {
            SgEstCategoriaIndicador cin = servicio.obtenerPorId(id);
            if (cin == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(cin).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgEstCategoriaIndicador
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgEstCategoriaIndicador.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CATEGORIA_INDICADOR})
    public Response crear(SgEstCategoriaIndicador cin) {
        try {
            cin.setCinPk(null);
            cin.setCinVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(cin)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CATEGORIA_INDICADOR})
    public Response actualizar(@PathParam("estCategoriaIndicadorId") Long id, SgEstCategoriaIndicador cin) {
        try {
            if (servicio.objetoExistePorId(id)) {
                cin.setCinPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(cin)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CATEGORIA_INDICADOR})
    public Response eliminarPorId(@PathParam("estCategoriaIndicadorId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{estCategoriaIndicadorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("estCategoriaIndicadorId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgEstCategoriaIndicador.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgEstCategoriaIndicador.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
