...
import sv.gob.mined.siges.negocio.servicios.CargoBean;
...
import sv.gob.mined.siges.persistencia.entidades.SgCargoRoles;

@RequestScoped
@Path("/v1/cargo")
@Tag(name = "Cargo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CARGO,
    ConstantesOperaciones.ACTUALIZAR_CARGO,
    ConstantesOperaciones.ELIMINAR_CARGO,
    ConstantesOperaciones.AUTENTICADO})
public class CargoRecurso {

    private static final Logger LOGGER = Logger.getLogger(CargoRecurso.class.getName());

    @Inject
    private CargoBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCargo.class)))
        ,
    ...
    @Path("/{cargoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("cargoId") Long id) {
        try {
            SgCargo sex = servicio.obtenerPorId(id);
            if (sex == null) {
     ...
     * @return Response Lista de SgCargo
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgCargo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CARGO})
    public Response crear(SgCargo car) {
        try {
            car.setCarPk(null);
            car.setCarVersion(0);
            if (car.getCarCargoRoles() != null) {
                for (SgCargoRoles op : car.getCarCargoRoles()) {
                    op.setCarCargo(car);
                }
            }
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(car)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CARGO})
    public Response actualizar(@PathParam("cargoId") Long id, SgCargo sex) {
        try {
            if (servicio.objetoExistePorId(id)) {
                sex.setCarPk(id);
                if (sex.getCarCargoRoles() != null) {
                    for (SgCargoRoles op : sex.getCarCargoRoles()) {
                        op.setCarCargo(sex);
                    }
                }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CARGO})
    public Response eliminarPorId(@PathParam("cargoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{cargoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("cargoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgCargo.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
