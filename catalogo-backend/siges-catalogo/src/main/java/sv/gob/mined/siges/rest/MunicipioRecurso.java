...
import sv.gob.mined.siges.negocio.servicios.MunicipioBean;
...
@RequestScoped
@Path("/v1/municipios")
@Tag(name = "Municipios API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MUNICIPIO,
    ConstantesOperaciones.ACTUALIZAR_MUNICIPIO,
    ConstantesOperaciones.ELIMINAR_MUNICIPIO,
    ConstantesOperaciones.AUTENTICADO})
public class MunicipioRecurso {

    private static final Logger LOGGER = Logger.getLogger(MunicipioRecurso.class.getName());

    @Inject
    private MunicipioBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMunicipio.class)))
        ,
    ...
    @Path("/{municipioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("municipioId") Long id) {
        try {
            SgMunicipio mun = servicio.obtenerPorId(id);
            if (mun == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(mun).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMunicipio 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgMunicipio.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroMunicipio filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroMunicipio filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MUNICIPIO})
    public Response crear(SgMunicipio mun) {
        try {
            mun.setMunPk(null);
            mun.setMunVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(mun)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MUNICIPIO})
    public Response actualizar(@PathParam("municipioId") Long id, SgMunicipio mun) {
        try {
            if (servicio.objetoExistePorId(id)) {
                mun.setMunPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(mun)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MUNICIPIO})
    public Response eliminarPorId(@PathParam("municipioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{municipioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("municipioId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMunicipio.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
