...
import sv.gob.mined.siges.negocio.servicios.RecursoEducativoBean;
...
@RequestScoped
@Path("/v1/recursoseducativos")
@Tag(name = "RecursoEducativo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_RECURSO_EDUCATIVO,
    ConstantesOperaciones.ACTUALIZAR_RECURSO_EDUCATIVO,
    ConstantesOperaciones.ELIMINAR_RECURSO_EDUCATIVO})
public class RecursoEducativoRecurso {

    private static final Logger LOGGER = Logger.getLogger(RecursoEducativoBean.class.getName());

    @Inject
    private RecursoEducativoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRecursoEducativo
     */
    @GET
    ...
    @Path("/{recursoEducativoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("recursoEducativoId") Long id) {
        try {
            SgRecursoEducativo red = servicio.obtenerPorId(id);
            if (red == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(red).build();
            }
        } catch (Exception ex) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_RECURSO_EDUCATIVO})
    public Response crear(SgRecursoEducativo red) {
        try {
            red.setRedPk(null);
            red.setRedVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(red)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_RECURSO_EDUCATIVO})
    public Response actualizar(@PathParam("recursoEducativoId") Long id, SgRecursoEducativo red) {
        try {
            if (servicio.objetoExistePorId(id)) {
                red.setRedPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(red)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_RECURSO_EDUCATIVO})
    public Response eliminarPorId(@PathParam("recursoEducativoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{recursoEducativoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("recursoEducativoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgRecursoEducativo.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
