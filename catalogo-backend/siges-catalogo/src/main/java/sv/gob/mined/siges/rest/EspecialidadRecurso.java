...
import sv.gob.mined.siges.negocio.servicios.EspecialidadBean;
...
@Tag(name = "Especialidades API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ESPECIALIDADES,
    ConstantesOperaciones.ACTUALIZAR_ESPECIALIDADES,
    ConstantesOperaciones.ELIMINAR_ESPECIALIDADES,
    ConstantesOperaciones.AUTENTICADO})
public class EspecialidadRecurso {

    private static final Logger LOGGER = Logger.getLogger(EspecialidadRecurso.class.getName());
    
    @Inject
    private EspecialidadBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgEspecialidad
     */    
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEspecialidad.class)))
        ,
    ...
    @Path("/{especialidadId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("especialidadId") Long id) {
        try {
            SgEspecialidad esp = servicio.obtenerPorId(id);
            if (esp == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(esp).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgEspecialidad 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgEspecialidad.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroEspecialidad filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroEspecialidad filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ESPECIALIDADES})
    public Response crear(SgEspecialidad esp) {
        try {
            esp.setEspPk(null);
            esp.setEspVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(esp)).build();
...
/**
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ESPECIALIDADES})
    public Response actualizar(@PathParam("especialidadId") Long id, SgEspecialidad esp) {
        try {
            if (servicio.objetoExistePorId(id)) {
                esp.setEspPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(esp)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ESPECIALIDADES})
    public Response eliminarPorId(@PathParam("especialidadId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
...
/**
    ...
    @Path("/historial/{especialidadId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("especialidadId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgEspecialidad.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
