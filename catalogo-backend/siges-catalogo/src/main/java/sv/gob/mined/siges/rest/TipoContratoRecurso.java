...
import sv.gob.mined.siges.negocio.servicios.TipoContratoBean;
...
@RequestScoped
@Path("/v1/tipocontrato")
@Tag(name = "TipoContrato API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPO_CONTRATO,
    ConstantesOperaciones.ACTUALIZAR_TIPO_CONTRATO,
    ConstantesOperaciones.ELIMINAR_TIPO_CONTRATO,
    ConstantesOperaciones.AUTENTICADO})
public class TipoContratoRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoContratoRecurso.class.getName());

    @Inject
    private TipoContratoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTipoContrato
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoContrato.class)))
        ,
    ...
    @Path("/{tipoContratoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoContratoId") Long id) {
        try {
            SgTipoContrato sex = servicio.obtenerPorId(id);
            if (sex == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(sex).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTipoContrato 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgTipoContrato.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPO_CONTRATO})
    public Response crear(SgTipoContrato sex) {
        try {
            sex.setTcoPk(null);
            sex.setTcoVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(sex)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPO_CONTRATO})
    public Response actualizar(@PathParam("tipoContratoId") Long id, SgTipoContrato sex) {
        try {
            if (servicio.objetoExistePorId(id)) {
                sex.setTcoPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(sex)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPO_CONTRATO})
    public Response eliminarPorId(@PathParam("tipoContratoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tipoContratoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoContratoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoContrato.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
