...
import sv.gob.mined.siges.negocio.servicios.ProcesoNocturnoBean;
 ...
 */
@RequestScoped
@Path("/v1/procesonocturno")
@Tag(name = "Procesos nocturnos API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_PROCESO_NOCTURNO,
    ConstantesOperaciones.ACTUALIZAR_PROCESO_NOCTURNO,
    ConstantesOperaciones.ELIMINAR_PROCESO_NOCTURNO})
public class ProcesoNocturnoRecurso {

    private static final Logger LOGGER = Logger.getLogger(ProcesoNocturnoRecurso.class.getName());

    @Inject
    private ProcesoNocturnoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgProcesoNocturno
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgProcesoNocturno.class)))
        ,
    ...
    @Path("/{procesoNocturnoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("procesoNocturnoId") Long id) {
        try {
            SgProcesoNocturno prn = servicio.obtenerPorId(id);
            if (prn == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(prn).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgProcesoNocturno
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgProcesoNocturno.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_PROCESO_NOCTURNO})
    public Response crear(SgProcesoNocturno prn) {
        try {
            prn.setPrnPk(null);
            prn.setPrnVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(prn)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PROCESO_NOCTURNO})
    public Response actualizar(@PathParam("procesoNocturnoId") Long id, SgProcesoNocturno prn) {
        try {
            if (servicio.objetoExistePorId(id)) {
                prn.setPrnPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(prn)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PROCESO_NOCTURNO})
    public Response eliminarPorId(@PathParam("procesoNocturnoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{procesoNocturnoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("procesoNocturnoId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgProcesoNocturno.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgProcesoNocturno.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
