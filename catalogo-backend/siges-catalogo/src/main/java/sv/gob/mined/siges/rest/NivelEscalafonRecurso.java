...
import sv.gob.mined.siges.negocio.servicios.NivelEscalafonBean;
...
@RequestScoped
@Path("/v1/nivelescalafon")
@Tag(name = "NivelEscalafon API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_AYUDA,
    ConstantesOperaciones.ACTUALIZAR_AYUDA,
    ConstantesOperaciones.ELIMINAR_AYUDA,
    ConstantesOperaciones.AUTENTICADO})
public class NivelEscalafonRecurso {

    private static final Logger LOGGER = Logger.getLogger(NivelEscalafonRecurso.class.getName());

    @Inject
    private NivelEscalafonBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgNivelEscalafon
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgNivelEscalafon.class)))
        ,
    ...
    @Path("/{nivelEscalafonId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("nivelEscalafonId") Long id) {
        try {
            SgNivelEscalafon ayu = servicio.obtenerPorId(id);
            if (ayu == null) {
     ...
     * @return Response Lista de SgNivelEscalafon 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgNivelEscalafon.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_AYUDA})
    public Response crear(SgNivelEscalafon ayu) {
        try {
            ayu.setNesPk(null);
            ayu.setNesVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_AYUDA})
    public Response actualizar(@PathParam("nivelEscalafonId") Long id, SgNivelEscalafon ayu) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_AYUDA})
    public Response eliminarPorId(@PathParam("nivelEscalafonId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{nivelEscalafonId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("nivelEscalafonId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgNivelEscalafon.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
