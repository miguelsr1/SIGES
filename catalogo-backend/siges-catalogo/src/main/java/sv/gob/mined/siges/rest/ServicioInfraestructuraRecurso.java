...
import sv.gob.mined.siges.negocio.servicios.ServicioInfraestructuraBean;
import sv.gob.mined.siges.persistencia.entidades.SgServicioInfraestructura;
 ...
 */
@RequestScoped
@Path("/v1/servicioinfraestructura")
@Tag(name = "Servicios Infraestructura API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SERVICIO_INFRAESTRUCTURA,
    ConstantesOperaciones.ACTUALIZAR_SERVICIO_INFRAESTRUCTURA,
    ConstantesOperaciones.ELIMINAR_SERVICIO_INFRAESTRUCTURA})
public class ServicioInfraestructuraRecurso {

    private static final Logger LOGGER = Logger.getLogger(ServicioInfraestructuraRecurso.class.getName());

    @Inject
    private ServicioInfraestructuraBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgServicioInfraestructura
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgServicioInfraestructura.class)))
        ,
    ...
    @Path("/{servicioInfraestructuraId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("servicioInfraestructuraId") Long id) {
        try {
            SgServicioInfraestructura sin = servicio.obtenerPorId(id);
            if (sin == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(sin).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgServicioInfraestructura
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgServicioInfraestructura.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
     ...
     *
     * @param sin SgServicioInfraestructura
     * @return Response  SgServicioInfraestructura
     */
    @POST
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SERVICIO_INFRAESTRUCTURA})
    public Response crear(SgServicioInfraestructura sin) {
        try {
            sin.setSinPk(null);
            sin.setSinVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(sin)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SERVICIO_INFRAESTRUCTURA})
    public Response actualizar(@PathParam("servicioInfraestructuraId") Long id, SgServicioInfraestructura sin) {
        try {
            if (servicio.objetoExistePorId(id)) {
                sin.setSinPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(sin)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_SERVICIO_INFRAESTRUCTURA})
    public Response eliminarPorId(@PathParam("servicioInfraestructuraId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{servicioInfraestructuraId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("servicioInfraestructuraId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgServicioInfraestructura.class, id))).build();
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgServicioInfraestructura.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
