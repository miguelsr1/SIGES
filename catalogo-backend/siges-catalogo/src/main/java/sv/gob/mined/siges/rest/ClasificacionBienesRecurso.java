...
import sv.gob.mined.siges.negocio.servicios.ClasificacionBienesBean;
...
@RequestScoped
@Path("/v1/clasificacionbienes")
@Tag(name = "Clasificación de Bienes API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CLASIFICACION_BIENES,
    ConstantesOperaciones.ACTUALIZAR_CLASIFICACION_BIENES,
    ConstantesOperaciones.ELIMINAR_CLASIFICACION_BIENES,
    ConstantesOperaciones.AUTENTICADO})
public class ClasificacionBienesRecurso {
    private static final Logger LOGGER = Logger.getLogger(ClasificacionBienesRecurso.class.getName());

    @Inject
    private ClasificacionBienesBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAfClasificacionBienes
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAfClasificacionBienes.class)))
        ,
    ...
    @Path("/{clasBienesId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("clasBienesId") Long id) {
        try {
            SgAfClasificacionBienes etn = servicio.obtenerPorId(id);
            if (etn == null) {
     ...
     * @return Response Lista de SgAfClasificacionBienes 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgAfClasificacionBienes.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CLASIFICACION_BIENES})
    public Response crear(SgAfClasificacionBienes etn) {
        try {
            etn.setCbiPk(null);
            etn.setCbiVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CLASIFICACION_BIENES})
    public Response actualizar(@PathParam("clasBienesId") Long id, SgAfClasificacionBienes etn) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CLASIFICACION_BIENES})
    public Response eliminarPorId(@PathParam("clasBienesId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{clasBienesId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("clasBienesId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAfClasificacionBienes.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
