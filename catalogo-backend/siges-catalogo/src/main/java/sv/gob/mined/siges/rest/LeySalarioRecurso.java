...
import sv.gob.mined.siges.negocio.servicios.LeySalarioBean;
...
@RequestScoped
@Path("/v1/leysalario")
@Tag(name = "Instituciones API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_LEY_SALARIO,
    ConstantesOperaciones.ACTUALIZAR_LEY_SALARIO,
    ConstantesOperaciones.ELIMINAR_LEY_SALARIO,
    ConstantesOperaciones.AUTENTICADO})
public class LeySalarioRecurso {

    private static final Logger LOGGER = Logger.getLogger(LeySalarioRecurso.class.getName());
    
    @Inject
    private LeySalarioBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgLeySalario.class)))
        ,
    ...
    @Path("/{leySalarioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("leySalarioId") Long id) {
        try {
            SgLeySalario ins = servicio.obtenerPorId(id);
            if (ins == null) {
     ...
     * @return Response Lista de SgLeySalario 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgLeySalario.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_LEY_SALARIO})
    public Response crear(SgLeySalario ins) {
        try {
            ins.setLsaPk(null);
            ins.setLsaVersion(0);
...
/**
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_LEY_SALARIO})
    public Response actualizar(@PathParam("leySalarioId") Long id, SgLeySalario ins) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_LEY_SALARIO})
    public Response eliminarPorId(@PathParam("leySalarioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
...
/**
    ...
    @Path("/historial/{leySalarioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("leySalarioId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgLeySalario.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
