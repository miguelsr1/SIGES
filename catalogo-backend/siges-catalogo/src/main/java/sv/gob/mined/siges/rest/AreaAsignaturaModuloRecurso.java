...
import sv.gob.mined.siges.negocio.servicios.AreaAsignaturaModuloBean;
 ...
 */
@RequestScoped
@Path("/v1/areasasignaturamodulo")
@Tag(name = "Areas Asignatura Modulo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_AREAS_ASIGNATURA_MODULO,
    ConstantesOperaciones.ACTUALIZAR_AREAS_ASIGNATURA_MODULO,
    ConstantesOperaciones.ELIMINAR_AREAS_ASIGNATURA_MODULO})
public class AreaAsignaturaModuloRecurso {

    private static final Logger LOGGER = Logger.getLogger(AreaAsignaturaModuloRecurso.class.getName());

    @Inject
    private AreaAsignaturaModuloBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAreaAsignaturaModulo
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAreaAsignaturaModulo.class)))
        ,
    ...
    @Path("/{areaAsignaturaModuloId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("areaAsignaturaModuloId") Long id) {
        try {
            SgAreaAsignaturaModulo aream = servicio.obtenerPorId(id);
            if (aream == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(aream).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgAreaAsignaturaModulo
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAreaAsignaturaModulo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_AREAS_ASIGNATURA_MODULO})
    public Response crear(SgAreaAsignaturaModulo aream) {
        try {
            aream.setAamPk(null);
            aream.setAamVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(aream)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_AREAS_ASIGNATURA_MODULO})
    public Response actualizar(@PathParam("areaAsignaturaModuloId") Long id, SgAreaAsignaturaModulo aream) {
        try {
            if (servicio.objetoExistePorId(id)) {
                aream.setAamPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(aream)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_AREAS_ASIGNATURA_MODULO})
    public Response eliminarPorId(@PathParam("areaAsignaturaModuloId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{areaAsignaturaModuloId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("areaAsignaturaModuloId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAreaAsignaturaModulo.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgAreaAsignaturaModulo.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
