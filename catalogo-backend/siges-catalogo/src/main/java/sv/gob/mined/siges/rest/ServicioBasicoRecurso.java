...
import sv.gob.mined.siges.negocio.servicios.ServicioBasicoBean;
 ...
 */
@RequestScoped
@Path("/v1/serviciosbasicos")
@Tag(name = "Servicio Básico API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SERVICIOS_BASICOS,
    ConstantesOperaciones.ACTUALIZAR_SERVICIOS_BASICOS,
    ConstantesOperaciones.ELIMINAR_SERVICIOS_BASICOS})
public class ServicioBasicoRecurso {

    private static final Logger LOGGER = Logger.getLogger(ServicioBasicoRecurso.class.getName());

    @Inject
    private ServicioBasicoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgServicioBasico
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgServicioBasico.class)))
        ,
    ...
    @Path("/{servicioBasicoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("servicioBasicoId") Long id) {
        try {
            SgServicioBasico sba = servicio.obtenerPorId(id);
            if (sba == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(sba).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgServicioBasico
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgServicioBasico.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroServicioBasico filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroServicioBasico filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SERVICIOS_BASICOS})
    public Response crear(SgServicioBasico sba) {
        try {
            sba.setSbaPk(null);
            sba.setSbaVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(sba)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SERVICIOS_BASICOS})
    public Response actualizar(@PathParam("servicioBasicoId") Long id, SgServicioBasico sba) {
        try {
            if (servicio.objetoExistePorId(id)) {
                sba.setSbaPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(sba)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_SERVICIOS_BASICOS})
    public Response eliminarPorId(@PathParam("servicioBasicoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{servicioBasicoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("servicioBasicoId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgServicioBasico.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgServicioBasico.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
