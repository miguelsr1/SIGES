...
import sv.gob.mined.siges.negocio.servicios.MotivoInasistenciaBean;
...
@RequestScoped
@Path("/v1/motivosinasistencia")
@Tag(name = "MotivoInasistencia API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MOTIVO_INASISTENCIA,
    ConstantesOperaciones.ACTUALIZAR_MOTIVO_INASISTENCIA,
    ConstantesOperaciones.ELIMINAR_MOTIVO_INASISTENCIA,
    ConstantesOperaciones.AUTENTICADO})
public class MotivoInasistenciaRecurso {

    private static final Logger LOGGER = Logger.getLogger(MotivoInasistenciaRecurso.class.getName());

    @Inject
    private MotivoInasistenciaBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMotivoInasistencia
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMotivoInasistencia.class)))
        ,
    ...
    @Path("/{motivoInasistenciaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("motivoInasistenciaId") Long id) {
        try {
            SgMotivoInasistencia min = servicio.obtenerPorId(id);
            if (min == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(min).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMotivoInasistencia 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgMotivoInasistencia.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MOTIVO_INASISTENCIA})
    public Response crear(SgMotivoInasistencia min) {
        try {
            min.setMinPk(null);
            min.setMinVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(min)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MOTIVO_INASISTENCIA})
    public Response actualizar(@PathParam("motivoInasistenciaId") Long id, SgMotivoInasistencia min) {
        try {
            if (servicio.objetoExistePorId(id)) {
                min.setMinPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(min)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MOTIVO_INASISTENCIA})
    public Response eliminarPorId(@PathParam("motivoInasistenciaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{motivoInasistenciaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("motivoInasistenciaId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMotivoInasistencia.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
