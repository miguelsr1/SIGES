...
import sv.gob.mined.siges.negocio.servicios.MotivosSeleccionPLazaBean;
 ...
 */
@RequestScoped
@Path("/v1/motivosseleccionplaza")
@Tag(name = "Motivos de selección de plaza API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MOTIVOS_SELECCION_PLAZA,
    ConstantesOperaciones.ACTUALIZAR_MOTIVOS_SELECCION_PLAZA,
    ConstantesOperaciones.ELIMINAR_MOTIVOS_SELECCION_PLAZA,
	ConstantesOperaciones.BUSCAR_MOTIVOS_SELECCION_PLAZA})
public class MotivosSeleccionPLazaRecurso {

    private static final Logger LOGGER = Logger.getLogger(MotivosSeleccionPLazaRecurso.class.getName());

    @Inject
    private MotivosSeleccionPLazaBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMotivosSeleccionPLaza
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMotivosSeleccionPLaza.class)))
        ,
    ...
    @Path("/{motivosSeleccionPLazaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("motivosSeleccionPLazaId") Long id) {
        try {
            SgMotivosSeleccionPLaza msp = servicio.obtenerPorId(id);
            if (msp == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(msp).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMotivosSeleccionPLaza
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgMotivosSeleccionPLaza.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MOTIVOS_SELECCION_PLAZA})
    public Response crear(SgMotivosSeleccionPLaza msp) {
        try {
            msp.setMspPk(null);
            msp.setMspVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(msp)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MOTIVOS_SELECCION_PLAZA})
    public Response actualizar(@PathParam("motivosSeleccionPLazaId") Long id, SgMotivosSeleccionPLaza msp) {
        try {
            if (servicio.objetoExistePorId(id)) {
                msp.setMspPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(msp)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MOTIVOS_SELECCION_PLAZA})
    public Response eliminarPorId(@PathParam("motivosSeleccionPLazaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{motivosSeleccionPLazaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("motivosSeleccionPLazaId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMotivosSeleccionPLaza.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgMotivosSeleccionPLaza.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
