...
import sv.gob.mined.siges.negocio.servicios.TipoTelefonoBean;
...
@RequestScoped
@Path("/v1/tipostelefono")
@Tag(name = "TipoTelefono API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPO_TELEFONO,
    ConstantesOperaciones.ACTUALIZAR_TIPO_TELEFONO,
    ConstantesOperaciones.ELIMINAR_TIPO_TELEFONO,
    ConstantesOperaciones.AUTENTICADO})
public class TipoTelefonoRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoTelefonoBean.class.getName());

    @Inject
    private TipoTelefonoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTipoTelefono
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoTelefono.class)))
        ,
    ...
    @Path("/{tipoTelefonoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoTelefonoId") Long id) {
        try {
            SgTipoTelefono tto = servicio.obtenerPorId(id);
            if (tto == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tto).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTipoTelefono 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgTipoTelefono.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPO_TELEFONO})
    public Response crear(SgTipoTelefono tto) {
        try {
            tto.setTtoPk(null);
            tto.setTtoVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tto)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPO_TELEFONO})
    public Response actualizar(@PathParam("tipoTelefonoId") Long id, SgTipoTelefono tto) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tto.setTtoPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tto)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPO_TELEFONO})
    public Response eliminarPorId(@PathParam("tipoTelefonoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tipoTelefonoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoTelefonoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoTelefono.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
