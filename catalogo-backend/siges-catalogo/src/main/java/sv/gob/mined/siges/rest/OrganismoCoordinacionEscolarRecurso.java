...
import sv.gob.mined.siges.negocio.servicios.OrganismoCoordinacionEscolarBean;
 ...
 */
@RequestScoped
@Path("/v1/organismoscoordinacionescolar")
@Tag(name = "Organismos de Coordinación Escolar API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ORGANISMOS_COORDINACION_ESCOLAR,
    ConstantesOperaciones.ACTUALIZAR_ORGANISMOS_COORDINACION_ESCOLAR,
    ConstantesOperaciones.ELIMINAR_ORGANISMOS_COORDINACION_ESCOLAR})
public class OrganismoCoordinacionEscolarRecurso {

    private static final Logger LOGGER = Logger.getLogger(OrganismoCoordinacionEscolarRecurso.class.getName());

    @Inject
    private OrganismoCoordinacionEscolarBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgOrganismoCoordinacionEscolar
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgOrganismoCoordinacionEscolar.class)))
        ,
    ...
    @Path("/{organismoCoordinacionEscolarId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("organismoCoordinacionEscolarId") Long id) {
        try {
            SgOrganismoCoordinacionEscolar oce = servicio.obtenerPorId(id);
            if (oce == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(oce).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgOrganismoCoordinacionEscolar
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgOrganismoCoordinacionEscolar.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ORGANISMOS_COORDINACION_ESCOLAR})
    public Response crear(SgOrganismoCoordinacionEscolar oce) {
        try {
            oce.setOcePk(null);
            oce.setOceVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(oce)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ORGANISMOS_COORDINACION_ESCOLAR})
    public Response actualizar(@PathParam("organismoCoordinacionEscolarId") Long id, SgOrganismoCoordinacionEscolar oce) {
        try {
            if (servicio.objetoExistePorId(id)) {
                oce.setOcePk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(oce)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ORGANISMOS_COORDINACION_ESCOLAR})
    public Response eliminarPorId(@PathParam("organismoCoordinacionEscolarId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{organismoCoordinacionEscolarId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("organismoCoordinacionEscolarId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgOrganismoCoordinacionEscolar.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgOrganismoCoordinacionEscolar.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
