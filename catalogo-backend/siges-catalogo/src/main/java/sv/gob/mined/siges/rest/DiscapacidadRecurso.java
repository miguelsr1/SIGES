...
import sv.gob.mined.siges.negocio.servicios.DiscapacidadBean;
...
@RequestScoped
@Path("/v1/discapacidades")
@Tag(name = "Discapacidad API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_DISCAPACIADAD,
    ConstantesOperaciones.ACTUALIZAR_DISCAPACIADAD,
    ConstantesOperaciones.ELIMINAR_DISCAPACIADAD,
    ConstantesOperaciones.AUTENTICADO})
public class DiscapacidadRecurso {

    private static final Logger LOGGER = Logger.getLogger(DiscapacidadRecurso.class.getName());

    @Inject
    private DiscapacidadBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgDiscapacidad
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgDiscapacidad.class)))
        ,
    ...
    @Path("/{discapacidadId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("discapacidadId") Long id) {
        try {
            SgDiscapacidad dis = servicio.obtenerPorId(id);
            if (dis == null) {
     ...
     * @return Response Lista de SgDiscapacidad 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgDiscapacidad.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DISCAPACIADAD})
    public Response crear(SgDiscapacidad dis) {
        try {
            dis.setDisPk(null);
            dis.setDisVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_DISCAPACIADAD})
    public Response actualizar(@PathParam("discapacidadId") Long id, SgDiscapacidad dis) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_DISCAPACIADAD})
    public Response eliminarPorId(@PathParam("discapacidadId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{discapacidadId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("discapacidadId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgDiscapacidad.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
