...
import sv.gob.mined.siges.negocio.servicios.InfItemObraExteriorBean;
 ...
 */
@RequestScoped
@Path("/v1/itemobraexterior")
@Tag(name = "Item obras exteriores API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ITEM_OBRA_EXTERIOR,
    ConstantesOperaciones.ACTUALIZAR_ITEM_OBRA_EXTERIOR,
    ConstantesOperaciones.ELIMINAR_ITEM_OBRA_EXTERIOR})
public class InfItemObraExteriorRecurso {

    private static final Logger LOGGER = Logger.getLogger(InfItemObraExteriorRecurso.class.getName());

    @Inject
    private InfItemObraExteriorBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgInfItemObraExterior
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgInfItemObraExterior.class)))
        ,
    ...
    @Path("/{infItemObraExteriorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("infItemObraExteriorId") Long id) {
        try {
            SgInfItemObraExterior ioe = servicio.obtenerPorId(id);
            if (ioe == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(ioe).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgInfItemObraExterior
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgInfItemObraExterior.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ITEM_OBRA_EXTERIOR})
    public Response crear(SgInfItemObraExterior ioe) {
        try {
            ioe.setIoePk(null);
            ioe.setIoeVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ioe)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ITEM_OBRA_EXTERIOR})
    public Response actualizar(@PathParam("infItemObraExteriorId") Long id, SgInfItemObraExterior ioe) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ioe.setIoePk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ioe)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ITEM_OBRA_EXTERIOR})
    public Response eliminarPorId(@PathParam("infItemObraExteriorId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{infItemObraExteriorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("infItemObraExteriorId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgInfItemObraExterior.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgInfItemObraExterior.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
