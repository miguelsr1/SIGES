...
import sv.gob.mined.siges.negocio.servicios.SiTipoDocumentoBean;
 ...
 */
@RequestScoped
@Path("/v1/sitipodocumento")
@Tag(name = "Tipo documento API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPO_IMAGEN,
    ConstantesOperaciones.ACTUALIZAR_TIPO_IMAGEN,
    ConstantesOperaciones.ELIMINAR_TIPO_IMAGEN,
    ConstantesOperaciones.CREAR_TIPO_DOCUMENTO_SI,
    ConstantesOperaciones.ACTUALIZAR_TIPO_DOCUMENTO_SI,
    ConstantesOperaciones.ELIMINAR_TIPO_DOCUMENTO_SI})
public class SiTipoDocumentoRecurso {

    private static final Logger LOGGER = Logger.getLogger(SiTipoDocumentoRecurso.class.getName());

    @Inject
    private SiTipoDocumentoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgSiTipoDocumento
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgSiTipoDocumento.class)))
        ,
    ...
    @Path("/{siTipoDocumentoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("siTipoDocumentoId") Long id) {
        try {
            SgSiTipoDocumento tii = servicio.obtenerPorId(id);
            if (tii == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tii).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgSiTipoDocumento
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgSiTipoDocumento.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPO_DOCUMENTO_SI})
    public Response crear(SgSiTipoDocumento tid) {
        try {
            tid.setTidPk(null);
            tid.setTidVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tid)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPO_DOCUMENTO_SI})
    public Response actualizar(@PathParam("siTipoDocumentoId") Long id, SgSiTipoDocumento tid) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tid.setTidPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tid)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPO_DOCUMENTO_SI})
    public Response eliminarPorId(@PathParam("siTipoDocumentoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{siTipoDocumentoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("siTipoDocumentoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgSiTipoDocumento.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgSiTipoDocumento.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
