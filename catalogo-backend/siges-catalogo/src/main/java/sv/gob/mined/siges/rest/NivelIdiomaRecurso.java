...
import sv.gob.mined.siges.negocio.servicios.NivelIdiomaBean;
...
@RequestScoped
@Path("/v1/nivelesidiomas")
@Tag(name = "NivelIdioma API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_NIVELES_IDIOMAS,
    ConstantesOperaciones.ACTUALIZAR_NIVELES_IDIOMAS,
    ConstantesOperaciones.ELIMINAR_NIVELES_IDIOMAS,
    ConstantesOperaciones.AUTENTICADO})
public class NivelIdiomaRecurso {

    private static final Logger LOGGER = Logger.getLogger(NivelIdiomaRecurso.class.getName());

    @Inject
    private NivelIdiomaBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgNivelIdioma.class)))
        ,
    ...
    @Path("/{nivelIdiomaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("nivelIdiomaId") Long id) {
        try {
            SgNivelIdioma nid = servicio.obtenerPorId(id);
            if (nid == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(nid).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgNivelIdioma 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgNivelIdioma.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_NIVELES_IDIOMAS})
    public Response crear(SgNivelIdioma nid) {
        try {
            nid.setNidPk(null);
            nid.setNidVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(nid)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_NIVELES_IDIOMAS})
    public Response actualizar(@PathParam("nivelIdiomaId") Long id, SgNivelIdioma nid) {
        try {
            if (servicio.objetoExistePorId(id)) {
                nid.setNidPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(nid)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_NIVELES_IDIOMAS})
    public Response eliminarPorId(@PathParam("nivelIdiomaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{nivelIdiomaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("nivelIdiomaId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgNivelIdioma.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
