...
import sv.gob.mined.siges.negocio.servicios.TipoRequerimientoInfraBean;
 ...
 */
@RequestScoped
@Path("/v1/tiposrequerimientoinfra")
@Tag(name = "Tipos de requerimientos de infraestructura API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPOS_REQUERIMIENTOS_INFRA,
    ConstantesOperaciones.ACTUALIZAR_TIPOS_REQUERIMIENTOS_INFRA,
    ConstantesOperaciones.ELIMINAR_TIPOS_REQUERIMIENTOS_INFRA})
public class TipoRequerimientoInfraRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoRequerimientoInfraRecurso.class.getName());

    @Inject
    private TipoRequerimientoInfraBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTipoRequerimientoInfra
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoRequerimientoInfra.class)))
        ,
    ...
    @Path("/{tipoRequerimientoInfraId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoRequerimientoInfraId") Long id) {
        try {
            SgTipoRequerimientoInfra tri = servicio.obtenerPorId(id);
            if (tri == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tri).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTipoRequerimientoInfra
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgTipoRequerimientoInfra.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPOS_REQUERIMIENTOS_INFRA})
    public Response crear(SgTipoRequerimientoInfra tri) {
        try {
            tri.setTriPk(null);
            tri.setTriVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tri)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPOS_REQUERIMIENTOS_INFRA})
    public Response actualizar(@PathParam("tipoRequerimientoInfraId") Long id, SgTipoRequerimientoInfra tri) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tri.setTriPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tri)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPOS_REQUERIMIENTOS_INFRA})
    public Response eliminarPorId(@PathParam("tipoRequerimientoInfraId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tipoRequerimientoInfraId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoRequerimientoInfraId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoRequerimientoInfra.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgTipoRequerimientoInfra.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
