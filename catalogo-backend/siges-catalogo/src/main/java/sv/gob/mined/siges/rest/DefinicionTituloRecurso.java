...
import sv.gob.mined.siges.negocio.servicios.DefinicionTituloBean;
 ...
 */
@RequestScoped
@Path("/v1/definiciontitulo")
@Tag(name = "Definiciones Titulo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_DEFINICION_TITULO,
    ConstantesOperaciones.ACTUALIZAR_DEFINICION_TITULO,
    ConstantesOperaciones.ELIMINAR_DEFINICION_TITULO})
public class DefinicionTituloRecurso {

    private static final Logger LOGGER = Logger.getLogger(DefinicionTituloRecurso.class.getName());

    @Inject
    private DefinicionTituloBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgDefinicionTitulo
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgDefinicionTitulo.class)))
        ,
    ...
    @Path("/{definicionTituloId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("definicionTituloId") Long id) {
        try {
            SgDefinicionTitulo dti = servicio.obtenerPorId(id);
            if (dti == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(dti).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgDefinicionTitulo
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgDefinicionTitulo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroDefinicionTitulo filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroDefinicionTitulo filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DEFINICION_TITULO})
    public Response crear(SgDefinicionTitulo dti) {
        try {
            dti.setDtiPk(null);
            dti.setDtiVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(dti)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_DEFINICION_TITULO})
    public Response actualizar(@PathParam("definicionTituloId") Long id, SgDefinicionTitulo dti) {
        try {
            if (servicio.objetoExistePorId(id)) {
                dti.setDtiPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(dti)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_DEFINICION_TITULO})
    public Response eliminarPorId(@PathParam("definicionTituloId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{definicionTituloId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("definicionTituloId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgDefinicionTitulo.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgDefinicionTitulo.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
