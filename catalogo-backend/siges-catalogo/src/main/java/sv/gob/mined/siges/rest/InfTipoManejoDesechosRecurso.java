...
import sv.gob.mined.siges.negocio.servicios.InfTipoManejoDesechosBean;
 ...
 */
@RequestScoped
@Path("/v1/tipomanejodesechos")
@Tag(name = "Tipo Manejo Desechos API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPO_MANEJO_DESECHOS,
    ConstantesOperaciones.ACTUALIZAR_TIPO_MANEJO_DESECHOS,
    ConstantesOperaciones.ELIMINAR_TIPO_MANEJO_DESECHOS})
public class InfTipoManejoDesechosRecurso {

    private static final Logger LOGGER = Logger.getLogger(InfTipoManejoDesechosRecurso.class.getName());

    @Inject
    private InfTipoManejoDesechosBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgInfTipoManejoDesechos
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgInfTipoManejoDesechos.class)))
        ,
    ...
    @Path("/{infTipoManejoDesechosId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("infTipoManejoDesechosId") Long id) {
        try {
            SgInfTipoManejoDesechos tmd = servicio.obtenerPorId(id);
            if (tmd == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tmd).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgInfTipoManejoDesechos
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgInfTipoManejoDesechos.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPO_MANEJO_DESECHOS})
    public Response crear(SgInfTipoManejoDesechos tmd) {
        try {
            tmd.setTmdPk(null);
            tmd.setTmdVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tmd)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPO_MANEJO_DESECHOS})
    public Response actualizar(@PathParam("infTipoManejoDesechosId") Long id, SgInfTipoManejoDesechos tmd) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tmd.setTmdPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tmd)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPO_MANEJO_DESECHOS})
    public Response eliminarPorId(@PathParam("infTipoManejoDesechosId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{infTipoManejoDesechosId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("infTipoManejoDesechosId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgInfTipoManejoDesechos.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgInfTipoManejoDesechos.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
