...
import sv.gob.mined.siges.negocio.servicios.TipoIdentificacionBean;
...
@RequestScoped
@Path("/v1/tiposidentificacion")
@Tag(name = "TipoIdentificacion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPO_IDENTIFICACION,
    ConstantesOperaciones.ACTUALIZAR_TIPO_IDENTIFICACION,
    ConstantesOperaciones.ELIMINAR_TIPO_IDENTIFICACION,
    ConstantesOperaciones.AUTENTICADO})
public class TipoIdentificacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoIdentificacionBean.class.getName());

    @Inject
    private TipoIdentificacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTipoIdentificacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoIdentificacion.class)))
        ,
    ...
    @Path("/{tipoIdentificacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoIdentificacionId") Long id) {
        try {
            SgTipoIdentificacion tin = servicio.obtenerPorId(id);
            if (tin == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tin).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTipoIdentificacion 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgTipoIdentificacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPO_IDENTIFICACION})
    public Response crear(SgTipoIdentificacion tin) {
        try {
            tin.setTinPk(null);
            tin.setTinVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tin)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPO_IDENTIFICACION})
    public Response actualizar(@PathParam("tipoIdentificacionId") Long id, SgTipoIdentificacion tin) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tin.setTinPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tin)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPO_IDENTIFICACION})
    public Response eliminarPorId(@PathParam("tipoIdentificacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tipoIdentificacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoIdentificacionId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoIdentificacion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
