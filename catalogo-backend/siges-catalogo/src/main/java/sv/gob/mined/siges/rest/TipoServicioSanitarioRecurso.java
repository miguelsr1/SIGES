...
import sv.gob.mined.siges.negocio.servicios.TipoServicioSanitarioBean;
...
@RequestScoped
@Path("/v1/tiposserviciosanitario")
@Tag(name = "TipoServicioSanitario API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPOS_SERVICIO_SANITARIO,
    ConstantesOperaciones.ACTUALIZAR_TIPOS_SERVICIO_SANITARIO,
    ConstantesOperaciones.ELIMINAR_TIPOS_SERVICIO_SANITARIO})
public class TipoServicioSanitarioRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoServicioSanitarioRecurso.class.getName());
    
    @Inject
    private TipoServicioSanitarioBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTipoServicioSanitario
     */    
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoServicioSanitario.class)))
        ,
    ...
    @Path("/{tipoServicioSanitarioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoServicioSanitarioId") Long id) {
        try {
            SgTipoServicioSanitario ins = servicio.obtenerPorId(id);
            if (ins == null) {
     ...
     * @return Response Lista de SgTipoServicioSanitario 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgTipoServicioSanitario.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroTipoServicioSanitario filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroTipoServicioSanitario filtro) {
        try {
     ...
     *
     * @param ins SgTipoServicioSanitario
     * @return Response SgTipoServicioSanitario
     */
    @POST
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPOS_SERVICIO_SANITARIO})
    public Response crear(SgTipoServicioSanitario ins) {
        try {
            ins.setTssPk(null);
            ins.setTssVersion(0);
...
/**
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPOS_SERVICIO_SANITARIO})
    public Response actualizar(@PathParam("tipoServicioSanitarioId") Long id, SgTipoServicioSanitario ins) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPOS_SERVICIO_SANITARIO})
    public Response eliminarPorId(@PathParam("tipoServicioSanitarioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
...
/**
    ...
    @Path("/historial/{tipoServicioSanitarioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoServicioSanitarioId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoServicioSanitario.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
