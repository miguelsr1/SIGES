...
import sv.gob.mined.siges.negocio.servicios.FormaAdquisicionBean;
...
@RequestScoped
@Path("/v1/formaadquisicion")
@Tag(name = "Forma Adquisición API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_FORMA_ADQUISICION,
    ConstantesOperaciones.ACTUALIZAR_FORMA_ADQUISICION,
    ConstantesOperaciones.ELIMINAR_FORMA_ADQUISICION,
    ConstantesOperaciones.AUTENTICADO})
public class FormaAdquisicionRecurso {
    private static final Logger LOGGER = Logger.getLogger(FormaAdquisicionRecurso.class.getName());

    @Inject
    private FormaAdquisicionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAfFormaAdquisicion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAfFormaAdquisicion.class)))
        ,
    ...
    @Path("/{formaAdqId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("formaAdqId") Long id) {
        try {
            SgAfFormaAdquisicion etn = servicio.obtenerPorId(id);
            if (etn == null) {
     ...
     * @return Response Lista de SgAfFormaAdquisicion 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgAfFormaAdquisicion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_FORMA_ADQUISICION})
    public Response crear(SgAfFormaAdquisicion etn) {
        try {
            etn.setFadPk(null);
            etn.setFadVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_FORMA_ADQUISICION})
    public Response actualizar(@PathParam("formaAdqId") Long id, SgAfFormaAdquisicion etn) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_FORMA_ADQUISICION})
    public Response eliminarPorId(@PathParam("formaAdqId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{formaAdqId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("formaAdqId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAfFormaAdquisicion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
