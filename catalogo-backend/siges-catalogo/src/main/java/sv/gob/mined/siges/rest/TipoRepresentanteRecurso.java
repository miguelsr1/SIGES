...
import sv.gob.mined.siges.negocio.servicios.TipoRepresentanteBean;
 ...
 */
@RequestScoped
@Path("/v1/tiporepresentante")
@Tag(name = "Tipos Representante API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPO_REPRESENTANTE,
    ConstantesOperaciones.ACTUALIZAR_TIPO_REPRESENTANTE,
    ConstantesOperaciones.ELIMINAR_TIPO_REPRESENTANTE})
public class TipoRepresentanteRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoRepresentanteRecurso.class.getName());

    @Inject
    private TipoRepresentanteBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTipoRepresentante
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoRepresentante.class)))
        ,
    ...
    @Path("/{tipoRepresentanteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoRepresentanteId") Long id) {
        try {
            SgTipoRepresentante tre = servicio.obtenerPorId(id);
            if (tre == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tre).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTipoRepresentante
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgTipoRepresentante.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroTipoRepresentante filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroTipoRepresentante filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPO_REPRESENTANTE})
    public Response crear(SgTipoRepresentante tre) {
        try {
            tre.setTrePk(null);
            tre.setTreVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tre)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPO_REPRESENTANTE})
    public Response actualizar(@PathParam("tipoRepresentanteId") Long id, SgTipoRepresentante tre) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tre.setTrePk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tre)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPO_REPRESENTANTE})
    public Response eliminarPorId(@PathParam("tipoRepresentanteId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tipoRepresentanteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoRepresentanteId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoRepresentante.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgTipoRepresentante.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
