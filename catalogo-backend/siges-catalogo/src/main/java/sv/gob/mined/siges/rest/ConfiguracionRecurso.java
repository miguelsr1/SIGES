 ...
 */
@RequestScoped
@Path("/v1/configuraciones")
@Tag(name = "Configuraciones API V1")
...
public class ConfiguracionRecurso {
    ...
    private static final Logger LOGGER = Logger.getLogger(ConfiguracionRecurso.class.getName());

    @Inject
    private ConfiguracionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgConfiguracion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgConfiguracion.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("configuracionId") Long id) {
        try {
            SgConfiguracion con = servicio.obtenerPorId(id);
            if (con == null) {
     ...
     * @return Response Lista de SgConfiguracion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgConfiguracion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    public Response crear(SgConfiguracion con) {
        try {
            con.setConPk(null);
            con.setConVersion(0);
    ...
    public Response actualizar(@PathParam("configuracionId") Long id, SgConfiguracion con) {
        try {
    ...
    public Response obtenerHistorialPorId(@PathParam("configuracionId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgConfiguracion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
