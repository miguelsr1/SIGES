...
import sv.gob.mined.siges.negocio.servicios.MotivoFallecimientoBean;
 ...
 */
@RequestScoped
@Path("/v1/motivofallecimiento")
@Tag(name = "Motivos de fallecimientos API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MOTIVOS_FALLECIMIENTO,
    ConstantesOperaciones.ACTUALIZAR_MOTIVOS_FALLECIMIENTO,
    ConstantesOperaciones.ELIMINAR_MOTIVOS_FALLECIMIENTO})
public class MotivoFallecimientoRecurso {

    private static final Logger LOGGER = Logger.getLogger(MotivoFallecimientoRecurso.class.getName());

    @Inject
    private MotivoFallecimientoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMotivoFallecimiento
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMotivoFallecimiento.class)))
        ,
    ...
    @Path("/{motivoFallecimientoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("motivoFallecimientoId") Long id) {
        try {
            SgMotivoFallecimiento mfa = servicio.obtenerPorId(id);
            if (mfa == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(mfa).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMotivoFallecimiento
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgMotivoFallecimiento.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MOTIVOS_FALLECIMIENTO})
    public Response crear(SgMotivoFallecimiento mfa) {
        try {
            mfa.setMfaPk(null);
            mfa.setMfaVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(mfa)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MOTIVOS_FALLECIMIENTO})
    public Response actualizar(@PathParam("motivoFallecimientoId") Long id, SgMotivoFallecimiento mfa) {
        try {
            if (servicio.objetoExistePorId(id)) {
                mfa.setMfaPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(mfa)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MOTIVOS_FALLECIMIENTO})
    public Response eliminarPorId(@PathParam("motivoFallecimientoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{motivoFallecimientoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("motivoFallecimientoId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMotivoFallecimiento.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgMotivoFallecimiento.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
