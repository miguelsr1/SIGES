...
import sv.gob.mined.siges.negocio.servicios.CargaArchivoBean;
 ...
 */
@RequestScoped
@Path("/v1/cargasarchivos")
@Tag(name = "Cargas de Archivos API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CARGAS_ARCHIVO,
    ConstantesOperaciones.ACTUALIZAR_CARGAS_ARCHIVO,
    ConstantesOperaciones.ELIMINAR_CARGAS_ARCHIVO})
public class CargaArchivoRecurso {

    private static final Logger LOGGER = Logger.getLogger(CargaArchivoRecurso.class.getName());

    @Inject
    private CargaArchivoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgCargaArchivo
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCargaArchivo.class)))
        ,
    ...
    @Path("/{cargaArchivoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("cargaArchivoId") Long id) {
        try {
            SgCargaArchivo cga = servicio.obtenerPorId(id);
            if (cga == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(cga).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgCargaArchivo
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgCargaArchivo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CARGAS_ARCHIVO})
    public Response crear(SgCargaArchivo cga) {
        try {
            cga.setCgaPk(null);
            cga.setCgaVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(cga)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CARGAS_ARCHIVO})
    public Response actualizar(@PathParam("cargaArchivoId") Long id, SgCargaArchivo cga) {
        try {
            if (servicio.objetoExistePorId(id)) {
                cga.setCgaPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(cga)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CARGAS_ARCHIVO})
    public Response eliminarPorId(@PathParam("cargaArchivoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{cargaArchivoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("cargaArchivoId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgCargaArchivo.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCargaArchivo.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
