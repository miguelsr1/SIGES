...
import sv.gob.mined.siges.negocio.servicios.CategoriaCursoBean;
 ...
 */
@RequestScoped
@Path("/v1/categoriascursos")
@Tag(name = "Categorías de curso API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CATEGORIAS_CURSO,
    ConstantesOperaciones.ACTUALIZAR_CATEGORIAS_CURSO,
    ConstantesOperaciones.ELIMINAR_CATEGORIAS_CURSO})
public class CategoriaCursoRecurso {

    private static final Logger LOGGER = Logger.getLogger(CategoriaCursoRecurso.class.getName());

    @Inject
    private CategoriaCursoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgCategoriaCurso
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCategoriaCurso.class)))
        ,
    ...
    @Path("/{categoriaCursoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("categoriaCursoId") Long id) {
        try {
            SgCategoriaCurso ccu = servicio.obtenerPorId(id);
            if (ccu == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(ccu).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgCategoriaCurso
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgCategoriaCurso.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CATEGORIAS_CURSO})
    public Response crear(SgCategoriaCurso ccu) {
        try {
            ccu.setCcuPk(null);
            ccu.setCcuVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ccu)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CATEGORIAS_CURSO})
    public Response actualizar(@PathParam("categoriaCursoId") Long id, SgCategoriaCurso ccu) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ccu.setCcuPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ccu)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CATEGORIAS_CURSO})
    public Response eliminarPorId(@PathParam("categoriaCursoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{categoriaCursoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("categoriaCursoId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgCategoriaCurso.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCategoriaCurso.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
