...
import sv.gob.mined.siges.negocio.servicios.FuenteFinanciamientoBean;
...
@RequestScoped
@Path("/v1/fuentefinanciamiento")
@Tag(name = "FuenteFinanciamiento API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_FUENTE_FINANCIAMIENTO,
    ConstantesOperaciones.ACTUALIZAR_FUENTE_FINANCIAMIENTO,
    ConstantesOperaciones.ELIMINAR_FUENTE_FINANCIAMIENTO,
    ConstantesOperaciones.AUTENTICADO})
public class FuenteFinanciamientoRecurso {

    private static final Logger LOGGER = Logger.getLogger(FuenteFinanciamientoRecurso.class.getName());
    
    @Inject
    private FuenteFinanciamientoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgFuenteFinanciamiento
     */    
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgFuenteFinanciamiento.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("fuenteFinanciamientoId") Long id) {
        try {
            SgFuenteFinanciamiento ins = servicio.obtenerPorId(id);
            if (ins == null) {
     ...
     * @return Response Lista de SgFuenteFinanciamiento 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgFuenteFinanciamiento.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
     ...
     *
     * @param ins SgFuenteFinanciamiento
     * @return Response SgFuenteFinanciamiento
     */
    @POST
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_FUENTE_FINANCIAMIENTO})
    public Response crear(SgFuenteFinanciamiento ins) {
        try {
            ins.setFfiPk(null);
            ins.setFfiVersion(0);
...
/**
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_FUENTE_FINANCIAMIENTO})
    public Response actualizar(@PathParam("fuenteFinanciamientoId") Long id, SgFuenteFinanciamiento ins) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_FUENTE_FINANCIAMIENTO})
...
/**
    ...
    public Response obtenerHistorialPorId(@PathParam("fuenteFinanciamientoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgFuenteFinanciamiento.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
