...
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_UNIDAD_ADMINISTRATIVA,
    ConstantesOperaciones.ACTUALIZAR_UNIDAD_ADMINISTRATIVA,
    ConstantesOperaciones.ELIMINAR_UNIDAD_ADMINISTRATIVA
})
public class UnidadesAdministrativasRecurso {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_UNIDAD_ADMINISTRATIVA})
    public Response crear(SgAfUnidadesAdministrativas entity) {
        try {
            entity.setUadPk(null);
            entity.setUadVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_UNIDAD_ADMINISTRATIVA})
    public Response actualizar(@PathParam("unidadId") Long id, SgAfUnidadesAdministrativas entity) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response validar(SgAfUnidadesAdministrativas entity) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(UnidadesAdministrativasValidacionNegocio.validar(entity)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_UNIDAD_ADMINISTRATIVA})
     ...
     *
     * @param id Long
     * @return Response Lista de ode_pk
     */
    @GET
    ...
    public Response obtenerHistorialPorId(@PathParam("unidadId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAfUnidadesAdministrativas.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
