...
import sv.gob.mined.siges.negocio.servicios.AreaIndicadorBean;
 ...
 */
@RequestScoped
@Path("/v1/areasindicadores")
@Tag(name = "Areas Indicador API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_AREAS_INDICADOR,
    ConstantesOperaciones.ACTUALIZAR_AREAS_INDICADOR,
    ConstantesOperaciones.ELIMINAR_AREAS_INDICADOR,
    ConstantesOperaciones.AUTENTICADO})
public class AreaIndicadorRecurso {

    private static final Logger LOGGER = Logger.getLogger(AreaIndicadorRecurso.class.getName());

    @Inject
    private AreaIndicadorBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAreaIndicador
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAreaIndicador.class)))
        ,
    ...
    @Path("/{areaIndicadorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("areaIndicadorId") Long id) {
        try {
            SgAreaIndicador arein = servicio.obtenerPorId(id);
            if (arein == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(arein).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgAreaIndicador
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAreaIndicador.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_AREAS_INDICADOR})
    public Response crear(SgAreaIndicador arein) {
        try {
            arein.setAriPk(null);
            arein.setAriVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(arein)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_AREAS_INDICADOR})
    public Response actualizar(@PathParam("areaIndicadorId") Long id, SgAreaIndicador arein) {
        try {
            if (servicio.objetoExistePorId(id)) {
                arein.setAriPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(arein)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_AREAS_INDICADOR})
    public Response eliminarPorId(@PathParam("areaIndicadorId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{areaIndicadorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("areaIndicadorId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAreaIndicador.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgAreaIndicador.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
