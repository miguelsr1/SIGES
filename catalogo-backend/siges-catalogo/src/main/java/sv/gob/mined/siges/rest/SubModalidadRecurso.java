...
import sv.gob.mined.siges.negocio.servicios.SubModalidadBean;
...
@RequestScoped
@Path("/v1/submodalidades")
@Tag(name = "SubModalidad API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SUB_MODALIDADES,
    ConstantesOperaciones.ACTUALIZAR_SUB_MODALIDADES,
    ConstantesOperaciones.ELIMINAR_SUB_MODALIDADES,
    ConstantesOperaciones.AUTENTICADO})
public class SubModalidadRecurso {

    private static final Logger LOGGER = Logger.getLogger(SubModalidadRecurso.class.getName());

    @Inject
    private SubModalidadBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgSubModalidadAtencion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgSubModalidadAtencion.class)))
        ,
    ...
    @Path("/{subModalidadId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("subModalidadId") Long id) {
        try {
            SgSubModalidadAtencion smo = servicio.obtenerPorId(id);
            if (smo == null) {
     ...
     * @return Response Lista de SgSubModalidadAtencion 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgSubModalidadAtencion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroSubModalidad filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroSubModalidad filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SUB_MODALIDADES})
    public Response crear(SgSubModalidadAtencion smo) {
        try {
            smo.setSmoPk(null);
            smo.setSmoVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SUB_MODALIDADES})
    public Response actualizar(@PathParam("subModalidadId") Long id, SgSubModalidadAtencion smo) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_SUB_MODALIDADES})
    public Response eliminarPorId(@PathParam("subModalidadId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{subModalidadId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("subModalidadId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgSubModalidadAtencion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
