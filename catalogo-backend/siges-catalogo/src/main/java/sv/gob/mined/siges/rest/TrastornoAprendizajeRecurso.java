...
import sv.gob.mined.siges.negocio.servicios.TrastornoAprendizajeBean;
...
@RequestScoped
@Path("/v1/trastornosaprendizaje")
@Tag(name = "Trastornos Aprendizaje API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TRASTORNO_APRENDIZAJE,
    ConstantesOperaciones.ACTUALIZAR_TRASTORNO_APRENDIZAJE,
    ConstantesOperaciones.ELIMINAR_TRASTORNO_APRENDIZAJE})
public class TrastornoAprendizajeRecurso {

    private static final Logger LOGGER = Logger.getLogger(TrastornoAprendizajeRecurso.class.getName());

    @Inject
    private TrastornoAprendizajeBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTrastornoAprendizaje
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTrastornoAprendizaje.class)))
        ,
    ...
    @Path("/{trastornoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("trastornoId") Long id) {
        try {
            SgTrastornoAprendizaje dis = servicio.obtenerPorId(id);
            if (dis == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(dis).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTrastornoAprendizaje 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgTrastornoAprendizaje.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
     ...
     *
     * @param dis SgTrastornoAprendizaje
     * @return Response SgTrastornoAprendizaje
     */
    @POST
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TRASTORNO_APRENDIZAJE})
    public Response crear(SgTrastornoAprendizaje dis) {
        try {
            dis.setTraPk(null);
            dis.setTraVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(dis)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TRASTORNO_APRENDIZAJE})
    public Response actualizar(@PathParam("trastornoId") Long id, SgTrastornoAprendizaje dis) {
        try {
            if (servicio.objetoExistePorId(id)) {
                dis.setTraPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(dis)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TRASTORNO_APRENDIZAJE})
    public Response eliminarPorId(@PathParam("trastornoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{trastornoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("trastornoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTrastornoAprendizaje.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
