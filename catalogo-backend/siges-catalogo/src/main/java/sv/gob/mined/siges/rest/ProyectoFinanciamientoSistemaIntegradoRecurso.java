...
import sv.gob.mined.siges.negocio.servicios.ProyectoFinanciamientoSistemaIntegradoBean;
 ...
 */
@RequestScoped
@Path("/v1/proyectosfinanciamientosistemaintegrado")
@Tag(name = "Proyectos Financiamiento Sistemas Integrados API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_PROYECTOS_FINANCIAMIENTO_SISTEMA_INTEGRADO,
    ConstantesOperaciones.ACTUALIZAR_PROYECTOS_FINANCIAMIENTO_SISTEMA_INTEGRADO,
    ConstantesOperaciones.ELIMINAR_PROYECTOS_FINANCIAMIENTO_SISTEMA_INTEGRADO})
public class ProyectoFinanciamientoSistemaIntegradoRecurso {

    private static final Logger LOGGER = Logger.getLogger(ProyectoFinanciamientoSistemaIntegradoRecurso.class.getName());

    @Inject
    private ProyectoFinanciamientoSistemaIntegradoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgProyectoFinanciamientoSistemaIntegrado
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgProyectoFinanciamientoSistemaIntegrado.class)))
        ,
    ...
    @Path("/{proyectoFinanciamientoSistemaIntegradoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("proyectoFinanciamientoSistemaIntegradoId") Long id) {
        try {
            SgProyectoFinanciamientoSistemaIntegrado pfs = servicio.obtenerPorId(id);
            if (pfs == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(pfs).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgProyectoFinanciamientoSistemaIntegrado
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgProyectoFinanciamientoSistemaIntegrado.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_PROYECTOS_FINANCIAMIENTO_SISTEMA_INTEGRADO})
    public Response crear(SgProyectoFinanciamientoSistemaIntegrado pfs) {
        try {
            pfs.setPfsPk(null);
            pfs.setPfsVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(pfs)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PROYECTOS_FINANCIAMIENTO_SISTEMA_INTEGRADO})
    public Response actualizar(@PathParam("proyectoFinanciamientoSistemaIntegradoId") Long id, SgProyectoFinanciamientoSistemaIntegrado pfs) {
        try {
            if (servicio.objetoExistePorId(id)) {
                pfs.setPfsPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(pfs)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PROYECTOS_FINANCIAMIENTO_SISTEMA_INTEGRADO})
    public Response eliminarPorId(@PathParam("proyectoFinanciamientoSistemaIntegradoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{proyectoFinanciamientoSistemaIntegradoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("proyectoFinanciamientoSistemaIntegradoId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgProyectoFinanciamientoSistemaIntegrado.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgProyectoFinanciamientoSistemaIntegrado.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
