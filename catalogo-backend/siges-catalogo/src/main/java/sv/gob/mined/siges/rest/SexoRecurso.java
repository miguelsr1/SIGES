...
import sv.gob.mined.siges.negocio.servicios.SexoBean;
...
@RequestScoped
@Path("/v1/sexos")
@Tag(name = "Sexo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SEXO,
    ConstantesOperaciones.ACTUALIZAR_SEXO,
    ConstantesOperaciones.ELIMINAR_SEXO,
    ConstantesOperaciones.AUTENTICADO})
public class SexoRecurso {

    private static final Logger LOGGER = Logger.getLogger(SexoBean.class.getName());

    @Inject
    private SexoBean servicio;
    ...
    public Response obtenerPorId(@PathParam("sexoId") Long id) {
        try {
            SgSexo sex = servicio.obtenerPorId(id);
            if (sex == null) {
     ...
     * @return Response Lista de SgSexo 
     */
    @POST
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SEXO})
    public Response crear(SgSexo sex) {
        try {
            sex.setSexPk(null);
            sex.setSexVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SEXO})
    public Response actualizar(@PathParam("sexoId") Long id, SgSexo sex) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_SEXO})
    public Response eliminarPorId(@PathParam("sexoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{sexoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("sexoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgSexo.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
