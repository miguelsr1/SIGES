...
import sv.gob.mined.siges.negocio.servicios.TipoValoracionBean;
...
@RequestScoped
@Path("/v1/tiposvaloracion")
@Tag(name = "Tipos de Valoración API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPOS_VALORACION,
    ConstantesOperaciones.ACTUALIZAR_TIPOS_VALORACION,
    ConstantesOperaciones.ELIMINAR_TIPOS_VALORACION,
    ConstantesOperaciones.AUTENTICADO})
public class TipoValoracionRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoValoracionRecurso.class.getName());

    @Inject
    private TipoValoracionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTipoValoracion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoValoracion.class)))
        ,
    ...
    @Path("/{tipoValoracionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoValoracionId") Long id) {
        try {
            SgTipoValoracion tva = servicio.obtenerPorId(id);
            if (tva == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tva).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTipoValoracion 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgTipoValoracion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPOS_VALORACION})
    public Response crear(SgTipoValoracion tva) {
        try {
            tva.setTvaPk(null);
            tva.setTvaVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tva)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPOS_VALORACION})
    public Response actualizar(@PathParam("tipoValoracionId") Long id, SgTipoValoracion tva) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tva.setTvaPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tva)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPOS_VALORACION})
    public Response eliminarPorId(@PathParam("tipoValoracionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tipoValoracionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoValoracionId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoValoracion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
