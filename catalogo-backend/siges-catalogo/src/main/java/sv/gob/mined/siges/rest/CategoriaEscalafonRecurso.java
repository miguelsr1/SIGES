...
import sv.gob.mined.siges.negocio.servicios.CategoriaEscalafonBean;
...
@RequestScoped
@Path("/v1/categoriaescalafon")
@Tag(name = "CategoriaEscalafon API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CATEGORIA_ESCALAFON,
    ConstantesOperaciones.ACTUALIZAR_CATEGORIA_ESCALAFON,
    ConstantesOperaciones.ELIMINAR_CATEGORIA_ESCALAFON,
    ConstantesOperaciones.AUTENTICADO})
public class CategoriaEscalafonRecurso {

    private static final Logger LOGGER = Logger.getLogger(CategoriaEscalafonRecurso.class.getName());

    @Inject
    private CategoriaEscalafonBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgCategoriaEscalafon
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCategoriaEscalafon.class)))
        ,
    ...
    @Path("/{categoriaEscalafonId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("categoriaEscalafonId") Long id) {
        try {
            SgCategoriaEscalafon ayu = servicio.obtenerPorId(id);
            if (ayu == null) {
     ...
     * @return Response Lista de SgCategoriaEscalafon 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgCategoriaEscalafon.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
     ...
     *
     * @param ayu SgCategoriaEscalafon
     * @return Response SgCategoriaEscalafon
     */
    @POST
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CATEGORIA_ESCALAFON})
    public Response crear(SgCategoriaEscalafon ayu) {
        try {
            ayu.setCesPk(null);
            ayu.setCesVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ayu)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CATEGORIA_ESCALAFON})
    public Response actualizar(@PathParam("categoriaEscalafonId") Long id, SgCategoriaEscalafon ayu) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ayu.setCesPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ayu)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CATEGORIA_ESCALAFON})
    public Response eliminarPorId(@PathParam("categoriaEscalafonId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{categoriaEscalafonId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("categoriaEscalafonId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgCategoriaEscalafon.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
