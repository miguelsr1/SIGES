...
import sv.gob.mined.siges.negocio.servicios.MotivoEgresoBean;
...
@RequestScoped
@Path("/v1/motivosegreso")
@Tag(name = "MotivoEgreso API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MOTIVO_EGRESO,
    ConstantesOperaciones.ACTUALIZAR_MOTIVO_EGRESO,
    ConstantesOperaciones.ELIMINAR_MOTIVO_EGRESO,
    ConstantesOperaciones.AUTENTICADO})
public class MotivoEgresoRecurso {

    private static final Logger LOGGER = Logger.getLogger(MotivoEgresoRecurso.class.getName());

    @Inject
    private MotivoEgresoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMotivoEgreso
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMotivoEgreso.class)))
        ,
    ...
    @Path("/{motivoEgresoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("motivoEgresoId") Long id) {
        try {
            SgMotivoEgreso meg = servicio.obtenerPorId(id);
            if (meg == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(meg).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMotivoEgreso 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgMotivoEgreso.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MOTIVO_EGRESO})
    public Response crear(SgMotivoEgreso meg) {
        try {
            meg.setMegPk(null);
            meg.setMegVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(meg)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MOTIVO_EGRESO})
    public Response actualizar(@PathParam("motivoEgresoId") Long id, SgMotivoEgreso meg) {
        try {
            if (servicio.objetoExistePorId(id)) {
                meg.setMegPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(meg)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MOTIVO_EGRESO})
    public Response eliminarPorId(@PathParam("motivoEgresoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{motivoEgresoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("motivoEgresoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMotivoEgreso.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
