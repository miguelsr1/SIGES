...
import sv.gob.mined.siges.negocio.servicios.IdentificacionOAEBean;
 ...
 */
@RequestScoped
@Path("/v1/identificacionoae")
@Tag(name = "Identificación OAE API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_IDENTIFICACION_OAE,
    ConstantesOperaciones.ACTUALIZAR_IDENTIFICACION_OAE,
    ConstantesOperaciones.ELIMINAR_IDENTIFICACION_OAE})
public class IdentificacionOAERecurso {

    private static final Logger LOGGER = Logger.getLogger(IdentificacionOAERecurso.class.getName());

    @Inject
    private IdentificacionOAEBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgIdentificacionOAE
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgIdentificacionOAE.class)))
        ,
    ...
    @Path("/{identificacionOAEId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("identificacionOAEId") Long id) {
        try {
            SgIdentificacionOAE ioa = servicio.obtenerPorId(id);
            if (ioa == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(ioa).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgIdentificacionOAE
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgIdentificacionOAE.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_IDENTIFICACION_OAE})
    public Response crear(SgIdentificacionOAE ioa) {
        try {
            ioa.setIoaPk(null);
            ioa.setIoaVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ioa)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_IDENTIFICACION_OAE})
    public Response actualizar(@PathParam("identificacionOAEId") Long id, SgIdentificacionOAE ioa) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ioa.setIoaPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ioa)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_IDENTIFICACION_OAE})
    public Response eliminarPorId(@PathParam("identificacionOAEId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{identificacionOAEId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("identificacionOAEId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgIdentificacionOAE.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgIdentificacionOAE.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
