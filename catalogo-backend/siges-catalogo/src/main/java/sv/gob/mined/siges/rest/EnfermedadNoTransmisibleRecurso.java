...
import sv.gob.mined.siges.negocio.servicios.EnfermedadNoTransmisibleBean;
...
@RequestScoped
@Path("/v1/enfermedadesnotransmisibles")
@Tag(name = "EnfermedadesNoTransmisible API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ENFERMEADES_NO_TRANSMISIBLE,
    ConstantesOperaciones.ACTUALIZAR_ENFERMEADES_NO_TRANSMISIBLE,
    ConstantesOperaciones.ELIMINAR_ENFERMEADES_NO_TRANSMISIBLE,
    ConstantesOperaciones.AUTENTICADO})
public class EnfermedadNoTransmisibleRecurso {

    private static final Logger LOGGER = Logger.getLogger(EnfermedadNoTransmisibleRecurso.class.getName());

    @Inject
    private EnfermedadNoTransmisibleBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgEnfermedadNoTransmisible
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEnfermedadNoTransmisible.class)))
        ,
    ...
    @Path("/{enfermedadNoTransmisibleId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("enfermedadNoTransmisibleId") Long id) {
        try {
            SgEnfermedadNoTransmisible ent = servicio.obtenerPorId(id);
            if (ent == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(ent).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgEnfermedadNoTransmisible 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgEnfermedadNoTransmisible.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ENFERMEADES_NO_TRANSMISIBLE})
    public Response crear(SgEnfermedadNoTransmisible ent) {
        try {
            ent.setEntPk(null);
            ent.setEntVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ent)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ENFERMEADES_NO_TRANSMISIBLE})
    public Response actualizar(@PathParam("enfermedadNoTransmisibleId") Long id, SgEnfermedadNoTransmisible ent) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ent.setEntPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ent)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ENFERMEADES_NO_TRANSMISIBLE})
    public Response eliminarPorId(@PathParam("enfermedadNoTransmisibleId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{enfermedadNoTransmisibleId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("enfermedadNoTransmisibleId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgEnfermedadNoTransmisible.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
