...
import sv.gob.mined.siges.negocio.servicios.ReferenciasApoyoBean;
 ...
 */
@RequestScoped
@Path("/v1/referenciasdeapoyo")
@Tag(name = "Referencias de apoyo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REFERENCIAS_DE_APOYO,
    ConstantesOperaciones.ACTUALIZAR_REFERENCIAS_DE_APOYO,
    ConstantesOperaciones.ELIMINAR_REFERENCIAS_DE_APOYO,
	ConstantesOperaciones.BUSCAR_REFERENCIAS_DE_APOYO})
public class ReferenciasApoyoRecurso {

    private static final Logger LOGGER = Logger.getLogger(ReferenciasApoyoRecurso.class.getName());

    @Inject
    private ReferenciasApoyoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgReferenciasApoyo
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgReferenciasApoyo.class)))
        ,
    ...
    @Path("/{referenciasApoyoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("referenciasApoyoId") Long id) {
        try {
            SgReferenciasApoyo rea = servicio.obtenerPorId(id);
            if (rea == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(rea).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgReferenciasApoyo
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgReferenciasApoyo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REFERENCIAS_DE_APOYO})
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REFERENCIAS_DE_APOYO})
    public Response crear(SgReferenciasApoyo rea) {
        try {
            rea.setReaPk(null);
            rea.setReaVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(rea)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REFERENCIAS_DE_APOYO})
    public Response actualizar(@PathParam("referenciasApoyoId") Long id, SgReferenciasApoyo rea) {
        try {
            if (servicio.objetoExistePorId(id)) {
                rea.setReaPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(rea)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REFERENCIAS_DE_APOYO})
    public Response eliminarPorId(@PathParam("referenciasApoyoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{referenciasApoyoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("referenciasApoyoId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgReferenciasApoyo.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgReferenciasApoyo.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
