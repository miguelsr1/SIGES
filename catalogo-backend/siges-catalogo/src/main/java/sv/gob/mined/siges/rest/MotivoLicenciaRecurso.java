...
import sv.gob.mined.siges.negocio.servicios.MotivoLicenciaBean;
...
@RequestScoped
@Path("/v1/motivoslicencia")
@Tag(name = "MotivoLicencia API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MOTIVOS_LICENCIA,
    ConstantesOperaciones.ACTUALIZAR_MOTIVOS_LICENCIA,
    ConstantesOperaciones.ELIMINAR_MOTIVOS_LICENCIA})
public class MotivoLicenciaRecurso {

    private static final Logger LOGGER = Logger.getLogger(MotivoLicenciaRecurso.class.getName());

    @Inject
    private MotivoLicenciaBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMotivoLicencia
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMotivoLicencia.class)))
        ,
    ...
    @Path("/{motivoLicenciaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("motivoLicenciaId") Long id) {
        try {
            SgMotivoLicencia pin = servicio.obtenerPorId(id);
            if (pin == null) {
     ...
     * @return Response Lista de SgMotivoLicencia 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgMotivoLicencia.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MOTIVOS_LICENCIA})
    public Response crear(SgMotivoLicencia pin) {
        try {
            pin.setMliPk(null);
            pin.setMliVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MOTIVOS_LICENCIA})
    public Response actualizar(@PathParam("motivoLicenciaId") Long id, SgMotivoLicencia pin) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MOTIVOS_LICENCIA})
    public Response eliminarPorId(@PathParam("motivoLicenciaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{motivoLicenciaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("motivoLicenciaId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMotivoLicencia.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
