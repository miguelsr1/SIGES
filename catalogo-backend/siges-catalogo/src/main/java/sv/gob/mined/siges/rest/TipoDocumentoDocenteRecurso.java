...
import sv.gob.mined.siges.negocio.servicios.TipoDocumentoDocenteBean;
 ...
 */
@RequestScoped
@Path("/v1/tiposdocumentosdocentes")
@Tag(name = "Tipos de documentos de docentes API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPOS_DOCUMENTOS_DOCENTES,
    ConstantesOperaciones.ACTUALIZAR_TIPOS_DOCUMENTOS_DOCENTES,
    ConstantesOperaciones.ELIMINAR_TIPOS_DOCUMENTOS_DOCENTES})
public class TipoDocumentoDocenteRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoDocumentoDocenteRecurso.class.getName());

    @Inject
    private TipoDocumentoDocenteBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTipoDocumentoDocente
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoDocumentoDocente.class)))
        ,
    ...
    @Path("/{tipoDocumentoDocenteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoDocumentoDocenteId") Long id) {
        try {
            SgTipoDocumentoDocente tdd = servicio.obtenerPorId(id);
            if (tdd == null) {
     ...
     * @return Response Lista de SgTipoDocumentoDocente
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgTipoDocumentoDocente.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPOS_DOCUMENTOS_DOCENTES})
    public Response crear(SgTipoDocumentoDocente tdd) {
        try {
            tdd.setTddPk(null);
            tdd.setTddVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tdd)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPOS_DOCUMENTOS_DOCENTES})
    public Response actualizar(@PathParam("tipoDocumentoDocenteId") Long id, SgTipoDocumentoDocente tdd) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tdd.setTddPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tdd)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPOS_DOCUMENTOS_DOCENTES})
    public Response eliminarPorId(@PathParam("tipoDocumentoDocenteId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tipoDocumentoDocenteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoDocumentoDocenteId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoDocumentoDocente.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgTipoDocumentoDocente.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
