...
import sv.gob.mined.siges.negocio.servicios.TipoCalleBean;
...
@RequestScoped
@Path("/v1/tiposcalle")
@Tag(name = "TipoCalle API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPOS_CALLE,
    ConstantesOperaciones.ACTUALIZAR_TIPOS_CALLE,
    ConstantesOperaciones.ELIMINAR_TIPOS_CALLE})
public class TipoCalleRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoCalleRecurso.class.getName());
    
    @Inject
    private TipoCalleBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoCalle.class)))
        ,
    ...
    @Path("/{tipoCalleId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoCalleId") Long id) {
        try {
            SgTipoCalle crr = servicio.obtenerPorId(id);
            if (crr == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(crr).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTipoCalle 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgTipoCalle.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPOS_CALLE})
    public Response crear(SgTipoCalle crr) {
        try {
            crr.setTllPk(null);
            crr.setTllVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(crr)).build();
...
/**
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPOS_CALLE})
    public Response actualizar(@PathParam("tipoCalleId") Long id, SgTipoCalle crr) {
        try {
            if (servicio.objetoExistePorId(id)) {
                crr.setTllPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(crr)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPOS_CALLE})
    public Response eliminarPorId(@PathParam("tipoCalleId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
...
/**
    ...
    @Path("/historial/{tipoCalleId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoCalleId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoCalle.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
