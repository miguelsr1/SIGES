...
import sv.gob.mined.siges.negocio.servicios.InstitucionPagoSalarioBean;
...
@RequestScoped
@Path("/v1/institucionpagosalario")
@Tag(name = "InstitucionPagoSalario API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_INSTITUCION_PAGO_SALARIO,
    ConstantesOperaciones.ACTUALIZAR_INSTITUCION_PAGO_SALARIO,
    ConstantesOperaciones.ELIMINAR_INSTITUCION_PAGO_SALARIO,
    ConstantesOperaciones.AUTENTICADO})
public class InstitucionPagoSalarioRecurso {

    private static final Logger LOGGER = Logger.getLogger(InstitucionPagoSalarioRecurso.class.getName());
    
    @Inject
    private InstitucionPagoSalarioBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgInstitucionPagaSalario.class)))
        ,
    ...
    @Path("/{institucionPagoSalarioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("institucionPagoSalarioId") Long id) {
        try {
            SgInstitucionPagaSalario ins = servicio.obtenerPorId(id);
            if (ins == null) {
     ...
     * @return Response Lista de SgInstitucion 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgInstitucionPagaSalario.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_INSTITUCION_PAGO_SALARIO})
    public Response crear(SgInstitucionPagaSalario ins) {
        try {
            ins.setIpsPk(null);
            ins.setIpsVersion(0);
...
/**
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_INSTITUCION_PAGO_SALARIO})
    public Response actualizar(@PathParam("institucionPagoSalarioId") Long id, SgInstitucionPagaSalario ins) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_INSTITUCION_PAGO_SALARIO})
    public Response eliminarPorId(@PathParam("institucionPagoSalarioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
...
/**
    ...
    @Path("/historial/{institucionPagoSalarioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("institucionPagoSalarioId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgInstitucionPagaSalario.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
