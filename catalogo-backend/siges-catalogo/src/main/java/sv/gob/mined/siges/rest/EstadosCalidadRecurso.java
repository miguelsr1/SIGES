...
import sv.gob.mined.siges.negocio.servicios.EstadosCalidadBean;
...
@RequestScoped
@Path("/v1/estadoscalidad")
@Tag(name = "Estados Calidad API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ESTADO_CALIDAD,
    ConstantesOperaciones.ELIMINAR_ESTADO_CALIDAD,
    ConstantesOperaciones.ELIMINAR_ETNIA,
    ConstantesOperaciones.AUTENTICADO})
public class EstadosCalidadRecurso {
    private static final Logger LOGGER = Logger.getLogger(EtniaBean.class.getName());

    @Inject
    private EstadosCalidadBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAfEstadosCalidad
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAfEstadosCalidad.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("estadoId") Long id) {
        try {
            SgAfEstadosCalidad etn = servicio.obtenerPorId(id);
            if (etn == null) {
     ...
     * @return Response Lista de SgAfEstadosCalidad 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgAfEstadosCalidad.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ESTADO_CALIDAD})
    public Response crear(SgAfEstadosCalidad etn) {
        try {
            etn.setEcaPk(null);
            etn.setEcaVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ESTADO_CALIDAD})
    public Response actualizar(@PathParam("estadoId") Long id, SgAfEstadosCalidad etn) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ESTADO_CALIDAD})
    ...
    public Response obtenerHistorialPorId(@PathParam("estadoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAfEstadosCalidad.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
