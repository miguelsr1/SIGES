...
import sv.gob.mined.siges.negocio.servicios.SistemaGestionContenidoBean;
...
@RequestScoped
@Path("/v1/sistemasgestioncontenido")
@Tag (name = "SistemaGestionContenido API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SISTEMAS_GESTION_CONTENIDO,
    ConstantesOperaciones.ACTUALIZAR_SISTEMAS_GESTION_CONTENIDO,
    ConstantesOperaciones.ELIMINAR_SISTEMAS_GESTION_CONTENIDO,
    ConstantesOperaciones.AUTENTICADO})
public class SistemaGestionContenidoRecurso {

    private static final Logger LOGGER = Logger.getLogger(SistemaGestionContenidoRecurso.class.getName());

    @Inject
    private SistemaGestionContenidoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgSistemaGestionContenido
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgSistemaGestionContenido.class)))
        ,
    ...
    @Path("/{sistemaGestionContenidoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("sistemaGestionContenidoId") Long id) {
        try {
            SgSistemaGestionContenido sgc = servicio.obtenerPorId(id);
            if (sgc == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(sgc).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgSistemaGestionContenido 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description ="Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgSistemaGestionContenido.class, type = SchemaType.ARRAY))),
        @APIResponse(responseCode = "400", description ="Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description ="Error interno del servidor.")}
    )
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SISTEMAS_GESTION_CONTENIDO})
    public Response crear(SgSistemaGestionContenido sgc) {
        try {
            sgc.setSgcPk(null);
            sgc.setSgcVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(sgc)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SISTEMAS_GESTION_CONTENIDO})
    public Response actualizar(@PathParam("sistemaGestionContenidoId") Long id, SgSistemaGestionContenido sgc) {
        try {
            if (servicio.objetoExistePorId(id)) {
                sgc.setSgcPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(sgc)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_SISTEMAS_GESTION_CONTENIDO})
    public Response eliminarPorId(@PathParam("sistemaGestionContenidoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{sistemaGestionContenidoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("sistemaGestionContenidoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgSistemaGestionContenido.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
