...
import sv.gob.mined.siges.negocio.servicios.ComisionDescargoActivoFijoBean;
...
@RequestScoped
@Path("/v1/comisionAF")
@Tag(name = "Comision de Unidades de Activo Fijo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_COMISION_DESCARGO_AF,
    ConstantesOperaciones.ACTUALIZAR_COMISION_DESCARGO_AF,
    ConstantesOperaciones.ELIMINAR_COMISION_DESCARGO_AF
})
public class ComisionUnidadActivoFijoRecurso {
    private static final Logger LOGGER = Logger.getLogger(UnidadesActivoFijoRecurso.class.getName());

    @Inject
    private ComisionDescargoActivoFijoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAfComisionDescargo
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAfComisionDescargo.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("unidadId") Long id) {
        try {
            SgAfComisionDescargo entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgAfComisionDescargo
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAfComisionDescargo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
     ...
     *
     * @param entity SgAfComisionDescargo
     * @return Response SgAfComisionDescargo
     */
    @POST
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_COMISION_DESCARGO_AF})
    public Response crear(SgAfComisionDescargo entity) {
        try {
            entity.setCdePk(null);
            entity.setCdeVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_COMISION_DESCARGO_AF})
    public Response actualizar(@PathParam("unidadId") Long id, SgAfComisionDescargo entity) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response validar(SgAfComisionDescargo entity) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(ComisionDescargoValidacionNegocio.validar(entity)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_COMISION_DESCARGO_AF})
    ...
    public Response obtenerHistorialPorId(@PathParam("unidadId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAfComisionDescargo.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
