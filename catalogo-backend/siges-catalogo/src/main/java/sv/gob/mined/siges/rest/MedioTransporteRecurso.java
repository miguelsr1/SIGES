...
import sv.gob.mined.siges.negocio.servicios.MedioTransporteBean;
...
@RequestScoped
@Path("/v1/mediostransporte")
@Tag(name = "MedioTransporte API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MEDIO_TRANSPORTE,
    ConstantesOperaciones.ACTUALIZAR_MEDIO_TRANSPORTE,
    ConstantesOperaciones.ELIMINAR_MEDIO_TRANSPORTE,
    ConstantesOperaciones.AUTENTICADO})
public class MedioTransporteRecurso {

    private static final Logger LOGGER = Logger.getLogger(MedioTransporteRecurso.class.getName());

    @Inject
    private MedioTransporteBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMedioTransporte
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMedioTransporte.class)))
        ,
    ...
    @Path("/{medioTransporteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("medioTransporteId") Long id) {
        try {
            SgMedioTransporte mtr = servicio.obtenerPorId(id);
            if (mtr == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(mtr).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMedioTransporte 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description ="Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgMedioTransporte.class, type = SchemaType.ARRAY))),
        @APIResponse(responseCode = "400", description ="Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description ="Error interno del servidor.")}
    )
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MEDIO_TRANSPORTE})
    public Response crear(SgMedioTransporte mtr) {
        try {
            mtr.setMtrPk(null);
            mtr.setMtrVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(mtr)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MEDIO_TRANSPORTE})
    public Response actualizar(@PathParam("medioTransporteId") Long id, SgMedioTransporte mtr) {
        try {
            if (servicio.objetoExistePorId(id)) {
                mtr.setMtrPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(mtr)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Path("/{medioTransporte:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MEDIO_TRANSPORTE})
    public Response eliminarPorId(@PathParam("medioTransporte") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{medioTransporteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("medioTransporteId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMedioTransporte.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
