...
import sv.gob.mined.siges.negocio.servicios.TipoAlfabetizadorBean;
 ...
 */
@RequestScoped
@Path("/v1/tipoalfabetizador")
@Tag(name = "Tipos Alfabetizador API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPO_ALFABETIZADOR,
    ConstantesOperaciones.ACTUALIZAR_TIPO_ALFABETIZADOR,
    ConstantesOperaciones.ELIMINAR_TIPO_ALFABETIZADOR})
public class TipoAlfabetizadorRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoAlfabetizadorRecurso.class.getName());

    @Inject
    private TipoAlfabetizadorBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTipoAlfabetizador
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoAlfabetizador.class)))
        ,
    ...
    @Path("/{tipoAlfabetizadorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoAlfabetizadorId") Long id) {
        try {
            SgTipoAlfabetizador tal = servicio.obtenerPorId(id);
            if (tal == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tal).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTipoAlfabetizador
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgTipoAlfabetizador.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPO_ALFABETIZADOR})
    public Response crear(SgTipoAlfabetizador tal) {
        try {
            tal.setTalPk(null);
            tal.setTalVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tal)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPO_ALFABETIZADOR})
    public Response actualizar(@PathParam("tipoAlfabetizadorId") Long id, SgTipoAlfabetizador tal) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tal.setTalPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tal)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPO_ALFABETIZADOR})
    public Response eliminarPorId(@PathParam("tipoAlfabetizadorId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tipoAlfabetizadorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoAlfabetizadorId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoAlfabetizador.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgTipoAlfabetizador.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
