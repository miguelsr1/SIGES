...
import sv.gob.mined.siges.negocio.servicios.MotivoPermutaBean;
...
@RequestScoped
@Path("/v1/motivospermuta")
@Tag(name = "MotivoPermuta API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MOTIVOS_PERMUTA,
    ConstantesOperaciones.ACTUALIZAR_MOTIVOS_PERMUTA,
    ConstantesOperaciones.ELIMINAR_MOTIVOS_PERMUTA})
public class MotivoPermutaRecurso {

    private static final Logger LOGGER = Logger.getLogger(MotivoPermutaRecurso.class.getName());

    @Inject
    private MotivoPermutaBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMotivoPermuta
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMotivoPermuta.class)))
        ,
    ...
    @Path("/{motivoPermutaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("motivoPermutaId") Long id) {
        try {
            SgMotivoPermuta pin = servicio.obtenerPorId(id);
            if (pin == null) {
     ...
     * @return Response Lista de SgMotivoPermuta 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgMotivoPermuta.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MOTIVOS_PERMUTA})
    public Response crear(SgMotivoPermuta pin) {
        try {
            pin.setMpePk(null);
            pin.setMpeVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MOTIVOS_PERMUTA})
    public Response actualizar(@PathParam("motivoPermutaId") Long id, SgMotivoPermuta pin) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MOTIVOS_PERMUTA})
    public Response eliminarPorId(@PathParam("motivoPermutaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{motivoPermutaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("motivoPermutaId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMotivoPermuta.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
