...
import sv.gob.mined.siges.negocio.servicios.EstadosTrasladoBean;
...
@RequestScoped
@Path("/v1/estadostraslado")
@Tag(name = "Estados Traslado API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ESTADO_TRASLADO,
    ConstantesOperaciones.ACTUALIZAR_ESTADO_TRASLADO,
    ConstantesOperaciones.ELIMINAR_ESTADO_TRASLADO,
    ConstantesOperaciones.AUTENTICADO})
public class EstadosTrasladoRecurso {
    private static final Logger LOGGER = Logger.getLogger(EstadosTrasladoRecurso.class.getName());

    @Inject
    private EstadosTrasladoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAfEstadosTraslado
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAfEstadosTraslado.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("estadoId") Long id) {
        try {
            SgAfEstadosTraslado etn = servicio.obtenerPorId(id);
            if (etn == null) {
     ...
     * @return Response Lista de SgAfEstadosTraslado 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgAfEstadosTraslado.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ESTADO_TRASLADO})
    public Response crear(SgAfEstadosTraslado etn) {
        try {
            etn.setEtrPk(null);
            etn.setEtrVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ESTADO_TRASLADO})
    public Response actualizar(@PathParam("estadoId") Long id, SgAfEstadosTraslado etn) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ESTADO_TRASLADO})
    ...
    public Response obtenerHistorialPorId(@PathParam("estadoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAfEstadosTraslado.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
