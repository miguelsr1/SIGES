...
import sv.gob.mined.siges.negocio.servicios.CargoTitularBean;
 ...
 */
@RequestScoped
@Path("/v1/cargotitular")
@Tag(name = "Cargos Titular API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CARGO_TITULAR,
    ConstantesOperaciones.ACTUALIZAR_CARGO_TITULAR,
    ConstantesOperaciones.ELIMINAR_CARGO_TITULAR})
public class CargoTitularRecurso {

    private static final Logger LOGGER = Logger.getLogger(CargoTitularRecurso.class.getName());

    @Inject
    private CargoTitularBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgCargoTitular
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCargoTitular.class)))
        ,
    ...
    @Path("/{cargoTitularId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("cargoTitularId") Long id) {
        try {
            SgCargoTitular cti = servicio.obtenerPorId(id);
            if (cti == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(cti).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgCargoTitular
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgCargoTitular.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroCargoTitular filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroCargoTitular filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CARGO_TITULAR})
    public Response crear(SgCargoTitular cti) {
        try {
            cti.setCtiPk(null);
            cti.setCtiVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(cti)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CARGO_TITULAR})
    public Response actualizar(@PathParam("cargoTitularId") Long id, SgCargoTitular cti) {
        try {
            if (servicio.objetoExistePorId(id)) {
                cti.setCtiPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(cti)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CARGO_TITULAR})
    public Response eliminarPorId(@PathParam("cargoTitularId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{cargoTitularId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("cargoTitularId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgCargoTitular.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCargoTitular.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
