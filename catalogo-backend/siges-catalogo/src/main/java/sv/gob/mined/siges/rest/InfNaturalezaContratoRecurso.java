...
import sv.gob.mined.siges.negocio.servicios.InfNaturalezaContratoBean;
 ...
 */
@RequestScoped
@Path("/v1/naturalezacontrato")
@Tag(name = "Naturaleza contrato API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_NATURALEZA_CONTRATO,
    ConstantesOperaciones.ACTUALIZAR_NATURALEZA_CONTRATO,
    ConstantesOperaciones.ELIMINAR_NATURALEZA_CONTRATO})
public class InfNaturalezaContratoRecurso {

    private static final Logger LOGGER = Logger.getLogger(InfNaturalezaContratoRecurso.class.getName());

    @Inject
    private InfNaturalezaContratoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgInfNaturalezaContrato
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgInfNaturalezaContrato.class)))
        ,
    ...
    @Path("/{infNaturalezaContratoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("infNaturalezaContratoId") Long id) {
        try {
            SgInfNaturalezaContrato nac = servicio.obtenerPorId(id);
            if (nac == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(nac).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgInfNaturalezaContrato
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgInfNaturalezaContrato.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_NATURALEZA_CONTRATO})
    public Response crear(SgInfNaturalezaContrato nac) {
        try {
            nac.setNacPk(null);
            nac.setNacVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(nac)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_NATURALEZA_CONTRATO})
    public Response actualizar(@PathParam("infNaturalezaContratoId") Long id, SgInfNaturalezaContrato nac) {
        try {
            if (servicio.objetoExistePorId(id)) {
                nac.setNacPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(nac)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_NATURALEZA_CONTRATO})
    public Response eliminarPorId(@PathParam("infNaturalezaContratoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{infNaturalezaContratoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("infNaturalezaContratoId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgInfNaturalezaContrato.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgInfNaturalezaContrato.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
