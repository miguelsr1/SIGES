...
import sv.gob.mined.siges.negocio.servicios.MenuPentahoBean;
...
@RequestScoped
@Path("/v1/menupentaho")
@Tag(name = "Menú PENTAHO API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.TIENE_ACCESO_REPORTES_PENTAHO})
public class MenuPentahoRecurso {

    private static final Logger LOGGER = Logger.getLogger(MenuPentahoRecurso.class.getName());

    @Inject
    private MenuPentahoBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMenuPentaho.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("menuId") Long id) {
        try {
            SgMenuPentaho mpe = servicio.obtenerPorId(id);
            if (mpe == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(mpe).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMenuPentaho
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgMenuPentaho.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroMenuPentaho filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroMenuPentaho filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.TIENE_ACCESO_REPORTES_PENTAHO})
    public Response crear(SgMenuPentaho pen) {
        try {
            pen.setMpePk(null);
            pen.setMpeVersion(0);
                        return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(pen)).build();
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.TIENE_ACCESO_REPORTES_PENTAHO})
    public Response actualizar(@PathParam("menuId") Long id, SgMenuPentaho pen) {
        try {
            if (servicio.objetoExistePorId(id)) {
                pen.setMpePk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(pen)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.TIENE_ACCESO_REPORTES_PENTAHO})
    public Response eliminarPorId(@PathParam("menuId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{menuId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("menuId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMenuPentaho.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgMenuPentaho.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
