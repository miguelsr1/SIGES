...
import sv.gob.mined.siges.negocio.servicios.EscolaridadBean;
...
@RequestScoped
@Path("/v1/escolaridades")
@Tag(name = "Escolaridad API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ESCOLARIDAD,
    ConstantesOperaciones.ACTUALIZAR_ESCOLARIDAD,
    ConstantesOperaciones.ELIMINAR_ESCOLARIDAD,
    ConstantesOperaciones.AUTENTICADO})
public class EscolaridadRecurso {

    private static final Logger LOGGER = Logger.getLogger(EscolaridadRecurso.class.getName());

    @Inject
    private EscolaridadBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEscolaridad.class)))
        ,
    ...
    @Path("/{escolaridadId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("escolaridadId") Long id) {
        try {
            SgEscolaridad esc = servicio.obtenerPorId(id);
            if (esc == null) {
     ...
     * @return Response Lista de SgEscolaridad 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgEscolaridad.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ESCOLARIDAD})
    public Response crear(SgEscolaridad esc) {
        try {
            esc.setEscPk(null);
            esc.setEscVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ESCOLARIDAD})
    public Response actualizar(@PathParam("escolaridadId") Long id, SgEscolaridad esc) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ESCOLARIDAD})
    public Response eliminarPorId(@PathParam("escolaridadId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{escolaridadId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("escolaridadId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgEscolaridad.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
