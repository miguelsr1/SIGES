...
import sv.gob.mined.siges.negocio.servicios.ClasificacionBean;
...
@RequestScoped
@Path("/v1/clasificaciones")
@Tag(name = "Clasificaciones API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CLASIFICACIONES,
    ConstantesOperaciones.ACTUALIZAR_CLASIFICACIONES,
    ConstantesOperaciones.ELIMINAR_CLASIFICACIONES,
    ConstantesOperaciones.AUTENTICADO})

public class ClasificacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(ClasificacionRecurso.class.getName());

    @Inject
    private ClasificacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgClasificacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgClasificacion.class)))
        ,
    ...
    @Path("/{clasificacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("clasificacionId") Long id) {
        try {
            SgClasificacion cla = servicio.obtenerPorId(id);
            if (cla == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(cla).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgClasificacion 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgClasificacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DEPARTAMENTO})
    public Response crear(SgClasificacion cla) {
        try {
            cla.setClaPk(null);
            cla.setClaVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(cla)).build();
     ...
     */
    @PUT
    @Operation(summary = "Actualiza el objeto con la id indicada. Si no existe, lo crea con id autogenerada.")
    @APIResponses(value = {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_DEPARTAMENTO})
    public Response actualizar(@PathParam("clasificacionId") Long id, SgClasificacion cla) {
        try {
            if (servicio.objetoExistePorId(id)) {
                cla.setClaPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(cla)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    public Response eliminarPorId(@PathParam("clasificacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{clasificacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("clasificacionId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgClasificacion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
