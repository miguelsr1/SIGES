...
import sv.gob.mined.siges.negocio.servicios.IdiomaBean;
...
@RequestScoped
@Path("/v1/idiomas")
@Tag (name = "Idiomas API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_IDIOMAS,
    ConstantesOperaciones.ACTUALIZAR_IDIOMAS,
    ConstantesOperaciones.ELIMINAR_IDIOMAS,
    ConstantesOperaciones.AUTENTICADO})
public class IdiomaRecurso {

    private static final Logger LOGGER = Logger.getLogger(IdiomaRecurso.class.getName());

    @Inject
    private IdiomaBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgIdioma.class)))
        ,
    ...
    @Path("/{idiomaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("idiomaId") Long id) {
        try {
            SgIdioma idi = servicio.obtenerPorId(id);
            if (idi == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(idi).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgIdioma 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description ="Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgIdioma.class, type = SchemaType.ARRAY))),
        @APIResponse(responseCode = "400", description ="Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description ="Error interno del servidor.")}
    )
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_IDIOMAS})
    public Response crear(SgIdioma idi) {
        try {
            idi.setIdiPk(null);
            idi.setIdiVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(idi)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_IDIOMAS})
    public Response actualizar(@PathParam("idiomaId") Long id, SgIdioma idi) {
        try {
            if (servicio.objetoExistePorId(id)) {
                idi.setIdiPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(idi)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_IDIOMAS})
    public Response eliminarPorId(@PathParam("idiomaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{idiomaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("idiomaId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgIdioma.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
