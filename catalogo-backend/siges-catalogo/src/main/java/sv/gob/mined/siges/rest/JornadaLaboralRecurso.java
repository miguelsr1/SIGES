...
import sv.gob.mined.siges.negocio.servicios.JornadaLaboralBean;
...
@RequestScoped
@Path("/v1/jornadaslaborales")
@Tag(name = "JornadaLaboral API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_JORNADA_LABORAL,
    ConstantesOperaciones.ACTUALIZAR_JORNADA_LABORAL,
    ConstantesOperaciones.ELIMINAR_JORNADA_LABORAL,
    ConstantesOperaciones.AUTENTICADO})
public class JornadaLaboralRecurso {

    private static final Logger LOGGER = Logger.getLogger(JornadaLaboralRecurso.class.getName());

    @Inject
    private JornadaLaboralBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgJornadaLaboral
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgJornadaLaboral.class)))
        ,
    ...
    @Path("/{jornadaLaboralId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("jornadaLaboralId") Long id) {
        try {
            SgJornadaLaboral jla = servicio.obtenerPorId(id);
            if (jla == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(jla).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgJornadaLaboral
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgJornadaLaboral.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_JORNADA_LABORAL})
    public Response crear(SgJornadaLaboral jla) {
        try {
            jla.setJlaPk(null);
            jla.setJlaVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(jla)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_JORNADA_LABORAL})
    public Response actualizar(@PathParam("jornadaLaboralId") Long id, SgJornadaLaboral jla) {
        try {
            if (servicio.objetoExistePorId(id)) {
                jla.setJlaPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(jla)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_JORNADA_LABORAL})
    public Response eliminarPorId(@PathParam("jornadaLaboralId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{jornadaLaboralId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("jornadaLaboralId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgJornadaLaboral.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
