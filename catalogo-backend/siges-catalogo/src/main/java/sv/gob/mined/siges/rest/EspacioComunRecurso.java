...
import sv.gob.mined.siges.negocio.servicios.EspacioComunBean;
 ...
 */
@RequestScoped
@Path("/v1/espaciocomun")
@Tag(name = "Espacio Común API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ESPACIOS_COMUNES,
    ConstantesOperaciones.ACTUALIZAR_ESPACIOS_COMUNES,
    ConstantesOperaciones.ELIMINAR_ESPACIOS_COMUNES})
public class EspacioComunRecurso {

    private static final Logger LOGGER = Logger.getLogger(EspacioComunRecurso.class.getName());

    @Inject
    private EspacioComunBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgEspacioComun
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEspacioComun.class)))
        ,
    ...
    @Path("/{espacioComunId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("espacioComunId") Long id) {
        try {
            SgEspacioComun eco = servicio.obtenerPorId(id);
            if (eco == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(eco).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgEspacioComun
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgEspacioComun.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroEspacioComplementario filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroEspacioComplementario filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ESPACIOS_COMUNES})
    public Response crear(SgEspacioComun eco) {
        try {
            eco.setEcoPk(null);
            eco.setEcoVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(eco)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ESPACIOS_COMUNES})
    public Response actualizar(@PathParam("espacioComunId") Long id, SgEspacioComun eco) {
        try {
            if (servicio.objetoExistePorId(id)) {
                eco.setEcoPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(eco)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ESPACIOS_COMUNES})
    public Response eliminarPorId(@PathParam("espacioComunId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{espacioComunId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("espacioComunId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgEspacioComun.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgEspacioComun.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
