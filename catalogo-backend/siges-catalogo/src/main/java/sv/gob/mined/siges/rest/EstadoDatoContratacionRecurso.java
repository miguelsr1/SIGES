...
import sv.gob.mined.siges.negocio.servicios.EstadoDatoContratacionBean;
 ...
 */
@RequestScoped
@Path("/v1/estadosdatocontratacion")
@Tag(name = "Estados de datos de contratación API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ESTADO_DATO_CONTRATACION,
    ConstantesOperaciones.ACTUALIZAR_ESTADO_DATO_CONTRATACION,
    ConstantesOperaciones.ELIMINAR_ESTADO_DATO_CONTRATACION})
public class EstadoDatoContratacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(EstadoDatoContratacionRecurso.class.getName());

    @Inject
    private EstadoDatoContratacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgEstadoDatoContratacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEstadoDatoContratacion.class)))
        ,
    ...
    @Path("/{estadoDatoContratacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("estadoDatoContratacionId") Long id) {
        try {
            SgEstadoDatoContratacion edc = servicio.obtenerPorId(id);
            if (edc == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(edc).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgEstadoDatoContratacion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgEstadoDatoContratacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ESTADO_DATO_CONTRATACION})
    public Response crear(SgEstadoDatoContratacion edc) {
        try {
            edc.setEdcPk(null);
            edc.setEdcVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(edc)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ESTADO_DATO_CONTRATACION})
    public Response actualizar(@PathParam("estadoDatoContratacionId") Long id, SgEstadoDatoContratacion edc) {
        try {
            if (servicio.objetoExistePorId(id)) {
                edc.setEdcPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(edc)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ESTADO_DATO_CONTRATACION})
    public Response eliminarPorId(@PathParam("estadoDatoContratacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{estadoDatoContratacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("estadoDatoContratacionId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgEstadoDatoContratacion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgEstadoDatoContratacion.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
