...
import sv.gob.mined.siges.negocio.servicios.TipoProveedorBean;
 ...
 */
@RequestScoped
@Path("/v1/tiposproveedor")
@Tag(name = "Tipos de Proveedores API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPOS_PROVEEDOR,
    ConstantesOperaciones.ACTUALIZAR_TIPOS_PROVEEDOR,
    ConstantesOperaciones.ELIMINAR_TIPOS_PROVEEDOR})
public class TipoProveedorRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoProveedorRecurso.class.getName());

    @Inject
    private TipoProveedorBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTipoProveedor
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoProveedor.class)))
        ,
    ...
    @Path("/{tipoProveedorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoProveedorId") Long id) {
        try {
            SgTipoProveedor tpr = servicio.obtenerPorId(id);
            if (tpr == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tpr).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTipoProveedor
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgTipoProveedor.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPOS_PROVEEDOR})
    public Response crear(SgTipoProveedor tpr) {
        try {
            tpr.setTprPk(null);
            tpr.setTprVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tpr)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPOS_PROVEEDOR})
    public Response actualizar(@PathParam("tipoProveedorId") Long id, SgTipoProveedor tpr) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tpr.setTprPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tpr)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPOS_PROVEEDOR})
    public Response eliminarPorId(@PathParam("tipoProveedorId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tipoProveedorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoProveedorId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoProveedor.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgTipoProveedor.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
