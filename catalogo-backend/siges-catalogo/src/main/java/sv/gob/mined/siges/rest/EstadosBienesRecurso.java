...
import sv.gob.mined.siges.negocio.servicios.EstadosBienesBean;
...
@RequestScoped
@Path("/v1/estadosbienes")
@Tag(name = "Estados Bienes API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ESTADO_BIENES,
    ConstantesOperaciones.ACTUALIZAR_ESTADO_BIENES,
    ConstantesOperaciones.ELIMINAR_ESTADO_BIENES,
    ConstantesOperaciones.AUTENTICADO})
public class EstadosBienesRecurso {
    private static final Logger LOGGER = Logger.getLogger(EstadosBienesRecurso.class.getName());

    @Inject
    private EstadosBienesBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAfEstadosBienes
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAfEstadosBienes.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("estadoId") Long id) {
        try {
            SgAfEstadosBienes etn = servicio.obtenerPorId(id);
            if (etn == null) {
     ...
     * @return Response Lista de SgAfEstadosBienes 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgAfEstadosBienes.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ESTADO_BIENES})
    public Response crear(SgAfEstadosBienes etn) {
        try {
            etn.setEbiPk(null);
            etn.setEbiVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ESTADO_BIENES})
    public Response actualizar(@PathParam("estadoId") Long id, SgAfEstadosBienes etn) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ESTADO_BIENES})
    ...
    public Response obtenerHistorialPorId(@PathParam("estadoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAfEstadosBienes.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
