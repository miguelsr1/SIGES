...
import sv.gob.mined.siges.negocio.servicios.AccionSolicitudTrasladoBean;
...
@RequestScoped
@Path("/v1/acciones")
@Tag(name = "Accion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ACCION,
    ConstantesOperaciones.ACTUALIZAR_ACCION,
    ConstantesOperaciones.ELIMINAR_ACCION})
public class AccionSolicitudTrasladoRecurso {

    private static final Logger LOGGER = Logger.getLogger(AccionSolicitudTrasladoRecurso.class.getName());

    @Inject
    private AccionSolicitudTrasladoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAccionSolicitudTraslado
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAccionSolicitudTraslado.class)))
        ,
    ...
    @Path("/{accionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("accionId") Long id) {
        try {
            SgAccionSolicitudTraslado ayu = servicio.obtenerPorId(id);
            if (ayu == null) {
     ...
     * @return Response Lista de SgAccionSolicitudTraslado 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgAccionSolicitudTraslado.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroAccion filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroAccion filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ACCION})
    public Response crear(SgAccionSolicitudTraslado ayu) {
        try {
            ayu.setAccPk(null);
            ayu.setAccVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ACCION})
    public Response actualizar(@PathParam("accionId") Long id, SgAccionSolicitudTraslado ayu) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ACCION})
    public Response eliminarPorId(@PathParam("accionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{accionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("accionId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAccionSolicitudTraslado.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
