...
import sv.gob.mined.siges.negocio.servicios.ConfiguracionPfeaBean;
 ...
 */
@RequestScoped
@Path("/v1/configuracionespfea")
@Tag(name = "Configuraciones PFEA API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CONFIGURACIONES,
    ConstantesOperaciones.ACTUALIZAR_CONFIGURACIONES,
    ConstantesOperaciones.ELIMINAR_CONFIGURACIONES,
    ConstantesOperaciones.AUTENTICADO})
public class ConfiguracionPfeaRecurso {

    private static final Logger LOGGER = Logger.getLogger(ConfiguracionPfeaRecurso.class.getName());

    @Inject
    private ConfiguracionPfeaBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgConfiguracionPfea
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgConfiguracionPfea.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("configuracionId") Long id) {
        try {
            SgConfiguracionPfea con = servicio.obtenerPorId(id);
            if (con == null) {
     ...
     * @return Response Lista de SgConfiguracionPfea
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgConfiguracionPfea.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CONFIGURACIONES})
    public Response crear(SgConfiguracionPfea con) {
        try {
            con.setConPk(null);
            con.setConVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(con)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CONFIGURACIONES})
    public Response actualizar(@PathParam("configuracionId") Long id, SgConfiguracionPfea con) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CONFIGURACIONES})
    public Response eliminarPorId(@PathParam("configuracionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    public Response obtenerHistorialPorId(@PathParam("configuracionId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgConfiguracionPfea.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
