...
import sv.gob.mined.siges.negocio.servicios.EscalaCalificacionBean;
...
@RequestScoped
@Path("/v1/escalacalificacion")
@Tag(name = "EscalasCalificacion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ESCALAS_CALIFICACION,
    ConstantesOperaciones.ACTUALIZAR_ESCALAS_CALIFICACION,
    ConstantesOperaciones.ELIMINAR_ESCALAS_CALIFICACION,
    ConstantesOperaciones.AUTENTICADO})
public class EscalaCalificacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(EscalaCalificacionRecurso.class.getName());

    @Inject
    private EscalaCalificacionBean servicio;
     ...
     *
     * @param id
     * @return Response SgEscalaCalificacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEscalaCalificacion.class)))
        ,
    ...
    @Path("/{escalaCalificacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("escalaCalificacionId") Long id) {
        try {
            SgEscalaCalificacion eca = servicio.obtenerPorId(id);
            if (eca == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(eca).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgEscalaCalificacion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgEscalaCalificacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroEscalaCalificacion filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroEscalaCalificacion filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ESCALAS_CALIFICACION})
    public Response crear(SgEscalaCalificacion eca) {
        try {
            eca.setEcaPk(null);
            eca.setEcaVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(eca)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ESCALAS_CALIFICACION})
    public Response actualizar(@PathParam("escalaCalificacionId") Long id, SgEscalaCalificacion eca) {
        try {
            if (servicio.objetoExistePorId(id)) {
                eca.setEcaPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(eca)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ESCALAS_CALIFICACION})
    public Response eliminarPorId(@PathParam("escalaCalificacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{escalaCalificacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("escalaCalificacionId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgEscalaCalificacion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
