...
import sv.gob.mined.siges.negocio.servicios.TipoManifestacionBean;
...
@RequestScoped
@Path("/v1/tiposmanifestacion")
@Tag(name = "Tipos de Manifestación API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPOS_MANIFESTACION,
    ConstantesOperaciones.ACTUALIZAR_TIPOS_MANIFESTACION,
    ConstantesOperaciones.ELIMINAR_TIPOS_MANIFESTACION,
    ConstantesOperaciones.AUTENTICADO})
public class TipoManifestacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoManifestacionRecurso.class.getName());

    @Inject
    private TipoManifestacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTipoManifestacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoManifestacion.class)))
        ,
    ...
    @Path("/{tipoManifestacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoManifestacionId") Long id) {
        try {
            SgTipoManifestacion tma = servicio.obtenerPorId(id);
            if (tma == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tma).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTipoManifestacion 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgTipoManifestacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroTiposManifestacionViolencia filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroTiposManifestacionViolencia filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPOS_MANIFESTACION})
    public Response crear(SgTipoManifestacion tma) {
        try {
            tma.setTmaPk(null);
            tma.setTmaVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tma)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPOS_MANIFESTACION})
    public Response actualizar(@PathParam("tipoManifestacionId") Long id, SgTipoManifestacion tma) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tma.setTmaPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tma)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPOS_MANIFESTACION})
    public Response eliminarPorId(@PathParam("tipoManifestacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tipoManifestacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoManifestacionId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoManifestacion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
