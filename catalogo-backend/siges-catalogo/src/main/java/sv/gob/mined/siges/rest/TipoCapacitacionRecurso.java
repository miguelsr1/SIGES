...
import sv.gob.mined.siges.negocio.servicios.TipoCapacitacionBean;
...
@RequestScoped
@Path("/v1/tiposcapacitacion")
@Tag(name = "TipoCapacitacion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPOS_CAPACITACION,
    ConstantesOperaciones.ACTUALIZAR_TIPOS_CAPACITACION,
    ConstantesOperaciones.ELIMINAR_TIPOS_CAPACITACION})
public class TipoCapacitacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoCapacitacionRecurso.class.getName());

    @Inject
    private TipoCapacitacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTipoCapacitacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoCapacitacion.class)))
        ,
    ...
    @Path("/{tipoCapacitacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoCapacitacionId") Long id) {
        try {
            SgTipoCapacitacion ttr = servicio.obtenerPorId(id);
            if (ttr == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(ttr).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTipoCapacitacion 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgTipoCapacitacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPOS_CAPACITACION})
    public Response crear(SgTipoCapacitacion ttr) {
        try {
            ttr.setTcaPk(null);
            ttr.setTcaVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ttr)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPOS_CAPACITACION})
    public Response actualizar(@PathParam("tipoCapacitacionId") Long id, SgTipoCapacitacion ttr) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ttr.setTcaPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ttr)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPOS_CAPACITACION})
    public Response eliminarPorId(@PathParam("tipoCapacitacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tipoCapacitacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoCapacitacionId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoCapacitacion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
