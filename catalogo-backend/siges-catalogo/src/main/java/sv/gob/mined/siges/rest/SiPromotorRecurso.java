...
import sv.gob.mined.siges.negocio.servicios.SiPromotorBean;
import sv.gob.mined.siges.persistencia.entidades.SgSiPromotor;
...
@RequestScoped
@Path("/v1/promotores")
@Tag(name = "Promotores API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_PROMOTORES,
    ConstantesOperaciones.ACTUALIZAR_PROMOTORES,
    ConstantesOperaciones.ELIMINAR_PROMOTORES})
public class SiPromotorRecurso {

    private static final Logger LOGGER = Logger.getLogger(SiPromotorRecurso.class.getName());

    @Inject
    private SiPromotorBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgSiPromotor.class)))
        ,
    ...
    @Path("/{siPromotorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("siPromotorId") Long id) {
        try {
            SgSiPromotor pro = servicio.obtenerPorId(id);
            if (pro == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(pro).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgSiPromotor
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgSiPromotor.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_PROMOTORES})
    public Response crear(SgSiPromotor pro) {
        try {
            pro.setProPk(null);
            pro.setProVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(pro)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PROMOTORES})
    public Response actualizar(@PathParam("siPromotorId") Long id, SgSiPromotor pro) {
        try {
            if (servicio.objetoExistePorId(id)) {
                pro.setProPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(pro)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PROMOTORES})
    public Response eliminarPorId(@PathParam("siPromotorId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{siPromotorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("siPromotorId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgSiPromotor.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgSiPromotor.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
