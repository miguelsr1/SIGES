...
import sv.gob.mined.siges.negocio.servicios.CompaniaTelecomunicacionBean;
 ...
 */
@RequestScoped
@Path("/v1/companiastelecomunicacion")
@Tag(name = "Compañías de telecomunicación API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_COMPANIAS_TELECOMUNICACION,
    ConstantesOperaciones.ACTUALIZAR_COMPANIAS_TELECOMUNICACION,
    ConstantesOperaciones.ELIMINAR_COMPANIAS_TELECOMUNICACION})
public class CompaniaTelecomunicacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(CompaniaTelecomunicacionRecurso.class.getName());

    @Inject
    private CompaniaTelecomunicacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgCompaniaTelecomunicacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCompaniaTelecomunicacion.class)))
        ,
    ...
    @Path("/{companiaTelecomunicacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("companiaTelecomunicacionId") Long id) {
        try {
            SgCompaniaTelecomunicacion cte = servicio.obtenerPorId(id);
            if (cte == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(cte).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgCompaniaTelecomunicacion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgCompaniaTelecomunicacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_COMPANIAS_TELECOMUNICACION})
    public Response crear(SgCompaniaTelecomunicacion cte) {
        try {
            cte.setCtePk(null);
            cte.setCteVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(cte)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_COMPANIAS_TELECOMUNICACION})
    public Response actualizar(@PathParam("companiaTelecomunicacionId") Long id, SgCompaniaTelecomunicacion cte) {
        try {
            if (servicio.objetoExistePorId(id)) {
                cte.setCtePk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(cte)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_COMPANIAS_TELECOMUNICACION})
    public Response eliminarPorId(@PathParam("companiaTelecomunicacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{companiaTelecomunicacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("companiaTelecomunicacionId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgCompaniaTelecomunicacion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCompaniaTelecomunicacion.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
