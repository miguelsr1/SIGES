...
import sv.gob.mined.siges.negocio.servicios.ProyectoInstitucionalBean;
...
@RequestScoped
@Path("/v1/proyectosinstitucionales")
@Tag(name = "ProyectoInstitucional API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_PROYECTOS_INSTITUCIONALES,
    ConstantesOperaciones.ACTUALIZAR_PROYECTOS_INSTITUCIONALES,
    ConstantesOperaciones.ELIMINAR_PROYECTOS_INSTITUCIONALES})
public class ProyectoInstitucionalRecurso {

    private static final Logger LOGGER = Logger.getLogger(ProyectoInstitucionalRecurso.class.getName());

    @Inject
    private ProyectoInstitucionalBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgProyectoInstitucional
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgProyectoInstitucional.class)))
        ,
    ...
    @Path("/{proyectoInstitucionalId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("proyectoInstitucionalId") Long id) {
        try {
            SgProyectoInstitucional pin = servicio.obtenerPorId(id);
            if (pin == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(pin).build();
            }
        } catch (Exception ex) {
     ...
     *
     * @param filtro FiltroProyectoInstitucional
     * @return Response Lista de SgProyectoInstitucional 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgProyectoInstitucional.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroProyectoInstitucional filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroProyectoInstitucional filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_PROYECTOS_INSTITUCIONALES})
    public Response crear(SgProyectoInstitucional pin) {
        try {
            pin.setPinPk(null);
            pin.setPinVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(pin)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PROYECTOS_INSTITUCIONALES})
    public Response actualizar(@PathParam("proyectoInstitucionalId") Long id, SgProyectoInstitucional pin) {
        try {
            if (servicio.objetoExistePorId(id)) {
                pin.setPinPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(pin)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PROYECTOS_INSTITUCIONALES})
    public Response eliminarPorId(@PathParam("proyectoInstitucionalId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{proyectoInstitucionalId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("proyectoInstitucionalId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgProyectoInstitucional.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
