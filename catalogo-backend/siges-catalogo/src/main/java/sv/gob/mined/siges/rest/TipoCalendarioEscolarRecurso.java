...
import sv.gob.mined.siges.negocio.servicios.TipoCalendarioEscolarBean;
...
@RequestScoped
@Path("/v1/tiposcalendarioescolar")
@Tag(name = "TiposCalendarioEscolar API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPO_CALENDARIO_ESCOLAR,
    ConstantesOperaciones.ACTUALIZAR_TIPO_CALENDARIO_ESCOLAR,
    ConstantesOperaciones.ELIMINAR_TIPO_CALENDARIO_ESCOLAR,
    ConstantesOperaciones.AUTENTICADO})
public class TipoCalendarioEscolarRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoCalendarioEscolarRecurso.class.getName());

    @Inject
    private TipoCalendarioEscolarBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTipoCalendarioEscolar
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoCalendarioEscolar.class)))
        ,
    ...
    @Path("/{tipoCalendarioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoCalendarioId") Long id) {
        try {
            SgTipoCalendarioEscolar tce = servicio.obtenerPorId(id);
            if (tce == null) {
     ...
     * @return Response Lista de SgTipoCalendarioEscolar 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgTipoCalendarioEscolar.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPO_CALENDARIO_ESCOLAR})
    public Response crear(SgTipoCalendarioEscolar tce) {
        try {
            tce.setTcePk(null);
            tce.setTceVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tce)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPO_CALENDARIO_ESCOLAR})
    public Response actualizar(@PathParam("tipoCalendarioId") Long id, SgTipoCalendarioEscolar tce) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tce.setTcePk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tce)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPO_CALENDARIO_ESCOLAR})
    public Response eliminarPorId(@PathParam("tipoCalendarioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tipoCalendarioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoCalendarioId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoCalendarioEscolar.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
