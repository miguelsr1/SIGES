...
import sv.gob.mined.siges.negocio.servicios.CategoriaBienesBean;
import sv.gob.mined.siges.negocio.servicios.ConsultaHistoricoBean;
import sv.gob.mined.siges.negocio.servicios.EtniaBean;
import sv.gob.mined.siges.persistencia.entidades.SgAfCategoriaBienes;
import sv.gob.mined.siges.persistencia.entidades.SgAfSeccionCategoria;
import sv.gob.mined.siges.persistencia.utilidades.PersistenceHelper;

@RequestScoped
@Path("/v1/categoriabienes")
@Tag(name = "Clasificación de Bienes API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CATEGORIA_BIENES,
    ConstantesOperaciones.ACTUALIZAR_CATEGORIA_BIENES,
    ConstantesOperaciones.ELIMINAR_CATEGORIA_BIENES,
    ConstantesOperaciones.AUTENTICADO})
public class CategoriaBienesRecurso {
    private static final Logger LOGGER = Logger.getLogger(EtniaBean.class.getName());

    @Inject
    private CategoriaBienesBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAfCategoriaBienes
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAfCategoriaBienes.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("catBienesId") Long id) {
        try {
            SgAfCategoriaBienes etn = servicio.obtenerPorId(id);
            if (etn == null) {
     ...
     * @return Response Lista de SgAfCategoriaBienes 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgAfCategoriaBienes.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroCategoriaBienes filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroCategoriaBienes filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CATEGORIA_BIENES})
    public Response crear(SgAfCategoriaBienes etn) {
        try {
            etn.setCabPk(null);
            etn.setCabVersion(0);
            if (etn.getSgAfSeccionesCategoriaList()!= null) {
                for (SgAfSeccionCategoria op : etn.getSgAfSeccionesCategoriaList()) {
                  op.setScaCategoriaFk(etn);
               }
            }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CATEGORIA_BIENES})
    public Response actualizar(@PathParam("catBienesId") Long id, SgAfCategoriaBienes etn) {
        try {
    ...
    public Response obtenerHistorialPorId(@PathParam("catBienesId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAfCategoriaBienes.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
