...
import sv.gob.mined.siges.negocio.servicios.IpAccesoBean;
import sv.gob.mined.siges.persistencia.entidades.SgIpsAcceso;

@RequestScoped
@Path("/v1/ipsacceso")
@Tag(name = "Ips de Acceso API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_IP_ACCESO,
    ConstantesOperaciones.ACTUALIZAR_IP_ACCESO,
    ConstantesOperaciones.ELIMINAR_IP_ACCESO,
    ConstantesOperaciones.AUTENTICADO})
public class IpsAccesoRecurso {
    private static final Logger LOGGER = Logger.getLogger(IpsAccesoRecurso.class.getName());

    @Inject
    private IpAccesoBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgIpsAcceso.class)))
        ,
    ...
    @Path("/{ipacceso:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("ipacceso") Long id) {
        try {
            SgIpsAcceso etn = servicio.obtenerPorId(id);
            if (etn == null) {
     ...
     * @return Response Lista de SgIpsAcceso 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgIpsAcceso.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroIpAcceso filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroIpAcceso filtro) {
        try {
    ...
    public Response crear(SgIpsAcceso etn) {
        try {
            etn.setIpaPk(null);
            etn.setIpaVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_IP_ACCESO})
    public Response actualizar(@PathParam("ipacceso") Long id, SgIpsAcceso etn) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_IP_ACCESO})
    public Response eliminarPorId(@PathParam("ipacceso") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{ipacceso:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("ipacceso") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgIpsAcceso.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
