...
import sv.gob.mined.siges.negocio.servicios.TiemposComidaDiaBean;
 ...
 */
@RequestScoped
@Path("/v1/tiempocomidadia")
@Tag(name = "Tiempos de comida al día API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIEMPO_COMIDA_DIA,
    ConstantesOperaciones.ACTUALIZAR_TIEMPO_COMIDA_DIA,
    ConstantesOperaciones.ELIMINAR_TIEMPO_COMIDA_DIA})
public class TiemposComidaDiaRecurso {

    private static final Logger LOGGER = Logger.getLogger(TiemposComidaDiaRecurso.class.getName());

    @Inject
    private TiemposComidaDiaBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTiemposComidaDia
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTiemposComidaDia.class)))
        ,
    ...
    @Path("/{tiemposComidaDiaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tiemposComidaDiaId") Long id) {
        try {
            SgTiemposComidaDia tcd = servicio.obtenerPorId(id);
            if (tcd == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tcd).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTiemposComidaDia
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgTiemposComidaDia.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIEMPO_COMIDA_DIA})
    public Response crear(SgTiemposComidaDia tcd) {
        try {
            tcd.setTcdPk(null);
            tcd.setTcdVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tcd)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIEMPO_COMIDA_DIA})
    public Response actualizar(@PathParam("tiemposComidaDiaId") Long id, SgTiemposComidaDia tcd) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tcd.setTcdPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tcd)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIEMPO_COMIDA_DIA})
    public Response eliminarPorId(@PathParam("tiemposComidaDiaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tiemposComidaDiaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tiemposComidaDiaId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTiemposComidaDia.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgTiemposComidaDia.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
