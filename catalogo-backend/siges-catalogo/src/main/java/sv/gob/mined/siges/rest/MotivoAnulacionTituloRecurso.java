...
import sv.gob.mined.siges.negocio.servicios.MotivoAnulacionTituloBean;
 ...
 */
@RequestScoped
@Path("/v1/motivoanulaciontitulo")
@Tag(name = "Motivo de anulación de título API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MOTIVO_ANULACION_TITULO,
    ConstantesOperaciones.ACTUALIZAR_MOTIVO_ANULACION_TITULO,
    ConstantesOperaciones.ELIMINAR_MOTIVO_ANULACION_TITULO})
public class MotivoAnulacionTituloRecurso {

    private static final Logger LOGGER = Logger.getLogger(MotivoAnulacionTituloRecurso.class.getName());

    @Inject
    private MotivoAnulacionTituloBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMotivoAnulacionTitulo
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMotivoAnulacionTitulo.class)))
        ,
    ...
    @Path("/{motivoAnulacionTituloId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("motivoAnulacionTituloId") Long id) {
        try {
            SgMotivoAnulacionTitulo mat = servicio.obtenerPorId(id);
            if (mat == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(mat).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMotivoAnulacionTitulo
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgMotivoAnulacionTitulo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MOTIVO_ANULACION_TITULO})
    public Response crear(SgMotivoAnulacionTitulo mat) {
        try {
            mat.setMatPk(null);
            mat.setMatVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(mat)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MOTIVO_ANULACION_TITULO})
    public Response actualizar(@PathParam("motivoAnulacionTituloId") Long id, SgMotivoAnulacionTitulo mat) {
        try {
            if (servicio.objetoExistePorId(id)) {
                mat.setMatPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(mat)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MOTIVO_ANULACION_TITULO})
    public Response eliminarPorId(@PathParam("motivoAnulacionTituloId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{motivoAnulacionTituloId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("motivoAnulacionTituloId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMotivoAnulacionTitulo.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgMotivoAnulacionTitulo.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
