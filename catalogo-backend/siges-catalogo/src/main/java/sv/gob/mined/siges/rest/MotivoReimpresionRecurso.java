...
import sv.gob.mined.siges.negocio.servicios.MotivoReimpresionBean;
 ...
 */
@RequestScoped
@Path("/v1/motivoreimpresion")
@Tag(name = "Motivo de Reimpresión API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MOTIVO_REIMPRESION,
    ConstantesOperaciones.ACTUALIZAR_MOTIVO_REIMPRESION,
    ConstantesOperaciones.ELIMINAR_MOTIVO_REIMPRESION})
public class MotivoReimpresionRecurso {

    private static final Logger LOGGER = Logger.getLogger(MotivoReimpresionRecurso.class.getName());

    @Inject
    private MotivoReimpresionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMotivoReimpresion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMotivoReimpresion.class)))
        ,
    ...
    @Path("/{motivoReimpresionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("motivoReimpresionId") Long id) {
        try {
            SgMotivoReimpresion mri = servicio.obtenerPorId(id);
            if (mri == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(mri).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMotivoReimpresion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgMotivoReimpresion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MOTIVO_REIMPRESION})
    public Response crear(SgMotivoReimpresion mri) {
        try {
            mri.setMriPk(null);
            mri.setMriVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(mri)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MOTIVO_REIMPRESION})
    public Response actualizar(@PathParam("motivoReimpresionId") Long id, SgMotivoReimpresion mri) {
        try {
            if (servicio.objetoExistePorId(id)) {
                mri.setMriPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(mri)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MOTIVO_REIMPRESION})
    public Response eliminarPorId(@PathParam("motivoReimpresionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{motivoReimpresionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("motivoReimpresionId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMotivoReimpresion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgMotivoReimpresion.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
