...
import sv.gob.mined.siges.negocio.servicios.ElementoHogarBean;
 ...
 */
@RequestScoped
@Path("/v1/elementoshogar")
@Tag(name = "Elementos de hogar API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ELEMENTOS_HOGAR,
    ConstantesOperaciones.ACTUALIZAR_ELEMENTOS_HOGAR,
    ConstantesOperaciones.ELIMINAR_ELEMENTOS_HOGAR})
public class ElementoHogarRecurso {

    private static final Logger LOGGER = Logger.getLogger(ElementoHogarRecurso.class.getName());

    @Inject
    private ElementoHogarBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgElementoHogar
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgElementoHogar.class)))
        ,
    ...
    @Path("/{elementoHogarId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("elementoHogarId") Long id) {
        try {
            SgElementoHogar eho = servicio.obtenerPorId(id);
            if (eho == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(eho).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgElementoHogar
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgElementoHogar.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ELEMENTOS_HOGAR})
    public Response crear(SgElementoHogar eho) {
        try {
            eho.setEhoPk(null);
            eho.setEhoVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(eho)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ELEMENTOS_HOGAR})
    public Response actualizar(@PathParam("elementoHogarId") Long id, SgElementoHogar eho) {
        try {
            if (servicio.objetoExistePorId(id)) {
                eho.setEhoPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(eho)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ELEMENTOS_HOGAR})
    public Response eliminarPorId(@PathParam("elementoHogarId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{elementoHogarId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("elementoHogarId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgElementoHogar.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgElementoHogar.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
