...
import sv.gob.mined.siges.negocio.servicios.GlosarioBean;
...
@RequestScoped
@Path("/v1/glosarios")
@Tag(name = "Glosarios API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_GLOSARIOS,
    ConstantesOperaciones.ACTUALIZAR_GLOSARIOS,
    ConstantesOperaciones.ELIMINAR_GLOSARIOS,
    ConstantesOperaciones.AUTENTICADO})
public class GlosarioRecurso {

    private static final Logger LOGGER = Logger.getLogger(GlosarioRecurso.class.getName());

    @Inject
    private GlosarioBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgGlosario.class)))
        ,
    ...
    @Path("/{glosarioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("glosarioId") Long id) {
        try {
            SgGlosario glo = servicio.obtenerPorId(id);
            if (glo == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(glo).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgGlosario
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgGlosario.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_GLOSARIOS})
    public Response crear(SgGlosario glo) {
        try {
            glo.setGloPk(null);
            glo.setGloVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(glo)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_GLOSARIOS})
    public Response actualizar(@PathParam("glosarioId") Long id, SgGlosario glo) {
        try {
            if (servicio.objetoExistePorId(id)) {
                glo.setGloPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(glo)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_GLOSARIOS})
    public Response eliminarPorId(@PathParam("glosarioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{glosarioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("glosarioId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgGlosario.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgGlosario.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
