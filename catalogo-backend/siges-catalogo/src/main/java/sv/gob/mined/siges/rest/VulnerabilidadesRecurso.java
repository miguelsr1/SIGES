...
import sv.gob.mined.siges.negocio.servicios.VulnerabilidadesBean;
...
@RequestScoped
@Path("/v1/vulnerabilidades")
@Tag(name = "Vulnerabilidades API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_VULNERABILIDAD,
    ConstantesOperaciones.ACTUALIZAR_VULNERABILIDAD,
    ConstantesOperaciones.ELIMINAR_VULNERABILIDAD,
    ConstantesOperaciones.AUTENTICADO})
public class VulnerabilidadesRecurso {
   private static final Logger LOGGER = Logger.getLogger(VulnerabilidadesRecurso.class.getName());

    @Inject
    private VulnerabilidadesBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgVulnerabilidades
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgVulnerabilidades.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("vulId") Long id) {
        try {
            SgVulnerabilidades etn = servicio.obtenerPorId(id);
            if (etn == null) {
     ...
     * @return Response Lista de SgVulnerabilidades 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgVulnerabilidades.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_VULNERABILIDAD})
    public Response crear(SgVulnerabilidades etn) {
        try {
            etn.setVulPk(null);
            etn.setVulVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_VULNERABILIDAD})
    public Response actualizar(@PathParam("vulId") Long id, SgVulnerabilidades etn) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_VULNERABILIDAD})
    public Response eliminarPorId(@PathParam("vulId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{vulId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("vulId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgVulnerabilidades.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
