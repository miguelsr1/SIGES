...
import sv.gob.mined.siges.negocio.servicios.DependenciaEconomicaBean;
...
@RequestScoped
@Path("/v1/dependenciaseconomica")
@Tag(name = "DependenciaEconomica API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_DEPENDENCIA_ECONOMICA,
    ConstantesOperaciones.ACTUALIZAR_DEPENDENCIA_ECONOMICA,
    ConstantesOperaciones.ELIMINAR_DEPENDENCIA_ECONOMICA,
    ConstantesOperaciones.AUTENTICADO})

public class DependenciaEconomicaRecurso {

    private static final Logger LOGGER = Logger.getLogger(DependenciaEconomicaRecurso.class.getName());

    @Inject
    private DependenciaEconomicaBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgDependenciaEconomica
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgDependenciaEconomica.class)))
        ,
    ...
    @Path("/{dependenciaEconomicaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("dependenciaEconomicaId") Long id) {
        try {
            SgDependenciaEconomica dec = servicio.obtenerPorId(id);
            if (dec == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(dec).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgDependenciaEconomica 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgDependenciaEconomica.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DEPENDENCIA_ECONOMICA})
    public Response crear(SgDependenciaEconomica dec) {
        try {
            dec.setDecPk(null);
            dec.setDecVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(dec)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_DEPENDENCIA_ECONOMICA})
    public Response actualizar(@PathParam("dependenciaEconomicaId") Long id, SgDependenciaEconomica dec) {
        try {
            if (servicio.objetoExistePorId(id)) {
                dec.setDecPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(dec)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_DEPENDENCIA_ECONOMICA})
    public Response eliminarPorId(@PathParam("dependenciaEconomicaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{dependenciaEconomicaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("dependenciaEconomicaId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgDependenciaEconomica.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
