...
import sv.gob.mined.siges.negocio.servicios.PreguntaCierreAnioBean;
 ...
 */
@RequestScoped
@Path("/v1/preguntacierreanio")
@Tag(name = "Preguntas de cierre de año API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_PREGUNTA_CIERRE_ANIO,
    ConstantesOperaciones.ACTUALIZAR_PREGUNTA_CIERRE_ANIO,
    ConstantesOperaciones.ELIMINAR_PREGUNTA_CIERRE_ANIO})
public class PreguntaCierreAnioRecurso {

    private static final Logger LOGGER = Logger.getLogger(PreguntaCierreAnioRecurso.class.getName());

    @Inject
    private PreguntaCierreAnioBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgPreguntaCierreAnio
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgPreguntaCierreAnio.class)))
        ,
    ...
    @Path("/{preguntaCierreAnioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("preguntaCierreAnioId") Long id) {
        try {
            SgPreguntaCierreAnio pca = servicio.obtenerPorId(id);
            if (pca == null) {
     ...
     * @return Response Lista de SgPreguntaCierreAnio
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgPreguntaCierreAnio.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_PREGUNTA_CIERRE_ANIO})
    public Response crear(SgPreguntaCierreAnio pca) {
        try {
            pca.setPcaPk(null);
            pca.setPcaVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PREGUNTA_CIERRE_ANIO})
    public Response actualizar(@PathParam("preguntaCierreAnioId") Long id, SgPreguntaCierreAnio pca) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PREGUNTA_CIERRE_ANIO})
    public Response eliminarPorId(@PathParam("preguntaCierreAnioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{preguntaCierreAnioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("preguntaCierreAnioId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgPreguntaCierreAnio.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgPreguntaCierreAnio.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
