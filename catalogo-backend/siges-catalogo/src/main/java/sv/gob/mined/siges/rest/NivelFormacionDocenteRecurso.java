...
import sv.gob.mined.siges.negocio.servicios.NivelFormacionDocenteBean;
...
@RequestScoped
@Path("/v1/nivelesformaciondocente")
@Tag(name = "NivelFormacionDocente API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_NIVELES_FORMACION_DOCENTE,
    ConstantesOperaciones.ACTUALIZAR_NIVELES_FORMACION_DOCENTE,
    ConstantesOperaciones.ELIMINAR_NIVELES_FORMACION_DOCENTE,
    ConstantesOperaciones.AUTENTICADO})
public class NivelFormacionDocenteRecurso {

    private static final Logger LOGGER = Logger.getLogger(NivelFormacionDocenteRecurso.class.getName());
    
    @Inject
    private NivelFormacionDocenteBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgNivelFormacionDocente
     */    
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgNivelFormacionDocente.class)))
        ,
    ...
    @Path("/{nivelFormacionDocenteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("nivelFormacionDocenteId") Long id) {
        try {
            SgNivelFormacionDocente nfd = servicio.obtenerPorId(id);
            if (nfd == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(nfd).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgNivelFormacionDocente 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgNivelFormacionDocente.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_NIVELES_FORMACION_DOCENTE})
    public Response crear(SgNivelFormacionDocente nfd) {
        try {
            nfd.setNfdPk(null);
            nfd.setNfdVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(nfd)).build();
...
/**
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_NIVELES_FORMACION_DOCENTE})
    public Response actualizar(@PathParam("nivelFormacionDocenteId") Long id, SgNivelFormacionDocente nfd) {
        try {
            if (servicio.objetoExistePorId(id)) {
                nfd.setNfdPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(nfd)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_NIVELES_FORMACION_DOCENTE})
    public Response eliminarPorId(@PathParam("nivelFormacionDocenteId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
...
/**
    ...
    @Path("/historial/{nivelFormacionDocenteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("nivelFormacionDocenteId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgNivelFormacionDocente.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
