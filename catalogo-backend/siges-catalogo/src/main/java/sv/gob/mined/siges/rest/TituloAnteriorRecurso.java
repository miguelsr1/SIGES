...
import sv.gob.mined.siges.negocio.servicios.TituloAnteriorBean;
 ...
 */
@RequestScoped
@Path("/v1/tituloanterior")
@Tag(name = "Títulos anteriores a 2008 API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TITULO_ANTERIOR,
    ConstantesOperaciones.ACTUALIZAR_TITULO_ANTERIOR,
    ConstantesOperaciones.ELIMINAR_TITULO_ANTERIOR})
public class TituloAnteriorRecurso {

    private static final Logger LOGGER = Logger.getLogger(TituloAnteriorRecurso.class.getName());

    @Inject
    private TituloAnteriorBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTituloAnterior
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTituloAnterior.class)))
        ,
    ...
    @Path("/{tituloAnteriorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tituloAnteriorId") Long id) {
        try {
            SgTituloAnterior tan = servicio.obtenerPorId(id);
            if (tan == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tan).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTituloAnterior
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgTituloAnterior.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TITULO_ANTERIOR})
    public Response crear(SgTituloAnterior tan) {
        try {
            tan.setTanPk(null);
            tan.setTanVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tan)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TITULO_ANTERIOR})
    public Response actualizar(@PathParam("tituloAnteriorId") Long id, SgTituloAnterior tan) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tan.setTanPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tan)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TITULO_ANTERIOR})
    public Response eliminarPorId(@PathParam("tituloAnteriorId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tituloAnteriorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tituloAnteriorId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTituloAnterior.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgTituloAnterior.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
