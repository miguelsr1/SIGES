...
import sv.gob.mined.siges.negocio.servicios.ImplementadoraBean;
 ...
 */
@RequestScoped
@Path("/v1/implementadoras")
@Tag(name = "Implementadoras API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_IMPLEMENTADORAS,
    ConstantesOperaciones.ACTUALIZAR_IMPLEMENTADORAS,
    ConstantesOperaciones.ELIMINAR_IMPLEMENTADORAS})
public class ImplementadoraRecurso {

    private static final Logger LOGGER = Logger.getLogger(ImplementadoraRecurso.class.getName());

    @Inject
    private ImplementadoraBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgImplementadoras
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgImplementadora.class)))
        ,
    ...
    @Path("/{implementadorasId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("implementadorasId") Long id) {
        try {
            SgImplementadora imp = servicio.obtenerPorId(id);
            if (imp == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(imp).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgImplementadoras
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgImplementadora.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_IMPLEMENTADORAS})
    public Response crear(SgImplementadora imp) {
        try {
            imp.setImpPk(null);
            imp.setImpVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(imp)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_IMPLEMENTADORAS})
    public Response actualizar(@PathParam("implementadorasId") Long id, SgImplementadora imp) {
        try {
            if (servicio.objetoExistePorId(id)) {
                imp.setImpPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(imp)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_IMPLEMENTADORAS})
    public Response eliminarPorId(@PathParam("implementadorasId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{implementadorasId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("implementadorasId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgImplementadora.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgImplementadora.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
