...
import sv.gob.mined.siges.negocio.servicios.UnidadesActivoFijoBean;
...
@RequestScoped
@Path("/v1/unidadesAF")
@Tag(name = "Unidades de Activo Fijo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_UNIDAD_ACTIVO_FIJO,
    ConstantesOperaciones.ACTUALIZAR_UNIDAD_ACTIVO_FIJO,
    ConstantesOperaciones.ELIMINAR_UNIDAD_ACTIVO_FIJO
})
public class UnidadesActivoFijoRecurso {
    
    private static final Logger LOGGER = Logger.getLogger(UnidadesActivoFijoRecurso.class.getName());

    @Inject
    private UnidadesActivoFijoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAfUnidadesActivoFijo
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAfUnidadesActivoFijo.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("unidadId") Long id) {
        try {
            SgAfUnidadesActivoFijo entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgAfUnidadesActivoFijo
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAfUnidadesActivoFijo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroUnidadesActivoFijo filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroUnidadesActivoFijo filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_UNIDAD_ACTIVO_FIJO})
    public Response crear(SgAfUnidadesActivoFijo entity) {
        try {
            entity.setUafPk(null);
            entity.setUafVersion(0);
            if(entity.getSgAfComisionDescargoList() != null) {
                for(SgAfComisionDescargo comision : entity.getSgAfComisionDescargoList()) {
                    comision.setCdeActivoFijoFk(entity);
                }
            }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_UNIDAD_ACTIVO_FIJO})
    public Response actualizar(@PathParam("unidadId") Long id, SgAfUnidadesActivoFijo entity) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response validar(SgAfUnidadesActivoFijo entity) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(UnidadesActivoFijoValidacionNegocio.validar(entity)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_UNIDAD_ACTIVO_FIJO})
    public Response eliminarPorId(@PathParam("unidadId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{unidadId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
     @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("unidadId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAfUnidadesActivoFijo.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
