...
import sv.gob.mined.siges.negocio.servicios.TipoNombramientoBean;
...
@RequestScoped
@Path("/v1/tiponombramiento")
@Tag(name = "TipoNombramiento API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPO_NOMBRAMIENTO,
    ConstantesOperaciones.ACTUALIZAR_TIPO_NOMBRAMIENTO,
    ConstantesOperaciones.ELIMINAR_TIPO_NOMBRAMIENTO,
    ConstantesOperaciones.AUTENTICADO})
public class TipoNombramientoRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoNombramientoRecurso.class.getName());
    
    @Inject
    private TipoNombramientoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTipoNombramiento
     */    
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoNombramiento.class)))
        ,
    ...
    @Path("/{tipoNombramientoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoNombramientoId") Long id) {
        try {
            SgTipoNombramiento ins = servicio.obtenerPorId(id);
            if (ins == null) {
     ...
     * @return Response Lista de SgTipoNombramiento 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgTipoNombramiento.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPO_NOMBRAMIENTO})
    public Response crear(SgTipoNombramiento ins) {
        try {
            ins.setTnoPk(null);
            ins.setTnoVersion(0);
...
/**
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPO_NOMBRAMIENTO})
    public Response actualizar(@PathParam("tipoNombramientoId") Long id, SgTipoNombramiento ins) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPO_NOMBRAMIENTO})
    public Response eliminarPorId(@PathParam("tipoNombramientoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
...
/**
    ...
    @Path("/historial/{tipoNombramientoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoNombramientoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoNombramiento.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
