...
import sv.gob.mined.siges.negocio.servicios.MotivoTrasladoBean;
...
@RequestScoped
@Path("/v1/motivostraslado")
@Tag(name = "MotivoTraslado API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MOTIVO_TRASLADO,
    ConstantesOperaciones.ACTUALIZAR_MOTIVO_TRASLADO,
    ConstantesOperaciones.ELIMINAR_MOTIVO_TRASLADO})
public class MotivoTrasladoRecurso {

    private static final Logger LOGGER = Logger.getLogger(MotivoTrasladoBean.class.getName());

    @Inject
    private MotivoTrasladoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMotivoTraslado
     */
    @GET
    ...
    @Path("/{motivoTrasladoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("motivoTrasladoId") Long id) {
        try {
            SgMotivoTraslado mot = servicio.obtenerPorId(id);
            if (mot == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(mot).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMotivoTraslado 
     */
    @POST
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MOTIVO_TRASLADO})
    public Response crear(SgMotivoTraslado mot) {
        try {
            mot.setMotPk(null);
            mot.setMotVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(mot)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MOTIVO_TRASLADO})
    public Response actualizar(@PathParam("motivoTrasladoId") Long id, SgMotivoTraslado mot) {
        try {
            if (servicio.objetoExistePorId(id)) {
                mot.setMotPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(mot)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MOTIVO_TRASLADO})
    public Response eliminarPorId(@PathParam("motivoTrasladoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{motivoTrasladoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("motivoTrasladoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMotivoTraslado.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
