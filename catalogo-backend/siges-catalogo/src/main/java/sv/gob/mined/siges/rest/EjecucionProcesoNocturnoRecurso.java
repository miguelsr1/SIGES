...
import sv.gob.mined.siges.negocio.servicios.EjecucionProcesoNocturnoBean;
 ...
 */
@RequestScoped
@Path("/v1/ejecucionprocesonocturno")
@Tag(name = "Ejecución Procesos nocturnos API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_EJECUCION_PROCESO_NOCTURNO,
    ConstantesOperaciones.ACTUALIZAR_EJECUCION_PROCESO_NOCTURNO,
    ConstantesOperaciones.ELIMINAR_EJECUCION_PROCESO_NOCTURNO})
public class EjecucionProcesoNocturnoRecurso {

    private static final Logger LOGGER = Logger.getLogger(EjecucionProcesoNocturnoRecurso.class.getName());

    @Inject
    private EjecucionProcesoNocturnoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgEjecucionProcesoNocturno
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEjecucionProcesoNocturno.class)))
        ,
    ...
    @Path("/{ejecucionProcesoNocturnoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("ejecucionProcesoNocturnoId") Long id) {
        try {
            SgEjecucionProcesoNocturno epr = servicio.obtenerPorId(id);
            if (epr == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(epr).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgEjecucionProcesoNocturno
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgEjecucionProcesoNocturno.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_EJECUCION_PROCESO_NOCTURNO})
    public Response crear(SgEjecucionProcesoNocturno epr) {
        try {
            epr.setEprPk(null);
            epr.setEprVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(epr)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_EJECUCION_PROCESO_NOCTURNO})
    public Response actualizar(@PathParam("ejecucionProcesoNocturnoId") Long id, SgEjecucionProcesoNocturno epr) {
        try {
            if (servicio.objetoExistePorId(id)) {
                epr.setEprPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(epr)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_EJECUCION_PROCESO_NOCTURNO})
    public Response eliminarPorId(@PathParam("ejecucionProcesoNocturnoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{ejecucionProcesoNocturnoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("ejecucionProcesoNocturnoId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgEjecucionProcesoNocturno.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgEjecucionProcesoNocturno.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
