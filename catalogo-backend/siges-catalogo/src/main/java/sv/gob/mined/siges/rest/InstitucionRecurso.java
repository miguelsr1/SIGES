...
import sv.gob.mined.siges.negocio.servicios.InstitucionBean;
...
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_INSTITUCION,
    ConstantesOperaciones.ACTUALIZAR_INSTITUCION,
    ConstantesOperaciones.ELIMINAR_INSTITUCION,
    ConstantesOperaciones.AUTENTICADO})
public class InstitucionRecurso {

    private static final Logger LOGGER = Logger.getLogger(InstitucionBean.class.getName());
    
    @Inject
    private InstitucionBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgInstitucion.class)))
        ,
    ...
    @Path("/{institucionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("institucionId") Long id) {
        try {
            SgInstitucion ins = servicio.obtenerPorId(id);
            if (ins == null) {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgInstitucion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_INSTITUCION})
    public Response crear(SgInstitucion ins) {
        try {
            ins.setInsPk(null);
            ins.setInsVersion(0);
...
/**
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_INSTITUCION})
    public Response actualizar(@PathParam("institucionId") Long id, SgInstitucion ins) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_INSTITUCION})
    public Response eliminarPorId(@PathParam("institucionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
...
/**
    ...
    @Path("/historial/{institucionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("institucionId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgInstitucion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
