...
import sv.gob.mined.siges.negocio.servicios.ModoPagoBean;
...
@RequestScoped
@Path("/v1/modopago")
@Tag(name = "ModoPago API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MODO_PAGO,
    ConstantesOperaciones.ACTUALIZAR_MODO_PAGO,
    ConstantesOperaciones.ELIMINAR_MODO_PAGO,
    ConstantesOperaciones.AUTENTICADO})
public class ModoPagoRecurso {

    private static final Logger LOGGER = Logger.getLogger(ModoPagoRecurso.class.getName());
    
    @Inject
    private ModoPagoBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgModoPago.class)))
        ,
    ...
    @Path("/{modoPagoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("modoPagoId") Long id) {
        try {
            SgModoPago ins = servicio.obtenerPorId(id);
            if (ins == null) {
     ...
     * @return Response Lista de SgModoPago 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgModoPago.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MODO_PAGO})
    public Response crear(SgModoPago ins) {
        try {
            ins.setMpaPk(null);
            ins.setMpaVersion(0);
...
/**
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MODO_PAGO})
    public Response actualizar(@PathParam("modoPagoId") Long id, SgModoPago ins) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MODO_PAGO})
    public Response eliminarPorId(@PathParam("modoPagoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
...
/**
    ...
    @Path("/historial/{modoPagoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("modoPagoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgModoPago.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
