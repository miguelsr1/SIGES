...
import sv.gob.mined.siges.negocio.servicios.InfTipoAbastecimientoBean;
 ...
 */
@RequestScoped
@Path("/v1/tipoabastecimiento")
@Tag(name = "Tipo Abastecimiento API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPO_ABASTECIMIENTO,
    ConstantesOperaciones.ACTUALIZAR_TIPO_ABASTECIMIENTO,
    ConstantesOperaciones.ELIMINAR_TIPO_ABASTECIMIENTO})
public class InfTipoAbastecimientoRecurso {

    private static final Logger LOGGER = Logger.getLogger(InfTipoAbastecimientoRecurso.class.getName());

    @Inject
    private InfTipoAbastecimientoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgInfTipoAbastecimiento
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgInfTipoAbastecimiento.class)))
        ,
    ...
    @Path("/{infTipoAbastecimientoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("infTipoAbastecimientoId") Long id) {
        try {
            SgInfTipoAbastecimiento tab = servicio.obtenerPorId(id);
            if (tab == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tab).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgInfTipoAbastecimiento
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgInfTipoAbastecimiento.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPO_ABASTECIMIENTO})
    public Response crear(SgInfTipoAbastecimiento tab) {
        try {
            tab.setTabPk(null);
            tab.setTabVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tab)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPO_ABASTECIMIENTO})
    public Response actualizar(@PathParam("infTipoAbastecimientoId") Long id, SgInfTipoAbastecimiento tab) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tab.setTabPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tab)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPO_ABASTECIMIENTO})
    public Response eliminarPorId(@PathParam("infTipoAbastecimientoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{infTipoAbastecimientoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("infTipoAbastecimientoId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgInfTipoAbastecimiento.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgInfTipoAbastecimiento.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
