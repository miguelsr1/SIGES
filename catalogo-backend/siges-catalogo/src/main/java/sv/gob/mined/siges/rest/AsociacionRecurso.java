...
import sv.gob.mined.siges.negocio.servicios.AsociacionBean;
 ...
 */
@RequestScoped
@Path("/v1/asociaciones")
@Tag(name = "Asociacion API V1")
...
public class AsociacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(AsociacionRecurso.class.getName());

    @Inject
    private AsociacionBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAsociacion.class)))
        ,
    ...
    @Path("/{asociacionesId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("asociacionesId") Long id) {
        try {
            SgAsociacion aso = servicio.obtenerPorId(id);
            if (aso == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(aso).build();
            }
        } catch (Exception ex) {
     ...
     *
     * @param filtro FiltroAsociacion
     * @return Response Lista de SgAsociacion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAsociacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroAsociacion filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroAsociacion filtro) {
        try {
    ...
    public Response crear(SgAsociacion aso) {
        try {
            aso.setAsoPk(null);
            aso.setAsoVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(aso)).build();
    ...
    public Response actualizar(@PathParam("asociacionesId") Long id, SgAsociacion aso) {
        try {
            if (servicio.objetoExistePorId(id)) {
                aso.setAsoPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(aso)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    public Response eliminarPorId(@PathParam("asociacionesId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{asociacionesId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("asociacionesId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAsociacion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgAsociacion.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
