...
import sv.gob.mined.siges.negocio.servicios.ProgramaEducativoBean;
...
@RequestScoped
@Path("/v1/programaseducativos")
@Tag(name = "ProgramaEducativo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_PROGRAMAS_EDUCATIVOS,
    ConstantesOperaciones.ACTUALIZAR_PROGRAMAS_EDUCATIVOS,
    ConstantesOperaciones.ELIMINAR_PROGRAMAS_EDUCATIVOS})
public class ProgramaEducativoRecurso {

    private static final Logger LOGGER = Logger.getLogger(ProgramaEducativoRecurso.class.getName());

    @Inject
    private ProgramaEducativoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgProgramaEducativo
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgProgramaEducativo.class)))
        ,
    ...
    @Path("/{programaEducativoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("programaEducativoId") Long id) {
        try {
            SgProgramaEducativo ped = servicio.obtenerPorId(id);
            if (ped == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(ped).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgProgramaEducativo 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgProgramaEducativo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_PROGRAMAS_EDUCATIVOS})
    public Response crear(SgProgramaEducativo ped) {
        try {
            ped.setPedPk(null);
            ped.setPedVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ped)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PROGRAMAS_EDUCATIVOS})
    public Response actualizar(@PathParam("programaEducativoId") Long id, SgProgramaEducativo ped) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ped.setPedPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ped)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PROGRAMAS_EDUCATIVOS})
    public Response eliminarPorId(@PathParam("programaEducativoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{programaEducativoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("programaEducativoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgProgramaEducativo.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
