...
import sv.gob.mined.siges.negocio.servicios.EstadoProcesoLegalizacionBean;
 ...
 */
@RequestScoped
@Path("/v1/estadoprocesolegalizacion")
@Tag(name = "Estados proceso legalizacion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ESTADO_PROCESO_LEGALIZACION,
    ConstantesOperaciones.ACTUALIZAR_ESTADO_PROCESO_LEGALIZACION,
    ConstantesOperaciones.ELIMINAR_ESTADO_PROCESO_LEGALIZACION})
public class EstadoProcesoLegalizacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(EstadoProcesoLegalizacionRecurso.class.getName());

    @Inject
    private EstadoProcesoLegalizacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgEstadoProcesoLegalizacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEstadoProcesoLegalizacion.class)))
        ,
    ...
    @Path("/{estadoProcesoLegalizacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("estadoProcesoLegalizacionId") Long id) {
        try {
            SgEstadoProcesoLegalizacion epl = servicio.obtenerPorId(id);
            if (epl == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(epl).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgEstadoProcesoLegalizacion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgEstadoProcesoLegalizacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroEstadoProcesoLegalizacion filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroEstadoProcesoLegalizacion filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ESTADO_PROCESO_LEGALIZACION})
    public Response crear(SgEstadoProcesoLegalizacion epl) {
        try {
            epl.setEplPk(null);
            epl.setEplVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(epl)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ESTADO_PROCESO_LEGALIZACION})
    public Response actualizar(@PathParam("estadoProcesoLegalizacionId") Long id, SgEstadoProcesoLegalizacion epl) {
        try {
            if (servicio.objetoExistePorId(id)) {
                epl.setEplPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(epl)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ESTADO_PROCESO_LEGALIZACION})
    public Response eliminarPorId(@PathParam("estadoProcesoLegalizacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{estadoProcesoLegalizacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("estadoProcesoLegalizacionId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgEstadoProcesoLegalizacion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgEstadoProcesoLegalizacion.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
