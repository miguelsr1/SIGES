...
import sv.gob.mined.siges.negocio.servicios.AlcanceCapacitacionBean;
...
@RequestScoped
@Path("/v1/alcancescapacitacion")
@Tag(name = "AlcanceCapacitacion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ALCANCE_CAPACITACION,
    ConstantesOperaciones.ACTUALIZAR_ALCANCE_CAPACITACION,
    ConstantesOperaciones.ELIMINAR_ALCANCE_CAPACITACION,
    ConstantesOperaciones.AUTENTICADO})
public class AlcanceCapacitacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(AlcanceCapacitacionRecurso.class.getName());

    @Inject
    private AlcanceCapacitacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAlcanceCapacitacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAlcanceCapacitacion.class)))
        ,
    ...
    @Path("/{alcanceCapacitacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("alcanceCapacitacionId") Long id) {
        try {
            SgAlcanceCapacitacion aca = servicio.obtenerPorId(id);
            if (aca == null) {
     ...
     * @return Response Lista de SgAlcanceCapacitacion 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgAlcanceCapacitacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ALCANCE_CAPACITACION})
    public Response crear(SgAlcanceCapacitacion aca) {
        try {
            aca.setAcaPk(null);
            aca.setAcaVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ALCANCE_CAPACITACION})
    public Response actualizar(@PathParam("alcanceCapacitacionId") Long id, SgAlcanceCapacitacion aca) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ALCANCE_CAPACITACION})
    public Response eliminarPorId(@PathParam("alcanceCapacitacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{alcanceCapacitacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("alcanceCapacitacionId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAlcanceCapacitacion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
