...
import sv.gob.mined.siges.negocio.servicios.EstadosInventarioBean;
...
@RequestScoped
@Path("/v1/estadosinventario")
@Tag(name = "Estados Inventario API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ESTADO_INVENTARIO,
    ConstantesOperaciones.ACTUALIZAR_ESTADO_INVENTARIO,
    ConstantesOperaciones.ELIMINAR_ESTADO_INVENTARIO,
    ConstantesOperaciones.AUTENTICADO})
public class EstadosInventarioRecurso {
    
    private static final Logger LOGGER = Logger.getLogger(EstadosInventarioRecurso.class.getName());

    @Inject
    private EstadosInventarioBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAfEstadosInventario
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAfEstadosInventario.class)))
        ,
    ...
    @Path("/{estadoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("estadoId") Long id) {
        try {
            SgAfEstadosInventario etn = servicio.obtenerPorId(id);
            if (etn == null) {
     ...
     * @return Response Lista de SgAfEstadosInventario 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgAfEstadosInventario.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ESTADO_INVENTARIO})
    public Response crear(SgAfEstadosInventario etn) {
        try {
            etn.setEinPk(null);
            etn.setEinVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ESTADO_INVENTARIO})
    public Response actualizar(@PathParam("estadoId") Long id, SgAfEstadosInventario etn) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ESTADO_INVENTARIO})
    public Response eliminarPorId(@PathParam("estadoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{estadoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("estadoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAfEstadosInventario.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
