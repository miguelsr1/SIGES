...
import sv.gob.mined.siges.negocio.servicios.PerfilesUsuariosIngresadosCeBean;
 ...
 */
@RequestScoped
@Path("/v1/perfilesdeusuariosingresadosce")
@Tag(name = "Perfiles de Usuarios Ingresados por el CE API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_PERFILES_USUARIOS_INGRESADOS_CE,
    ConstantesOperaciones.ACTUALIZAR_PERFILES_USUARIOS_INGRESADOS_CE,
    ConstantesOperaciones.ELIMINAR_PERFILES_USUARIOS_INGRESADOS_CE,
	ConstantesOperaciones.BUSCAR_PERFILES_USUARIOS_INGRESADOS_CE})
public class PerfilesUsuariosIngresadosCeRecurso {

    private static final Logger LOGGER = Logger.getLogger(PerfilesUsuariosIngresadosCeRecurso.class.getName());

    @Inject
    private PerfilesUsuariosIngresadosCeBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgPerfilesUsuariosIngresadosCe
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgPerfilesUsuariosIngresadosCe.class)))
        ,
    ...
    @Path("/{perfilesUsuariosIngresadosCeId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PERFILES_USUARIOS_INGRESADOS_CE})
    public Response obtenerPorId(@PathParam("perfilesUsuariosIngresadosCeId") Long id) {
        try {
            SgPerfilesUsuariosIngresadosCe pui = servicio.obtenerPorId(id);
            if (pui == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(pui).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgPerfilesUsuariosIngresadosCe
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgPerfilesUsuariosIngresadosCe.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_PERFILES_USUARIOS_INGRESADOS_CE})
    public Response crear(SgPerfilesUsuariosIngresadosCe pui) {
        try {
            pui.setPuiPk(null);
            pui.setPuiVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(pui)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PERFILES_USUARIOS_INGRESADOS_CE})
    public Response actualizar(@PathParam("perfilesUsuariosIngresadosCeId") Long id, SgPerfilesUsuariosIngresadosCe pui) {
        try {
            if (servicio.objetoExistePorId(id)) {
                pui.setPuiPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(pui)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PERFILES_USUARIOS_INGRESADOS_CE})
    public Response eliminarPorId(@PathParam("perfilesUsuariosIngresadosCeId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{perfilesUsuariosIngresadosCeId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("perfilesUsuariosIngresadosCeId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgPerfilesUsuariosIngresadosCe.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgPerfilesUsuariosIngresadosCe.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
