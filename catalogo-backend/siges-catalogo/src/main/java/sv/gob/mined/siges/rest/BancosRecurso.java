...
import sv.gob.mined.siges.negocio.servicios.BancosBean;
...
@RequestScoped
@Path("/v1/bancos")
@Tag(name = "Bancos API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CATALOGO_BANCOS,
    ConstantesOperaciones.ACTUALIZAR_CATALOGO_BANCOS,
    ConstantesOperaciones.ELIMINAR_CATALOGO_BANCOS})
public class BancosRecurso {

    private static final Logger LOGGER = Logger.getLogger(BancosRecurso.class.getName());

    @Inject
    private BancosBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgBancos.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("cuentasBancariasId") Long id) {
        try {
            SgBancos bnc = servicio.obtenerPorId(id);
            if (bnc == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(bnc).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgBancos
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgBancos.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CATALOGO_BANCOS})
    public Response crear(SgBancos bnc) {
        try {
            bnc.setBncPk(null);
            bnc.setBncVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(bnc)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CATALOGO_BANCOS})
    public Response actualizar(@PathParam("cuentasBancariasId") Long id, SgBancos bnc) {
        try {
            if (servicio.objetoExistePorId(id)) {
                bnc.setBncPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(bnc)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CATALOGO_BANCOS})
    ...
    public Response obtenerHistorialPorId(@PathParam("cuentasBancariasId") Long id) {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgBancos.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgBancos.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
