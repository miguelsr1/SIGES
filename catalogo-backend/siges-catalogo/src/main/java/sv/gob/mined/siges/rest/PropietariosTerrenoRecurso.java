...
import sv.gob.mined.siges.negocio.servicios.PropietariosTerrenoBean;
 ...
 */
@RequestScoped
@Path("/v1/propietarioterreno")
@Tag(name = "Propietarios de terreno API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_PROPIETARIO_TERRENO,
    ConstantesOperaciones.ACTUALIZAR_PROPIETARIO_TERRENO,
    ConstantesOperaciones.ELIMINAR_PROPIETARIO_TERRENO})
public class PropietariosTerrenoRecurso {

    private static final Logger LOGGER = Logger.getLogger(PropietariosTerrenoRecurso.class.getName());

    @Inject
    private PropietariosTerrenoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgPropietariosTerreno
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgPropietariosTerreno.class)))
        ,
    ...
    @Path("/{propietariosTerrenoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("propietariosTerrenoId") Long id) {
        try {
            SgPropietariosTerreno pdt = servicio.obtenerPorId(id);
            if (pdt == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(pdt).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgPropietariosTerreno
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgPropietariosTerreno.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_PROPIETARIO_TERRENO})
    public Response crear(SgPropietariosTerreno pdt) {
        try {
            pdt.setPdtPk(null);
            pdt.setPdtVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(pdt)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PROPIETARIO_TERRENO})
    public Response actualizar(@PathParam("propietariosTerrenoId") Long id, SgPropietariosTerreno pdt) {
        try {
            if (servicio.objetoExistePorId(id)) {
                pdt.setPdtPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(pdt)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PROPIETARIO_TERRENO})
    public Response eliminarPorId(@PathParam("propietariosTerrenoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{propietariosTerrenoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("propietariosTerrenoId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgPropietariosTerreno.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgPropietariosTerreno.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
