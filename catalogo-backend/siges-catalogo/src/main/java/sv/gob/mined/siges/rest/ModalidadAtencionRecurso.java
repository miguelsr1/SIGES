...
import sv.gob.mined.siges.negocio.servicios.ModalidadAtencionBean;
...
@RequestScoped
@Path("/v1/modalidadesatencion")
@Tag(name = "ModalidadAtencion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MODALIDADES_ATENCION,
    ConstantesOperaciones.ACTUALIZAR_MODALIDADES_ATENCION,
    ConstantesOperaciones.ELIMINAR_MODALIDADES_ATENCION,
    ConstantesOperaciones.AUTENTICADO})
public class ModalidadAtencionRecurso {

    private static final Logger LOGGER = Logger.getLogger(ModalidadAtencionRecurso.class.getName());

    @Inject
    private ModalidadAtencionBean servicio;
     ...
     *
     * @param id
     * @return Response SgModalidadAtencion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgModalidadAtencion.class)))
        ,
    ...
    @Path("/{modalidadAtencionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("modalidadAtencionId") Long id) {
        try {
            SgModalidadAtencion mat = servicio.obtenerPorId(id);
            if (mat == null) {
     ...
     * @return Response Lista de SgModalidadAtencion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgModalidadAtencion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroModalidadAtencion filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroModalidadAtencion filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MODALIDADES_ATENCION})
    public Response crear(SgModalidadAtencion mat) {
        try {
            mat.setMatPk(null);
            mat.setMatVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MODALIDADES_ATENCION})
    public Response actualizar(@PathParam("modalidadAtencionId") Long id, SgModalidadAtencion mat) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MODALIDADES_ATENCION})
    public Response eliminarPorId(@PathParam("modalidadAtencionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{modalidadAtencionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("modalidadAtencionId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgModalidadAtencion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
