...
import sv.gob.mined.siges.negocio.servicios.TipoOrganismoAdministrativoBean;
 ...
 */
@RequestScoped
@Path("/v1/tiposorganismoadministrativo")
@Tag(name = "Tipos de Organismo Administrativo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPOS_ORGANISMO_ADMINISTRATIVO,
    ConstantesOperaciones.ACTUALIZAR_TIPOS_ORGANISMO_ADMINISTRATIVO,
    ConstantesOperaciones.ELIMINAR_TIPOS_ORGANISMO_ADMINISTRATIVO})
public class TipoOrganismoAdministrativoRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoOrganismoAdministrativoRecurso.class.getName());

    @Inject
    private TipoOrganismoAdministrativoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTipoOrganismoAdministrativo
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoOrganismoAdministrativo.class)))
        ,
    ...
    @Path("/{tipoOrganismoAdministrativoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoOrganismoAdministrativoId") Long id) {
        try {
            SgTipoOrganismoAdministrativo toa = servicio.obtenerPorId(id);
            if (toa == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(toa).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTipoOrganismoAdministrativo
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgTipoOrganismoAdministrativo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPOS_ORGANISMO_ADMINISTRATIVO})
    public Response crear(SgTipoOrganismoAdministrativo toa) {
        try {
            toa.setToaPk(null);
            toa.setToaVersion(0);
            if(toa.getToaItems()!=null){
                for(SgItemEvaluarOrganismo item : toa.getToaItems()){
                    item.setIeoTipoOrganismo(toa);
                }
            }
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(toa)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPOS_ORGANISMO_ADMINISTRATIVO})
    public Response actualizar(@PathParam("tipoOrganismoAdministrativoId") Long id, SgTipoOrganismoAdministrativo toa) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ...
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(toa)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPOS_ORGANISMO_ADMINISTRATIVO})
    public Response eliminarPorId(@PathParam("tipoOrganismoAdministrativoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tipoOrganismoAdministrativoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoOrganismoAdministrativoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoOrganismoAdministrativo.class, id))).build();         
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgTipoOrganismoAdministrativo.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
