...
import sv.gob.mined.siges.negocio.servicios.CategoriaFormacionDocenteBean;
...
@RequestScoped
@Path("/v1/categoriasformaciondocente")
@Tag(name = "CategoriaFormacionDocente API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CATEGORIAS_FORMACION_DOCENTE,
    ConstantesOperaciones.ACTUALIZAR_CATEGORIAS_FORMACION_DOCENTE,
    ConstantesOperaciones.ELIMINAR_CATEGORIAS_FORMACION_DOCENTE,
    ConstantesOperaciones.AUTENTICADO})
public class CategoriaFormacionDocenteRecurso {

    private static final Logger LOGGER = Logger.getLogger(CategoriaFormacionDocenteRecurso.class.getName());
    
    @Inject
    private CategoriaFormacionDocenteBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgCategoriaFormacionDocente
     */    
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCategoriaFormacionDocente.class)))
        ,
    ...
    @Path("/{categoriaFormacionDocenteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("categoriaFormacionDocenteId") Long id) {
        try {
            SgCategoriaFormacionDocente cfd = servicio.obtenerPorId(id);
            if (cfd == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(cfd).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgCategoriaFormacionDocente 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgCategoriaFormacionDocente.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CATEGORIAS_FORMACION_DOCENTE})
    public Response crear(SgCategoriaFormacionDocente cfd) {
        try {
            cfd.setCfdPk(null);
            cfd.setCfdVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(cfd)).build();
...
/**
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CATEGORIAS_FORMACION_DOCENTE})
    public Response actualizar(@PathParam("categoriaFormacionDocenteId") Long id, SgCategoriaFormacionDocente cfd) {
        try {
            if (servicio.objetoExistePorId(id)) {
                cfd.setCfdPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(cfd)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CATEGORIAS_FORMACION_DOCENTE})
    public Response eliminarPorId(@PathParam("categoriaFormacionDocenteId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
...
/**
    ...
    @Path("/historial/{categoriaFormacionDocenteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("categoriaFormacionDocenteId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgCategoriaFormacionDocente.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
