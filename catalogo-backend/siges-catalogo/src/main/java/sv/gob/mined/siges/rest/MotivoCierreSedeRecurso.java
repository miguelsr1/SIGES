...
import sv.gob.mined.siges.negocio.servicios.MotivoCierreSedeBean;
 ...
 */
@RequestScoped
@Path("/v1/motivoscierresede")
@Tag(name = "Motivos de cierre de sede API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MOTIVOS_CIERRE_SEDE,
    ConstantesOperaciones.ACTUALIZAR_MOTIVOS_CIERRE_SEDE,
    ConstantesOperaciones.ELIMINAR_MOTIVOS_CIERRE_SEDE
})
public class MotivoCierreSedeRecurso {

    private static final Logger LOGGER = Logger.getLogger(MotivoCierreSedeRecurso.class.getName());

    @Inject
    private MotivoCierreSedeBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMotivoCierreSede
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMotivoCierreSede.class)))
        ,
    ...
    @Path("/{motivoCierreSedeId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("motivoCierreSedeId") Long id) {
        try {
            SgMotivoCierreSede mcs = servicio.obtenerPorId(id);
            if (mcs == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(mcs).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMotivoCierreSede
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgMotivoCierreSede.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MOTIVOS_CIERRE_SEDE})
    public Response crear(SgMotivoCierreSede mcs) {
        try {
            mcs.setMcsPk(null);
            mcs.setMcsVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(mcs)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MOTIVOS_CIERRE_SEDE})
    public Response actualizar(@PathParam("motivoCierreSedeId") Long id, SgMotivoCierreSede mcs) {
        try {
            if (servicio.objetoExistePorId(id)) {
                mcs.setMcsPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(mcs)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MOTIVOS_CIERRE_SEDE})
    public Response eliminarPorId(@PathParam("motivoCierreSedeId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{motivoCierreSedeId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("motivoCierreSedeId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMotivoCierreSede.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgMotivoCierreSede.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
