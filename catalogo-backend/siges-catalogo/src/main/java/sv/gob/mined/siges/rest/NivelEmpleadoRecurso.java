...
import sv.gob.mined.siges.negocio.servicios.NivelEmpleadoBean;
...
@RequestScoped
@Path("/v1/nivelempleado")
@Tag(name = "NivelEmpleado API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_NIVEL_EMPLEADO,
    ConstantesOperaciones.ACTUALIZAR_NIVEL_EMPLEADO,
    ConstantesOperaciones.ELIMINAR_NIVEL_EMPLEADO,
    ConstantesOperaciones.AUTENTICADO})
public class NivelEmpleadoRecurso {

    private static final Logger LOGGER = Logger.getLogger(NivelEmpleadoRecurso.class.getName());

    @Inject
    private NivelEmpleadoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgNivelEmpleado
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgNivelEmpleado.class)))
        ,
    ...
    @Path("/{nivelEmpleadoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("nivelEmpleadoId") Long id) {
        try {
            SgNivelEmpleado etn = servicio.obtenerPorId(id);
            if (etn == null) {
     ...
     * @return Response Lista de SgNivelEmpleado 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgNivelEmpleado.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_NIVEL_EMPLEADO})
    public Response crear(SgNivelEmpleado etn) {
        try {
            etn.setNemPk(null);
            etn.setNemVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_NIVEL_EMPLEADO})
    public Response actualizar(@PathParam("nivelEmpleadoId") Long id, SgNivelEmpleado etn) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_NIVEL_EMPLEADO})
    public Response eliminarPorId(@PathParam("nivelEmpleadoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{nivelEmpleadoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("nivelEmpleadoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgNivelEmpleado.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
