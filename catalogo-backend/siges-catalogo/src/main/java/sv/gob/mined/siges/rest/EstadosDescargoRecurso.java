...
import sv.gob.mined.siges.negocio.servicios.EstadosDescargoBean;
...
@RequestScoped
@Path("/v1/estadosdescargo")
@Tag(name = "Estados Descargo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ESTADO_DESCARGO,
    ConstantesOperaciones.ACTUALIZAR_ESTADO_DESCARGO,
    ConstantesOperaciones.ELIMINAR_ESTADO_DESCARGO,
    ConstantesOperaciones.AUTENTICADO})
public class EstadosDescargoRecurso {
    private static final Logger LOGGER = Logger.getLogger(EstadosDescargoRecurso.class.getName());

    @Inject
    private EstadosDescargoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAfEstadosDescargo
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAfEstadosDescargo.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("estadoId") Long id) {
        try {
            SgAfEstadosDescargo etn = servicio.obtenerPorId(id);
            if (etn == null) {
     ...
     * @return Response Lista de SgAfEstadosDescargo 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgAfEstadosDescargo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ESTADO_DESCARGO})
    public Response crear(SgAfEstadosDescargo etn) {
        try {
            etn.setEdePk(null);
            etn.setEdeVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ESTADO_DESCARGO})
    public Response actualizar(@PathParam("estadoId") Long id, SgAfEstadosDescargo etn) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ESTADO_DESCARGO})
    ...
    public Response obtenerHistorialPorId(@PathParam("estadoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAfEstadosDescargo.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
