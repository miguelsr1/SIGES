...
import sv.gob.mined.siges.negocio.servicios.AyudaBean;
...
public class AyudaRecurso {
    ...
    private static final Logger LOGGER = Logger.getLogger(AyudaRecurso.class.getName());

    @Inject
    private AyudaBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAyuda.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("ayudaId") Long id) {
        try {
            SgAyuda ayu = servicio.obtenerPorId(id);
            if (ayu == null) {
     ...
     * @return Response Lista de SgAyuda
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAyuda.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    public Response crear(SgAyuda ayu) {
        try {
            ayu.setAyuPk(null);
            ayu.setAyuVersion(0);
    ...
    public Response actualizar(@PathParam("ayudaId") Long id, SgAyuda ayu) {
        try {
    ...
    @Path("/historial/{ayudaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("ayudaId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAyuda.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
