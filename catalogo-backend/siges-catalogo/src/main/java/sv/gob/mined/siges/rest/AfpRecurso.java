...
import sv.gob.mined.siges.negocio.servicios.AfpBean;
...
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_AFP,
    ConstantesOperaciones.ACTUALIZAR_AFP,
    ConstantesOperaciones.ELIMINAR_AFP,
    ConstantesOperaciones.AUTENTICADO})
public class AfpRecurso {

    private static final Logger LOGGER = Logger.getLogger(AfpRecurso.class.getName());

    @Inject
    private AfpBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAfp.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("afpId") Long id) {
        try {
            SgAfp ayu = servicio.obtenerPorId(id);
            if (ayu == null) {
     ...
     * @return Response Lista de SgAfp 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgAfp.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_AFP})
    public Response crear(SgAfp ayu) {
        try {
            ayu.setAfpPk(null);
            ayu.setAfpVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_AFP})
    public Response actualizar(@PathParam("afpId") Long id, SgAfp ayu) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_AFP})
    public Response eliminarPorId(@PathParam("afpId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{afpId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("afpId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAfp.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
