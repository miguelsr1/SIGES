...
import sv.gob.mined.siges.negocio.servicios.TipoVacunaBean;
...
@RequestScoped
@Path("/v1/tiposvacuna")
@Tag(name = "TiposVacuna API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPOS_VACUNA,
    ConstantesOperaciones.ACTUALIZAR_TIPOS_VACUNA,
    ConstantesOperaciones.ELIMINAR_TIPOS_VACUNA,
    ConstantesOperaciones.AUTENTICADO})
public class TipoVacunaRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoVacunaRecurso.class.getName());

    @Inject
    private TipoVacunaBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoVacuna.class)))
        ,
    ...
    @Path("/{tipoVacunaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoVacunaId") Long id) {
        try {
            SgTipoVacuna tvc = servicio.obtenerPorId(id);
            if (tvc == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tvc).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTipoVacuna 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgTipoVacuna.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPOS_VACUNA})
    public Response crear(SgTipoVacuna tvc) {
        try {
            tvc.setTvcPk(null);
            tvc.setTvcVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tvc)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPOS_VACUNA})
    public Response actualizar(@PathParam("tipoVacunaId") Long id, SgTipoVacuna tvc) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tvc.setTvcPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tvc)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPOS_VACUNA})
    public Response eliminarPorId(@PathParam("tipoVacunaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tipoVacunaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoVacunaId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoVacuna.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
