...
import sv.gob.mined.siges.negocio.servicios.PaisBean;
...
@RequestScoped
@Path("/v1/paises")
@Tag(name = "Pais API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_PAIS,
    ConstantesOperaciones.ACTUALIZAR_PAIS,
    ConstantesOperaciones.ELIMINAR_PAIS,
    ConstantesOperaciones.AUTENTICADO})
public class PaisRecurso {
    private static final Logger LOGGER = Logger.getLogger(PaisRecurso.class.getName());

    @Inject
    private PaisBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgPais.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("paisId") Long id) {
        try {
            SgPais pai = servicio.obtenerPorId(id);
            if (pai == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(pai).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgPais 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgPais.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_PAIS})
    public Response crear(SgPais pai) {
        try {
            pai.setPaiPk(null);
            pai.setPaiVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(pai)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PAIS})
    public Response actualizar(@PathParam("paisId") Long id, SgPais pai) {
        try {
            if (servicio.objetoExistePorId(id)) {
                pai.setPaiPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(pai)).build();
            } else {
                pai.setPaiPk(null);
                return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(pai)).build();
            }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PAIS})
    public Response eliminarPorId(@PathParam("paisId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{paisId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("paisId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgPais.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
