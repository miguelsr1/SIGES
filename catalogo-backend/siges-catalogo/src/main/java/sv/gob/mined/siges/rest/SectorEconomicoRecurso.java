...
import sv.gob.mined.siges.negocio.servicios.SectorEconomicoBean;
...
@RequestScoped
@Path("/v1/sectoreseconomico")
@Tag(name = "TiposCalendarioEscolar API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SECTORES_ECONOMICOS,
    ConstantesOperaciones.ACTUALIZAR_SECTORES_ECONOMICOS,
    ConstantesOperaciones.ELIMINAR_SECTORES_ECONOMICOS,
    ConstantesOperaciones.AUTENTICADO})
public class SectorEconomicoRecurso {

    private static final Logger LOGGER = Logger.getLogger(SectorEconomicoRecurso.class.getName());

    @Inject
    private SectorEconomicoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgSectorEconomico
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgSectorEconomico.class)))
        ,
    ...
    @Path("/{sectorEconomicoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("sectorEconomicoId") Long id) {
        try {
            SgSectorEconomico sec = servicio.obtenerPorId(id);
            
            if (sec == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(sec).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgSectorEconomico 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgSectorEconomico.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SECTORES_ECONOMICOS})
    public Response crear(SgSectorEconomico sec) {
        try {
            sec.setSecPk(null);
            sec.setSecVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(sec)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SECTORES_ECONOMICOS})
    public Response actualizar(@PathParam("sectorEconomicoId") Long id, SgSectorEconomico sec) {
        try {
            if (servicio.objetoExistePorId(id)) {
                sec.setSecPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(sec)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_SECTORES_ECONOMICOS})
    public Response eliminarPorId(@PathParam("sectorEconomicoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{sectorEconomicoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("sectorEconomicoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgSectorEconomico.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
