...
import sv.gob.mined.siges.negocio.servicios.EstadoInmuebleBean;
 ...
 */
@RequestScoped
@Path("/v1/estadosinmuebles")
@Tag(name = "Estado Inmueble API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ESTADO_INMUEBLE,
    ConstantesOperaciones.ACTUALIZAR_ESTADO_INMUEBLE,
    ConstantesOperaciones.ELIMINAR_ESTADO_INMUEBLE})
public class EstadoInmuebleRecurso {

    private static final Logger LOGGER = Logger.getLogger(EstadoInmuebleRecurso.class.getName());

    @Inject
    private EstadoInmuebleBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgEstadoInmueble
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEstadoInmueble.class)))
        ,
    ...
    @Path("/{estadoInmuebleId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("estadoInmuebleId") Long id) {
        try {
            SgEstadoInmueble ein = servicio.obtenerPorId(id);
            if (ein == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(ein).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgEstadoInmueble
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgEstadoInmueble.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ESTADO_INMUEBLE})
    public Response crear(SgEstadoInmueble ein) {
        try {
            ein.setEinPk(null);
            ein.setEinVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ein)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ESTADO_INMUEBLE})
    public Response actualizar(@PathParam("estadoInmuebleId") Long id, SgEstadoInmueble ein) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ein.setEinPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ein)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ESTADO_INMUEBLE})
    public Response eliminarPorId(@PathParam("estadoInmuebleId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{estadoInmuebleId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("estadoInmuebleId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgEstadoInmueble.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgEstadoInmueble.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
