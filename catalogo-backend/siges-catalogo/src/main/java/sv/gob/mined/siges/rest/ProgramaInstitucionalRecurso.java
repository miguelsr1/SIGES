...
import sv.gob.mined.siges.negocio.servicios.ProgramaInstitucionalBean;
 ...
 */
@RequestScoped
@Path("/v1/programasinstitucional")
@Tag(name = "Programas Institucionales API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_PROGRAMAS_INSTITUCIONAL,
    ConstantesOperaciones.ACTUALIZAR_PROGRAMAS_INSTITUCIONAL,
    ConstantesOperaciones.ELIMINAR_PROGRAMAS_INSTITUCIONAL})
public class ProgramaInstitucionalRecurso {

    private static final Logger LOGGER = Logger.getLogger(ProgramaInstitucionalRecurso.class.getName());

    @Inject
    private ProgramaInstitucionalBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgProgramaInstitucional
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgProgramaInstitucional.class)))
        ,
    ...
    @Path("/{programaInstitucionalId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("programaInstitucionalId") Long id) {
        try {
            SgProgramaInstitucional pin = servicio.obtenerPorId(id);
            if (pin == null) {
     ...
     * @return Response Lista de SgProgramaInstitucional
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgProgramaInstitucional.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_PROGRAMAS_INSTITUCIONAL})
    public Response crear(SgProgramaInstitucional pin) {
        try {
            pin.setPinPk(null);
            pin.setPinVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PROGRAMAS_INSTITUCIONAL})
    public Response actualizar(@PathParam("programaInstitucionalId") Long id, SgProgramaInstitucional pin) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PROGRAMAS_INSTITUCIONAL})
    public Response eliminarPorId(@PathParam("programaInstitucionalId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{programaInstitucionalId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("programaInstitucionalId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgProgramaInstitucional.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgProgramaInstitucional.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
