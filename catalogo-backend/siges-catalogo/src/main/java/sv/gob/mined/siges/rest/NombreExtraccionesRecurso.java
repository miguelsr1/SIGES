...
import sv.gob.mined.siges.negocio.servicios.NombreExtraccionesBean;
 ...
 */
@RequestScoped
@Path("/v1/nombresextracciones")
@Tag(name = "Nombres de extracciones API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_NOMBRE_EXTRACCION,
    ConstantesOperaciones.ACTUALIZAR_NOMBRE_EXTRACCION,
    ConstantesOperaciones.ELIMINAR_NOMBRE_EXTRACCION})
public class NombreExtraccionesRecurso {

    private static final Logger LOGGER = Logger.getLogger(NombreExtraccionesRecurso.class.getName());

    @Inject
    private NombreExtraccionesBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgNombreExtracciones
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEstNombreExtraccion.class)))
        ,
    ...
    @Path("/{nombreExtraccionesId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("nombreExtraccionesId") Long id) {
        try {
            SgEstNombreExtraccion nex = servicio.obtenerPorId(id);
            if (nex == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(nex).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgNombreExtracciones
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgEstNombreExtraccion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_NOMBRE_EXTRACCION})
    public Response crear(SgEstNombreExtraccion nex) {
        try {
            nex.setNexPk(null);
            nex.setNexVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(nex)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_NOMBRE_EXTRACCION})
    public Response actualizar(@PathParam("nombreExtraccionesId") Long id, SgEstNombreExtraccion nex) {
        try {
            if (servicio.objetoExistePorId(id)) {
                nex.setNexPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(nex)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_NOMBRE_EXTRACCION})
    public Response eliminarPorId(@PathParam("nombreExtraccionesId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{nombreExtraccionesId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("nombreExtraccionesId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgEstNombreExtraccion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgEstNombreExtraccion.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
