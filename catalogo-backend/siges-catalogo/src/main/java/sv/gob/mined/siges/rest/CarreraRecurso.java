...
import sv.gob.mined.siges.negocio.servicios.CarreraBean;
...
@RequestScoped
@Path("/v1/carreras")
@Tag(name = "Carrera API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CARRERAS,
    ConstantesOperaciones.ACTUALIZAR_CARRERAS,
    ConstantesOperaciones.ELIMINAR_CARRERAS,
    ConstantesOperaciones.AUTENTICADO})
public class CarreraRecurso {

    private static final Logger LOGGER = Logger.getLogger(CarreraRecurso.class.getName());
    
    @Inject
    private CarreraBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCarrera.class)))
        ,
    ...
    @Path("/{carreraId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("carreraId") Long id) {
        try {
            SgCarrera crr = servicio.obtenerPorId(id);
            if (crr == null) {
     ...
     * @return Response Lista de SgCarrera 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgCarrera.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CARRERAS})
    public Response crear(SgCarrera crr) {
        try {
            crr.setCrrPk(null);
            crr.setCrrVersion(0);
...
/**
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CARRERAS})
    public Response actualizar(@PathParam("carreraId") Long id, SgCarrera crr) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CARRERAS})
    public Response eliminarPorId(@PathParam("carreraId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
...
/**
    ...
    @Path("/historial/{carreraId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("carreraId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgCarrera.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
