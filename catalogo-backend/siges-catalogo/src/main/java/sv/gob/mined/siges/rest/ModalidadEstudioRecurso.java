...
import sv.gob.mined.siges.negocio.servicios.ModalidadEstudioBean;
...
@RequestScoped
@Path("/v1/modalidadesestudio")
@Tag(name = "ModalidadEstudio API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MODALIDADES_ESTUDIO,
    ConstantesOperaciones.ACTUALIZAR_MODALIDADES_ESTUDIO,
    ConstantesOperaciones.ELIMINAR_MODALIDADES_ESTUDIO,
    ConstantesOperaciones.AUTENTICADO})
public class ModalidadEstudioRecurso {

    private static final Logger LOGGER = Logger.getLogger(ModalidadEstudioRecurso.class.getName());
    
    @Inject
    private ModalidadEstudioBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgModalidadEstudio
     */    
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgModalidadEstudio.class)))
        ,
    ...
    @Path("/{modalidadEstudioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("modalidadEstudioId") Long id) {
        try {
            SgModalidadEstudio mes = servicio.obtenerPorId(id);
            if (mes == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(mes).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgModalidadEstudio 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgModalidadEstudio.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MODALIDADES_ESTUDIO})
    public Response crear(SgModalidadEstudio mes) {
        try {
            mes.setMesPk(null);
            mes.setMesVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(mes)).build();
...
/**
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MODALIDADES_ESTUDIO})
    public Response actualizar(@PathParam("modalidadEstudioId") Long id, SgModalidadEstudio mes) {
        try {
            if (servicio.objetoExistePorId(id)) {
                mes.setMesPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(mes)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MODALIDADES_ESTUDIO})
    public Response eliminarPorId(@PathParam("modalidadEstudioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
...
/**
    ...
    @Path("/historial/{modalidadEstudioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("modalidadEstudioId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgModalidadEstudio.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
