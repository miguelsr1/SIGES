...
import sv.gob.mined.siges.negocio.servicios.BeneficioBean;
...
@RequestScoped
@Path("/v1/beneficio")
@Tag(name = "Beneficios API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_BENEFICIOS,
    ConstantesOperaciones.ACTUALIZAR_BENEFICIOS,
    ConstantesOperaciones.ELIMINAR_BENEFICIOS})
public class BeneficioRecurso {

    private static final Logger LOGGER = Logger.getLogger(BeneficioRecurso.class.getName());

    @Inject
    private BeneficioBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgBeneficio.class)))
        ,
    ...
    @Path("/{beneficioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("beneficioId") Long id) {
        try {
            SgBeneficio bnf = servicio.obtenerPorId(id);
            if (bnf == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(bnf).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgBeneficio
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgBeneficio.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_BENEFICIOS})
    public Response crear(SgBeneficio bnf) {
        try {
            bnf.setBnfPk(null);
            bnf.setBnfVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(bnf)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_BENEFICIOS})
    public Response actualizar(@PathParam("beneficioId") Long id, SgBeneficio bnf) {
        try {
            if (servicio.objetoExistePorId(id)) {
                bnf.setBnfPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(bnf)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_BENEFICIOS})
    public Response eliminarPorId(@PathParam("beneficioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{beneficioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("beneficioId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgBeneficio.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgBeneficio.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
