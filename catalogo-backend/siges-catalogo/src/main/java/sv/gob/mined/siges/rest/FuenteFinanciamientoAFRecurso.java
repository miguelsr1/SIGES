...
import sv.gob.mined.siges.negocio.servicios.FuenteFinanciamientoAFBean;
import sv.gob.mined.siges.persistencia.entidades.SgAfFuenteFinanciamiento;

@RequestScoped
@Path("/v1/fuentefinanciamientoAF")
@Tag(name = "Fuente Financiamiento de Activo Fijo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_FUENTE_FINANCIAMIENTO_AF,
    ConstantesOperaciones.ACTUALIZAR_FUENTE_FINANCIAMIENTO_AF,
    ConstantesOperaciones.ELIMINAR_FUENTE_FINANCIAMIENTO_AF,
    ConstantesOperaciones.AUTENTICADO})
public class FuenteFinanciamientoAFRecurso {
    private static final Logger LOGGER = Logger.getLogger(FuenteFinanciamientoAFRecurso.class.getName());
    
    @Inject
    private FuenteFinanciamientoAFBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAfFuenteFinanciamiento
     */    
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAfFuenteFinanciamiento.class)))
        ,
    ...
    @Path("/{fuenteFinanciamientoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("fuenteFinanciamientoId") Long id) {
        try {
            SgAfFuenteFinanciamiento ins = servicio.obtenerPorId(id);
            if (ins == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(ins).build();
            }
        } catch (Exception ex) {
     ...
     *
     * @param filtro FiltroFuenteFinanciamientoAF
     * @return Response Lista de SgAfFuenteFinanciamiento 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgAfFuenteFinanciamiento.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroFuenteFinanciamientoAF filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroFuenteFinanciamientoAF filtro) {
        try {
     ...
     *
     * @param ins SgAfFuenteFinanciamiento
     * @return Response SgAfFuenteFinanciamiento
     */
    @POST
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_FUENTE_FINANCIAMIENTO_AF})
    public Response crear(SgAfFuenteFinanciamiento ins) {
        try {
            ins.setFfiPk(null);
            ins.setFfiVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ins)).build();
...
/**
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_FUENTE_FINANCIAMIENTO_AF})
    public Response actualizar(@PathParam("fuenteFinanciamientoId") Long id, SgAfFuenteFinanciamiento ins) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ins.setFfiPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ins)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_FUENTE_FINANCIAMIENTO_AF})
    public Response eliminarPorId(@PathParam("fuenteFinanciamientoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
...
/**
    ...
    @Path("/historial/{fuenteFinanciamientoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("fuenteFinanciamientoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAfFuenteFinanciamiento.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
