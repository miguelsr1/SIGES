...
import sv.gob.mined.siges.negocio.servicios.TelefonoBean;
...
@RequestScoped
@Path("/v1/telefonos")
@Tag(name = "Telefono API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ASOCIACIONES,
    ConstantesOperaciones.ACTUALIZAR_ASOCIACIONES,
    ConstantesOperaciones.ELIMINAR_ASOCIACIONES})
public class TelefonoRecurso {

    private static final Logger LOGGER = Logger.getLogger(TelefonoRecurso.class.getName());

    @Inject
    private TelefonoBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTelefono.class)))
        ,
    ...
    @Path("/{telefonoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("telefonoId") Long id) {
        try {
            SgTelefono entity = servicio.obtenerPorId(id);
            if (entity == null) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ASOCIACIONES})
    public Response crear(SgTelefono entity) {
        try {
            entity.setTelPk(null);
            entity.setTelVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ASOCIACIONES})
    public Response actualizar(@PathParam("telefonoId") Long id, SgTelefono entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ASOCIACIONES})
    public Response eliminarPorId(@PathParam("telefonoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgTelefono.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    public Response obtenerHistorialPorId(@PathParam("subModalidadId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTelefono.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
