...
import sv.gob.mined.siges.negocio.servicios.FuenteAbastecimientoAguaBean;
 ...
 */
@RequestScoped
@Path("/v1/fuentesabastecimientoagua")
@Tag(name = "Fuentes de abastecimiento de agua API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_FUENTES_ABASTECIMIENTO_AGUA,
    ConstantesOperaciones.ACTUALIZAR_FUENTES_ABASTECIMIENTO_AGUA,
    ConstantesOperaciones.ELIMINAR_FUENTES_ABASTECIMIENTO_AGUA})
public class FuenteAbastecimientoAguaRecurso {

    private static final Logger LOGGER = Logger.getLogger(FuenteAbastecimientoAguaRecurso.class.getName());

    @Inject
    private FuenteAbastecimientoAguaBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgFuenteAbastecimientoAgua
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgFuenteAbastecimientoAgua.class)))
        ,
    ...
    @Path("/{fuenteAbastecimientoAguaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("fuenteAbastecimientoAguaId") Long id) {
        try {
            SgFuenteAbastecimientoAgua faa = servicio.obtenerPorId(id);
            if (faa == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(faa).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgFuenteAbastecimientoAgua
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgFuenteAbastecimientoAgua.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_FUENTES_ABASTECIMIENTO_AGUA})
    public Response crear(SgFuenteAbastecimientoAgua faa) {
        try {
            faa.setFaaPk(null);
            faa.setFaaVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(faa)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_FUENTES_ABASTECIMIENTO_AGUA})
    public Response actualizar(@PathParam("fuenteAbastecimientoAguaId") Long id, SgFuenteAbastecimientoAgua faa) {
        try {
            if (servicio.objetoExistePorId(id)) {
                faa.setFaaPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(faa)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_FUENTES_ABASTECIMIENTO_AGUA})
    public Response eliminarPorId(@PathParam("fuenteAbastecimientoAguaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{fuenteAbastecimientoAguaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("fuenteAbastecimientoAguaId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgFuenteAbastecimientoAgua.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgFuenteAbastecimientoAgua.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
