...
import sv.gob.mined.siges.negocio.servicios.OpcionesDescargoBean;
...
@RequestScoped
@Path("/v1/opcionesdescargo")
@Tag(name = "Estados Traslado API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_OPCIONES_DESCARGO,
    ConstantesOperaciones.ACTUALIZAR_OPCIONES_DESCARGO,
    ConstantesOperaciones.ELIMINAR_OPCIONES_DESCARGO,
    ConstantesOperaciones.AUTENTICADO})
public class OpcionesDescargoRecurso {
 private static final Logger LOGGER = Logger.getLogger(OpcionesDescargoRecurso.class.getName());

    @Inject
    private OpcionesDescargoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAfOpcionesDescargo
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAfOpcionesDescargo.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("opcionId") Long id) {
        try {
            SgAfOpcionesDescargo etn = servicio.obtenerPorId(id);
            if (etn == null) {
     ...
     * @return Response Lista de SgAfOpcionesDescargo 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgAfOpcionesDescargo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_OPCIONES_DESCARGO})
    public Response crear(SgAfOpcionesDescargo etn) {
        try {
            etn.setOdePk(null);
            etn.setOdeVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_OPCIONES_DESCARGO})
    public Response actualizar(@PathParam("opcionId") Long id, SgAfOpcionesDescargo etn) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_OPCIONES_DESCARGO})
    ...
    public Response obtenerHistorialPorId(@PathParam("opcionId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAfOpcionesDescargo.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
