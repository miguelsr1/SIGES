...
import sv.gob.mined.siges.negocio.servicios.MensajeBean;
...
@RequestScoped
@Path("/v1/mensajes")
@Tag(name = "Mensaje API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MENSAJE,
    ConstantesOperaciones.ACTUALIZAR_MENSAJE,
    ConstantesOperaciones.ELIMINAR_MENSAJE,
    ConstantesOperaciones.AUTENTICADO})
public class MensajeRecurso {

    private static final Logger LOGGER = Logger.getLogger(MensajeRecurso.class.getName());

    @Inject
    private MensajeBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMensaje.class)))
        ,
    ...
    @Path("/{mensajeId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("mensajeId") Long id) {
        try {
            SgMensaje msj = servicio.obtenerPorId(id);
            if (msj == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(msj).build();
            }
        } catch (Exception ex) {
     ...
     */
    @GET
    @Operation(summary = "Devuelve los mensajes en el formato necesario para un Resource Bundle.")
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
     ...
     * @return Response Lista de SgMensaje 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgMensaje.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroMensaje filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroMensaje filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MENSAJE})
    public Response crear(SgMensaje msj) {
        try {
            msj.setMsjPk(null);
            msj.setMsjVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(msj)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MENSAJE})
    public Response actualizar(@PathParam("mensajeId") Long id, SgMensaje msj) {
        try {
            if (servicio.objetoExistePorId(id)) {
                msj.setMsjPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(msj)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MENSAJE})
    public Response eliminarPorId(@PathParam("mensajeId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{mensajeId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("mensajeId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMensaje.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
