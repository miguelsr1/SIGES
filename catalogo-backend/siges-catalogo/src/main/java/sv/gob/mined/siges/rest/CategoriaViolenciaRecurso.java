...
import sv.gob.mined.siges.negocio.servicios.CategoriaViolenciaBean;
 ...
 */
@RequestScoped
@Path("/v1/categoriaviolencia")
@Tag(name = "Categorias Violencia API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CATEGORIA_VIOLENCIA,
    ConstantesOperaciones.ACTUALIZAR_CATEGORIA_VIOLENCIA,
    ConstantesOperaciones.ELIMINAR_CATEGORIA_VIOLENCIA,
    ConstantesOperaciones.AUTENTICADO})
public class CategoriaViolenciaRecurso {

    private static final Logger LOGGER = Logger.getLogger(CategoriaViolenciaRecurso.class.getName());

    @Inject
    private CategoriaViolenciaBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgCategoriaViolencia
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCategoriaViolencia.class)))
        ,
    ...
    @Path("/{categoriaViolenciaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("categoriaViolenciaId") Long id) {
        try {
            SgCategoriaViolencia cav = servicio.obtenerPorId(id);
            if (cav == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(cav).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgCategoriaViolencia
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgCategoriaViolencia.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CATEGORIA_VIOLENCIA})
    public Response crear(SgCategoriaViolencia cav) {
        try {
            cav.setCavPk(null);
            cav.setCavVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(cav)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CATEGORIA_VIOLENCIA})
    public Response actualizar(@PathParam("categoriaViolenciaId") Long id, SgCategoriaViolencia cav) {
        try {
            if (servicio.objetoExistePorId(id)) {
                cav.setCavPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(cav)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CATEGORIA_VIOLENCIA})
    public Response eliminarPorId(@PathParam("categoriaViolenciaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{categoriaViolenciaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("categoriaViolenciaId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgCategoriaViolencia.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCategoriaViolencia.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
