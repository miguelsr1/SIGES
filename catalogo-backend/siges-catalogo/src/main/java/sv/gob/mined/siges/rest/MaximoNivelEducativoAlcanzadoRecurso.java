...
import sv.gob.mined.siges.negocio.servicios.MaximoNivelEducativoAlcanzadoBean;
 ...
 */
@RequestScoped
@Path("/v1/maximoniveleducativoalcanzado")
@Tag(name = "Máximo nivel educativo alcanzado API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MAXIMO_NIVEL_EDUCATIVO_ALCANZADO,
    ConstantesOperaciones.ACTUALIZAR_MAXIMO_NIVEL_EDUCATIVO_ALCANZADO,
    ConstantesOperaciones.ELIMINAR_MAXIMO_NIVEL_EDUCATIVO_ALCANZADO})
public class MaximoNivelEducativoAlcanzadoRecurso {

    private static final Logger LOGGER = Logger.getLogger(MaximoNivelEducativoAlcanzadoRecurso.class.getName());

    @Inject
    private MaximoNivelEducativoAlcanzadoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMaximoNivelEducativoAlcanzado
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMaximoNivelEducativoAlcanzado.class)))
        ,
    ...
    @Path("/{maximoNivelEducativoAlcanzadoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("maximoNivelEducativoAlcanzadoId") Long id) {
        try {
            SgMaximoNivelEducativoAlcanzado mne = servicio.obtenerPorId(id);
            if (mne == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(mne).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMaximoNivelEducativoAlcanzado
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgMaximoNivelEducativoAlcanzado.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MAXIMO_NIVEL_EDUCATIVO_ALCANZADO})
    public Response crear(SgMaximoNivelEducativoAlcanzado mne) {
        try {
            mne.setMnePk(null);
            mne.setMneVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(mne)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MAXIMO_NIVEL_EDUCATIVO_ALCANZADO})
    public Response actualizar(@PathParam("maximoNivelEducativoAlcanzadoId") Long id, SgMaximoNivelEducativoAlcanzado mne) {
        try {
            if (servicio.objetoExistePorId(id)) {
                mne.setMnePk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(mne)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MAXIMO_NIVEL_EDUCATIVO_ALCANZADO})
    public Response eliminarPorId(@PathParam("maximoNivelEducativoAlcanzadoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{maximoNivelEducativoAlcanzadoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("maximoNivelEducativoAlcanzadoId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMaximoNivelEducativoAlcanzado.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgMaximoNivelEducativoAlcanzado.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
