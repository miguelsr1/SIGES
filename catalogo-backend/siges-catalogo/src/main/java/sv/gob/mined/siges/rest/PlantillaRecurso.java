...
import sv.gob.mined.siges.negocio.servicios.PlantillaBean;
...
@RequestScoped
@Path("/v1/plantilla")
@Tag(name = "Plantillas API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_PLANTILLA,
    ConstantesOperaciones.ACTUALIZAR_PLANTILLA,
    ConstantesOperaciones.ELIMINAR_PLANTILLA,
    ConstantesOperaciones.ACTUALIZAR_ORGANIZACION_CURRICULAR})
public class PlantillaRecurso {

    private static final Logger LOGGER = Logger.getLogger(PlantillaRecurso.class.getName());

    @Inject
    private PlantillaBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgPlantilla.class)))
        ,
    ...
    @Path("/{plantillaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("plantillaId") Long id) {
        try {
            SgPlantilla pla = servicio.obtenerPorId(id);
            if (pla == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(pla).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgPlantilla
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgPlantilla.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroPlantilla filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroPlantilla filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_PLANTILLA, ConstantesOperaciones.ACTUALIZAR_ORGANIZACION_CURRICULAR})
    public Response crear(SgPlantilla pla) {
        try {
            pla.setPlaPk(null);
            pla.setPlaVersion(0);
            if (pla.getPlaOrganizacionCurricular() == null && !jwt.getGroups().contains(ConstantesOperaciones.CREAR_PLANTILLA)) {
                return Response.status(HttpStatus.SC_FORBIDDEN).build();
            }
            if (pla.getPlaOrganizacionCurricular() != null && !jwt.getGroups().contains(ConstantesOperaciones.ACTUALIZAR_ORGANIZACION_CURRICULAR)) {
                return Response.status(HttpStatus.SC_FORBIDDEN).build();
            }
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(pla)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PLANTILLA, ConstantesOperaciones.ACTUALIZAR_ORGANIZACION_CURRICULAR})
    public Response actualizar(@PathParam("plantillaId") Long id, SgPlantilla pla) {
        try {
            if (servicio.objetoExistePorId(id)) {
                pla.setPlaPk(id);

                if (pla.getPlaOrganizacionCurricular() == null && !jwt.getGroups().contains(ConstantesOperaciones.ACTUALIZAR_PLANTILLA)) {
                    return Response.status(HttpStatus.SC_FORBIDDEN).build();
                }
                if (pla.getPlaOrganizacionCurricular() != null && !jwt.getGroups().contains(ConstantesOperaciones.ACTUALIZAR_ORGANIZACION_CURRICULAR)) {
                    return Response.status(HttpStatus.SC_FORBIDDEN).build();
                ...
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(pla)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PLANTILLA, ConstantesOperaciones.ACTUALIZAR_ORGANIZACION_CURRICULAR})
    public Response eliminarPorId(@PathParam("plantillaId") Long id) {
        try {
            SgPlantilla pla = servicio.obtenerPorId(id);
            if (pla != null) {
                if (pla.getPlaOrganizacionCurricular() == null && !jwt.getGroups().contains(ConstantesOperaciones.ACTUALIZAR_PLANTILLA)) {
    ...
    @Path("/historial/{plantillaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("plantillaId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgPlantilla.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgPlantilla.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
