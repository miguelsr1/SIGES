...
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MODULOS_FORMACION_DOCENTE,
    ConstantesOperaciones.ACTUALIZAR_MODULOS_FORMACION_DOCENTE,
    ConstantesOperaciones.ELIMINAR_MODULOS_FORMACION_DOCENTE,
...
public class ModuloFormacionDocenteRecurso {
    ...
    public Response obtenerPorId(@PathParam("moduloFormacionDocenteId") Long id) {
        try {
            SgModuloFormacionDocente mfd = servicio.obtenerPorId(id);
            if (mfd == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(mfd).build();
            }
        } catch (Exception ex) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MODULOS_FORMACION_DOCENTE})
    public Response crear(SgModuloFormacionDocente mfd) {
        try {
            mfd.setMfdPk(null);
            mfd.setMfdVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(mfd)).build();
...
/**
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MODULOS_FORMACION_DOCENTE})
    public Response actualizar(@PathParam("moduloFormacionDocenteId") Long id, SgModuloFormacionDocente mfd) {
        try {
            if (servicio.objetoExistePorId(id)) {
                mfd.setMfdPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(mfd)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MODULOS_FORMACION_DOCENTE})
...
/**
    ...
    public Response obtenerHistorialPorId(@PathParam("moduloFormacionDocenteId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgModuloFormacionDocente.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
