...
import sv.gob.mined.siges.negocio.servicios.TipoAsociacionBean;
...
@RequestScoped
@Path("/v1/tiposasociaciones")
@Tag(name = "TipoAsociacion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPOS_ASOCIACIONES,
    ConstantesOperaciones.ACTUALIZAR_TIPOS_ASOCIACIONES,
    ConstantesOperaciones.ELIMINAR_TIPOS_ASOCIACIONES,
    ConstantesOperaciones.AUTENTICADO})
public class TipoAsociacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoAsociacionRecurso.class.getName());

    @Inject
    private TipoAsociacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTipoAsociacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoAsociacion.class)))
        ,
    ...
    @Path("/{tipoAsociacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoAsociacionId") Long id) {
        try {
            SgTipoAsociacion tce = servicio.obtenerPorId(id);
            if (tce == null) {
     ...
     * @return Response Lista de SgTipoAsociacion 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgTipoAsociacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPOS_ASOCIACIONES})
    public Response crear(SgTipoAsociacion tce) {
        try {
            tce.setTasPk(null);
            tce.setTasVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPOS_ASOCIACIONES})
    public Response actualizar(@PathParam("tipoAsociacionId") Long id, SgTipoAsociacion tce) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPOS_ASOCIACIONES})
    public Response eliminarPorId(@PathParam("tipoAsociacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tipoAsociacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoAsociacionId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoAsociacion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
