...
import sv.gob.mined.siges.negocio.servicios.EtiquetaBean;
...
@RequestScoped
@Path("/v1/etiquetas")
@Tag(name = "Etiqueta API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ETIQUETA,
    ConstantesOperaciones.ACTUALIZAR_ETIQUETA,
    ConstantesOperaciones.ELIMINAR_ETIQUETA,
    ConstantesOperaciones.AUTENTICADO})
public class EtiquetaRecurso {

    private static final Logger LOGGER = Logger.getLogger(EtiquetaRecurso.class.getName());

    @Inject
    private EtiquetaBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEtiqueta.class)))
        ,
    ...
    @Path("/{etiquetaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("etiquetaId") Long id) {
        try {
            SgEtiqueta eti = servicio.obtenerPorId(id);
            if (eti == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(eti).build();
            }
        } catch (Exception ex) {
     ...
     */
    @GET
    @Operation(summary = "Devuelve las etiquetas en el formato necesario para un Resource Bundle.")
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = Object.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    )
    @Path("/obtenerpararesourcebundle")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerParaResourceBundle() {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.obtenerParaResourceBundle()).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
     ...
     * @return Response Lista de SgEtiqueta 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgEtiqueta.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroEtiqueta filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroEtiqueta filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ETIQUETA})
    public Response crear(SgEtiqueta eti) {
        try {
            eti.setEtiPk(null);
            eti.setEtiVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(eti)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ETIQUETA})
    public Response actualizar(@PathParam("etiquetaId") Long id, SgEtiqueta eti) {
        try {
            if (servicio.objetoExistePorId(id)) {
                eti.setEtiPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(eti)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ETIQUETA})
    public Response eliminarPorId(@PathParam("etiquetaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{etiquetaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("etiquetaId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgEtiqueta.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
