...
import sv.gob.mined.siges.negocio.servicios.TipoTrabajoBean;
...
@RequestScoped
@Path("/v1/tipostrabajo")
@Tag(name = "TipoTrabajo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPO_TRABAJO,
    ConstantesOperaciones.ACTUALIZAR_TIPO_TRABAJO,
    ConstantesOperaciones.ELIMINAR_TIPO_TRABAJO,
    ConstantesOperaciones.AUTENTICADO})
public class TipoTrabajoRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoTrabajoRecurso.class.getName());

    @Inject
    private TipoTrabajoBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoTrabajo.class)))
        ,
    ...
    @Path("/{tipoTrabajoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoTrabajoId") Long id) {
        try {
            SgTipoTrabajo ttr = servicio.obtenerPorId(id);
            if (ttr == null) {
     ...
     * @return Response Lista de SgTipoTrabajo 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgTipoTrabajo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPO_TRABAJO})
    public Response crear(SgTipoTrabajo ttr) {
        try {
            ttr.setTtrPk(null);
            ttr.setTtrVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPO_TRABAJO})
    public Response actualizar(@PathParam("tipoTrabajoId") Long id, SgTipoTrabajo ttr) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPO_TRABAJO})
    public Response eliminarPorId(@PathParam("tipoTrabajoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tipoTrabajoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoTrabajoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoTrabajo.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
