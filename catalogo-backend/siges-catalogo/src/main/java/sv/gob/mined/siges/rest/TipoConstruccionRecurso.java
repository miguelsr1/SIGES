...
import sv.gob.mined.siges.negocio.servicios.TipoConstruccionBean;
 ...
 */
@RequestScoped
@Path("/v1/tipoconstruccion")
@Tag(name = "Tipos de Construcción API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPO_CONSTRUCCION,
    ConstantesOperaciones.ACTUALIZAR_TIPO_CONSTRUCCION,
    ConstantesOperaciones.ELIMINAR_TIPO_CONSTRUCCION})
public class TipoConstruccionRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoConstruccionRecurso.class.getName());

    @Inject
    private TipoConstruccionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTipoConstruccion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoConstruccion.class)))
        ,
    ...
    @Path("/{tipoConstruccionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoConstruccionId") Long id) {
        try {
            SgTipoConstruccion tco = servicio.obtenerPorId(id);
            if (tco == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tco).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTipoConstruccion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgTipoConstruccion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPO_CONSTRUCCION})
    public Response crear(SgTipoConstruccion tco) {
        try {
            tco.setTcoPk(null);
            tco.setTcoVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tco)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPO_CONSTRUCCION})
    public Response actualizar(@PathParam("tipoConstruccionId") Long id, SgTipoConstruccion tco) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tco.setTcoPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tco)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPO_CONSTRUCCION})
    public Response eliminarPorId(@PathParam("tipoConstruccionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{tipoConstruccionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoConstruccionId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoConstruccion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgTipoConstruccion.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
