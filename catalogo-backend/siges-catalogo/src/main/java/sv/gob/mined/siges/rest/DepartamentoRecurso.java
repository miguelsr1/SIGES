...
import sv.gob.mined.siges.negocio.servicios.DepartamentoBean;
...
@Tag(name = "Departamentos API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_DEPARTAMENTO,
    ConstantesOperaciones.ACTUALIZAR_DEPARTAMENTO,
    ConstantesOperaciones.ELIMINAR_DEPARTAMENTO,
...
public class DepartamentoRecurso {
    ...
    @Inject
    private DepartamentoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgDepartamento
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgDepartamento.class)))
        ,
    ...
    @Path("/{departamentoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("departamentoId") Long id) {
        try {
            SgDepartamento dep = servicio.obtenerPorId(id);
            if (dep == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(dep).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgDepartamento 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgDepartamento.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    public Response crear(SgDepartamento dep) {
        try {
            dep.setDepPk(null);
            dep.setDepVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(dep)).build();
    ...
    public Response actualizar(@PathParam("departamentoId") Long id, SgDepartamento dep) {
        try {
            if (servicio.objetoExistePorId(id)) {
                dep.setDepPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(dep)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_DEPARTAMENTO})
    public Response eliminarPorId(@PathParam("departamentoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{departamentoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("departamentoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgDepartamento.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
