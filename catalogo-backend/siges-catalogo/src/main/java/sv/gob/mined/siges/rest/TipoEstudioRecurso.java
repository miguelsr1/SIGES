...
import sv.gob.mined.siges.negocio.servicios.TipoEstudioBean;
...
@RequestScoped
@Path("/v1/tiposestudio")
@Tag(name = "TipoEstudio API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPOS_ESTUDIO,
    ConstantesOperaciones.ACTUALIZAR_TIPOS_ESTUDIO,
    ConstantesOperaciones.ELIMINAR_TIPOS_ESTUDIO,
    ConstantesOperaciones.AUTENTICADO})
public class TipoEstudioRecurso {

    private static final Logger LOGGER = Logger.getLogger(TipoEstudioRecurso.class.getName());
    
    @Inject
    private TipoEstudioBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTipoEstudio.class)))
        ,
    ...
    @Path("/{tipoEstudioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("tipoEstudioId") Long id) {
        try {
            SgTipoEstudio tes = servicio.obtenerPorId(id);
            if (tes == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tes).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTipoEstudio 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgTipoEstudio.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPOS_ESTUDIO})
    public Response crear(SgTipoEstudio tes) {
        try {
            tes.setTesPk(null);
            tes.setTesVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tes)).build();
...
/**
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPOS_ESTUDIO})
    public Response actualizar(@PathParam("tipoEstudioId") Long id, SgTipoEstudio tes) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tes.setTesPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tes)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPOS_ESTUDIO})
    public Response eliminarPorId(@PathParam("tipoEstudioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
...
/**
    ...
    @Path("/historial/{tipoEstudioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("tipoEstudioId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTipoEstudio.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
