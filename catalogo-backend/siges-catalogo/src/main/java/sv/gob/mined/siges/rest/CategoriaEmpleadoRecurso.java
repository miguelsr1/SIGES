...
import sv.gob.mined.siges.negocio.servicios.CategoriaEmpleadoBean;
...
@RequestScoped
@Path("/v1/categoriaempleado")
@Tag(name = "CategoriaEmpleado API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CATEGORIA_EMPLEADO,
    ConstantesOperaciones.ACTUALIZAR_CATEGORIA_EMPLEADO,
    ConstantesOperaciones.ELIMINAR_CATEGORIA_EMPLEADO,
    ConstantesOperaciones.AUTENTICADO})
public class CategoriaEmpleadoRecurso {

    private static final Logger LOGGER = Logger.getLogger(CategoriaEmpleadoRecurso.class.getName());

    @Inject
    private CategoriaEmpleadoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgCategoriaEmpleado
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCategoriaEmpleado.class)))
        ,
    ...
    @Path("/{categoriaEmpleadoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("categoriaEmpleadoId") Long id) {
        try {
            SgCategoriaEmpleado etn = servicio.obtenerPorId(id);
            if (etn == null) {
     ...
     * @return Response Lista de SgCategoriaEmpleado 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgCategoriaEmpleado.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CATEGORIA_EMPLEADO})
    public Response crear(SgCategoriaEmpleado etn) {
        try {
            etn.setCemPk(null);
            etn.setCemVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CATEGORIA_EMPLEADO})
    public Response actualizar(@PathParam("categoriaEmpleadoId") Long id, SgCategoriaEmpleado etn) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CATEGORIA_EMPLEADO})
    public Response eliminarPorId(@PathParam("categoriaEmpleadoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{categoriaEmpleadoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("categoriaEmpleadoId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgCategoriaEmpleado.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
