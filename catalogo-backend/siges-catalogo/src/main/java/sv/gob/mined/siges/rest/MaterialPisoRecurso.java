...
import sv.gob.mined.siges.negocio.servicios.MaterialPisoBean;
 ...
 */
@RequestScoped
@Path("/v1/materialespiso")
@Tag(name = "Materiales de piso API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MATERIALES_PISO,
    ConstantesOperaciones.ACTUALIZAR_MATERIALES_PISO,
    ConstantesOperaciones.ELIMINAR_MATERIALES_PISO})
public class MaterialPisoRecurso {

    private static final Logger LOGGER = Logger.getLogger(MaterialPisoRecurso.class.getName());

    @Inject
    private MaterialPisoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMaterialPiso
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMaterialPiso.class)))
        ,
    ...
    @Path("/{materialPisoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("materialPisoId") Long id) {
        try {
            SgMaterialPiso map = servicio.obtenerPorId(id);
            if (map == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(map).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMaterialPiso
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgMaterialPiso.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MATERIALES_PISO})
    public Response crear(SgMaterialPiso map) {
        try {
            map.setMapPk(null);
            map.setMapVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(map)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MATERIALES_PISO})
    public Response actualizar(@PathParam("materialPisoId") Long id, SgMaterialPiso map) {
        try {
            if (servicio.objetoExistePorId(id)) {
                map.setMapPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(map)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MATERIALES_PISO})
    public Response eliminarPorId(@PathParam("materialPisoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{materialPisoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("materialPisoId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMaterialPiso.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgMaterialPiso.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
