...
import sv.gob.mined.siges.negocio.servicios.InfAccesibilidadBean;
 ...
 */
@RequestScoped
@Path("/v1/accesibilidad")
@Tag(name = "Accesibilidad API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ACCESIBILIDAD,
    ConstantesOperaciones.ACTUALIZAR_ACCESIBILIDAD,
    ConstantesOperaciones.ELIMINAR_ACCESIBILIDAD})
public class InfAccesibilidadRecurso {

    private static final Logger LOGGER = Logger.getLogger(InfAccesibilidadRecurso.class.getName());

    @Inject
    private InfAccesibilidadBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgInfAccesibilidad
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgInfAccesibilidad.class)))
        ,
    ...
    @Path("/{infAccesibilidadId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("infAccesibilidadId") Long id) {
        try {
            SgInfAccesibilidad acc = servicio.obtenerPorId(id);
            if (acc == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(acc).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgInfAccesibilidad
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgInfAccesibilidad.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ACCESIBILIDAD})
    public Response crear(SgInfAccesibilidad acc) {
        try {
            acc.setAccPk(null);
            acc.setAccVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(acc)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ACCESIBILIDAD})
    public Response actualizar(@PathParam("infAccesibilidadId") Long id, SgInfAccesibilidad acc) {
        try {
            if (servicio.objetoExistePorId(id)) {
                acc.setAccPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(acc)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ACCESIBILIDAD})
    public Response eliminarPorId(@PathParam("infAccesibilidadId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{infAccesibilidadId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("infAccesibilidadId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgInfAccesibilidad.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgInfAccesibilidad.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
