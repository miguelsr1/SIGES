...
import sv.gob.mined.siges.negocio.servicios.EstadoCivilBean;
...
@RequestScoped
@Path("/v1/estadoscivil")
@Tag(name = "EstadoCivil API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ESTADO_CIVIL,
    ConstantesOperaciones.CREAR_ESTADO_CIVIL,
    ConstantesOperaciones.ELIMINAR_ESTADO_CIVIL,
    ConstantesOperaciones.AUTENTICADO})
public class EstadoCivilRecurso {

    private static final Logger LOGGER = Logger.getLogger(EstadoCivilBean.class.getName());

    @Inject
    private EstadoCivilBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEstadoCivil.class)))
        ,
    ...
    @Path("/{estadoCivilId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("estadoCivilId") Long id) {
        try {
            SgEstadoCivil eci = servicio.obtenerPorId(id);
            if (eci == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(eci).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgEstadoCivil 
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgEstadoCivil.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ESTADO_CIVIL})
    public Response crear(SgEstadoCivil eci) {
        try {
            eci.setEciPk(null);
            eci.setEciVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(eci)).build();
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ESTADO_CIVIL})
    public Response actualizar(@PathParam("estadoCivilId") Long id, SgEstadoCivil eci) {
        try {
            if (servicio.objetoExistePorId(id)) {
                eci.setEciPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(eci)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ESTADO_CIVIL})
    public Response eliminarPorId(@PathParam("estadoCivilId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{estadoCivilId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("estadoCivilId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgEstadoCivil.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
