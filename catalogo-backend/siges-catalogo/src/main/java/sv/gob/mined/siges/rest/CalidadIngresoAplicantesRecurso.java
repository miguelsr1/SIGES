...
import sv.gob.mined.siges.negocio.servicios.CalidadIngresoAplicantesBean;
 ...
 */
@RequestScoped
@Path("/v1/calidadesingresoaplicantesplaza")
@Tag(name = "Calidades de ingreso de aplicantes a plaza API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CALIDADES_INGRESO_APLICANTES,
    ConstantesOperaciones.ACTUALIZAR_CALIDADES_INGRESO_APLICANTES,
    ConstantesOperaciones.ELIMINAR_CALIDADES_INGRESO_APLICANTES,
	ConstantesOperaciones.BUSCAR_CALIDADES_INGRESO_APLICANTES})
public class CalidadIngresoAplicantesRecurso {

    private static final Logger LOGGER = Logger.getLogger(CalidadIngresoAplicantesRecurso.class.getName());

    @Inject
    private CalidadIngresoAplicantesBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgCalidadIngresoAplicantes
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCalidadIngresoAplicantes.class)))
        ,
    ...
    @Path("/{calidadIngresoAplicantesId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("calidadIngresoAplicantesId") Long id) {
        try {
            SgCalidadIngresoAplicantes cia = servicio.obtenerPorId(id);
            if (cia == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(cia).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgCalidadIngresoAplicantes
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgCalidadIngresoAplicantes.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CALIDADES_INGRESO_APLICANTES})
    public Response crear(SgCalidadIngresoAplicantes cia) {
        try {
            cia.setCiaPk(null);
            cia.setCiaVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(cia)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CALIDADES_INGRESO_APLICANTES})
    public Response actualizar(@PathParam("calidadIngresoAplicantesId") Long id, SgCalidadIngresoAplicantes cia) {
        try {
            if (servicio.objetoExistePorId(id)) {
                cia.setCiaPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(cia)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CALIDADES_INGRESO_APLICANTES})
    public Response eliminarPorId(@PathParam("calidadIngresoAplicantesId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{calidadIngresoAplicantesId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("calidadIngresoAplicantesId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgCalidadIngresoAplicantes.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCalidadIngresoAplicantes.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
