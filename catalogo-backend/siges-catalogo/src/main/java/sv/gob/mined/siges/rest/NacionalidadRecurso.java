...
import sv.gob.mined.siges.negocio.servicios.NacionalidadBean;
import sv.gob.mined.siges.persistencia.entidades.SgNacionalidad;
 ...
 */
@RequestScoped
@Path("/v1/nacionalidades")
@Tag(name = "Nacionalidades API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_NACIONALIDADES,
    ConstantesOperaciones.ACTUALIZAR_NACIONALIDADES,
    ConstantesOperaciones.ELIMINAR_NACIONALIDADES,
    ConstantesOperaciones.AUTENTICADO})
public class NacionalidadRecurso {

    private static final Logger LOGGER = Logger.getLogger(NacionalidadRecurso.class.getName());

    @Inject
    private NacionalidadBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgNacionalidad
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgNacionalidad.class)))
        ,
    ...
    @Path("/{nacionalidadId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("nacionalidadId") Long id) {
        try {
            SgNacionalidad nac = servicio.obtenerPorId(id);
            if (nac == null) {
     ...
     * @return Response Lista de SgNacionalidad
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgNacionalidad.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_NACIONALIDADES})
    public Response crear(SgNacionalidad nac) {
        try {
            nac.setNacPk(null);
            nac.setNacVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_NACIONALIDADES})
    public Response actualizar(@PathParam("nacionalidadId") Long id, SgNacionalidad nac) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_NACIONALIDADES})
    public Response eliminarPorId(@PathParam("nacionalidadId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{nacionalidadId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("nacionalidadId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgNacionalidad.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgNacionalidad.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
