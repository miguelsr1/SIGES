/*
 *  SIGES
 *  Desarrollado por Sofis Solutions
 */
package sv.gob.mined.siges.negocio.validaciones;

import org.apache.commons.lang3.StringUtils;
import sv.gob.mined.siges.excepciones.BusinessException;
import sv.gob.mined.siges.negocio.mensajes.Errores;
import sv.gob.mined.siges.persistencia.entidades.SgAfProyectos;
import sv.gob.mined.siges.utils.BooleanUtils;

public class ProyectosValidacionNegocio {
    /**
     * Valida que los datos del elemento sean correctos
     * 
     * @param etn SgAfProyectos
     * @return Boolean
     * @throws BusinessException Devuelve los códigos de error de la validación
     */
    public static boolean validar(SgAfProyectos etn) throws BusinessException {
        boolean respuesta = true;
        BusinessException ge = new BusinessException();
        if (etn==null) {
            ge.addError(Errores.ERROR_DATO_VACIO);
        } else {          
            if (StringUtils.isBlank(etn.getProCodigo())){
                ge.addError("proCodigo", Errores.ERROR_CODIGO_VACIO);
            } else if (etn.getProCodigo().length() > 4){
                ge.addError("proCodigo", Errores.ERROR_LARGO_CODIGO_4);
            }
            
            if (StringUtils.isBlank(etn.getProNombre())){
                ge.addError("proNombre", Errores.ERROR_NOMBRE_VACIO);
            } else if (etn.getProNombre().length() > 255){
                ge.addError("proNombre", Errores.ERROR_LARGO_NOMBRE_255);
            }
        }
        if (ge.getErrores().size()>0) {
            throw ge;
        }
        return respuesta;
    }
    
    /**
     * Compara dos elementos del tipo. 
     * Indica si los elementos contienen la misma información.
     * 
     * @param c1 SgFormaAdquisicion
     * @param c2 SgFormaAdquisicion
     * @return Boolean
     */
    public static boolean compararParaGrabar(SgAfProyectos c1, SgAfProyectos c2) {
 
        boolean respuesta = true;
        if (c1!=null) {
            if (c2!=null) {
                respuesta = StringUtils.equals(c1.getProCodigo(), c2.getProCodigo());
                respuesta = respuesta && StringUtils.equals(c1.getProNombre(), c2.getProNombre());
                respuesta = respuesta && BooleanUtils.sonIguales(c1.getProHabilitado(), c2.getProHabilitado());       
            }  
        } else {
            respuesta = c2==null;
        }
        return respuesta;
    }  
}
