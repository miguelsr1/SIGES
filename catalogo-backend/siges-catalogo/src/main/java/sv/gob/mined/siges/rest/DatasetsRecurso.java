...
import sv.gob.mined.siges.negocio.servicios.DatasetsBean;
import sv.gob.mined.siges.persistencia.entidades.SgEstDatasets;
...
@RequestScoped
@Path("/v1/datasets")
@Tag(name = "Datasets API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_DATASETS,
    ConstantesOperaciones.ACTUALIZAR_DATASETS,
    ConstantesOperaciones.ELIMINAR_DATASETS})
public class DatasetsRecurso {

    private static final Logger LOGGER = Logger.getLogger(DatasetsRecurso.class.getName());

    @Inject
    private DatasetsBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEstDatasets.class)))
        ,
    ...
    @Path("/{datasetsId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("datasetsId") Long id) {
        try {
            SgEstDatasets dat = servicio.obtenerPorId(id);
            if (dat == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(dat).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgDatasets
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgEstDatasets.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DATASETS})
    public Response crear(SgEstDatasets dat) {
        try {
            dat.setDatPk(null);
            dat.setDatVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(dat)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_DATASETS})
    public Response actualizar(@PathParam("datasetsId") Long id, SgEstDatasets dat) {
        try {
            if (servicio.objetoExistePorId(id)) {
                dat.setDatPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(dat)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_DATASETS})
    public Response eliminarPorId(@PathParam("datasetsId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{datasetsId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("datasetsId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgEstDatasets.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgEstDatasets.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
