...
import sv.gob.mined.siges.negocio.servicios.MotivoInasistenciaPersonalBean;
 ...
 */
@RequestScoped
@Path("/v1/motivosinasistenciapersonal")
@Tag(name = "Motivos de Inasistencia del Personal API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MOTIVOS_INASISTENCIA_PERSONAL,
    ConstantesOperaciones.ACTUALIZAR_MOTIVOS_INASISTENCIA_PERSONAL,
    ConstantesOperaciones.ELIMINAR_MOTIVOS_INASISTENCIA_PERSONAL})
public class MotivoInasistenciaPersonalRecurso {

    private static final Logger LOGGER = Logger.getLogger(MotivoInasistenciaPersonalRecurso.class.getName());

    @Inject
    private MotivoInasistenciaPersonalBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMotivoInasistenciaPersonal
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMotivoInasistenciaPersonal.class)))
        ,
    ...
    @Path("/{motivoInasistenciaPersonalId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("motivoInasistenciaPersonalId") Long id) {
        try {
            SgMotivoInasistenciaPersonal mip = servicio.obtenerPorId(id);
            if (mip == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(mip).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMotivoInasistenciaPersonal
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgMotivoInasistenciaPersonal.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MOTIVOS_INASISTENCIA_PERSONAL})
    public Response crear(SgMotivoInasistenciaPersonal mip) {
        try {
            mip.setMipPk(null);
            mip.setMipVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(mip)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MOTIVOS_INASISTENCIA_PERSONAL})
    public Response actualizar(@PathParam("motivoInasistenciaPersonalId") Long id, SgMotivoInasistenciaPersonal mip) {
        try {
            if (servicio.objetoExistePorId(id)) {
                mip.setMipPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(mip)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MOTIVOS_INASISTENCIA_PERSONAL})
    public Response eliminarPorId(@PathParam("motivoInasistenciaPersonalId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{motivoInasistenciaPersonalId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("motivoInasistenciaPersonalId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMotivoInasistenciaPersonal.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgMotivoInasistenciaPersonal.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
