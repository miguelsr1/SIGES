...
import sv.gob.mined.siges.negocio.servicios.InfTipologiaConstruccionBean;
 ...
 */
@RequestScoped
@Path("/v1/tipologiaconstruccion")
@Tag(name = "Tiplogías construcción API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TIPOLOGIA_CONSTRUCCION,
    ConstantesOperaciones.ACTUALIZAR_TIPOLOGIA_CONSTRUCCION,
    ConstantesOperaciones.ELIMINAR_TIPOLOGIA_CONSTRUCCION})
public class InfTipologiaConstruccionRecurso {

    private static final Logger LOGGER = Logger.getLogger(InfTipologiaConstruccionRecurso.class.getName());

    @Inject
    private InfTipologiaConstruccionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgInfTipologiaConstruccion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgInfTipologiaConstruccion.class)))
        ,
    ...
    @Path("/{infTipologiaConstruccionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("infTipologiaConstruccionId") Long id) {
        try {
            SgInfTipologiaConstruccion tic = servicio.obtenerPorId(id);
            if (tic == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tic).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgInfTipologiaConstruccion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgInfTipologiaConstruccion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TIPOLOGIA_CONSTRUCCION})
    public Response crear(SgInfTipologiaConstruccion tic) {
        try {
            tic.setTicPk(null);
            tic.setTicVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tic)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_TIPOLOGIA_CONSTRUCCION})
    public Response actualizar(@PathParam("infTipologiaConstruccionId") Long id, SgInfTipologiaConstruccion tic) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tic.setTicPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tic)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TIPOLOGIA_CONSTRUCCION})
    public Response eliminarPorId(@PathParam("infTipologiaConstruccionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{infTipologiaConstruccionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("infTipologiaConstruccionId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgInfTipologiaConstruccion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgInfTipologiaConstruccion.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
