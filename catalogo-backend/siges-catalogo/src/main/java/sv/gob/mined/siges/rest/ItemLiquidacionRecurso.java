...
import sv.gob.mined.siges.negocio.servicios.ItemLiquidacionBean;
 ...
 */
@RequestScoped
@Path("/v1/itemsliquidacion")
@Tag(name = "Items Liquidacion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ITEM_LIQUIDACION,
    ConstantesOperaciones.ACTUALIZAR_ITEM_LIQUIDACION,
    ConstantesOperaciones.ELIMINAR_ITEM_LIQUIDACION})
public class ItemLiquidacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(ItemLiquidacionRecurso.class.getName());

    @Inject
    private ItemLiquidacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgItemLiquidacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgItemLiquidacion.class)))
        ,
    ...
    @Path("/{itemLiquidacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("itemLiquidacionId") Long id) {
        try {
            SgItemLiquidacion ili = servicio.obtenerPorId(id);
            if (ili == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(ili).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgItemLiquidacion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgItemLiquidacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ITEM_LIQUIDACION})
    public Response crear(SgItemLiquidacion ili) {
        try {
            ili.setIliPk(null);
            ili.setIliVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ili)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ITEM_LIQUIDACION})
    public Response actualizar(@PathParam("itemLiquidacionId") Long id, SgItemLiquidacion ili) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ili.setIliPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ili)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ITEM_LIQUIDACION})
    public Response eliminarPorId(@PathParam("itemLiquidacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{itemLiquidacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("itemLiquidacionId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgItemLiquidacion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgItemLiquidacion.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
