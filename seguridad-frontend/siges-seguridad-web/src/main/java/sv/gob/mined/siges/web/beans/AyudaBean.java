...
public class AyudaBean implements Serializable {
    ...
    private HashMap<String, SgAyuda> ayudas = new HashMap<>();
    ...
    public SgAyuda getAyuda(String codigo) {
        try {
            ...
            if (!ayudas.containsKey(codigo)) {
                List<SgAyuda> a = catalogoClient.buscarAyudas(filtro);
                if (!a.isEmpty()) {
                    ayudas.put(codigo, a.get(0));
                } else {
                    ayudas.put(codigo, null);
                }
            }
