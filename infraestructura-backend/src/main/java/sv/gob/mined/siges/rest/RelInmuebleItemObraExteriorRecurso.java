...
import sv.gob.mined.siges.negocio.servicios.RelInmuebleItemObraExteriorBean;
 ...
 */
@RequestScoped
@Path("/v1/relinmuebleitemobraexterior")
@Tag(name = "RelInmuebleItemObraExterior API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_INMUEBLE_ITEM_OBRA_EXTERIOR,
    ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_ITEM_OBRA_EXTERIOR,
    ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_ITEM_OBRA_EXTERIOR,
	ConstantesOperaciones.BUSCAR_REL_INMUEBLE_ITEM_OBRA_EXTERIOR})
public class RelInmuebleItemObraExteriorRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelInmuebleItemObraExteriorRecurso.class.getName());

    @Inject
    private RelInmuebleItemObraExteriorBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelInmuebleItemObraExterior
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelInmuebleItemObraExterior.class)))
        ,
    ...
    @Path("/{relInmuebleItemObraExteriorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_INMUEBLE_ITEM_OBRA_EXTERIOR})
    public Response obtenerPorId(@PathParam("relInmuebleItemObraExteriorId") Long id) {
        try {
            SgRelInmuebleItemObraExterior rix = servicio.obtenerPorId(id);
            if (rix == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(rix).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRelInmuebleItemObraExterior
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelInmuebleItemObraExterior.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_INMUEBLE_ITEM_OBRA_EXTERIOR})
    public Response buscar(FiltroRelInmuebleItemObraExterior filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_INMUEBLE_ITEM_OBRA_EXTERIOR})
    public Response buscarTotal(FiltroRelInmuebleItemObraExterior filtro) {
        try {
     ...
     *
     * @param rix SgRelInmuebleItemObraExterior
     * @return Response  SgRelInmuebleItemObraExterior
     */
    @POST
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_INMUEBLE_ITEM_OBRA_EXTERIOR})
    public Response crear(SgRelInmuebleItemObraExterior rix) {
        try {
            rix.setRixPk(null);
            rix.setRixVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(rix)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_ITEM_OBRA_EXTERIOR})
    public Response actualizar(@PathParam("relInmuebleItemObraExteriorId") Long id, SgRelInmuebleItemObraExterior rix) {
        try {
            if (servicio.objetoExistePorId(id)) {
                rix.setRixPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(rix)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_ITEM_OBRA_EXTERIOR})
    public Response eliminarPorId(@PathParam("relInmuebleItemObraExteriorId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relInmuebleItemObraExteriorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relInmuebleItemObraExteriorId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialRevisionesPorId(SgRelInmuebleItemObraExterior.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelInmuebleItemObraExterior.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
