...
import sv.gob.mined.siges.negocio.servicios.RelInmuebleServicioSanitarioBean;
 ...
 */
@RequestScoped
@Path("/v1/relinmuebleserviciosanitario")
@Tag(name = "Rel Inmueble Servicio Sanitario API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_INMUEBLE_SERVICIO_SANITARIO,
    ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_SERVICIO_SANITARIO,
    ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_SERVICIO_SANITARIO,
	ConstantesOperaciones.BUSCAR_REL_INMUEBLE_SERVICIO_SANITARIO})
public class RelInmuebleServicioSanitarioRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelInmuebleServicioSanitarioRecurso.class.getName());

    @Inject
    private RelInmuebleServicioSanitarioBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelInmuebleServicioSanitario
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelInmuebleServicioSanitario.class)))
        ,
    ...
    @Path("/{relInmuebleServicioSanitarioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_INMUEBLE_SERVICIO_SANITARIO})
    public Response obtenerPorId(@PathParam("relInmuebleServicioSanitarioId") Long id) {
        try {
            SgRelInmuebleServicioSanitario rit = servicio.obtenerPorId(id);
            if (rit == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(rit).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRelInmuebleServicioSanitario
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelInmuebleServicioSanitario.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_INMUEBLE_SERVICIO_SANITARIO})
    public Response buscar(FiltroRelInmuebleServicioSanitario filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_INMUEBLE_SERVICIO_SANITARIO})
    public Response buscarTotal(FiltroRelInmuebleServicioSanitario filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_INMUEBLE_SERVICIO_SANITARIO})
    public Response crear(SgRelInmuebleServicioSanitario rit) {
        try {
            rit.setRitPk(null);
            rit.setRitVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(rit)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_SERVICIO_SANITARIO})
    public Response actualizar(@PathParam("relInmuebleServicioSanitarioId") Long id, SgRelInmuebleServicioSanitario rit) {
        try {
            if (servicio.objetoExistePorId(id)) {
                rit.setRitPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(rit)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_SERVICIO_SANITARIO})
    public Response eliminarPorId(@PathParam("relInmuebleServicioSanitarioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relInmuebleServicioSanitarioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relInmuebleServicioSanitarioId") Long id) {
        try {
                return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRelInmuebleServicioSanitario.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelInmuebleServicioSanitario.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
