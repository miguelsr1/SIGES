...
import sv.gob.mined.siges.negocio.servicios.RelEdificioServicioBean;
 ...
 */
@RequestScoped
@Path("/v1/reledificioservicio")
@Tag(name = "Rel Edificio Servicio API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_EDIFICIO_SERVICIO,
    ConstantesOperaciones.ACTUALIZAR_REL_EDIFICIO_SERVICIO,
    ConstantesOperaciones.ELIMINAR_REL_EDIFICIO_SERVICIO,
    ConstantesOperaciones.BUSCAR_REL_EDIFICIO_SERVICIO})
public class RelEdificioServicioRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelEdificioServicioRecurso.class.getName());

    @Inject
    private RelEdificioServicioBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelEdificioServicio
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelEdificioServicio.class)))
        ,
    ...
    @Path("/{relEdificioServicioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_EDIFICIO_SERVICIO})
    public Response obtenerPorId(@PathParam("relEdificioServicioId") Long id) {
        try {
            SgRelEdificioServicio res = servicio.obtenerPorId(id);
            if (res == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(res).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRelEdificioServicio
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelEdificioServicio.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_EDIFICIO_SERVICIO})
    public Response buscar(FiltroRelEdificioServicio filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_EDIFICIO_SERVICIO})
    public Response buscarTotal(FiltroRelEdificioServicio filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_EDIFICIO_SERVICIO})
    public Response crear(SgRelEdificioServicio res) {
        try {
            res.setResPk(null);
            res.setResVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(res)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_EDIFICIO_SERVICIO})
    public Response actualizar(@PathParam("relEdificioServicioId") Long id, SgRelEdificioServicio res) {
        try {
            if (servicio.objetoExistePorId(id)) {
                res.setResPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(res)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_EDIFICIO_SERVICIO})
    public Response eliminarPorId(@PathParam("relEdificioServicioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relEdificioServicioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relEdificioServicioId") Long id) {
        try {
	    return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRelEdificioServicio.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelEdificioServicio.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
