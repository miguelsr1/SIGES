...
import sv.gob.mined.siges.negocio.servicios.RelEdificioEspacioBean;
 ...
 */
@RequestScoped
@Path("/v1/reledificioespacio")
@Tag(name = "Rel Edificio Espacio API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_EDIFICIO_ESPACIO,
    ConstantesOperaciones.ACTUALIZAR_REL_EDIFICIO_ESPACIO,
    ConstantesOperaciones.ELIMINAR_REL_EDIFICIO_ESPACIO,
    ConstantesOperaciones.BUSCAR_REL_EDIFICIO_ESPACIO})
public class RelEdificioEspacioRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelEdificioEspacioRecurso.class.getName());

    @Inject
    private RelEdificioEspacioBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelEdificioEspacio
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelEdificioEspacio.class)))
        ,
    ...
    @Path("/{relEdificioEspacioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_EDIFICIO_ESPACIO})
    public Response obtenerPorId(@PathParam("relEdificioEspacioId") Long id) {
        try {
            SgRelEdificioEspacio ree = servicio.obtenerPorId(id);
            if (ree == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(ree).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRelEdificioEspacio
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelEdificioEspacio.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_EDIFICIO_ESPACIO})
    public Response buscar(FiltroRelEdificioEspacio filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_EDIFICIO_ESPACIO})
    public Response buscarTotal(FiltroRelEdificioEspacio filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_EDIFICIO_ESPACIO})
    public Response crear(SgRelEdificioEspacio ree) {
        try {
            ree.setReePk(null);
            ree.setReeVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ree)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_EDIFICIO_ESPACIO})
    public Response actualizar(@PathParam("relEdificioEspacioId") Long id, SgRelEdificioEspacio ree) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ree.setReePk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ree)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_EDIFICIO_ESPACIO})
    public Response eliminarPorId(@PathParam("relEdificioEspacioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relEdificioEspacioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relEdificioEspacioId") Long id) {
        try {
	    return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRelEdificioEspacio.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelEdificioEspacio.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
