...
import sv.gob.mined.siges.negocio.servicios.RelEdificioServicioSanitarioBean;
 ...
 */
@RequestScoped
@Path("/v1/reledificioserviciosanitario")
@Tag(name = "Rel Edificio Servicio Sanitario API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_EDIFICIO_SERVICIO_SANITARIO,
    ConstantesOperaciones.ACTUALIZAR_REL_EDIFICIO_SERVICIO_SANITARIO,
    ConstantesOperaciones.ELIMINAR_REL_EDIFICIO_SERVICIO_SANITARIO,
	ConstantesOperaciones.BUSCAR_REL_EDIFICIO_SERVICIO_SANITARIO})
public class RelEdificioServicioSanitarioRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelEdificioServicioSanitarioRecurso.class.getName());

    @Inject
    private RelEdificioServicioSanitarioBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelEdificioServicioSanitario
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelEdificioServicioSanitario.class)))
        ,
    ...
    @Path("/{relEdificioServicioSanitarioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_EDIFICIO_SERVICIO_SANITARIO})
    public Response obtenerPorId(@PathParam("relEdificioServicioSanitarioId") Long id) {
        try {
            SgRelEdificioServicioSanitario ret = servicio.obtenerPorId(id);
            if (ret == null) {
     ...
     * @return Response Lista de SgRelEdificioServicioSanitario
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelEdificioServicioSanitario.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_EDIFICIO_SERVICIO_SANITARIO})
    public Response buscar(FiltroRelEdificioServicioSanitario filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_EDIFICIO_SERVICIO_SANITARIO})
    public Response buscarTotal(FiltroRelEdificioServicioSanitario filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_EDIFICIO_SERVICIO_SANITARIO})
    public Response crear(SgRelEdificioServicioSanitario ret) {
        try {
            ret.setRetPk(null);
            ret.setRetVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ret)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_EDIFICIO_SERVICIO_SANITARIO})
    public Response actualizar(@PathParam("relEdificioServicioSanitarioId") Long id, SgRelEdificioServicioSanitario ret) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ret.setRetPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ret)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_EDIFICIO_SERVICIO_SANITARIO})
    public Response eliminarPorId(@PathParam("relEdificioServicioSanitarioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relEdificioServicioSanitarioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relEdificioServicioSanitarioId") Long id) {
        try {
	     return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRelEdificioServicioSanitario.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelEdificioServicioSanitario.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
