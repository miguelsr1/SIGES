...
import sv.gob.mined.siges.negocio.servicios.RelInmuebleUnidadRespBean;
 ...
 */
@RequestScoped
@Path("/v1/relinmuebleunidadresp")
@Tag(name = "Rel Inmueble Unidad Resp API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_INMUEBLE_UNIDAD_RESP,
    ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_UNIDAD_RESP,
    ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_UNIDAD_RESP})
public class RelInmuebleUnidadRespRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelInmuebleUnidadRespRecurso.class.getName());

    @Inject
    private RelInmuebleUnidadRespBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelInmuebleUnidadResp
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelInmuebleUnidadResp.class)))
        ,
    ...
    @Path("/{relInmuebleUnidadRespId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("relInmuebleUnidadRespId") Long id) {
        try {
            SgRelInmuebleUnidadResp riu = servicio.obtenerPorId(id);
            if (riu == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(riu).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRelInmuebleUnidadResp
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelInmuebleUnidadResp.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroRelInmuebleUnidadResp filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroRelInmuebleUnidadResp filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_INMUEBLE_UNIDAD_RESP})
    public Response crear(SgRelInmuebleUnidadResp riu) {
        try {
            riu.setRiuPk(null);
            riu.setRiuVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(riu)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_UNIDAD_RESP})
    public Response actualizar(@PathParam("relInmuebleUnidadRespId") Long id, SgRelInmuebleUnidadResp riu) {
        try {
            if (servicio.objetoExistePorId(id)) {
                riu.setRiuPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(riu)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_UNIDAD_RESP})
    public Response eliminarPorId(@PathParam("relInmuebleUnidadRespId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relInmuebleUnidadRespId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relInmuebleUnidadRespId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialRevisionesPorId(SgRelInmuebleUnidadResp.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelInmuebleUnidadResp.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
