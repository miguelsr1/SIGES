...
import sv.gob.mined.siges.negocio.servicios.RelInmuebleTipoDrenajeBean;
 ...
 */
@RequestScoped
@Path("/v1/relinmuebletipodrenaje")
@Tag(name = "Rel Inmueble Tipo Drenaje API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_INMUEBLE_TIPO_DRENAJE,
    ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_TIPO_DRENAJE,
    ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_TIPO_DRENAJE})
public class RelInmuebleTipoDrenajeRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelInmuebleTipoDrenajeRecurso.class.getName());

    @Inject
    private RelInmuebleTipoDrenajeBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelInmuebleTipoDrenaje
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelInmuebleTipoDrenaje.class)))
        ,
    ...
    @Path("/{relInmuebleTipoDrenajeId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("relInmuebleTipoDrenajeId") Long id) {
        try {
            SgRelInmuebleTipoDrenaje itd = servicio.obtenerPorId(id);
            if (itd == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(itd).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRelInmuebleTipoDrenaje
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelInmuebleTipoDrenaje.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroRelInmuebleTipoDrenaje filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroRelInmuebleTipoDrenaje filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_INMUEBLE_TIPO_DRENAJE})
    public Response crear(SgRelInmuebleTipoDrenaje itd) {
        try {
            itd.setItdPk(null);
            itd.setItdVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(itd)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_TIPO_DRENAJE})
    public Response actualizar(@PathParam("relInmuebleTipoDrenajeId") Long id, SgRelInmuebleTipoDrenaje itd) {
        try {
            if (servicio.objetoExistePorId(id)) {
                itd.setItdPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(itd)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_TIPO_DRENAJE})
    public Response eliminarPorId(@PathParam("relInmuebleTipoDrenajeId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relInmuebleTipoDrenajeId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relInmuebleTipoDrenajeId") Long id) {
        try {
	    return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRelInmuebleTipoDrenaje.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelInmuebleTipoDrenaje.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
