...
import sv.gob.mined.siges.negocio.servicios.MejorasBean;
...
@RequestScoped
@Path("/v1/mejoras")
@Tag(name = "Mejoras API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MEJORAS,
    ConstantesOperaciones.ACTUALIZAR_MEJORAS,
    ConstantesOperaciones.ELIMINAR_MEJORAS,
	ConstantesOperaciones.BUSCAR_MEJORAS})
public class MejorasRecurso {

    private static final Logger LOGGER = Logger.getLogger(MejorasRecurso.class.getName());

    @Inject
    private MejorasBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMejoras.class)))
        ,
    ...
    @Path("/{mejorasId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MEJORAS})
    public Response obtenerPorId(@PathParam("mejorasId") Long id) {
        try {
            SgMejoras mej = servicio.obtenerPorId(id);
            if (mej == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(mej).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMejoras
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgMejoras.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MEJORAS})
    public Response buscar(FiltroMejora filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MEJORAS})
    public Response buscarTotal(FiltroMejora filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MEJORAS})
    public Response crear(SgMejoras mej) {
        try {
            mej.setMejPk(null);
            mej.setMejVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(mej)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MEJORAS})
    public Response actualizar(@PathParam("mejorasId") Long id, SgMejoras mej) {
        try {
            if (servicio.objetoExistePorId(id)) {
                mej.setMejPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(mej)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MEJORAS})
    public Response eliminarPorId(@PathParam("mejorasId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{mejorasId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("mejorasId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialRevisionesPorId(SgMejoras.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgMejoras.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
