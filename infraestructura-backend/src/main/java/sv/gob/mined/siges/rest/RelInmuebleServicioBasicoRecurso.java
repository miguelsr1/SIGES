...
import sv.gob.mined.siges.negocio.servicios.RelInmuebleServicioBean;
...
@RequestScoped
@Path("/v1/relInmuebleServicioBasico")
@Tag(name = "Inmuebles de la Sede API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_INMUEBLE_SERVICIO_BASICO,
    ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_SERVICIO_BASICO,
    ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_SERVICIO_BASICO,
    ConstantesOperaciones.BUSCAR_REL_INMUEBLE_SERVICIO_BASICO})
public class RelInmuebleServicioBasicoRecurso {
    private static final Logger LOGGER = Logger.getLogger(RelInmuebleServicioBasicoRecurso.class.getName());

    @Inject
    private RelInmuebleServicioBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelInmuebleServicioBasico.class)))
        ,
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_INMUEBLE_SERVICIO_BASICO})
    public Response obtenerPorId(@PathParam("inmuebleId") Long id) {
        try {
            SgRelInmuebleServicioBasico entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgRelInmuebleServicioBasico
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelInmuebleServicioBasico.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_INMUEBLE_SERVICIO_BASICO})
    public Response buscar(FiltroRelInmuebleServicio filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_INMUEBLE_SERVICIO_BASICO})
    public Response buscarTotal(FiltroRelInmuebleServicio filtro) {
        try {
     ...
     * @return Response SgRelInmuebleServicioBasico
     */
    @POST
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_INMUEBLE_SERVICIO_BASICO})
    public Response crear(SgRelInmuebleServicioBasico entity) {
        try {
            entity.setRisPk(null);
            entity.setRisVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_SERVICIO_BASICO})
    public Response actualizar(@PathParam("inmuebleId") Long id, SgRelInmuebleServicioBasico entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_SERVICIO_BASICO})
    ...
    public Response obtenerHistorialPorId(@PathParam("inmuebleId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRelInmuebleServicioBasico.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelInmuebleServicioBasico.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response validar(SgRelInmuebleServicioBasico entity) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(RelInmuebleServicioValidacionNegocio.validar(entity)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
