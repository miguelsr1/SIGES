...
import sv.gob.mined.siges.negocio.servicios.RelInmuebleEspacioComunBean;
...
@RequestScoped
@Path("/v1/relInmuebleEspacioComun")
@Tag(name = "Inmuebles de la Sede API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_INMUEBLE_ESPACIO_COMUN,
    ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_ESPACIO_COMUN,
    ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_ESPACIO_COMUN,
    ConstantesOperaciones.BUSCAR_REL_INMUEBLE_ESPACIO_COMUN})
public class RelInmuebleEspacioComunRecurso {
    private static final Logger LOGGER = Logger.getLogger(RelInmuebleEspacioComunRecurso.class.getName());

    @Inject
    private RelInmuebleEspacioComunBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelInmuebleEspacioComun.class)))
        ,
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_INMUEBLE_ESPACIO_COMUN})
    public Response obtenerPorId(@PathParam("inmuebleId") Long id) {
        try {
            SgRelInmuebleEspacioComun entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgRelInmuebleEspacioComun
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelInmuebleEspacioComun.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_INMUEBLE_ESPACIO_COMUN})
    public Response buscar(FiltroRelInmuebleEspacioComun filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_INMUEBLE_ESPACIO_COMUN})
    public Response buscarTotal(FiltroRelInmuebleEspacioComun filtro) {
        try {
     ...
     * @return Response SgRelInmuebleEspacioComun
     */
    @POST
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_INMUEBLE_ESPACIO_COMUN})
    public Response crear(SgRelInmuebleEspacioComun entity) {
        try {
            entity.setRiePk(null);
            entity.setRieVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_ESPACIO_COMUN})
    public Response actualizar(@PathParam("inmuebleId") Long id, SgRelInmuebleEspacioComun entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_ESPACIO_COMUN})
    ...
    public Response obtenerHistorialPorId(@PathParam("inmuebleId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRelInmuebleEspacioComun.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelInmuebleEspacioComun.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response validar(SgRelInmuebleEspacioComun entity) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(RelInmuebleEspacioComunValidacionNegocio.validar(entity)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
