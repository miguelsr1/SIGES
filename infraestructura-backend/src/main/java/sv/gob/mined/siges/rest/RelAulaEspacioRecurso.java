...
import sv.gob.mined.siges.negocio.servicios.RelAulaEspacioBean;
 ...
 */
@RequestScoped
@Path("/v1/relaulaespacio")
@Tag(name = "Rel Aula Espacio API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_AULA_ESPACIO,
    ConstantesOperaciones.ACTUALIZAR_REL_AULA_ESPACIO,
    ConstantesOperaciones.ELIMINAR_REL_AULA_ESPACIO,
    ConstantesOperaciones.BUSCAR_REL_AULA_ESPACIO})
public class RelAulaEspacioRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelAulaEspacioRecurso.class.getName());

    @Inject
    private RelAulaEspacioBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelAulaEspacio
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelAulaEspacio.class)))
        ,
    ...
    @Path("/{relAulaEspacioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_AULA_ESPACIO})
    public Response obtenerPorId(@PathParam("relAulaEspacioId") Long id) {
        try {
            SgRelAulaEspacio rae = servicio.obtenerPorId(id);
            if (rae == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(rae).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRelAulaEspacio
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelAulaEspacio.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_AULA_ESPACIO})
    public Response buscar(FiltroRelAulaEspacio filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_AULA_ESPACIO})
    public Response buscarTotal(FiltroRelAulaEspacio filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_AULA_ESPACIO})
    public Response crear(SgRelAulaEspacio rae) {
        try {
            rae.setRaePk(null);
            rae.setRaeVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(rae)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_AULA_ESPACIO})
    public Response actualizar(@PathParam("relAulaEspacioId") Long id, SgRelAulaEspacio rae) {
        try {
            if (servicio.objetoExistePorId(id)) {
                rae.setRaePk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(rae)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_AULA_ESPACIO})
    public Response eliminarPorId(@PathParam("relAulaEspacioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relAulaEspacioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relAulaEspacioId") Long id) {
        try {
	    return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRelAulaEspacio.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelAulaEspacio.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
