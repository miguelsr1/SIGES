...
import sv.gob.mined.siges.negocio.servicios.RelInmuebleManejoDesechosBean;
 ...
 */
@RequestScoped
@Path("/v1/relinmueblemanejodesechos")
@Tag(name = "Rel Inmueble Manejo Desechos API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_INMUEBLE_MANEJO_DESECHOS,
    ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_MANEJO_DESECHOS,
    ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_MANEJO_DESECHOS})
public class RelInmuebleManejoDesechosRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelInmuebleManejoDesechosRecurso.class.getName());

    @Inject
    private RelInmuebleManejoDesechosBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelInmuebleManejoDesechos
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelInmuebleManejoDesechos.class)))
        ,
    ...
    @Path("/{relInmuebleManejoDesechosId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("relInmuebleManejoDesechosId") Long id) {
        try {
            SgRelInmuebleManejoDesechos imd = servicio.obtenerPorId(id);
            if (imd == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(imd).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRelInmuebleManejoDesechos
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelInmuebleManejoDesechos.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroRelInmuebleManejoDesechos filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroRelInmuebleManejoDesechos filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_INMUEBLE_MANEJO_DESECHOS})
    public Response crear(SgRelInmuebleManejoDesechos imd) {
        try {
            imd.setImdPk(null);
            imd.setImdVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(imd)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_MANEJO_DESECHOS})
    public Response actualizar(@PathParam("relInmuebleManejoDesechosId") Long id, SgRelInmuebleManejoDesechos imd) {
        try {
            if (servicio.objetoExistePorId(id)) {
                imd.setImdPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(imd)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_MANEJO_DESECHOS})
    public Response eliminarPorId(@PathParam("relInmuebleManejoDesechosId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relInmuebleManejoDesechosId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relInmuebleManejoDesechosId") Long id) {
        try {
	    return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRelInmuebleManejoDesechos.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelInmuebleManejoDesechos.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
