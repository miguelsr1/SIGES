...
import sv.gob.mined.siges.negocio.servicios.RelInmuebleArchivoBean;
 ...
 */
@RequestScoped
@Path("/v1/relinmueblearchivo")
@Tag(name = "Rel Inmueble Archivo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_INMUEBLE_ARCHIVO,
    ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_ARCHIVO,
    ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_ARCHIVO,
    ConstantesOperaciones.BUSCAR_REL_INMUEBLE_ARCHIVO})
public class RelInmuebleArchivoRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelInmuebleArchivoRecurso.class.getName());

    @Inject
    private RelInmuebleArchivoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelInmuebleArchivo
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelInmuebleArchivo.class)))
        ,
    ...
    @Path("/{relInmuebleArchivoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_INMUEBLE_ARCHIVO})
    public Response obtenerPorId(@PathParam("relInmuebleArchivoId") Long id) {
        try {
            SgRelInmuebleArchivo ria = servicio.obtenerPorId(id);
            if (ria == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(ria).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRelInmuebleArchivo
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelInmuebleArchivo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_INMUEBLE_ARCHIVO})
    public Response buscar(FiltroRelInmuebleArchivo filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_INMUEBLE_ARCHIVO})
    public Response buscarTotal(FiltroRelInmuebleArchivo filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_INMUEBLE_ARCHIVO})
    public Response crear(SgRelInmuebleArchivo ria) {
        try {
            ria.setRiaPk(null);
            ria.setRiaVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ria)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_ARCHIVO})
    public Response actualizar(@PathParam("relInmuebleArchivoId") Long id, SgRelInmuebleArchivo ria) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ria.setRiaPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ria)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_ARCHIVO})
    public Response eliminarPorId(@PathParam("relInmuebleArchivoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relInmuebleArchivoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relInmuebleArchivoId") Long id) {
        try {
	    return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRelInmuebleArchivo.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelInmuebleArchivo.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
