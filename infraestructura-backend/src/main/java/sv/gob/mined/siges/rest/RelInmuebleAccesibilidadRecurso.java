...
import sv.gob.mined.siges.negocio.servicios.RelInmuebleAccesibilidadBean;
 ...
 */
@RequestScoped
@Path("/v1/relinmuebleaccesibilidad")
@Tag(name = "Rel Inmueble Accesibilidad API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_INMUEBLE_ACCESIBILIDAD,
    ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_ACCESIBILIDAD,
    ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_ACCESIBILIDAD})
public class RelInmuebleAccesibilidadRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelInmuebleAccesibilidadRecurso.class.getName());

    @Inject
    private RelInmuebleAccesibilidadBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelInmuebleAccesibilidad
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelInmuebleAccesibilidad.class)))
        ,
    ...
    @Path("/{relInmuebleAccesibilidadId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("relInmuebleAccesibilidadId") Long id) {
        try {
            SgRelInmuebleAccesibilidad iac = servicio.obtenerPorId(id);
            if (iac == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(iac).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRelInmuebleAccesibilidad
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelInmuebleAccesibilidad.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroRelInmuebleAccesibilidad filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroRelInmuebleAccesibilidad filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_INMUEBLE_ACCESIBILIDAD})
    public Response crear(SgRelInmuebleAccesibilidad iac) {
        try {
            iac.setIacPk(null);
            iac.setIacVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(iac)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_ACCESIBILIDAD})
    public Response actualizar(@PathParam("relInmuebleAccesibilidadId") Long id, SgRelInmuebleAccesibilidad iac) {
        try {
            if (servicio.objetoExistePorId(id)) {
                iac.setIacPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(iac)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_ACCESIBILIDAD})
    public Response eliminarPorId(@PathParam("relInmuebleAccesibilidadId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relInmuebleAccesibilidadId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relInmuebleAccesibilidadId") Long id) {
        try {
	    return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRelInmuebleAccesibilidad.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelInmuebleAccesibilidad.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
