...
import sv.gob.mined.siges.negocio.servicios.RelInmuebleAlmacenamientoAguaBean;
 ...
 */
@RequestScoped
@Path("/v1/relinmueblealmacenamientoagua")
@Tag(name = "Rel Inmueble Almacenamiento Agua API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_INMUEBLE_ALMACENAMIENTO_AGUA,
    ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_ALMACENAMIENTO_AGUA,
    ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_ALMACENAMIENTO_AGUA})
public class RelInmuebleAlmacenamientoAguaRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelInmuebleAlmacenamientoAguaRecurso.class.getName());

    @Inject
    private RelInmuebleAlmacenamientoAguaBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelInmuebleAlmacenamientoAgua
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelInmuebleAlmacenamientoAgua.class)))
        ,
    ...
    @Path("/{relInmuebleAlmacenamientoAguaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("relInmuebleAlmacenamientoAguaId") Long id) {
        try {
            SgRelInmuebleAlmacenamientoAgua ial = servicio.obtenerPorId(id);
            if (ial == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(ial).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRelInmuebleAlmacenamientoAgua
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelInmuebleAlmacenamientoAgua.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroRelInmuebleAlmacenamientoAgua filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroRelInmuebleAlmacenamientoAgua filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_INMUEBLE_ALMACENAMIENTO_AGUA})
    public Response crear(SgRelInmuebleAlmacenamientoAgua ial) {
        try {
            ial.setIalPk(null);
            ial.setIalVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ial)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_ALMACENAMIENTO_AGUA})
    public Response actualizar(@PathParam("relInmuebleAlmacenamientoAguaId") Long id, SgRelInmuebleAlmacenamientoAgua ial) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ial.setIalPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ial)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_ALMACENAMIENTO_AGUA})
    public Response eliminarPorId(@PathParam("relInmuebleAlmacenamientoAguaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relInmuebleAlmacenamientoAguaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relInmuebleAlmacenamientoAguaId") Long id) {
        try {
	    return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRelInmuebleAlmacenamientoAgua.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelInmuebleAlmacenamientoAgua.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
