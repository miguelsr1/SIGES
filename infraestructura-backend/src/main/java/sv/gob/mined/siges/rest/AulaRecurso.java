...
import sv.gob.mined.siges.negocio.servicios.AulaBean;
...
@RequestScoped
@Path("/v1/aula")
@Tag(name = "Aulas API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_AULAS,
    ConstantesOperaciones.ACTUALIZAR_AULAS,
    ConstantesOperaciones.ELIMINAR_AULAS,
    ConstantesOperaciones.BUSCAR_AULA})
public class AulaRecurso {

    private static final Logger LOGGER = Logger.getLogger(AulaRecurso.class.getName());

    @Inject
    private AulaBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAula.class)))
        ,
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_AULA})
    public Response obtenerPorId(@PathParam("aulaId") Long id) {
        try {
            SgAula aul = servicio.obtenerPorId(id);
            if (aul == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(aul).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgAula
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAula.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_AULA})
    public Response buscar(FiltroAula filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_AULA})
    public Response buscarTotal(FiltroAula filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_AULAS})
    public Response crear(SgAula aul) {
        try {
            aul.setAulPk(null);
            aul.setAulVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(aul)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_AULAS})
    public Response actualizar(@PathParam("aulaId") Long id, SgAula aul) {
        try {
            if (servicio.objetoExistePorId(id)) {
                aul.setAulPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(aul)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_AULAS})
    public Response eliminarPorId(@PathParam("aulaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{aulaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("aulaId") Long id) {
        try {
                return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgAula.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgAula.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response actualizarListaRelEspacioComun(@PathParam("inmuebleId") Long id, SgAulaLiteEspacio entity) {
        try {
            if (servicio.objetoExistePorId(id)) {
                entity.setAulPk(id);
                if (entity.getAulRelAulaEspacio()!= null) {
                    for (SgRelAulaEspacio espacioComun : entity.getAulRelAulaEspacio()) {
                        espacioComun.setRaeAula(new SgAula(entity.getAulPk(), entity.getAulVersion()));
                      
                    }
                }
                SgAulaLiteEspacio resp=servicio.guardarLiteEspacio(entity);
                resp.setAulRelAulaEspacio(null);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_INMUEBLE_O_TERRENO})
    public Response actualizarListaRelServicio(@PathParam("inmuebleId") Long id, SgAulaLiteServicio entity) {
        try {
            if (servicio.objetoExistePorId(id)) {
                entity.setAulPk(id);
                if (entity.getAulRelAulaServicio()!= null) {
                    for (SgRelAulaServicio espacioComun : entity.getAulRelAulaServicio()) {
                        espacioComun.setRasAula(new SgAula(entity.getAulPk(), entity.getAulVersion()));
                       
                    }
                }
                SgAulaLiteServicio resp=servicio.guardarLiteServicio(entity);
                resp.setAulRelAulaServicio(null);
                return Response.status(HttpStatus.SC_OK).entity(resp).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
