...
import sv.gob.mined.siges.negocio.servicios.RelInmuebleDocumentoBean;
 ...
 */
@RequestScoped
@Path("/v1/relinmuebledocumento")
@Tag(name = "Rel Inmueble Documento API V1")
...
public class RelInmuebleDocumentoRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelInmuebleDocumentoRecurso.class.getName());

    @Inject
    private RelInmuebleDocumentoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelInmuebleDocumento
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelInmuebleDocumento.class)))
        ,
    ...
    @Path("/{relInmuebleDocumentoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_EDIFICIO_DOCUMENTOS})
    public Response obtenerPorId(@PathParam("relInmuebleDocumentoId") Long id) {
        try {
            SgRelInmuebleDocumento ria = servicio.obtenerPorId(id);
            if (ria == null) {
     ...
     * @return Response Lista de SgRelInmuebleDocumento
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelInmuebleDocumento.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_EDIFICIO_DOCUMENTOS})
    public Response buscar(FiltroRelInmuebleDocumento filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_EDIFICIO_DOCUMENTOS})
    public Response buscarTotal(FiltroRelInmuebleDocumento filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_INMUEBLE_DOCUMENTO})
    public Response crear(SgRelInmuebleDocumento rid) {
        try {
            rid.setRidPk(null);
            rid.setRidVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(rid)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_INMUEBLE_DOCUMENTO})
    public Response actualizar(@PathParam("relInmuebleDocumentoId") Long id, SgRelInmuebleDocumento ria) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_INMUEBLE_DOCUMENTO})
    public Response eliminarPorId(@PathParam("relInmuebleDocumentoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relInmuebleDocumentoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relInmuebleDocumentoId") Long id) {
        try {
	    return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRelInmuebleDocumento.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelInmuebleDocumento.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
