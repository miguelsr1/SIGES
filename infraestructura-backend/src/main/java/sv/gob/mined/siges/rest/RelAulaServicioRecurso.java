...
import sv.gob.mined.siges.negocio.servicios.RelAulaServicioBean;
 ...
 */
@RequestScoped
@Path("/v1/relaulaservicio")
@Tag(name = "Rel Aula Servicio API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_AULA_SERVICIO,
    ConstantesOperaciones.ACTUALIZAR_REL_AULA_SERVICIO,
    ConstantesOperaciones.ELIMINAR_REL_AULA_SERVICIO,
   ConstantesOperaciones. BUSCAR_REL_AULA_SERVICIO})
public class RelAulaServicioRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelAulaServicioRecurso.class.getName());

    @Inject
    private RelAulaServicioBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelAulaServicio
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelAulaServicio.class)))
        ,
    ...
    @Path("/{relAulaServicioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_AULA_SERVICIO})
    public Response obtenerPorId(@PathParam("relAulaServicioId") Long id) {
        try {
            SgRelAulaServicio ras = servicio.obtenerPorId(id);
            if (ras == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(ras).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRelAulaServicio
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelAulaServicio.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_AULA_SERVICIO})
    public Response buscar(FiltroRelAulaServicio filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_AULA_SERVICIO})
    public Response buscarTotal(FiltroRelAulaServicio filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_AULA_SERVICIO})
    public Response crear(SgRelAulaServicio ras) {
        try {
            ras.setRasPk(null);
            ras.setRasVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ras)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_AULA_SERVICIO})
    public Response actualizar(@PathParam("relAulaServicioId") Long id, SgRelAulaServicio ras) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ras.setRasPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ras)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_AULA_SERVICIO})
    public Response eliminarPorId(@PathParam("relAulaServicioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relAulaServicioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relAulaServicioId") Long id) {
        try {
	    return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRelAulaServicio.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelAulaServicio.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
