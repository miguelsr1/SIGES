...
import sv.gob.mined.siges.negocio.servicios.EdificioBean;
...
@RequestScoped
@Path("/v1/edificio")
@Tag(name = "Edificios API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_EDIFICIO,
    ConstantesOperaciones.ACTUALIZAR_EDIFICIO,
    ConstantesOperaciones.ELIMINAR_EDIFICIO,
    ConstantesOperaciones.BUSCAR_EDIFICIO})
public class EdificioRecurso {

    private static final Logger LOGGER = Logger.getLogger(EdificioRecurso.class.getName());

    @Inject
    private EdificioBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEdificio.class)))
        ,
    ...
    @Path("/{edificioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_EDIFICIO})
    public Response obtenerPorId(@PathParam("edificioId") Long id) {
        try {
            SgEdificio edi = servicio.obtenerPorId(id);
            if (edi == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(edi).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgEdificio
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgEdificio.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_EDIFICIO})
    public Response buscar(FiltroEdificio filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_EDIFICIO})
    public Response buscarTotal(FiltroEdificio filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_EDIFICIO})
    public Response crear(SgEdificio edi) {
        try {
            edi.setEdiPk(null);
            edi.setEdiVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(edi)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_EDIFICIO})
    public Response actualizar(@PathParam("edificioId") Long id, SgEdificio edi) {
        try {
            if (servicio.objetoExistePorId(id)) {
                edi.setEdiPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(edi)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_EDIFICIO})
    public Response eliminarPorId(@PathParam("edificioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{edificioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("edificioId") Long id) {
        try {
	    return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgEdificio.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgEdificio.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response actualizarListaRelEspacioComun(@PathParam("inmuebleId") Long id, SgEdificioLiteEspacio entity) {
        try {
            if (servicio.objetoExistePorId(id)) {
                entity.setEdiPk(id);
                if (entity.getEdiRelEdificioEspacio()!= null) {
                    for (SgRelEdificioEspacio espacioComun : entity.getEdiRelEdificioEspacio()) {
                        espacioComun.setReeEdificio(new SgEdificio(entity.getEdiPk(), entity.getEdiVersion()));
                      
                    }
                }
                SgEdificioLiteEspacio resp=servicio.guardarLiteEspacio(entity);
                resp.setEdiRelEdificioEspacio(null);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_INMUEBLE_O_TERRENO})
    public Response actualizarListaRelServicio(@PathParam("inmuebleId") Long id, SgEdificioLiteServicio entity) {
        try {
            if (servicio.objetoExistePorId(id)) {
                entity.setEdiPk(id);
                if (entity.getEdiRelEdificioServicio()!= null) {
                    for (SgRelEdificioServicio espacioComun : entity.getEdiRelEdificioServicio()) {
                        espacioComun.setResEdificio(new SgEdificio(entity.getEdiPk(), entity.getEdiVersion()));
                       
                    }
                }
                if (entity.getEdiRelEdificioServicioSanitario()!= null) {
                    for (SgRelEdificioServicioSanitario espacioComun : entity.getEdiRelEdificioServicioSanitario()) {
                        espacioComun.setRetEdificio(new SgEdificio(entity.getEdiPk(), entity.getEdiVersion()));
                       
                    }
                }
                SgEdificioLiteServicio resp=servicio.guardarLiteServicio(entity);
                resp.setEdiRelEdificioServicio(null);
                resp.setEdiRelEdificioServicioSanitario(null);
                return Response.status(HttpStatus.SC_OK).entity(resp).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
