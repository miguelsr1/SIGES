...
import sv.gob.mined.siges.negocio.servicios.SistemaSedeBean;
import sv.gob.mined.siges.persistencia.entidades.centros.SgSede;

@RequestScoped
@Path("/v1/sistemassedes")
@Tag(name = "SistemaSede API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO})
public class SistemaSedeRecurso {

    private static final Logger LOGGER = Logger.getLogger(SistemaSedeRecurso.class.getName());

    @Inject
    private SistemaSedeBean servicio;
    ...
    public Response obtenerPorId(@PathParam("sedeId") Long id) {
        try {
            SgSede entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgSede
     */
    @POST
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroSistemaSede filtro) {
        try {
     ...
     *
     * @param filtro FiltroCodiguera
     * @return Response Lista de SgSistemaSede
     */
    @POST
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroSistemaSede filtro) {
        try {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "201", description = "Entidad creada satisfactoriamente.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgSistemaSede.class)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response crear(SgSistemaSede entity) {
        try {
    ...
    @Path("/{sinPk:[1-9][0-9]*}/{sedPk:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response eliminarPorId(@PathParam("sinPk") Long sinPk, @PathParam("sedPk") Long sedPk) {
        try {
            servicio.eliminar(sinPk, sedPk);
