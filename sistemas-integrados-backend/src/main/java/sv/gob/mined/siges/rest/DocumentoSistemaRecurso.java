...
import sv.gob.mined.siges.negocio.servicios.DocumentoSistemaBean;
...
@RequestScoped
@Path("/v1/documentossistemas")
@Tag(name = "DocumentoSistema API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_DOCUMENTO_SISTEMA,
    ConstantesOperaciones.ACTUALIZAR_DOCUMENTO_SISTEMA,
    ConstantesOperaciones.ELIMINAR_DOCUMENTO_SISTEMA})
public class DocumentoSistemaRecurso {

    private static final Logger LOGGER = Logger.getLogger(DocumentoSistemaRecurso.class.getName());

    @Inject
    private DocumentoSistemaBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgDocumentoSistema
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgDocumentoSistema.class)))
        ,
    ...
    @Path("/{documentoSistemaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("documentoSistemaId") Long id) {
        try {
            SgDocumentoSistema entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgDocumentoSistema
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgDocumentoSistema.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroDocumentoSistema filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroDocumentoSistema filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DOCUMENTO_SISTEMA})
    public Response crear(SgDocumentoSistema entity) {
        try {
            entity.setDsiPk(null);
            entity.setDsiVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_DOCUMENTO_SISTEMA})
    public Response actualizar(@PathParam("documentoSistemaId") Long id, SgDocumentoSistema entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_DOCUMENTO_SISTEMA})
    public Response eliminarPorId(@PathParam("documentoSistemaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{documentoSistemaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("documentoSistemaId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgDocumentoSistema.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
