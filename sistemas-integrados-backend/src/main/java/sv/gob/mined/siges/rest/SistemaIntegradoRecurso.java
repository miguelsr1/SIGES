...
import sv.gob.mined.siges.negocio.servicios.SistemaIntegradoBean;
...
@RequestScoped
@Path("/v1/sistemasintegrados")
@Tag(name = "SistemaIntegrado API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SISTEMA_INTEGRADO,
    ConstantesOperaciones.ACTUALIZAR_SISTEMA_INTEGRADO,
    ConstantesOperaciones.ELIMINAR_SISTEMA_INTEGRADO})
public class SistemaIntegradoRecurso {

    private static final Logger LOGGER = Logger.getLogger(SistemaIntegradoRecurso.class.getName());

    @Inject
    private SistemaIntegradoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgSistemaIntegrado
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgSistemaIntegrado.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("sistemaIntegradoId") Long id) {
        try {
            SgSistemaIntegrado entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgSistemaIntegrado
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgSistemaIntegrado.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroSistemaIntegrado filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroSistemaIntegrado filtro) {
        try {
    ...
    public Response crear(SgSistemaIntegrado entity) {
        try {
            entity.setSinPk(null);
            entity.setSinVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SISTEMA_INTEGRADO})
    public Response actualizar(@PathParam("sistemaIntegradoId") Long id, SgSistemaIntegrado entity) {
        try {
    ...
    public Response obtenerHistorialPorId(@PathParam("sistemaIntegradoId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgSistemaIntegrado.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    @POST
    @Path("obtenerpromediocalificacionesporarea")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPromedioCalificacionesPorArea(FiltroPromedioCalificaciones filtro) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.obtenerPromedioCalificacionesPorArea(filtro)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
