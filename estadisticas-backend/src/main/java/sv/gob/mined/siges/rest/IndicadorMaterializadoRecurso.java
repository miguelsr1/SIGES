...
import sv.gob.mined.siges.negocio.servicios.IndicadorMaterializadoBean;
 ...
 */
@RequestScoped
@Path("/v1/indicadoresmaterializados")
@Tag(name = "Indicadores Materializados API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_INDICADORES_MATERIALIZADOS,
    ConstantesOperaciones.ELIMINAR_INDICADORES_MATERIALIZADOS})
public class IndicadorMaterializadoRecurso {

    private static final Logger LOGGER = Logger.getLogger(IndicadorMaterializadoRecurso.class.getName());

    @Inject
    private IndicadorMaterializadoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgIndicadorMaterializado
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgIndicadorMaterializado.class)))
        ,
    ...
    @Path("/{indicadorMaterializadoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("indicadorMaterializadoId") Long id) {
        try {
            SgIndicadorMaterializado ese = servicio.obtenerPorId(id);
            if (ese == null) {
     ...
     * @return Response Lista de SgIndicadorMaterializado
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgIndicadorMaterializado.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroIndicadorMaterializado filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroIndicadorMaterializado filtro) {
        try {
     ...
     *
     * @param ese SgIndicadorMaterializado
     * @return Response
     */
    @POST
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_INDICADORES_MATERIALIZADOS})
    public Response crear(FiltroEstadisticas fil) {
        try {
            servicio.crearMaterializacionIndicadorAnio(fil);
            ...
            if (PersistenceHelper.isConstraintViolation(ex)) {
                return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(Errores.ERROR_CARGA_EXTERNA_DUPLICADA)).build();
            } else {
                LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
                return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
            }
    ...
    @Path("/{indicadorId:[1-9][0-9]*}/{anio:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_INDICADORES_MATERIALIZADOS})
    public Response eliminar(@PathParam("indicadorId") Long id, @PathParam("anio") Integer anio) {
        try {
            if (servicio.objetoExistePorIdIndicadorYAnio(id, anio)) {
                servicio.eliminar(id, anio);
    ...
    @Path("/historial/{indicadorMaterializadoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("indicadorMaterializadoId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgIndicadorMaterializado.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgIndicadorMaterializado.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    @POST
    @Path("obtenerestadistica")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerEstadisticaDeIndicadorMaterializado(FiltroEstadisticas filtro) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.obtenerEstadisticaDeIndicadorMaterializado(filtro)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
