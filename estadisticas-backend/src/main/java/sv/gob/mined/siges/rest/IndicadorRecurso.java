...
import sv.gob.mined.siges.negocio.servicios.IndicadorBean;
...
@RequestScoped
@Path("/v1/indicadores")
@Tag(name = "Indicadores API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_INDICADORES,
    ConstantesOperaciones.ACTUALIZAR_INDICADORES,
    ConstantesOperaciones.ELIMINAR_INDICADORES})
public class IndicadorRecurso {

    private static final Logger LOGGER = Logger.getLogger(IndicadorRecurso.class.getName());

    @Inject
    private IndicadorBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEstIndicador.class))),
    ...
    @Path("/{indicadorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("indicadorId") Long id) {
        try {
            SgEstIndicador ind = servicio.obtenerPorId(id);
            if (ind == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(ind).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgIndicador
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgEstIndicador.class, type = SchemaType.ARRAY))),
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta."),
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    )
    @Path("/buscar")
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroIndicadores filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroIndicadores filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_INDICADORES})
    public Response crear(SgEstIndicador ind) {
        try {
            ind.setIndPk(null);
            ind.setIndVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ind)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_INDICADORES})
    public Response actualizar(@PathParam("indicadorId") Long id, SgEstIndicador ind) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ind.setIndPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ind)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_INDICADORES})
    public Response eliminarPorId(@PathParam("indicadorId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{indicadorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("indicadorId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgEstIndicador.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgEstIndicador.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
