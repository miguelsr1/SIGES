...
import sv.gob.mined.siges.negocio.servicios.PaginaBean;
import sv.gob.mined.siges.persistencia.utilidades.RevHistorico;

@RequestScoped
@Path("/v1/paginas")
@Tag(name = "Paginas API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_RH_PAGINA,
    ConstantesOperaciones.ACTUALIZAR_RH_PAGINA,
    ConstantesOperaciones.ELIMINAR_RH_PAGINA,
    ConstantesOperaciones.BUSCAR_RH_PAGINA})
public class PaginaRecurso {

    private static final Logger LOGGER = Logger.getLogger(PaginaRecurso.class.getName());

    @Inject
    private PaginaBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRhPagina.class)))
        ,
    ...
    @Path("/{paginaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_RH_PAGINA})
    public Response obtenerPorId(@PathParam("paginaId") Long id) {
        try {
            SgRhPagina entity = servicio.obtenerPorId(id);
     ...
     * @return Response Lista de SgRhPagina
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRhPagina.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_RH_PAGINA})
    public Response buscar(FiltroPagina filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_RH_PAGINA})
    public Response buscarTotal(FiltroPagina filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_RH_PAGINA})
    public Response crear(SgRhPagina entity) {
        try {
            entity.setPagPk(null);
            entity.setPagVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_RH_PAGINA})
    public Response actualizar(@PathParam("paginaId") Long id, SgRhPagina entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_RH_PAGINA})
    public Response eliminarPorId(@PathParam("paginaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{paginaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("paginaId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRhPagina.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, jwt.getSubject(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRhPagina.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, jwt.getSubject(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
