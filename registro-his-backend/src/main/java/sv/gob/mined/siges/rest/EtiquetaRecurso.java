...
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ...
    ConstantesOperaciones.BUSCAR_ETIQUETA})
public class EtiquetaRecurso {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRhEtiqueta.class)))
        ,
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ETIQUETA})
    public Response obtenerPorId(@PathParam("etiquetaId") Long id) {
        try {
            SgRhEtiqueta entity = servicio.obtenerPorId(id);
     ...
     * @return Response Lista de SgRhEtiqueta
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRhEtiqueta.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    public Response crear(SgRhEtiqueta entity) {
        try {
            entity.setEtiPk(null);
            entity.setEtiVersion(0);
    ...
    public Response actualizar(@PathParam("etiquetaId") Long id, SgRhEtiqueta entity) {
        try {
    ...
    public Response obtenerHistorialPorId(@PathParam("etiquetaId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRhEtiqueta.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, jwt.getSubject(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRhEtiqueta.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, jwt.getSubject(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
