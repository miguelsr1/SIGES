...
/**
 * Servicio REST correspondiente a los requerimientos de fondo
 *
 * @author Sofis Solutions
 */
@RequestScoped
@Path("/v1/requerimientosFondo")
@Tag(name = "Solicitudes de Transferencias API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SOLICITUD_DE_TRANSFERENCIA,
    ConstantesOperaciones.ACTUALIZAR_SOLICITUD_DE_TRANSFERENCIA,
    ConstantesOperaciones.ELIMINAR_SOLICITUD_DE_TRANSFERENCIA,
    ConstantesOperaciones.BUSCAR_SOLICITUD_DE_TRANSFERENCIA})
public class RequerimientoFondoRecurso {

    private static final Logger LOGGER = Logger.getLogger(RequerimientoFondoRecurso.class.getName());

    @Inject
    private RequerimientoFondoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRequerimientoFondo
     */
    @POST
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_DE_TRANSFERENCIA})
    public Response buscar(FiltroRequerimientosFondo filtro) {
        try {
     ...
     *
     * @param filtro FiltroTransferenciaACed
     * @return Response Lista de SgTransferenciaACed
     */
    @POST
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_DE_TRANSFERENCIA})
    public Response buscarTotal(FiltroRequerimientosFondo filtro) {
        try {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRequerimientoFondo.class)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
    ...
    public Response obtenerPorId(@PathParam("ayudaId") Long id) {
        try {
            SgRequerimientoFondo ayu = servicio.obtenerPorId(id);
            if (ayu == null) {
     ...
     *
     * @param pres SgRequerimientoFondo
     * @return Response SgRequerimientoFondo
     */
    @POST
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SOLICITUD_DE_TRANSFERENCIA})
    public Response crear(SgRequerimientoFondo pres) {
        try {
            pres.setStrPk(null);
            pres.setStrVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SOLICITUD_DE_TRANSFERENCIA})
    public Response actualizar(@PathParam("strPk") Long id, SgRequerimientoFondo ayu) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_SOLICITUD_DE_TRANSFERENCIA})
    ...
    /**
     * Devuelve los requerimientos acumulados por recurso
     *
     * @return Response Lista de RequerimientoPorRecurso
     */
    @GET
    @Operation(summary = "Devuelve los requerimientos acumulados por recurso.")
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = RequerimientoPorRecurso.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    )
    @Path("/acumuladas/recurso")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO, ConstantesOperaciones.BUSCAR_SOLICITUD_DE_TRANSFERENCIA})
    public Response obtenerReqPorRecurso() {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.obtenerReqPorRecurso()).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
     ...
     *
     * @param id Long
     * @return Response Lista de SgRequerimientoFondo
     */
    @GET
    ...
    public Response obtenerHistorialPorId(@PathParam("entityId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRequerimientoFondo.class, id)).build();
        } catch (Exception ex) {
     ...
     *
     * @return Response SgCuentasBancariasDD
     */
    @GET
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRequerimientoFondo.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
