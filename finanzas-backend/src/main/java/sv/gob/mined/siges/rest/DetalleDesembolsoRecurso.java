...
import sv.gob.mined.siges.negocio.servicios.DetalleDesembolsoBean;
...
/**
 * Servicios REST para el detalle desembolso recurso
 * @author Sofis Solutions
 */
@RequestScoped
@Path("/v1/detalledesembolso")
@Tag(name = "Detalle de desembolsos API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_DETALLE_DESEMBOLSO,
    ConstantesOperaciones.ACTUALIZAR_DETALLE_DESEMBOLSO,
    ConstantesOperaciones.ELIMINAR_DETALLE_DESEMBOLSO,
	ConstantesOperaciones.BUSCAR_DETALLE_DESEMBOLSO})
public class DetalleDesembolsoRecurso {

    private static final Logger LOGGER = Logger.getLogger(DetalleDesembolsoRecurso.class.getName());

    @Inject
    private DetalleDesembolsoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgDetalleDesembolso
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgDetalleDesembolso.class)))
        ,
    ...
    @Path("/{detalleDesembolsosId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DETALLE_DESEMBOLSO})
    public Response obtenerPorId(@PathParam("detalleDesembolsosId") Long id) {
        try {
            SgDetalleDesembolso ded = servicio.obtenerPorId(id,true);
            if (ded == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(ded).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgDetalleDesembolso
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgDetalleDesembolso.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DETALLE_DESEMBOLSO})
    public Response buscar(FiltroDetalleDesembolso filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DETALLE_DESEMBOLSO})
    public Response buscarTotal(FiltroDetalleDesembolso filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DETALLE_DESEMBOLSO})
    public Response crear(SgDetalleDesembolso ded) {
        try {
            ded.setDedPk(null);
            ded.setDedVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ded,true)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_DETALLE_DESEMBOLSO})
    public Response actualizar(@PathParam("detalleDesembolsosId") Long id, SgDetalleDesembolso ded) {
        try {
            if (servicio.objetoExistePorId(id,true)) {
                ded.setDedPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ded,true)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_DETALLE_DESEMBOLSO})
    public Response eliminarPorId(@PathParam("detalleDesembolsosId") Long id) {
        try {
            if (servicio.objetoExistePorId(id,true)) {
    ...
    @Path("/historial/{detalleDesembolsosId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("detalleDesembolsosId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgDetalleDesembolso.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgDetalleDesembolso.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
