...
import sv.gob.mined.siges.negocio.servicios.PresupuestoEscolarBean;
...
/**
 * Servicio REST correspondiente al presupuesto escolar.
 *
 * @author Sofis Solutions
 */
@RequestScoped
@Path("/v1/presupuestosescolares")
@Tag(name = "Presupuesto Escolar API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_PRESUPUESTOS_ESCOLARES,
    ConstantesOperaciones.ACTUALIZAR_PRESUPUESTOS_ESCOLARES,
    ConstantesOperaciones.ELIMINAR_PRESUPUESTOS_ESCOLARES})

public class PresupuestoEscolarRecurso {

    private static final Logger LOGGER = Logger.getLogger(PresupuestoEscolarRecurso.class.getName());

    @Inject
    private PresupuestoEscolarBean servicio;
    ...
    public Response obtenerPorId(@PathParam("ayudaId") Long id) {
        try {
            SgPresupuestoEscolar ayu = servicio.obtenerPorId(id, Boolean.TRUE);
            if (ayu == null) {
     ...
     * @return Response Lista de SgPresupuestoEscolar
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgPresupuestoEscolar.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroPresupuestoEscolar filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroPresupuestoEscolar filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_PRESUPUESTOS_ESCOLARES})
    public Response crear(SgPresupuestoEscolar pres) {
        try {
            pres.setPesPk(null);
            pres.setPesVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PRESUPUESTOS_ESCOLARES})
    public Response actualizar(@PathParam("pesPk") Long id, SgPresupuestoEscolar ayu) {
        try {
    ...
    @Path("/historial/{presupuestoEscolarId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("presupuestoEscolarId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgPresupuestoEscolar.class, id)).build();
        } catch (Exception ex) {
    ...
    @Path("/prepararAnulacion/{pesPk:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PRESUPUESTOS_ESCOLARES})
    public Response prepararAnulacion(@PathParam("pesPk") Long id, SgPresupuestoEscolar ayu) {
        try {
            if (servicio.objetoExistePorId(id, true)) {
                ayu.setPesPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.prepararAnulacion(ayu, true)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    //////
    /**
     * Elimina todos los movimientos ya consolidados en presupuesto Edicion.
     * autogenerada.
     *
     * @param id Long
     * @param pres SgPresupuestoEscolar
    ...
    @Path("/eliminandoMovEditados/{pesPk:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PRESUPUESTOS_ESCOLARES})
    public Response eliminandoMovEditados(@PathParam("pesPk") Long id, SgPresupuestoEscolar ayu) {
        try {
            if (servicio.objetoExistePorId(id, true)) {
                ayu.setPesPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.eliminandoMovEditados(ayu, true)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarFinanzas(Long filtro) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.buscarFinanzas(filtro)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
