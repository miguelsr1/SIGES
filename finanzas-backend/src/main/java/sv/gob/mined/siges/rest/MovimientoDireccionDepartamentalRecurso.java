...
import sv.gob.mined.siges.negocio.servicios.MovimientoDireccionDepartamentalBean;
 ...
 */
@RequestScoped
@Path("/v1/movimientodirecciondepartamental")
@Tag(name = "Movimiento Dirección Departamental API V1")
...
/**
 * Servicios REST para los movimientos de las cuentas bancarias de las
 * direcciones departamentales.
 */
public class MovimientoDireccionDepartamentalRecurso {

    private static final Logger LOGGER = Logger.getLogger(MovimientoDireccionDepartamentalRecurso.class.getName());

    @Inject
    private MovimientoDireccionDepartamentalBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMovimientoDireccionDepartamental
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMovimientoDireccionDepartamental.class)))
        ,
    ...
    @Path("/{movimientoDireccionDepartamentalId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MOVIMIENTO_CUENTA_BANCARIA})
    public Response obtenerPorId(@PathParam("movimientoDireccionDepartamentalId") Long id) {
        try {
            SgMovimientoDireccionDepartamental mdd = servicio.obtenerPorId(id);
            if (mdd == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(mdd).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMovimientoDireccionDepartamental
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgMovimientoDireccionDepartamental.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MOVIMIENTO_CUENTA_BANCARIA})
    public Response buscar(FiltroMovimientoDireccionDepartamental filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MOVIMIENTO_CUENTA_BANCARIA})
    public Response buscarTotal(FiltroMovimientoDireccionDepartamental filtro) {
        try {
    ...
    public Response crear(SgMovimientoDireccionDepartamental mdd) {
        try {
            mdd.setMddPk(null);
            mdd.setMddVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(mdd)).build();
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MOVIMIENTO_CUENTA_BANCARIA})
    public Response actualizar(@PathParam("movimientoDireccionDepartamentalId") Long id, SgMovimientoDireccionDepartamental mdd) {
        try {
            if (servicio.objetoExistePorId(id)) {
                mdd.setMddPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(mdd)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response eliminarPorId(@PathParam("movimientoDireccionDepartamentalId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{movimientoDireccionDepartamentalId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("movimientoDireccionDepartamentalId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMovimientoDireccionDepartamental.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgMovimientoDireccionDepartamental.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
