...
/**
 * DAO correspondiente a planes de compra
 *
 * @author jgiron
 */
public class PlanesCompraDAO extends HibernateJpaDAOImp<SgEncabezadoPlanCompras, Integer> implements Serializable {
     ...
     * @throws Exception
     */
    public PlanesCompraDAO(EntityManager em) throws Exception {
        super(em);
    ...
    private List<CriteriaTO> generarCriteria(FiltroEncabezadoPlanCompra filtro) {
        ...
        if (filtro.getAnioFiscalFk() != null) {
                MatchCriteriaTO criterio = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.EQUALS, "plaPresupuestoFk.pesAnioFiscalFk.aniAnio", filtro.getAnioFiscalFk());
                criterios.add(criterio);
        }

        if (filtro.getSedeFk() != null) {
            MatchCriteriaTO criterio = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.EQUALS, "plaPresupuestoFk.pesSedeFk.sedPk", filtro.getSedeFk());
            criterios.add(criterio);
        }

        if (filtro.getPlaEstado() != null) {
            MatchCriteriaTO criterio = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.EQUALS, "plaEstado", filtro.getPlaEstado());
            criterios.add(criterio);
        }
