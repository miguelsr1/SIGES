...
import sv.gob.mined.siges.negocio.servicios.EvaluacionItemLiquidacionBean;
...
/**
 * Servicios REST para la evaluación de items de liquidación
 * @author Sofis Solutions
 */
@RequestScoped
@Path("/v1/evaluacionitemsliquidacion")
@Tag(name = "Evaluación Items de Liquidacion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_EVALUACION_ITEM_LIQUIDACION,
    ConstantesOperaciones.ACTUALIZAR_EVALUACION_ITEM_LIQUIDACION,
    ConstantesOperaciones.ELIMINAR_EVALUACION_ITEM_LIQUIDACION,
	ConstantesOperaciones.BUSCAR_EVALUACION_ITEM_LIQUIDACION})
public class EvaluacionItemLiquidacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(EvaluacionItemLiquidacionRecurso.class.getName());

    @Inject
    private EvaluacionItemLiquidacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgEvaluacionItemLiquidacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEvaluacionItemLiquidacion.class)))
        ,
    ...
    @Path("/{evaluacionItemLiquidacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_EVALUACION_ITEM_LIQUIDACION})
    public Response obtenerPorId(@PathParam("evaluacionItemLiquidacionId") Long id) {
        try {
            SgEvaluacionItemLiquidacion eil = servicio.obtenerPorId(id);
            if (eil == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(eil).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgEvaluacionItemLiquidacion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgEvaluacionItemLiquidacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_EVALUACION_ITEM_LIQUIDACION})
    public Response buscar(FiltroEvaluacionItemLiquidacion filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_EVALUACION_ITEM_LIQUIDACION})
    public Response buscarTotal(FiltroEvaluacionItemLiquidacion filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_EVALUACION_ITEM_LIQUIDACION})
    public Response crear(SgEvaluacionItemLiquidacion eil) {
        try {
            eil.setEilPk(null);
            eil.setEilVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(eil,true)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_EVALUACION_ITEM_LIQUIDACION})
    public Response actualizar(@PathParam("evaluacionItemLiquidacionId") Long id, SgEvaluacionItemLiquidacion eil) {
        try {
            if (servicio.objetoExistePorId(id)) {
                eil.setEilPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(eil,true)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_EVALUACION_ITEM_LIQUIDACION})
    public Response eliminarPorId(@PathParam("evaluacionItemLiquidacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{evaluacionItemLiquidacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("evaluacionItemLiquidacionId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgEvaluacionItemLiquidacion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgEvaluacionItemLiquidacion.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
