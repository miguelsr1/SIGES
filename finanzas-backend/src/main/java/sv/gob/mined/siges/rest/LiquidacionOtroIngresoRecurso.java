...
import sv.gob.mined.siges.negocio.servicios.LiquidacionOtroIngresoBean;
...
/**
 * Servicios REST para la liquidación de otros ingresos
 * @author Sofis Solutions
 */
@RequestScoped
@Path("/v1/liquidacionsotrosingresos")
@Tag(name = "Liquidaciones otros ingresos API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_LIQUIDACION_OTRO_INGRESO,
    ConstantesOperaciones.ACTUALIZAR_LIQUIDACION_OTRO_INGRESO,
    ConstantesOperaciones.ELIMINAR_LIQUIDACION_OTRO_INGRESO,
	ConstantesOperaciones.BUSCAR_LIQUIDACION_OTRO_INGRESO})
public class LiquidacionOtroIngresoRecurso {

    private static final Logger LOGGER = Logger.getLogger(LiquidacionOtroIngresoRecurso.class.getName());

    @Inject
    private LiquidacionOtroIngresoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgLiquidacionOtroIngreso
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgLiquidacionOtroIngreso.class)))
        ,
    ...
    @Path("/{liquidacionOtroIngresoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_LIQUIDACION_OTRO_INGRESO})
    public Response obtenerPorId(@PathParam("liquidacionOtroIngresoId") Long id) {
        try {
            SgLiquidacionOtroIngreso loi = servicio.obtenerPorId(id);
            if (loi == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(loi).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgLiquidacionOtroIngreso
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgLiquidacionOtroIngreso.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_LIQUIDACION_OTRO_INGRESO})
    public Response buscar(FiltroLiquidacionOtroIngreso filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_LIQUIDACION_OTRO_INGRESO})
    public Response buscarTotal(FiltroLiquidacionOtroIngreso filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_LIQUIDACION_OTRO_INGRESO})
    public Response crear(SgLiquidacionOtroIngreso loi) {
        try {
            loi.setLoiPk(null);
            loi.setLoiVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(loi,true)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_LIQUIDACION_OTRO_INGRESO})
    public Response actualizar(@PathParam("liquidacionOtroIngresoId") Long id, SgLiquidacionOtroIngreso loi) {
        try {
            if (servicio.objetoExistePorId(id)) {
                loi.setLoiPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(loi,true)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_LIQUIDACION_OTRO_INGRESO})
    public Response eliminarPorId(@PathParam("liquidacionOtroIngresoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{liquidacionOtroIngresoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("liquidacionOtroIngresoId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgLiquidacionOtroIngreso.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgLiquidacionOtroIngreso.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response guardarConMovimientos(SgLiquidacionOtroIngreso liq) {
        try {
            liq.setLoiPk(null);
            liq.setLoiVersion(0);
