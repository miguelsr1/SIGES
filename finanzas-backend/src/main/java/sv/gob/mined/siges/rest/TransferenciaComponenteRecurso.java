...
import sv.gob.mined.siges.negocio.servicios.TransferenciaComponenteBean;
import sv.gob.mined.siges.persistencia.entidades.siap2.SsTransferenciaComponente;

/**
 * Servicio REST correspondiente a las trasnferencias a las direcciones.
 *
 * @author Sofis Solutions
 */
@RequestScoped
@Path("/v1/transferencias/componentes")
...
public class TransferenciaComponenteRecurso {

    private static final Logger LOGGER = Logger.getLogger(TransferenciaComponenteRecurso.class.getName());

    @Inject
    private TransferenciaComponenteBean servicio;
     ...
     *
     * @param id Long
     * @return Response SsTransferenciaComponente
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SsTransferenciaComponente.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("transferenciaId") Long id) {
        try {
            SsTransferenciaComponente st = servicio.obtenerPorId(id);
            if (st == null) {
     ...
     * @return Response Lista de SsTransferenciaComponente
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SsTransferenciaComponente.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_TRANSFERENCIAS_COMPONENTES})
    public Response buscar(FiltroTransferenciaComponente filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_TRANSFERENCIAS_COMPONENTES})
    public Response buscarTotal(FiltroTransferenciaComponente filtro) {
        try {
    ...
    public Response obtenerHistorialPorId(@PathParam("transferenciaId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SsTransferenciaComponente.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SsTransferenciaComponente.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
