...
import sv.gob.mined.siges.negocio.servicios.EvaluacionItemLiquidacionOtrosBean;
 ...
 */
@RequestScoped
@Path("/v1/evaluacionitemsliquidacionotro")
@Tag(name = "Evaluación Items de Liquidacion Otros API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_EVALUACION_ITEM_LIQUIDACION_OTRO,
    ConstantesOperaciones.ACTUALIZAR_EVALUACION_ITEM_LIQUIDACION_OTRO,
    ConstantesOperaciones.ELIMINAR_EVALUACION_ITEM_LIQUIDACION_OTRO,
	ConstantesOperaciones.BUSCAR_EVALUACION_ITEM_LIQUIDACION_OTRO})
public class EvaluacionItemLiquidacionOtrosRecurso {

    private static final Logger LOGGER = Logger.getLogger(EvaluacionItemLiquidacionOtrosRecurso.class.getName());

    @Inject
    private EvaluacionItemLiquidacionOtrosBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgEvaluacionItemLiquidacionOtros
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEvaluacionItemLiquidacionOtros.class)))
        ,
    ...
    @Path("/{evaluacionItemLiquidacionOtrosId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_EVALUACION_ITEM_LIQUIDACION_OTRO})
    public Response obtenerPorId(@PathParam("evaluacionItemLiquidacionOtrosId") Long id) {
        try {
            SgEvaluacionItemLiquidacionOtros elo = servicio.obtenerPorId(id);
            if (elo == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(elo).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgEvaluacionItemLiquidacionOtros
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgEvaluacionItemLiquidacionOtros.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_EVALUACION_ITEM_LIQUIDACION_OTRO})
    public Response crear(SgEvaluacionItemLiquidacionOtros elo) {
        try {
            elo.setEloPk(null);
            elo.setEloVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(elo,true)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_EVALUACION_ITEM_LIQUIDACION_OTRO})
    public Response actualizar(@PathParam("evaluacionItemLiquidacionOtrosId") Long id, SgEvaluacionItemLiquidacionOtros elo) {
        try {
            if (servicio.objetoExistePorId(id)) {
                elo.setEloPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(elo,true)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_EVALUACION_ITEM_LIQUIDACION_OTRO})
    public Response eliminarPorId(@PathParam("evaluacionItemLiquidacionOtrosId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{evaluacionItemLiquidacionOtrosId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("evaluacionItemLiquidacionOtrosId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgEvaluacionItemLiquidacionOtros.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgEvaluacionItemLiquidacionOtros.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
