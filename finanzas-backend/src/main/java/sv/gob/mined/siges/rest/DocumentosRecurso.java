...
import sv.gob.mined.siges.negocio.servicios.DocumentosBean;
...
@RequestScoped
@Path("/v1/documentos")
@Tag(name = "Documentos API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_DOCUMENTOS,
    ConstantesOperaciones.ACTUALIZAR_DOCUMENTOS,
    ConstantesOperaciones.ELIMINAR_DOCUMENTOS,
    ConstantesOperaciones.BUSCAR_DOCUMENTOS})
/**
 * Servicios REST para los documentos del presupuesto.
 */
public class DocumentosRecurso {

    private static final Logger LOGGER = Logger.getLogger(DocumentosRecurso.class.getName());

    @Inject
    private DocumentosBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgDocumentos.class)))
        ,
    ...
    @Path("/{documentosId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DOCUMENTOS})
    public Response obtenerPorId(@PathParam("documentosId") Long id) {
        try {
            SgDocumentos doc = servicio.obtenerPorId(id);
            if (doc == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(doc).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgDocumentos
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgDocumentos.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DOCUMENTOS})
    public Response buscar(FiltroDocumentos filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DOCUMENTOS})
    public Response buscarTotal(FiltroDocumentos filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DOCUMENTOS})
    public Response crear(SgDocumentos doc) {
        try {
            doc.setDocPk(null);
            doc.setDocVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(doc)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_DOCUMENTOS})
    public Response actualizar(@PathParam("documentosId") Long id, SgDocumentos doc) {
        try {
            if (servicio.objetoExistePorId(id)) {
                doc.setDocPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(doc)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_DOCUMENTOS})
    public Response eliminarPorId(@PathParam("documentosId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{documentosId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("documentosId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgDocumentos.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgDocumentos.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
