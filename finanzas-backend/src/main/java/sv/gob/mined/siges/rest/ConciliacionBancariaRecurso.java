...
import sv.gob.mined.siges.negocio.servicios.ConciliacionBancariaBean;
...
/**
 * Servicios REST para las conciliaciones bancarias
 * @author Sofis Solutions
 */
@RequestScoped
@Path("/v1/conciliaciones")
@Tag(name = "Conciliaciones API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CONCILIACION_BANCARIA,
    ConstantesOperaciones.ACTUALIZAR_CONCILIACION_BANCARIA,
    ConstantesOperaciones.BUSCAR_CONCILIACION_BANCARIA,
    ConstantesOperaciones.ELIMINAR_CONCILIACION_BANCARIA})

public class ConciliacionBancariaRecurso {

    private static final Logger LOGGER = Logger.getLogger(ConciliacionBancariaRecurso.class.getName());

    @Inject
    private ConciliacionBancariaBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgConciliacionesBancarias
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgConciliacionesBancarias.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("conId") Long id) {
        try {
     ...
     * @return Response Lista de SgConciliacionesBancarias
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgConciliacionesBancarias.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroConciliacionBancaria filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CONCILIACION_BANCARIA})
    public Response buscarTotal(FiltroConciliacionBancaria filtro) {
        try {
     ...
     *
     * @param com SgConciliacionesBancarias
     * @return Response SgConciliacionesBancarias
     */
    @POST
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CONCILIACION_BANCARIA})
    public Response crear(SgConciliacionesBancarias com) {
        try {
            com.setConPk(null);
            com.setConVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CONCILIACION_BANCARIA})
    public Response actualizar(@PathParam("conPk") Long id, SgConciliacionesBancarias fac) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CONCILIACION_BANCARIA})
    ...
    @Path("/historial/{conciliacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("conciliacionId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgConciliacionesBancarias.class, id)).build();
        } catch (Exception ex) {
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgConciliacionesBancarias.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
