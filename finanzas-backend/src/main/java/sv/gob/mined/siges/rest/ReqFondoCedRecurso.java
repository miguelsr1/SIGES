...
import sv.gob.mined.siges.negocio.servicios.ReqFondoCedBean;
import sv.gob.mined.siges.negocio.servicios.RequerimientoFondoBean;
...
/**
 * Servicio REST correspondiente a los requerimientos de fondos de los centros
 * educativos.
 *
 * @author Sofis Solutions
 */
@RequestScoped
@Path("/v1/reqfondosced")
@Tag(name = "Requerimientos de Fondos a Centros Educativos API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REQUERIMIENTO_FONDO_A_CED,
    ConstantesOperaciones.ACTUALIZAR_REQUERIMIENTO_FONDO_A_CED,
    ConstantesOperaciones.ELIMINAR_REQUERIMIENTO_FONDO_A_CED,
    ConstantesOperaciones.BUSCAR_REQUERIMIENTO_FONDO_A_CED})

public class ReqFondoCedRecurso {

    private static final Logger LOGGER = Logger.getLogger(ReqFondoCedRecurso.class.getName());

    @Inject
    private ReqFondoCedBean servicio;
    
    @Inject
    private RequerimientoFondoBean requerimientoFondoServicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgReqFondoCed.class)))
        ,
    ...
    @Path("/{reqFondoCedId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REQUERIMIENTO_FONDO_A_CED})
    public Response obtenerPorId(@PathParam("reqFondoCedId") Long id) {
        try {
            SgReqFondoCed rfc = servicio.obtenerPorId(id);
            if (rfc == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(rfc).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgReqFondoCed
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgReqFondoCed.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REQUERIMIENTO_FONDO_A_CED})
    public Response buscar(FiltroReqFondoCed filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REQUERIMIENTO_FONDO_A_CED})
    public Response buscarTotal(FiltroReqFondoCed filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REQUERIMIENTO_FONDO_A_CED})
    public Response crear(SgReqFondoCed rfc) {
        try {
            rfc.setRfcPk(null);
            rfc.setRfcVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(rfc, true)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REQUERIMIENTO_FONDO_A_CED})
    public Response actualizar(@PathParam("reqFondoCedId") Long id, SgReqFondoCed rfc) {
        try {
            if (servicio.objetoExistePorId(id)) {
                rfc.setRfcPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(rfc, true)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REQUERIMIENTO_FONDO_A_CED})
    public Response eliminarPorId(@PathParam("reqFondoCedId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{reqFondoCedId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("reqFondoCedId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgReqFondoCed.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgReqFondoCed.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    /**
     * Devuelve el detalle de un requerimiento de fondo
     *
     * @param id 
     * @return Response List<ResultadoAcumuladorDTO>
     */
    @GET
    @Operation(summary = "Realiza la consulta del detalle de requerimientos de fondo")
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = DetalleRequerimientoFondo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    )
    @Path("/detalle/requerimiento/{reqId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerDetalleReqFondo(@PathParam("reqId") Long id) {
        try {
            if(requerimientoFondoServicio.objetoExistePorId(id,Boolean.TRUE)){
                return Response.status(HttpStatus.SC_OK).entity(servicio.obtenerDetalleReqFondo(id)).build();
            }
            else{
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
