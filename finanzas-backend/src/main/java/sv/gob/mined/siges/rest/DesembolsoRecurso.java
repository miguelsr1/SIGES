...
import sv.gob.mined.siges.negocio.servicios.DesembolsoBean;
...
@RequestScoped
@Path("/v1/desembolsos")
@Tag(name = "Desembolsos API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_DESEMBOLSO,
    ConstantesOperaciones.ACTUALIZAR_DESEMBOLSO,
    ConstantesOperaciones.ELIMINAR_DESEMBOLSO,
    ConstantesOperaciones.BUSCAR_DESEMBOLSO})
/**
 * Servicios REST para los desembolsos de las transferencias.
 */
public class DesembolsoRecurso {

    private static final Logger LOGGER = Logger.getLogger(DesembolsoRecurso.class.getName());

    @Inject
    private DesembolsoBean servicio;
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DESEMBOLSO})
    public Response buscar(FiltroDesembolso filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DESEMBOLSO})
    public Response buscarTotal(FiltroDesembolso filtro) {
        try {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgDesembolso.class)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
    ...
    public Response obtenerPorId(@PathParam("ayudaId") Long id) {
        try {
            SgDesembolso ayu = servicio.obtenerPorId(id, Boolean.TRUE);
            if (ayu == null) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DESEMBOLSO})
    public Response crear(SgDesembolso pres) {
        try {
            pres.setDsbPk(null);
            pres.setDsbVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_DESEMBOLSO})
    public Response actualizar(@PathParam("movPk") Long id, SgDesembolso ayu) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_DESEMBOLSO})
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DESEMBOLSO})
    public Response crearConDetalle(DesembolsoDDE des) {
        try {
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardarConDetalle(des, true)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
    ...
    /**
     * Crea los movimientos a las cuentas de las DDE desde los desembolsos
     *
     * @param pres SgDesembolso
     * @return Response SgDesembolso
     */
    @POST
    @Operation(summary = "Crea los movimientos a las cuentas de las DDE desde los desembolsos.")
    @APIResponses(value = {
        @APIResponse(responseCode = "201", description = "Entidad creada satisfactoriamente.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgDesembolso.class)))
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DESEMBOLSO})
    public Response crearMovimientos(DesembolsoMovimiento des) {
        try {
//            if(servicio.objetoExistePorId(des.getDesembolsoPk(), Boolean.TRUE)){
                return Response.status(HttpStatus.SC_CREATED).entity(servicio.generarMovimientos(des, Boolean.TRUE)).build();
//            }
//            else{
//                return Response.status(HttpStatus.SC_NOT_FOUND).build();
//            }
