...
import sv.gob.mined.siges.negocio.servicios.TransferenciaDireccionDepBean;
...
/**
 * Servicio REST correspondiente a las transferencias a las direcciones
 * departamentales
 *
 * @author Sofis Solutions
 */
@RequestScoped
@Path("/v1/transferenciasdirecciondepartamental")
@Tag(name = "Transferencias Dirección Departamental API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_TRANSFERENCIA_DIRECCION_DEPARTAMENTAL,
    ConstantesOperaciones.ACTUALIZAR_TRANSFERENCIA_DIRECCION_DEPARTAMENTAL,
    ConstantesOperaciones.ELIMINAR_TRANSFERENCIA_DIRECCION_DEPARTAMENTAL,
    ConstantesOperaciones.BUSCAR_TRANSFERENCIA_DIRECCION_DEPARTAMENTAL})
public class TransferenciaDireccionDepRecurso {

    private static final Logger LOGGER = Logger.getLogger(TransferenciaDireccionDepRecurso.class.getName());

    @Inject
    private TransferenciaDireccionDepBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgTransferenciaDireccionDep
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgTransferenciaDireccionDep.class)))
        ,
    ...
    @Path("/{transferenciaDireccionDepId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_TRANSFERENCIA_DIRECCION_DEPARTAMENTAL})
    public Response obtenerPorId(@PathParam("transferenciaDireccionDepId") Long id) {
        try {
            SgTransferenciaDireccionDep tdd = servicio.obtenerPorId(id);
            if (tdd == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tdd).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgTransferenciaDireccionDep
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgTransferenciaDireccionDep.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_TRANSFERENCIA_DIRECCION_DEPARTAMENTAL})
    public Response buscar(FiltroTransferenciaDireccionDep filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_TRANSFERENCIA_DIRECCION_DEPARTAMENTAL})
    public Response buscarTotal(FiltroTransferenciaDireccionDep filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_TRANSFERENCIA_DIRECCION_DEPARTAMENTAL})
    public Response crear(SgTransferenciaDireccionDep tdd) {
        try {
            tdd.setTddPk(null);
            tdd.setTddVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(tdd, true)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TRANSFERENCIA_DIRECCION_DEPARTAMENTAL})
    public Response actualizar(@PathParam("transferenciaDireccionDepId") Long id, SgTransferenciaDireccionDep tdd) {
        try {
            if (servicio.objetoExistePorId(id)) {
                tdd.setTddPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(tdd, true)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_TRANSFERENCIA_DIRECCION_DEPARTAMENTAL})
    public Response eliminarPorId(@PathParam("transferenciaDireccionDepId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{transferenciaDireccionDepId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("transferenciaDireccionDepId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgTransferenciaDireccionDep.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgTransferenciaDireccionDep.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
