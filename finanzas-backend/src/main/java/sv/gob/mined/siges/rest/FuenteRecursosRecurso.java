...
import sv.gob.mined.siges.negocio.servicios.FuenteRecursosBean;
import sv.gob.mined.siges.persistencia.entidades.siap2.SsFuenteRecursos;

/**
 * Servicios REST para las fuentes de recursos
 *
 * @author Sofis Solutions
 */
@RequestScoped
@Path("/v1/fuente/recursos")
@Tag(name = "Cuentas API V1")

public class FuenteRecursosRecurso {

    private static final Logger LOGGER = Logger.getLogger(FuenteRecursosRecurso.class.getName());

    @Inject
    private FuenteRecursosBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SsFuenteRecursos.class, type = SchemaType.ARRAY)))
        ,
    ...
    public Response buscar(FiltroFuenteRecursos filtro) {
        try {
     ...
     * @return Response Lista de SgPlanCompras
     */
    @POST
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PLAN_DE_COMPRAS})
    public Response buscarTotal(FiltroFuenteRecursos filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarFuente(Long movPk) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.objetoExistePorIdFuente(movPk)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
