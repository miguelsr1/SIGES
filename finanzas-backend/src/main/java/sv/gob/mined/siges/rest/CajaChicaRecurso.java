...
import sv.gob.mined.siges.negocio.servicios.CajaChicaBean;
 ...
 */
@RequestScoped
@Path("/v1/cuentasbancariascc")
@Tag(name = "Cuentas Bancarias Caja Chica API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CUENTASBANCARIASCC,
    ConstantesOperaciones.ACTUALIZAR_CUENTASBANCARIASCC,
    ConstantesOperaciones.ELIMINAR_CUENTASBANCARIASCC,
    ConstantesOperaciones.BUSCAR_CUENTASBANCARIASCC})
/**
 * Servicios REST para las cuentas bancarias de los centros educativos.
 */
public class CajaChicaRecurso {

    private static final Logger LOGGER = Logger.getLogger(CajaChicaRecurso.class.getName());

    @Inject
    private CajaChicaBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCajaChica.class)))
        ,
    ...
    @Path("/{cuentasBancariasCCId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CUENTASBANCARIASCC})
    public Response obtenerPorId(@PathParam("cuentasBancariasCCId") Long id) {
        try {
            SgCajaChica bcc = servicio.obtenerPorId(id);
            if (bcc == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(bcc).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgCajaChica
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgCajaChica.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CUENTASBANCARIASCC})
    public Response buscar(FiltroCajaChica filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CUENTASBANCARIASCC})
    public Response buscarTotal(FiltroCajaChica filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CUENTASBANCARIASCC})
    public Response crear(SgCajaChica bcc) {
        try {
            bcc.setBccPk(null);
            bcc.setBccVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(bcc)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CUENTASBANCARIASCC})
    public Response actualizar(@PathParam("cuentasBancariasCCId") Long id, SgCajaChica bcc) {
        try {
            if (servicio.objetoExistePorId(id)) {
                bcc.setBccPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(bcc)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CUENTASBANCARIASCC})
    public Response eliminarPorId(@PathParam("cuentasBancariasCCId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{cuentasBancariasCCId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("cuentasBancariasCCId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgCajaChica.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCajaChica.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
