...
import sv.gob.mined.siges.negocio.servicios.ChequeraBean;
...
/**
 * Servicios REST para las chequeras
 * @author Sofis Solutions
 */
@RequestScoped
@Path("/v1/chequeras")
@Tag(name = "Chequeras API V1")

public class ChequeraRecurso {

    private static final Logger LOGGER = Logger.getLogger(ChequeraRecurso.class.getName());

    @Inject
    private ChequeraBean servicio;
    @Inject
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgChequera.class, type = SchemaType.ARRAY)))
        ,
    ...
    @Timed
    public Response buscar(FiltroChequera filtro) {
        try {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgChequera.class)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
    ...
    public Response obtenerPorIdLazy(@PathParam("entityId") Long entityId) {
        try {
            SgChequera sede = servicio.obtenerPorIdLazy(entityId);
            if (sede == null) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CHEQUERA})
    public Response crear(SgChequera com) {
        try {
            com.setChePk(null);
            com.setCheVersion(0);
     ...
     * @return Response Lista de SgChequera
     */
    @POST
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroChequera filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CHEQUERA})
    ...
    public Response actualizar(@PathParam("chePk") Long id, SgChequera che) {
        try {
            if (servicio.objetoExistePorId(id, true)) {
                che.setChePk(id);
