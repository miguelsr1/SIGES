...
import sv.gob.mined.siges.negocio.servicios.ProveedorBean;
...
/**
 * Servicio REST correspondiente a los proveedores
 *
 * @author Sofis Solutions
 */
@RequestScoped
@Path("/v1/proveedores")
@Tag(name = "Proveedor API V1")

public class ProveedorRecurso {

    private static final Logger LOGGER = Logger.getLogger(ProveedorRecurso.class.getName());

    @Inject
    private ProveedorBean servicio;
    @Inject
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SsProveedor.class, type = SchemaType.ARRAY)))
        ,
    ...
    @Timed
    public Response buscar(FiltroProveedor filtro) {
        try {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SsProveedor.class)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
    ...
    public Response obtenerPorIdLazy(@PathParam("entityId") Long entityId) {
        try {
            SsProveedor sede = servicio.obtenerPorIdLazy(entityId);
            if (sede == null) {
    ...
    public Response crear(SsProveedor com) {
        try {
            com.setProId(null);
            com.setProVersion(0);
     ...
     * @return Response Lista de SsProveedor
     */
    @POST
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroProveedor filtro) {
        try {
    ...
    public Response actualizar(@PathParam("chePk") Long id, SsProveedor che) {
        try {
            if (servicio.objetoExistePorId(id, true)) {
                che.setProId(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(che, true)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
            ...
            if (PersistenceHelper.isOptimisticException(ex)) {
                ...
                return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(Errores.ERROR_CHEQUERA_DUPLICADA)).build();
            } else {
                LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
                return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
            }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PROVEEDOR})
