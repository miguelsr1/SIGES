...
import sv.gob.mined.siges.negocio.servicios.CuentaEsBean;
...
/**
 * Servicios REST para las cuentas
...
public class CuentaRecurso {
    ...
    private static final Logger LOGGER = Logger.getLogger(CuentaRecurso.class.getName());

    @Inject
    private CuentaEsBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SsCuenta.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    )
    @Path("/buscarUnidadPresupuestaria")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarUnidadPresupuestaria(FiltroCuenta filtro) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.buscarUnidadPresupuestaria(filtro)).build();
        } catch (Exception ex) {
    ...
    @Path("/buscarLineaPresupuestaria")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarLineaPresupuestaria(FiltroCuenta filtro) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.buscarLineaPresupuestaria(filtro)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroCuenta filtro) {
        try {
    ...
    public Response buscarTotal(FiltroCuenta filtro) {
        try {
