...
import sv.gob.mined.siges.negocio.servicios.TransferenciasBean;
import sv.gob.mined.siges.persistencia.entidades.siap2.SsTransferencia;

/**
 * Servicio REST correspondiente a las trasnferencias componente
 *
 * @author Sofis Solutions
 */
@RequestScoped
@Path("/v1/transferencias")
@Tag(name = "Transferencias API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.BUSCAR_TRANSFERENCIAS_COMPONENTES})
public class TransferenciasRecurso {

    private static final Logger LOGGER = Logger.getLogger(TransferenciasRecurso.class.getName());

    @Inject
    private TransferenciasBean servicio;
     ...
     *
     * @param id Long
     * @return Response SsTransferencia
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SsTransferencia.class)))
        ,
    ...
    @Path("/{transferenciaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_TRANSFERENCIAS_COMPONENTES})
    public Response obtenerPorId(@PathParam("transferenciaId") Long id) {
        try {
            SsTransferencia st = servicio.obtenerPorId(id);
            if (st == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(st).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SsTransferencia
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SsTransferencia.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_TRANSFERENCIAS_COMPONENTES})
    public Response buscar(FiltroTransferencia filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_TRANSFERENCIAS_COMPONENTES})
    public Response buscarTotal(FiltroTransferencia filtro) {
        try {
    ...
    @Path("/historial/{transferenciaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("transferenciaId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SsTransferencia.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SsTransferencia.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = IngresosEgresosReporte.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    public Response buscarParaReporte(LibroIngresosEgresos libTrans) {
        try {
            if (libTrans != null) {
                return Response.status(HttpStatus.SC_OK).entity(servicio.buscarParaReporte(libTrans)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
    ...
    @Path("/buscarParaReporteComponentes")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MOVIMIENTO_CUENTA_BANCARIA})
    public Response buscarParaReporteComponentes(LibroIngresosEgresos libTrans) {
        try {
            if (libTrans != null) {
                return Response.status(HttpStatus.SC_OK).entity(servicio.buscarParaReporteComponentes(libTrans)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
