...
import sv.gob.mined.siges.negocio.servicios.RecibosBean;
...
/**
 * Servicio REST correspondiente a los recibos.
 *
 * @author Sofis Solutions
 */
@RequestScoped
@Path("/v1/recibos")
@Tag(name = "Recibos API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_RECIBOS,
    ConstantesOperaciones.ACTUALIZAR_RECIBOS,
    ConstantesOperaciones.ELIMINAR_RECIBOS,
    ConstantesOperaciones.BUSCAR_RECIBOS})
public class RecibosRecurso {

    private static final Logger LOGGER = Logger.getLogger(RecibosRecurso.class.getName());

    @Inject
    private RecibosBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRecibos.class)))
        ,
    ...
    @Path("/{recibosId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_RECIBOS})
    public Response obtenerPorId(@PathParam("recibosId") Long id) {
        try {
            SgRecibos rec = servicio.obtenerPorId(id);
            if (rec == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(rec).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRecibos
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRecibos.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_RECIBOS})
    public Response buscar(FiltroRecibos filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_RECIBOS})
    public Response buscarTotal(FiltroRecibos filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_RECIBOS})
    public Response crear(SgRecibos rec) {
        try {
            rec.setRecPk(null);
            rec.setRecVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(rec)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_RECIBOS})
    public Response actualizar(@PathParam("recibosId") Long id, SgRecibos rec) {
        try {
            if (servicio.objetoExistePorId(id)) {
                rec.setRecPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(rec)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_RECIBOS})
    public Response eliminarPorId(@PathParam("recibosId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{recibosId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("recibosId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgRecibos.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRecibos.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
