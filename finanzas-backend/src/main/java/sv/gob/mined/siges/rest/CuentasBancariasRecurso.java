...
import sv.gob.mined.siges.negocio.servicios.CuentasBancariasBean;
 ...
 */
@RequestScoped
@Path("/v1/cuentas_bancarias")
@Tag(name = "Cuentas Bancarias API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CUENTAS_BANCARIAS,
    ConstantesOperaciones.ACTUALIZAR_CUENTAS_BANCARIAS,
    ConstantesOperaciones.ELIMINAR_CUENTAS_BANCARIAS,
    ConstantesOperaciones.BUSCAR_CUENTAS_BANCARIAS})
/**
 * Servicios REST para las cuentas bancarias (genérico).
 */
public class CuentasBancariasRecurso {

    private static final Logger LOGGER = Logger.getLogger(CuentasBancariasRecurso.class.getName());

    @Inject
    private CuentasBancariasBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgCuentasBancarias
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCuentasBancarias.class)))
        ,
    ...
    @Path("/{cuentasBancariasId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CUENTAS_BANCARIAS})
    public Response obtenerPorId(@PathParam("cuentasBancariasId") Long id) {
        try {
            SgCuentasBancarias cbc = servicio.obtenerPorId(id);
            if (cbc == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(cbc).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgCuentasBancarias
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgCuentasBancarias.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CUENTAS_BANCARIAS})
    public Response buscar(FiltroCuentasBancarias filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CUENTAS_BANCARIAS})
    public Response buscarTotal(FiltroCuentasBancarias filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CUENTAS_BANCARIAS})
    public Response crear(SgCuentasBancarias cbc) {
        try {
            cbc.setCbcPk(null);
            cbc.setCbcVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(cbc)).build();
            ...
            if (PersistenceHelper.isConstraintViolation(ex)) {
                return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(Errores.ERROR_NUM_CUENTA_DUPLICADO)).build();
            } else {
                LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
                return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
            }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CUENTAS_BANCARIAS})
    public Response actualizar(@PathParam("cuentasBancariasId") Long id, SgCuentasBancarias cbc) {
        try {
            if (servicio.objetoExistePorId(id)) {
                cbc.setCbcPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(cbc)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CUENTAS_BANCARIAS})
    public Response eliminarPorId(@PathParam("cuentasBancariasId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{cuentasBancariasId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("cuentasBancariasId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgCuentasBancarias.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCuentasBancarias.class, id, rev)).build();
        } catch (Exception ex) {
