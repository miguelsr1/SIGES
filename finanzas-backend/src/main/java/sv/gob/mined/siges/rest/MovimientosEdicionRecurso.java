...
import sv.gob.mined.siges.negocio.servicios.MovimientosEdicionBean;
 ...
 */
@RequestScoped
@Path("/v1/edicion_movimientos")
@Tag(name = "Presupuesto Escolar API V1")
@DeclareRoles({
    ConstantesOperaciones.CREAR_EDICION_PRESPUESTO_ESCOLAR_APROBADO,
    ConstantesOperaciones.ACTUALIZAR_EDICION_PRESPUESTO_ESCOLAR_APROBADO,
    ConstantesOperaciones.ELIMINAR_EDICION_PRESPUESTO_ESCOLAR_APROBADO})
/**
 * Servicios REST para los movimientos del presupuesto escolar.
 */
public class MovimientosEdicionRecurso {

    private static final Logger LOGGER = Logger.getLogger(MovimientosEdicionRecurso.class.getName());

    @Inject
    private MovimientosEdicionBean servicio;
    ...
    public Response obtenerPorId(@PathParam("ayudaId") Long id) {
        try {
            SgMovimientosEdicion ayu = servicio.obtenerPorId(id, Boolean.TRUE);
            if (ayu == null) {
    ...
    public Response crear(SgMovimientosEdicion pres) {
        try {
            pres.setMovPk(null);
    ...
    public Response actualizar(@PathParam("movPk") Long id, SgMovimientosEdicion ayu) {
        try {
     ...
     * @return Response Lista de SgMovimientosEdicion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgMovimientosEdicion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroMovimientosEdicion filtro) {
        try {
    ...
    @Path("/eliminarAnular/{ayudaId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PRESUPUESTOS_ESCOLARES})
    public Response eliminarAnularPorId(@PathParam("ayudaId") Long id) {
        try {
            if (servicio.objetoExistePorIdOriginal(id, true)) {
                servicio.eliminarAnular(id);
                return Response.status(HttpStatus.SC_OK).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
    ...
    /**
     * Elimina todos los objetos de la id indicada
     *
     * @param id Long
     * @return Response
     */
    ...
    @Path("/eliminarEditados/{ayudaId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PRESUPUESTOS_ESCOLARES})
    public Response eliminarEditados(@PathParam("ayudaId") Long id) {

        try {
            if (servicio.objetoPresPorId(id, true)) {
                servicio.eliminarEditados(id);
                return Response.status(HttpStatus.SC_OK).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
