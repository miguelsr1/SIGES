 ...
 */
@Entity
@Table(name = "sg_presupuesto_escolar_movimiento", schema = Constantes.SCHEMA_FINANZAS)
@XmlRootElement
...
public class SgMovimientos implements Serializable {
    ...
    @OneToMany(mappedBy = "movOriginalEditado", fetch = FetchType.LAZY)
    private List<SgMovimientosEdicion> editados;
    ...
    public List<SgMovimientosEdicion> getEditados() {
        return editados;
    }

    public void setEditados(List<SgMovimientosEdicion> editados) {
    ...
    public int hashCode() {
        int hash = 3;
        hash = 29 * hash + Objects.hashCode(this.movPk);
        return hash;
    }
