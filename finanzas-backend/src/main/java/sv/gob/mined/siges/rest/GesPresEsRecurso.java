...
import sv.gob.mined.siges.negocio.servicios.GesPresEsBean;
 ...
 */
@RequestScoped
@Path("/v1/gesPresEsRecurso")
@Tag(name = "GesPresEsRecurso API V1")
/**
 * Servicios REST para la gestión del presupuesto.
 */
public class GesPresEsRecurso {

    private static final Logger LOGGER = Logger.getLogger(GesPresEsRecurso.class.getName());

    @Inject
    private GesPresEsBean servicio;
    ...
    @Path("/buscarSubcomponente")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarSubcomponente(FiltroGesPresEs filtro) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.buscarSubcomponente(filtro)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
