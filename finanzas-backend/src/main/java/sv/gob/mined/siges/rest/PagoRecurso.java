...
import sv.gob.mined.siges.negocio.servicios.PagoBean;
...
/**
 * Servicio REST correspondiente a los pagos
 * @author Sofis Solutions
 */
@RequestScoped
@Path("/v1/pagos")
@Tag(name = "Pagos API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_PAGOS,
    ConstantesOperaciones.ACTUALIZAR_PAGOS,
    ConstantesOperaciones.ELIMINAR_PAGOS,
    ConstantesOperaciones.BUSCAR_PAGOS})
public class PagoRecurso {

    private static final Logger LOGGER = Logger.getLogger(PagoRecurso.class.getName());

    @Inject
    private PagoBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgPago.class)))
        ,
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PAGOS})
    public Response obtenerPorId(@PathParam("pagoId") Long id) {
        try {
            SgPago pgs = servicio.obtenerPorId(id);
            if (pgs == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(pgs).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgPago
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgPago.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PAGOS})
    public Response buscar(FiltroPago filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PAGOS})
    public Response buscarTotal(FiltroPago filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_PAGOS})
    public Response crear(SgPago pgs) {
        try {
            pgs.setPgsPk(null);
            pgs.setPgsVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(pgs)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PAGOS})
    public Response actualizar(@PathParam("pagoId") Long id, SgPago pgs) {
        try {
            if (servicio.objetoExistePorId(id)) {
                pgs.setPgsPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(pgs)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Path("/{pagoId:[1-9][0-9]*}/{razon}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PAGOS})
    public Response eliminarPorId(@PathParam("pagoId") Long id, @PathParam("razon") String razon) {
        try {
            if (servicio.objetoExistePorId(id)) {
                servicio.eliminar(id, razon);
    ...
    @Path("/historial/{pagoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("pagoId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgPago.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgPago.class, id, rev)).build();
        } catch (Exception ex) {
