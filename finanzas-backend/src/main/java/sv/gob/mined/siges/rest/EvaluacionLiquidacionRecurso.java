...
import sv.gob.mined.siges.negocio.servicios.EvaluacionLiquidacionBean;
...
/**
 * Servicios REST para la evaluación de liquidación
 * @author Sofis Solutions
 */
@RequestScoped
@Path("/v1/evaluacionesliquidaciones")
@Tag(name = "Evaluaciones de Liquidaciones API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_EVALUACION_LIQUIDACION,
    ConstantesOperaciones.ACTUALIZAR_EVALUACION_LIQUIDACION,
    ConstantesOperaciones.ELIMINAR_EVALUACION_LIQUIDACION,
	ConstantesOperaciones.BUSCAR_EVALUACION_LIQUIDACION})
public class EvaluacionLiquidacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(EvaluacionLiquidacionRecurso.class.getName());

    @Inject
    private EvaluacionLiquidacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgEvaluacionLiquidacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEvaluacionLiquidacion.class)))
        ,
    ...
    @Path("/{evaluacionLiquidacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_EVALUACION_LIQUIDACION})
    public Response obtenerPorId(@PathParam("evaluacionLiquidacionId") Long id) {
        try {
            SgEvaluacionLiquidacion elq = servicio.obtenerPorId(id);
            if (elq == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(elq).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgEvaluacionLiquidacion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgEvaluacionLiquidacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_EVALUACION_LIQUIDACION})
    public Response crear(SgEvaluacionLiquidacion elq) {
        try {
            elq.setElqPk(null);
            elq.setElqVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(elq,true)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_EVALUACION_LIQUIDACION})
    public Response actualizar(@PathParam("evaluacionLiquidacionId") Long id, SgEvaluacionLiquidacion elq) {
        try {
            if (servicio.objetoExistePorId(id)) {
                elq.setElqPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(elq,true)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_EVALUACION_LIQUIDACION})
    public Response eliminarPorId(@PathParam("evaluacionLiquidacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{evaluacionLiquidacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("evaluacionLiquidacionId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgEvaluacionLiquidacion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgEvaluacionLiquidacion.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
