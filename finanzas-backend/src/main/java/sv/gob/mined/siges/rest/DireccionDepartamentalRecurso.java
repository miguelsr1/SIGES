...
import sv.gob.mined.siges.negocio.servicios.DireccionDepartamentalBean;
 ...
 */
@RequestScoped
@Path("/v1/direcciondepartamental")
@Tag(name = "Dirección Departamental API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_DIRECCION_DEPARTAMENTAL,
    ConstantesOperaciones.ACTUALIZAR_DIRECCION_DEPARTAMENTAL,
    ConstantesOperaciones.ELIMINAR_DIRECCION_DEPARTAMENTAL,
    ConstantesOperaciones.BUSCAR_DIRECCION_DEPARTAMENTAL})
/**
 * Servicios REST para las direcciones departamentales (pagadurías).
 */
public class DireccionDepartamentalRecurso {

    private static final Logger LOGGER = Logger.getLogger(DireccionDepartamentalRecurso.class.getName());

    @Inject
    private DireccionDepartamentalBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgDireccionDepartamental
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgDireccionDepartamental.class)))
        ,
    ...
    @Path("/{direccionDepartamentalId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DIRECCION_DEPARTAMENTAL})
    public Response obtenerPorId(@PathParam("direccionDepartamentalId") Long id) {
        try {
            SgDireccionDepartamental ded = servicio.obtenerPorId(id);
            if (ded == null) {
     ...
     * @return Response Lista de SgDireccionDepartamental
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgDireccionDepartamental.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DIRECCION_DEPARTAMENTAL})
    public Response buscar(FiltroDireccionDepartamental filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DIRECCION_DEPARTAMENTAL})
    public Response buscarTotal(FiltroDireccionDepartamental filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DIRECCION_DEPARTAMENTAL})
    public Response crear(SgDireccionDepartamental ded) {
        try {
            ded.setDedPk(null);
            ded.setDedVersion(0);
            ...
            if (PersistenceHelper.isConstraintViolation(ex)) {
                return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(Errores.ERROR_DEPARTAMENTO_DUPLICADO)).build();
            } else {
                LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
                return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
            }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_DIRECCION_DEPARTAMENTAL})
    public Response actualizar(@PathParam("direccionDepartamentalId") Long id, SgDireccionDepartamental ded) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ded.setDedPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ded, Boolean.TRUE)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_DIRECCION_DEPARTAMENTAL})
    public Response eliminarPorId(@PathParam("direccionDepartamentalId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    public Response obtenerHistorialPorId(@PathParam("entityId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgDireccionDepartamental.class, id)).build();
        } catch (Exception ex) {
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgDireccionDepartamental.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
