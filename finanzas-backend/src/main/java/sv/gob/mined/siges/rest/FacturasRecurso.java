...
import sv.gob.mined.siges.negocio.servicios.FacturaBean;
...
/**
 * Servicios REST para las facturas
 *
 * @author Sofis Solutions
 */
@RequestScoped
@Path("/v1/facturas")
@Tag(name = "Facturas API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_FACTURAS,
    ConstantesOperaciones.ACTUALIZAR_FACTURAS,
    ConstantesOperaciones.BUSCAR_FACTURAS,
    ConstantesOperaciones.ELIMINAR_FACTURAS})

public class FacturasRecurso {

    private static final Logger LOGGER = Logger.getLogger(FacturasRecurso.class.getName());

    @Inject
    private FacturaBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgFactura.class)))
        ,
    ...
    @Path("/{facdaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("facdaId") Long id) {
        try {
            Boolean fac = servicio.objetoExistePorId(id, Boolean.TRUE);
            if (fac == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(fac).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgFactura
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgFactura.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroFactura filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_FACTURAS})
    public Response buscarTotal(FiltroFactura filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_FACTURAS})
    public Response crear(SgFactura com) {
        try {
            com.setFacPk(null);
            com.setFacVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_FACTURAS})
    public Response actualizar(@PathParam("facPk") Long id, SgFactura fac) {
        try {
            if (servicio.objetoExistePorId(id, true)) {
                fac.setFacPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(fac, true)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_FACTURAS})
    ...
    @Path("/historial/{facturaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("facturaId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgFactura.class, id)).build();
        } catch (Exception ex) {
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgFactura.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    /**
     * Anula el objeto con la id indicada. Si no existe, lo crea con id
     * autogenerada.
     *
     * @param id Long
     * @param com SgFactura
    ...
    @Path("/anular/{facPk:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_FACTURAS})
    public Response anular(@PathParam("facPk") Long id, SgFactura fac) {
        try {
            if (servicio.objetoExistePorId(id, true)) {
                fac.setFacPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.anular(fac, true)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    /**
     * Cambia a Pagado el objeto con la id indicada. Si no existe, lo crea con
     * id autogenerada.
     *
     * @param id Long
     * @param com SgFactura
    ...
    @Path("/pagar/{facPk:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_FACTURAS})
    public Response pagar(@PathParam("facPk") Long id, SgFactura fac) {
        try {
            if (servicio.objetoExistePorId(id, true)) {
                fac.setFacPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.pagar(fac, true)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    public Response obtenerPorIdLazy(@PathParam("entityId") Long entityId) {
        try {
            SgFactura factura = servicio.obtenerPorIdLazy(entityId);
            if (factura == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(factura).build();
            }
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
