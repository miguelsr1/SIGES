...
import sv.gob.mined.siges.negocio.servicios.EvaluacionLiquidacionOtroBean;
 ...
 */
@RequestScoped
@Path("/v1/evaluacionliquidacionotro")
@Tag(name = "Evaluación Liquidacion Otros API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_EVALUACION_LIQUIDACION_OTRO,
    ConstantesOperaciones.ACTUALIZAR_EVALUACION_LIQUIDACION_OTRO,
    ConstantesOperaciones.ELIMINAR_EVALUACION_LIQUIDACION_OTRO,
	ConstantesOperaciones.BUSCAR_EVALUACION_LIQUIDACION_OTRO})
public class EvaluacionLiquidacionOtroRecurso {

    private static final Logger LOGGER = Logger.getLogger(EvaluacionLiquidacionOtroRecurso.class.getName());

    @Inject
    private EvaluacionLiquidacionOtroBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgEvaluacionLiquidacionOtro
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEvaluacionLiquidacionOtro.class)))
        ,
    ...
    @Path("/{evaluacionLiquidacionOtroId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_EVALUACION_LIQUIDACION_OTRO})
    public Response obtenerPorId(@PathParam("evaluacionLiquidacionOtroId") Long id) {
        try {
            SgEvaluacionLiquidacionOtro eli = servicio.obtenerPorId(id);
            if (eli == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(eli).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgEvaluacionLiquidacionOtro
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgEvaluacionLiquidacionOtro.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_EVALUACION_LIQUIDACION_OTRO})
    public Response crear(SgEvaluacionLiquidacionOtro eli) {
        try {
            eli.setEliPk(null);
            eli.setEliVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(eli,true)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_EVALUACION_LIQUIDACION_OTRO})
    public Response actualizar(@PathParam("evaluacionLiquidacionOtroId") Long id, SgEvaluacionLiquidacionOtro eli) {
        try {
            if (servicio.objetoExistePorId(id)) {
                eli.setEliPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(eli,true)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_EVALUACION_LIQUIDACION_OTRO})
    public Response eliminarPorId(@PathParam("evaluacionLiquidacionOtroId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{evaluacionLiquidacionOtroId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("evaluacionLiquidacionOtroId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgEvaluacionLiquidacionOtro.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgEvaluacionLiquidacionOtro.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
