...
import sv.gob.mined.siges.negocio.servicios.CuentasBancariasDDBean;
 ...
 */
@RequestScoped
@Path("/v1/cuentasbancariasdd")
@Tag(name = "Cuentas Bancarias Dirección Departamental API V1")
...
/**
 * Servicios REST para las cuentas bancarias de las direcciones departamentales.
 */
public class CuentasBancariasDDRecurso {

    private static final Logger LOGGER = Logger.getLogger(CuentasBancariasDDRecurso.class.getName());

    @Inject
    private CuentasBancariasDDBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "CocbcPknsumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgCuentasBancariasDD.class)))
        ,
    ...
    @Path("/{cuentasBancariasDDId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CUENTAS_BANCARIAS})
    public Response obtenerPorId(@PathParam("cuentasBancariasDDId") Long id) {
        try {
            SgCuentasBancariasDD cbd = servicio.obtenerPorId(id);
            if (cbd == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(cbd).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgCuentasBancariasDD
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgCuentasBancariasDD.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CUENTAS_BANCARIAS})
    public Response buscar(FiltroCuentasBancariasDD filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CUENTAS_BANCARIAS})
    public Response buscarTotal(FiltroCuentasBancariasDD filtro) {
        try {
    ...
    public Response crear(SgCuentasBancariasDD cbd) {
        try {
            cbd.setCbdPk(null);
            cbd.setCbdVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(cbd)).build();
    ...
    public Response actualizar(@PathParam("cuentasBancariasDDId") Long id, SgCuentasBancariasDD cbd) {
        try {
            if (servicio.objetoExistePorId(id)) {
                cbd.setCbdPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(cbd)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    public Response eliminarPorId(@PathParam("cuentasBancariasDDId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    public Response obtenerHistorialPorId(@PathParam("entityId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgCuentasBancariasDD.class, id)).build();
        } catch (Exception ex) {
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCuentasBancariasDD.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
