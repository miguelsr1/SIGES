...
import sv.gob.mined.siges.negocio.servicios.MetodoAdqBean;
...
@RequestScoped
@Path("/v1/metodosadq")
@Tag(name = "Metodo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.BUSCAR_METODO_ADQ})
/**
 * Servicios REST para los métodos de adquisición.
 */
public class MetodoAdqRecurso {

    private static final Logger LOGGER = Logger.getLogger(MetodoAdqRecurso.class.getName());

    @Inject
    private MetodoAdqBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SsMetodoAdq.class)))
        ,
    ...
    @Path("/{MetodoAdqId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_METODO_ADQ})
    public Response obtenerPorId(@PathParam("MetodoAdqId") Long id) {
        try {
            SsMetodoAdq entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SsMetodoAdq
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SsMetodoAdq.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_METODO_ADQ})
    public Response buscar(FiltroMetodoAdq filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_METODO_ADQ})
    public Response buscarTotal(FiltroMetodoAdq filtro) {
        try {
    ...
    @Path("/historial/{MetodoAdqId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("MetodoAdqId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SsMetodoAdq.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SsMetodoAdq.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
