...
import sv.gob.mined.siges.negocio.servicios.MovimientoCajaChicaBean;
 ...
 */
@RequestScoped
@Path("/v1/movimientoscajachica")
@Tag(name = "Movimietos de Caja Chica API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MOVIMIENTOSCAJACHICA,
    ConstantesOperaciones.BUSCAR_MOVIMIENTOSCAJACHICA})
/**
 * Servicios REST para los movimientos de caja chica.
 */
public class MovimientoCajaChicaRecurso {

    private static final Logger LOGGER = Logger.getLogger(MovimientoCajaChicaRecurso.class.getName());

    @Inject
    private MovimientoCajaChicaBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMovimientoCajaChica
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMovimientoCajaChica.class)))
        ,
    ...
    @Path("/{movimientoCajaChicaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MOVIMIENTOSCAJACHICA})
    public Response obtenerPorId(@PathParam("movimientoCajaChicaId") Long id) {
        try {
            SgMovimientoCajaChica mcc = servicio.obtenerPorId(id);
            if (mcc == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(mcc).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMovimientoCajaChica
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgMovimientoCajaChica.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MOVIMIENTOSCAJACHICA})
    public Response buscar(FiltroMovimientoCajaChica filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MOVIMIENTOSCAJACHICA})
    public Response buscarTotal(FiltroMovimientoCajaChica filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MOVIMIENTOSCAJACHICA})
    public Response crear(SgMovimientoCajaChica mcc) {
        try {
            mcc.setMccPk(null);
            mcc.setMccVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(mcc)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response actualizar(@PathParam("movimientoCajaChicaId") Long id, SgMovimientoCajaChica mcc) {
        try {
            if (servicio.objetoExistePorId(id)) {
                mcc.setMccPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(mcc)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response eliminarPorId(@PathParam("movimientoCajaChicaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{movimientoCajaChicaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("movimientoCajaChicaId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMovimientoCajaChica.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgMovimientoCajaChica.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
     ...
     *
     * @param filtro FiltroCodiguera
     * @return Response Lista de CuentaBancariaReporte
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = CajaChicaReporte.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MOVIMIENTOSCAJACHICA})
    public Response buscarParaReporte(LibroCajaChica libCaja) {
        try {
            if (libCaja != null) {
                return Response.status(HttpStatus.SC_OK).entity(servicio.buscarParaReporte(libCaja)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
