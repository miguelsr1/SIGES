...
import sv.gob.mined.siges.negocio.servicios.MovimientoLiquidacionOtroBean;
...
/**
 * Servicios REST para los movimientos de la liquidación de otro ingresos
 * @author Sofis Solutions
 */
@RequestScoped
@Path("/v1/movimientosliquidacionotros")
@Tag(name = "Movimientos liquidaciones de otros ingresos API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MOVIMIENTOS_LIQUIDACION_OTROS,
    ConstantesOperaciones.ACTUALIZAR_MOVIMIENTOS_LIQUIDACION_OTROS,
    ConstantesOperaciones.ELIMINAR_MOVIMIENTOS_LIQUIDACION_OTROS,
	ConstantesOperaciones.BUSCAR_MOVIMIENTOS_LIQUIDACION_OTROS})
public class MovimientoLiquidacionOtroRecurso {

    private static final Logger LOGGER = Logger.getLogger(MovimientoLiquidacionOtroRecurso.class.getName());

    @Inject
    private MovimientoLiquidacionOtroBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMovimientoLiquidacionOtro
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMovimientoLiquidacionOtro.class)))
        ,
    ...
    @Path("/{movimientoLiquidacionOtroId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MOVIMIENTOS_LIQUIDACION_OTROS})
    public Response obtenerPorId(@PathParam("movimientoLiquidacionOtroId") Long id) {
        try {
            SgMovimientoLiquidacionOtro mlo = servicio.obtenerPorId(id);
            if (mlo == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(mlo).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMovimientoLiquidacionOtro
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgMovimientoLiquidacionOtro.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MOVIMIENTOS_LIQUIDACION_OTROS})
    public Response buscar(FiltroMovimientoLiquidacionOtro filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MOVIMIENTOS_LIQUIDACION_OTROS})
    public Response buscarTotal(FiltroMovimientoLiquidacionOtro filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MOVIMIENTOS_LIQUIDACION_OTROS})
    public Response crear(SgMovimientoLiquidacionOtro mlo) {
        try {
            mlo.setMloPk(null);
            mlo.setMloVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(mlo,true)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MOVIMIENTOS_LIQUIDACION_OTROS})
    public Response actualizar(@PathParam("movimientoLiquidacionOtroId") Long id, SgMovimientoLiquidacionOtro mlo) {
        try {
            if (servicio.objetoExistePorId(id)) {
                mlo.setMloPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(mlo,true)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MOVIMIENTOS_LIQUIDACION_OTROS})
    public Response eliminarPorId(@PathParam("movimientoLiquidacionOtroId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{movimientoLiquidacionOtroId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("movimientoLiquidacionOtroId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMovimientoLiquidacionOtro.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgMovimientoLiquidacionOtro.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
