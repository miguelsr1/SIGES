...
import sv.gob.mined.siges.negocio.servicios.MovimientoLiquidacionBean;
...
/**
 * Servicio REST correspondiente a los movimientos de liquidación
 * @author Sofis Solutions
 */
@RequestScoped
@Path("/v1/movimientosliquidacion")
@Tag(name = "Movimientos liquidacion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MOVIMIENTOS_LIQUIDACION,
    ConstantesOperaciones.ACTUALIZAR_MOVIMIENTOS_LIQUIDACION,
    ConstantesOperaciones.ELIMINAR_MOVIMIENTOS_LIQUIDACION,
	ConstantesOperaciones.BUSCAR_MOVIMIENTOS_LIQUIDACION})
public class MovimientoLiquidacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(MovimientoLiquidacionRecurso.class.getName());

    @Inject
    private MovimientoLiquidacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMovimientoLiquidacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMovimientoLiquidacion.class)))
        ,
    ...
    @Path("/{movimientoLiquidacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MOVIMIENTOS_LIQUIDACION})
    public Response obtenerPorId(@PathParam("movimientoLiquidacionId") Long id) {
        try {
            SgMovimientoLiquidacion mlq = servicio.obtenerPorId(id);
            if (mlq == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(mlq).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMovimientoLiquidacion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgMovimientoLiquidacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MOVIMIENTOS_LIQUIDACION})
    public Response buscar(FiltroMovimientoLiquidacion filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MOVIMIENTOS_LIQUIDACION})
    public Response buscarTotal(FiltroMovimientoLiquidacion filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MOVIMIENTOS_LIQUIDACION})
    public Response crear(SgMovimientoLiquidacion mlq) {
        try {
            mlq.setMlqPk(null);
            mlq.setMlqVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(mlq,true)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MOVIMIENTOS_LIQUIDACION})
    public Response actualizar(@PathParam("movimientoLiquidacionId") Long id, SgMovimientoLiquidacion mlq) {
        try {
            if (servicio.objetoExistePorId(id)) {
                mlq.setMlqPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(mlq,true)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MOVIMIENTOS_LIQUIDACION})
    public Response eliminarPorId(@PathParam("movimientoLiquidacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{movimientoLiquidacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("movimientoLiquidacionId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMovimientoLiquidacion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgMovimientoLiquidacion.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
