...
import sv.gob.mined.siges.negocio.servicios.RelPersonalEspecialidadBean;
...
@RequestScoped
@Path("/v1/relpersonalespecialidades")
@Tag(name = "RelacionPersonalEspecialidad API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_PERSONAL_ESPECIALIDAD,
    ConstantesOperaciones.ACTUALIZAR_REL_PERSONAL_ESPECIALIDAD,
    ConstantesOperaciones.ELIMINAR_REL_PERSONAL_ESPECIALIDAD})
public class RelPersonalEspecialidadRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelPersonalEspecialidadRecurso.class.getName());

    @Inject
    private RelPersonalEspecialidadBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelPersonalEspecialidad
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelPersonalEspecialidad.class)))
        ,
    ...
    @Path("/{relPersonalEspecialidadId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("relPersonalEspecialidadId") Long id) {
        try {
            SgRelPersonalEspecialidad entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgRelPersonalEspecialidad
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelPersonalEspecialidad.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroRelPersonalEspecialidad filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroRelPersonalEspecialidad filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_PERSONAL_ESPECIALIDAD})
    public Response crear(SgRelPersonalEspecialidad entity) {
        try {
            entity.setRpePk(null);
            entity.setRpeVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_PERSONAL_ESPECIALIDAD})
    public Response actualizar(@PathParam("relPersonalEspecialidadId") Long id, SgRelPersonalEspecialidad entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_PERSONAL_ESPECIALIDAD})
    public Response eliminarPorId(@PathParam("relPersonalEspecialidadId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relPersonalEspecialidadId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relPersonalEspecialidadId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRelPersonalEspecialidad.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelPersonalEspecialidad.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
