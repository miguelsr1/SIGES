...
import sv.gob.mined.siges.negocio.servicios.PlanEstudioBean;
...
public class PlanEstudioRescurso {

    private static final Logger LOGGER = Logger.getLogger(PlanEstudioRescurso.class.getName());

    @Inject
    private PlanEstudioBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgPlanEstudio.class)))
        ,
    ...
    @Path("/{planEstudioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PLANES_ESTUDIO})
    public Response obtenerPorId(@PathParam("planEstudioId") Long id) {
        try {
            SgPlanEstudio entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgPlanEstudio
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgPlanEstudio.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PLANES_ESTUDIO})
    public Response buscar(FiltroPlanEstudio filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PLANES_ESTUDIO})
    public Response buscarTotal(FiltroPlanEstudio filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_PLANES_ESTUDIO})
    public Response crear(SgPlanEstudio entity) {
        try {
            entity.setPesPk(null);
            entity.setPesVersion(0);
            LOGGER.info("crear");
    ...
    public Response actualizar(@PathParam("planEstudioId") Long id, SgPlanEstudio entity) {
        try {
    ...
    public Response eliminarPorId(@PathParam("planEstudioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{planEstudioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("planEstudioId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgPlanEstudio.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgPlanEstudio.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
