...
import sv.gob.mined.siges.negocio.servicios.RelGradoPrecedenciaBean;
 ...
 */
@RequestScoped
@Path("/v1/relgradoprecedencia")
@Tag(name = "Rel Grado Precedencia API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.MODIFICAR_REL_GRADO_PRECEDENCIA})
public class RelGradoPrecedenciaRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelGradoPrecedenciaRecurso.class.getName());

    @Inject
    private RelGradoPrecedenciaBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelGradoPrecedencia
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelGradoPrecedencia.class)))
        ,
    ...
    @Path("/{relGradoPrecedenciaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("relGradoPrecedenciaId") Long id) {
        try {
            SgRelGradoPrecedencia rgp = servicio.obtenerPorId(id);
            if (rgp == null) {
     ...
     * @return Response Lista de SgRelGradoPrecedencia
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelGradoPrecedencia.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroRelGradoPrecedencia filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroRelGradoPrecedencia filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.MODIFICAR_REL_GRADO_PRECEDENCIA})
    public Response crear(SgRelGradoPrecedencia rgp) {
        try {
            rgp.setRgpPk(null);
            rgp.setRgpVersion(0);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.MODIFICAR_REL_GRADO_PRECEDENCIA})
    public Response actualizar(@PathParam("relGradoPrecedenciaId") Long id, SgRelGradoPrecedencia rgp) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.MODIFICAR_REL_GRADO_PRECEDENCIA})
    public Response eliminarPorId(@PathParam("relGradoPrecedenciaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
