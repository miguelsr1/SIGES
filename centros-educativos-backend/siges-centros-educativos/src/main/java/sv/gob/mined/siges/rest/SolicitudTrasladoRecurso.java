...
import sv.gob.mined.siges.negocio.servicios.SolicitudTrasladoBean;
...
@RequestScoped
@Path("/v1/solicitudestraslado")
@Tag(name = "SolicitudTraslado API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SOLICITUD_TRASLADO,
    ConstantesOperaciones.ACTUALIZAR_SOLICITUD_TRASLADO,
    ConstantesOperaciones.ELIMINAR_SOLICITUD_TRASLADO,
    ConstantesOperaciones.BUSCAR_SOLICITUD_TRASLADO,
    ConstantesOperaciones.CONFIRMAR_TRASLADO})
public class SolicitudTrasladoRecurso {

    private static final Logger LOGGER = Logger.getLogger(SolicitudTrasladoRecurso.class.getName());

    @Inject
    private SolicitudTrasladoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgSolicitudTraslado
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgSolicitudTraslado.class)))
        ,
    ...
    @Path("/{solicitudTrasladoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_TRASLADO})
    public Response obtenerPorId(@PathParam("solicitudTrasladoId") Long id) {
        try {
            SgSolicitudTraslado entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     *
     * @param filtro FiltroSolicitudTraslado
     * @return Response Lista de SgSolicitudTraslado
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgSolicitudTraslado.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_TRASLADO})
    public Response buscar(FiltroSolicitudTraslado filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_TRASLADO})
    public Response buscarTotal(FiltroSolicitudTraslado filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SOLICITUD_TRASLADO})
    public Response crear(SgSolicitudTraslado entity) {
        try {
            entity.setSotPk(null);
            entity.setSotVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SOLICITUD_TRASLADO})
    public Response actualizar(@PathParam("solicitudTrasladoId") Long id, SgSolicitudTraslado entity) {
        try {
     ...
     * @return Response SgConfirmacionSolicitudTraslado
     */
    @POST
    @Operation(summary = "Crea el objeto indicado con id autogenerada.")
    @APIResponses(value = {
        @APIResponse(responseCode = "201", description = "Entidad creada satisfactoriamente.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgConfirmacionSolicitudTraslado.class)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "422", description = "Entidad no se puede procesar. Datos incorrectos.")
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CONFIRMAR_TRASLADO})
    public Response confirmarFirma(@PathParam("firmaTransactionId") String firmaTransactionId) {
        try {
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.confirmarFirma(firmaTransactionId)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
    ...
    /**
     * Preconfirmar solicitud de traslado
     *
     * @param cma SgConfirmacionSolicitudTraslado
     * @return Response SgConfirmacionSolicitudTraslado
     */
    @POST
    @Operation(summary = "Crea el objeto indicado con id autogenerada.")
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CONFIRMAR_TRASLADO})
    public Response preconfirmar(SgConfirmacionSolicitudTraslado cma) {
        try {
            SgConfirmacionSolicitudTraslado guardada = servicio.generarConfirmacionSolicitudTraslado(cma); //para poder generar el reporte, debe estar guardada
            cma.setSotPk(guardada.getSotPk());
            cma.setSotVersion(guardada.getSotVersion());
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.preconfirmar(cma)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_SOLICITUD_TRASLADO})
    public Response eliminarPorId(@PathParam("solicitudTrasladoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{solicitudTrasladoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("solicitudTrasladoId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgSolicitudTraslado.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgSolicitudTraslado.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
