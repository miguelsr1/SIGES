...
import sv.gob.mined.siges.negocio.servicios.AsistenciaPersonalBean;
...
@RequestScoped
@Path("/v1/asistenciaspersonal")
@Tag(name = "AsistenciaPersonalPersonal API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ASISTENCIA_PERSONAL,
    ConstantesOperaciones.ACTUALIZAR_ASISTENCIA_PERSONAL,
    ConstantesOperaciones.ELIMINAR_ASISTENCIA_PERSONAL,
    ConstantesOperaciones.BUSCAR_ASISTENCIA_PERSONAL})
public class AsistenciaPersonalRecurso {

    private static final Logger LOGGER = Logger.getLogger(AsistenciaPersonalRecurso.class.getName());

    @Inject
    private AsistenciaPersonalBean servicio;
     ...
     *
     * @param asistenciaPersonalId
     * @return Response -> SgAsistenciaPersonal
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAsistenciaPersonal.class)))
        ,
    ...
    @Path("/{asistenciaPersonalId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ASISTENCIA_PERSONAL})
    public Response obtenerPorId(@PathParam("asistenciaPersonalId") Long asistenciaPersonalId) {
        try {
            SgAsistenciaPersonal entity = servicio.obtenerPorId(asistenciaPersonalId);
            if (entity == null) {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAsistenciaPersonal.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ASISTENCIA_PERSONAL})
    public Response buscar(FiltroAsistenciaPersonal filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ASISTENCIA_PERSONAL})
    public Response buscarTotal(FiltroAsistenciaPersonal filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ASISTENCIA_PERSONAL})
    public Response crear(SgAsistenciaPersonal entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ASISTENCIA_PERSONAL})
    public Response actualizar(@PathParam("asistenciaPersonalId") Long id, SgAsistenciaPersonal entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ASISTENCIA_PERSONAL})
    public Response eliminarPorId(@PathParam("asistenciaPersonalId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{asistenciaPersonalId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("asistenciaPersonalId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgAsistenciaPersonal.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgAsistenciaPersonal.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
