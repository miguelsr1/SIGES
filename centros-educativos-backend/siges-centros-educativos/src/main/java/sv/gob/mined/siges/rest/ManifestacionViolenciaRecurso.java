...
import sv.gob.mined.siges.negocio.servicios.ManifestacionViolenciaBean;
...
@RequestScoped
@Path("/v1/manifestacionesviolencia")
@Tag(name = "ManifestacionViolencia API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MANIFESTACIONES_VIOLENCIA,
    ConstantesOperaciones.ACTUALIZAR_MANIFESTACIONES_VIOLENCIA,
    ConstantesOperaciones.ELIMINAR_MANIFESTACIONES_VIOLENCIA,
    ConstantesOperaciones.BUSCAR_MANIFESTACIONES_VIOLENCIA})
public class ManifestacionViolenciaRecurso {

    private static final Logger LOGGER = Logger.getLogger(ManifestacionViolenciaRecurso.class.getName());

    @Inject
    private ManifestacionViolenciaBean servicio;
     ...
     *
     * @param manifestacionViolenciaId
     * @return Response -> SgManifestacionViolencia
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgManifestacionViolencia.class)))
        ,
    ...
    @Path("/{manifestacionViolenciaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MANIFESTACIONES_VIOLENCIA})
    public Response obtenerPorId(@PathParam("manifestacionViolenciaId") Long manifestacionViolenciaId) {
        try {
            SgManifestacionViolencia entity = servicio.obtenerPorId(manifestacionViolenciaId);
            if (entity == null) {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgManifestacionViolencia.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MANIFESTACIONES_VIOLENCIA})
    public Response buscar(FiltroManifestacionViolencia filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MANIFESTACIONES_VIOLENCIA})
    public Response buscarTotal(FiltroManifestacionViolencia filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MANIFESTACIONES_VIOLENCIA})
    public Response crear(SgManifestacionViolencia entity) {
        try {
            entity.setMviPk(null);
            entity.setMviVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MANIFESTACIONES_VIOLENCIA, ConstantesOperaciones.MODIFICAR_MANIFESTACION_VIOLENCIA_PROPIA})
    public Response actualizar(@PathParam("manifestacionViolenciaId") Long id, SgManifestacionViolencia entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MANIFESTACIONES_VIOLENCIA, ConstantesOperaciones.MODIFICAR_MANIFESTACION_VIOLENCIA_PROPIA})
    public Response eliminarPorId(@PathParam("manifestacionViolenciaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{manifestacionViolenciaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("manifestacionViolenciaId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgManifestacionViolencia.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgManifestacionViolencia.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
