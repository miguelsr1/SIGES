...
import sv.gob.mined.siges.negocio.servicios.CalificacionDiplomadoEstudianteBean;
 ...
 */
@RequestScoped
@Path("/v1/calificaciondiplomadoestudiante")
@Tag(name = "Calificación diplomado estudiante API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CALIFICACION_DIPLOMADO_ESTUDIANTE,
    ConstantesOperaciones.ACTUALIZAR_CALIFICACION_DIPLOMADO_ESTUDIANTE,
    ConstantesOperaciones.ELIMINAR_CALIFICACION_DIPLOMADO_ESTUDIANTE,
	ConstantesOperaciones.BUSCAR_CALIFICACION_DIPLOMADO_ESTUDIANTE})
public class CalificacionDiplomadoEstudianteRecurso {

    private static final Logger LOGGER = Logger.getLogger(CalificacionDiplomadoEstudianteRecurso.class.getName());

    @Inject
    private CalificacionDiplomadoEstudianteBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgCalificacionDiplomadoEstudiante
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCalificacionDiplomadoEstudiante.class)))
        ,
    ...
    @Path("/{calificacionDiplomadoEstudianteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CALIFICACION_DIPLOMADO_ESTUDIANTE})
    public Response obtenerPorId(@PathParam("calificacionDiplomadoEstudianteId") Long id) {
        try {
            SgCalificacionDiplomadoEstudiante cde = servicio.obtenerPorId(id);
            if (cde == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(cde).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgCalificacionDiplomadoEstudiante
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgCalificacionDiplomadoEstudiante.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CALIFICACION_DIPLOMADO_ESTUDIANTE})
    public Response buscar(FiltroCalificacionDiplomadoEstudiante filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CALIFICACION_DIPLOMADO_ESTUDIANTE})
    public Response buscarTotal(FiltroCalificacionDiplomadoEstudiante filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CALIFICACION_DIPLOMADO_ESTUDIANTE})
    public Response crear(SgCalificacionDiplomadoEstudiante cde) {
        try {
            cde.setCdePk(null);
            cde.setCdeVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(cde)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CALIFICACION_DIPLOMADO_ESTUDIANTE})
    public Response actualizar(@PathParam("calificacionDiplomadoEstudianteId") Long id, SgCalificacionDiplomadoEstudiante cde) {
        try {
            if (servicio.objetoExistePorId(id)) {
                cde.setCdePk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(cde)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CALIFICACION_DIPLOMADO_ESTUDIANTE})
    public Response eliminarPorId(@PathParam("calificacionDiplomadoEstudianteId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{calificacionDiplomadoEstudianteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("calificacionDiplomadoEstudianteId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgCalificacionDiplomadoEstudiante.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCalificacionDiplomadoEstudiante.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
