...
import sv.gob.mined.siges.negocio.servicios.RelGradoPlanEstudioBean;
 ...
 */
@RequestScoped
@Path("/v1/relgradoplanestudio")
@Tag(name = "Rel Grado Plan Estudio API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_GRADO_PLAN_ESTUDIO,
    ConstantesOperaciones.ACTUALIZAR_REL_GRADO_PLAN_ESTUDIO,
    ConstantesOperaciones.ELIMINAR_REL_GRADO_PLAN_ESTUDIO})
public class RelGradoPlanEstudioRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelGradoPlanEstudioRecurso.class.getName());

    @Inject
    private RelGradoPlanEstudioBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelGradoPlanEstudio
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelGradoPlanEstudio.class)))
        ,
    ...
    @Path("/{relGradoPlanEstudioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("relGradoPlanEstudioId") Long id) {
        try {
            SgRelGradoPlanEstudio rgp = servicio.obtenerPorId(id);
            if (rgp == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(rgp).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRelGradoPlanEstudio
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelGradoPlanEstudio.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroRelGradoPlanEstudio filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroRelGradoPlanEstudio filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_GRADO_PLAN_ESTUDIO})
    public Response crear(SgRelGradoPlanEstudio rgp) {
        try {
            rgp.setRgpPk(null);
            rgp.setRgpVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(rgp)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_GRADO_PLAN_ESTUDIO})
    public Response actualizar(@PathParam("relGradoPlanEstudioId") Long id, SgRelGradoPlanEstudio rgp) {
        try {
            if (servicio.objetoExistePorId(id)) {
                rgp.setRgpPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(rgp)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_GRADO_PLAN_ESTUDIO})
    public Response eliminarPorId(@PathParam("relGradoPlanEstudioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relGradoPlanEstudioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relGradoPlanEstudioId") Long id) {
        try {

            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRelGradoPlanEstudio.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelGradoPlanEstudio.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    @Path("/buscarDefinicionTitulo")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarDefinicionTitulo(FiltroRelGradoPlanEstudio filtro) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.obtenerDefinicionTitulo(filtro)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
