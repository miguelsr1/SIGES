...
import sv.gob.mined.siges.negocio.servicios.ReglaEquivalenciaDetalleBean;
...
@RequestScoped
@Path("/v1/reglaeqdetalle")
@Tag(name = "Detalle Reglas de Equivalencia API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REGLA_EQUIVALENCIA,
    ConstantesOperaciones.ACTUALIZAR_REGLA_EQUIVALENCIA,
    ConstantesOperaciones.ELIMINAR_REGLA_EQUIVALENCIA,
    ConstantesOperaciones.BUSCAR_REGLA_EQUIVALENCIA})
public class ReglaEquivalenciaDetalleRecurso {

    private static final Logger LOGGER = Logger.getLogger(ReglaEquivalenciaRecurso.class.getName());

    @Inject
    private ReglaEquivalenciaDetalleBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgReglaEquivalenciaDetalle
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgReglaEquivalenciaDetalle.class)))
        ,
    ...
    @Path("/{reglaEqDetalleId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REGLA_EQUIVALENCIA})
    public Response obtenerPorId(@PathParam("reglaEqDetalleId") Long id) {
        try {
            SgReglaEquivalenciaDetalle entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgReglaEquivalenciaDetalle
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgReglaEquivalenciaDetalle.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REGLA_EQUIVALENCIA})
    public Response buscar(FiltroReglaEquivalenciaDetalle filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REGLA_EQUIVALENCIA})
    public Response buscarTotal(FiltroReglaEquivalenciaDetalle filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REGLA_EQUIVALENCIA})
    public Response crear(SgReglaEquivalenciaDetalle entity) {
        try {
            entity.setRedPk(null);
            entity.setRedVersion(0);
            ...
            if (PersistenceHelper.isConstraintViolation(ex)) {
                return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(PersistenceHelper.getReglaEquivalenciaViolationBusinessException(ex)).build();
            } else {
                tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
                tracer.activeSpan().log(ex.getMessage());
                LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REGLA_EQUIVALENCIA})
    public Response actualizar(@PathParam("reglaEqDetalleId") Long id, SgReglaEquivalenciaDetalle entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REGLA_EQUIVALENCIA})
    public Response eliminarPorId(@PathParam("reglaEqDetalleId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{reglaEqDetalleId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("reglaEqDetalleId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgReglaEquivalenciaDetalle.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgReglaEquivalenciaDetalle.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
