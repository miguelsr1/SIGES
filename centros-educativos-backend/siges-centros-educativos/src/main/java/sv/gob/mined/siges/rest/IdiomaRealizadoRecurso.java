...
import sv.gob.mined.siges.negocio.servicios.IdiomaRealizadoBean;
...
@RequestScoped
@Path("/v1/idiomasrealizados")
@Tag(name = "IdiomasRealizados API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_IDIOMAS_REALIZADOS,
    ConstantesOperaciones.ACTUALIZAR_IDIOMAS_REALIZADOS,
    ConstantesOperaciones.ELIMINAR_IDIOMAS_REALIZADOS,
    ConstantesOperaciones.BUSCAR_IDIOMAS_REALIZADOS})
public class IdiomaRealizadoRecurso {

    private static final Logger LOGGER = Logger.getLogger(IdiomaRealizadoRecurso.class.getName());

    @Inject
    private IdiomaRealizadoBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgIdiomaRealizado.class)))
        ,
    ...
    @Path("/{idiomaRealizadoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_IDIOMAS_REALIZADOS})
    public Response obtenerPorId(@PathParam("idiomaRealizadoId") Long id) {
        try {
            SgIdiomaRealizado entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgIdiomaRealizado
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgIdiomaRealizado.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_IDIOMAS_REALIZADOS})
    public Response crear(SgIdiomaRealizado entity) {
        try {
            entity.setIrePk(null);
            entity.setIreVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_IDIOMAS_REALIZADOS})
    public Response actualizar(@PathParam("idiomaRealizadoId") Long id, SgIdiomaRealizado entity) {
        try {
    ...
    /**
     * Valida idioma como realizado.
     *
     * @param id Long
     * @return Response SgIdiomaRealizado
     */
    @POST
    @Operation(summary = "Valida idioma como realizado.")
    @APIResponses(value = {
    ...
    @Path("/validarrealizado/{idiomaRealizadoId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.VALIDAR_INFORMACION_FICHA_DOCENTE})
    public Response validarRealizado(@PathParam("idiomaRealizadoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    /**
     * Invalida idioma como realizado.
     *
     * @param id Long
     * @return Response SgIdiomaRealizado
     */
    @POST
    @Operation(summary = "Invalida idioma como realizado.")
    @APIResponses(value = {
    ...
    @Path("/invalidarrealizado/{idiomaRealizadoId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.VALIDAR_INFORMACION_FICHA_DOCENTE})
    public Response invalidarRealizado(@PathParam("idiomaRealizadoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_IDIOMAS_REALIZADOS})
    public Response eliminarPorId(@PathParam("idiomaRealizadoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{idiomaRealizadoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("idiomaRealizadoId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgIdiomaRealizado.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgIdiomaRealizado.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response validar(SgIdiomaRealizado entity) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(IdiomaRealizadoValidacionNegocio.validar(entity)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
