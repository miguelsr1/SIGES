...
import sv.gob.mined.siges.negocio.servicios.HabilitacionPeriodoMatriculaBean;
 ...
 */
@RequestScoped
@Path("/v1/habiltacionperiodomatricula")
@Tag(name = "Habilitación período matrícula API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_HABILITACION_PERIODO_MATRICULA,
    ConstantesOperaciones.ACTUALIZAR_HABILITACION_PERIODO_MATRICULA,
    ConstantesOperaciones.ELIMINAR_HABILITACION_PERIODO_MATRICULA,
    ConstantesOperaciones.EVALUAR_HABILITACION_PERIODO_MATRICULA,
	ConstantesOperaciones.BUSCAR_HABILITACION_PERIODO_MATRICULA})
public class HabilitacionPeriodoMatriculaRecurso {

    private static final Logger LOGGER = Logger.getLogger(HabilitacionPeriodoMatriculaRecurso.class.getName());

    @Inject
    private HabilitacionPeriodoMatriculaBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgHabilitacionPeriodoMatricula
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgHabilitacionPeriodoMatricula.class)))
        ,
    ...
    @Path("/{habilitacionPeriodoMatriculaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_HABILITACION_PERIODO_MATRICULA})
    public Response obtenerPorId(@PathParam("habilitacionPeriodoMatriculaId") Long id) {
        try {
            SgHabilitacionPeriodoMatricula hmp = servicio.obtenerPorId(id);
            if (hmp == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(hmp).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgHabilitacionPeriodoMatricula
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgHabilitacionPeriodoMatricula.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_HABILITACION_PERIODO_MATRICULA})
    public Response buscar(FiltroHabilitacionPeriodoMatricula filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_HABILITACION_PERIODO_MATRICULA})
    public Response buscarTotal(FiltroHabilitacionPeriodoMatricula filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_HABILITACION_PERIODO_MATRICULA})
    public Response crear(SgHabilitacionPeriodoMatricula hmp) {
        try {
            hmp.setHmpPk(null);
            hmp.setHmpVersion(0);
            if(hmp.getHmpRelHabilitacionMatriculaNivel()!=null ){
                for(SgRelHabilitacionMatriculaNivel per:hmp.getHmpRelHabilitacionMatriculaNivel()){
                    per.setRhnHabilitacionPeriodoMatriculaFk(hmp);
                }
            }
            
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(hmp)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_HABILITACION_PERIODO_MATRICULA})
    public Response actualizar(@PathParam("habilitacionPeriodoMatriculaId") Long id, SgHabilitacionPeriodoMatricula hmp) {
        try {
            if (servicio.objetoExistePorId(id)) {
                hmp.setHmpPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(hmp)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_HABILITACION_PERIODO_MATRICULA})
    public Response eliminarPorId(@PathParam("habilitacionPeriodoMatriculaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{habilitacionPeriodoMatriculaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("habilitacionPeriodoMatriculaId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialRevisionesPorId(SgHabilitacionPeriodoMatricula.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgHabilitacionPeriodoMatricula.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.EVALUAR_HABILITACION_PERIODO_MATRICULA})
    public Response aprobarSolicitud(SgHabilitacionPeriodoMatricula hpc) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.EVALUAR_HABILITACION_PERIODO_MATRICULA})
    public Response rechazarSolicitud(SgHabilitacionPeriodoMatricula hpc) {
        try {
