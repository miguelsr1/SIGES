...
import sv.gob.mined.siges.negocio.servicios.ComponentePlanGradoBean;
...
@RequestScoped
@Path("/v1/componentesplangrado")
@Tag(name = "ComponentePlanGrado API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_PLANES_ESTUDIO,
    ConstantesOperaciones.ACTUALIZAR_PLANES_ESTUDIO,
    ConstantesOperaciones.ELIMINAR_PLANES_ESTUDIO,
    ConstantesOperaciones.CREAR_ACTIVIDAD_ESPECIAL,
    ConstantesOperaciones.EDITAR_ACTIVIDAD_ESPECIAL,
    ConstantesOperaciones.ELIMINAR_ACTIVIDAD_ESPECIAL,
    ConstantesOperaciones.BUSCAR_PLANES_ESTUDIO})
public class ComponentePlanGradoRecurso {

    private static final Logger LOGGER = Logger.getLogger(ComponentePlanGradoRecurso.class.getName());

    @Inject
    private ComponentePlanGradoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgComponentePlanGrado
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgComponentePlanGrado.class)))
        ,
    ...
    @Path("/{componentePlanGradoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PLANES_ESTUDIO})
    public Response obtenerPorId(@PathParam("componentePlanGradoId") Long id) {
        try {
            SgComponentePlanGrado entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgComponentePlanGrado
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgComponentePlanGrado.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PLANES_ESTUDIO})
    public Response buscar(FiltroComponentePlanGrado filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PLANES_ESTUDIO})
    public Response buscarTotal(FiltroComponentePlanGrado filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_COMPONENTES_GRADO, ConstantesOperaciones.CREAR_ACTIVIDAD_ESPECIAL})
    public Response crear(SgComponentePlanGrado entity) {
        try {
            entity.setCpgPk(null);
            entity.setCpgVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PLANES_ESTUDIO, ConstantesOperaciones.EDITAR_ACTIVIDAD_ESPECIAL})
    public Response actualizar(@PathParam("componentePlanGradoId") Long id, SgComponentePlanGrado entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PLANES_ESTUDIO, ConstantesOperaciones.ELIMINAR_ACTIVIDAD_ESPECIAL})
    public Response eliminarPorId(@PathParam("componentePlanGradoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/eliminarComponentePlanGrado/{componentePlanGradoId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PLANES_ESTUDIO, ConstantesOperaciones.EDITAR_ACTIVIDAD_ESPECIAL})
    public Response eliminarComponentePlanGrado(@PathParam("componentePlanGradoId") Long id, SgComponentePlanGrado entity) {
        try {
            if (servicio.objetoExistePorId(id)) {
                entity.setCpgPk(id);
                servicio.eliminarComponentePlanGrado(entity);
                return Response.status(HttpStatus.SC_OK).entity(entity).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
    ...
    @Path("/historial/{componentePlanGradoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("componentePlanGradoId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgComponentePlanGrado.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgComponentePlanGrado.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
