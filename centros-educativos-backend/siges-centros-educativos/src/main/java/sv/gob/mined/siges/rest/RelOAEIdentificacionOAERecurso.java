...
import sv.gob.mined.siges.negocio.servicios.RelOAEIdentificacionOAEBean;
 ...
 */
@RequestScoped
@Path("/v1/relacionoaeidentificacionoae")
@Tag(name = "Relación OAE Identificación API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_OAE_IDENTIFICACION_OAE,
    ConstantesOperaciones.ACTUALIZAR_REL_OAE_IDENTIFICACION_OAE,
    ConstantesOperaciones.ELIMINAR_REL_OAE_IDENTIFICACION_OAE})
public class RelOAEIdentificacionOAERecurso {

    private static final Logger LOGGER = Logger.getLogger(RelOAEIdentificacionOAERecurso.class.getName());

    @Inject
    private RelOAEIdentificacionOAEBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelOAEIdentificacionOAE
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelOAEIdentificacionOAE.class)))
        ,
    ...
    @Path("/{relOAEIdentificacionOAEId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("relOAEIdentificacionOAEId") Long id) {
        try {
            SgRelOAEIdentificacionOAE rio = servicio.obtenerPorId(id);
            if (rio == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(rio).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRelOAEIdentificacionOAE
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelOAEIdentificacionOAE.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroRelOAEIdentificacionOAE filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroRelOAEIdentificacionOAE filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_OAE_IDENTIFICACION_OAE})
    public Response crear(SgRelOAEIdentificacionOAE rio) {
        try {
            rio.setRioPk(null);
            rio.setRioVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(rio)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_OAE_IDENTIFICACION_OAE})
    public Response actualizar(@PathParam("relOAEIdentificacionOAEId") Long id, SgRelOAEIdentificacionOAE rio) {
        try {
            if (servicio.objetoExistePorId(id)) {
                rio.setRioPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(rio)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_OAE_IDENTIFICACION_OAE})
    public Response eliminarPorId(@PathParam("relOAEIdentificacionOAEId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relOAEIdentificacionOAEId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relOAEIdentificacionOAEId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgRelOAEIdentificacionOAE.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelOAEIdentificacionOAE.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
