...
import sv.gob.mined.siges.negocio.servicios.SedeEducameBean;
...
@RequestScoped
@Path("/v1/sedeseducame")
@Tag(name = "Sedes EDUCAME API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_EDUCAME,
    ConstantesOperaciones.ACTUALIZAR_EDUCAME,
    ConstantesOperaciones.ELIMINAR_EDUCAME,
    ConstantesOperaciones.BUSCAR_SEDES})
public class SedeEducameRecurso {

    private static final Logger LOGGER = Logger.getLogger(SedeEducameRecurso.class.getName());

    @Inject
    private SedeEducameBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgSedeEducame.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("entityId") Long entityId) {
        try {
            SgSedeEducame cedPri = servicio.obtenerPorId(entityId);
            if (cedPri == null) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_EDUCAME})
    public Response crear(SgSedeEducame entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_EDUCAME})
    public Response actualizar(@PathParam("entityId") Long id, SgSedeEducame entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_EDUCAME})
    ...
    public Response obtenerHistorialPorId(@PathParam("entityId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgSedeEducame.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgSedeEducame.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
