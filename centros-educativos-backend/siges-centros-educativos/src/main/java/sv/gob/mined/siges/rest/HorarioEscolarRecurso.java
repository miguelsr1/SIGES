...
import sv.gob.mined.siges.negocio.servicios.HorarioEscolarBean;
 ...
 */
@RequestScoped
@Path("/v1/horariosescolares")
@Tag(name = "Horarios Escolares API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_HORARIOS_ESCOLARES,
    ConstantesOperaciones.ACTUALIZAR_HORARIOS_ESCOLARES,
    ConstantesOperaciones.ELIMINAR_HORARIOS_ESCOLARES,
    ConstantesOperaciones.BUSCAR_HORARIOS_ESCOLARES})
public class HorarioEscolarRecurso {

    private static final Logger LOGGER = Logger.getLogger(HorarioEscolarRecurso.class.getName());

    @Inject
    private HorarioEscolarBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgHorarioEscolar
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgHorarioEscolar.class)))
        ,
    ...
    @Path("/{horarioEscolarId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_HORARIOS_ESCOLARES})
    //@Timed
    public Response obtenerPorId(@PathParam("horarioEscolarId") Long id) {
        try {
            SgHorarioEscolar hes = servicio.obtenerPorId(id);
            if (hes == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(hes).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgHorarioEscolar
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgHorarioEscolar.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_HORARIOS_ESCOLARES})
    //@Timed
    public Response buscar(FiltroHorarioEscolar filtro) {
        try {
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_HORARIOS_ESCOLARES})
    //@Timed
    public Response buscarTotal(FiltroHorarioEscolar filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_HORARIOS_ESCOLARES})
    //@Timed
    public Response crear(SgHorarioEscolar hes) {
        try {
            hes.setHesPk(null);
            hes.setHesVersion(0);
            if (hes.getHesCeldasDiaHora() != null) {
                for (SgCeldaDiaHora celda : hes.getHesCeldasDiaHora()) {
                    celda.setCdhHorarioEscolar(hes);
                }
            }
            if (hes.getHesDocentes() != null) {
                for (SgComponenteDocente doc : hes.getHesDocentes()) {
                    doc.setCdoHorarioEscolar(hes);
                }
            }
            if (hes.getHesDiasLectivos() != null) {
                for (SgDiaLectivoHorarioEscolar fdh : hes.getHesDiasLectivos()) {
                    fdh.setDlhHorarioEscolarFk(hes);
                }

            }
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(hes)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_HORARIOS_ESCOLARES})
    //@Timed
    public Response actualizar(@PathParam("horarioEscolarId") Long id, SgHorarioEscolar hes) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ...
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(hes)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Path("actualizarDocente/{horarioEscolarId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_HORARIOS_ESCOLARES})
    public Response guardarDocenteUnico(@PathParam("horarioEscolarId") Long id, SgHorarioEscolarLite hes) {
        try {
            if (servicio.objetoExistePorId(id)) {
                hes.setHesPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardarLite(hes)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_HORARIOS_ESCOLARES})
    public Response eliminarPorId(@PathParam("horarioEscolarId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{horarioEscolarId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("horarioEscolarId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgHorarioEscolar.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgHorarioEscolar.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
