...
import sv.gob.mined.siges.negocio.servicios.HabilitacionPeriodoCalificacionBean;
 ...
 */
@RequestScoped
@Path("/v1/habilitacionperiodocalificacion")
@Tag(name = "Habilitación de Período de calificación API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SOLICITUD_CORRECCION_CALIFICACION,
    ConstantesOperaciones.ACTUALIZAR_SOLICITUD_CORRECCION_CALIFICACION,
    ConstantesOperaciones.ELIMINAR_SOLICITUD_CORRECCION_CALIFICACION,
    ConstantesOperaciones.EVALUAR_SOLICITUD_CORRECCION_CALIFICACION,
    ConstantesOperaciones.BUSCAR_SOLICITUD_CORRECCION_CALIFICACION})
public class HabilitacionPeriodoCalificacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(HabilitacionPeriodoCalificacionRecurso.class.getName());

    @Inject
    private HabilitacionPeriodoCalificacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgHabilitacionPeriodoCalificacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgHabilitacionPeriodoCalificacion.class)))
        ,
    ...
    @Path("/{habilitacionPeriodoCalificacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_CORRECCION_CALIFICACION})
    public Response obtenerPorId(@PathParam("habilitacionPeriodoCalificacionId") Long id) {
        try {
            SgHabilitacionPeriodoCalificacion hpc = servicio.obtenerPorId(id);
            if (hpc == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(hpc).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgHabilitacionPeriodoCalificacion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgHabilitacionPeriodoCalificacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_CORRECCION_CALIFICACION})
    public Response buscar(FiltroHabilitacionPeriodoCalificacion filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_CORRECCION_CALIFICACION})
    public Response buscarTotal(FiltroHabilitacionPeriodoCalificacion filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SOLICITUD_CORRECCION_CALIFICACION})
    public Response crear(SgHabilitacionPeriodoCalificacion hpc) {
        try {
            hpc.setHpcPk(null);
            hpc.setHpcVersion(0);
            if(hpc.getRelPeriodosHabilitacionPeriodo()!=null ){
                for(SgRelPeriodosHabilitacionPeriodo per:hpc.getRelPeriodosHabilitacionPeriodo()){
                    per.setRphHabilitacionPeriodoCalificacionFk(hpc);
                }
            }
            
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(hpc)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SOLICITUD_CORRECCION_CALIFICACION})
    public Response actualizar(@PathParam("habilitacionPeriodoCalificacionId") Long id, SgHabilitacionPeriodoCalificacion hpc) {
        try {
            if (servicio.objetoExistePorId(id)) {
                hpc.setHpcPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(hpc)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_SOLICITUD_CORRECCION_CALIFICACION})
    public Response eliminarPorId(@PathParam("habilitacionPeriodoCalificacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{habilitacionPeriodoCalificacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("habilitacionPeriodoCalificacionId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialRevisionesPorId(SgHabilitacionPeriodoCalificacion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgHabilitacionPeriodoCalificacion.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_CORRECCION_CALIFICACION})
    public Response buscarEstudiante(FiltroEstudiante filtro) {
        try {

            return Response.status(HttpStatus.SC_OK).entity(servicio.buscarEstudiante(filtro)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "201", description = "Entidad creada satisfactoriamente.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgPeriodosOrdExord.class)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_CORRECCION_CALIFICACION})
    public Response buscarPeriodos(SgMatricula mat) {
        try {

            return Response.status(HttpStatus.SC_OK).entity(servicio.busarPeriodos(mat)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
     ...
     */
    @POST
    @Operation(summary = "Aprueba el objeto indicado con id autogenerada.")
    @APIResponses(value = {
        @APIResponse(responseCode = "201", description = "Entidad creada satisfactoriamente.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgHabilitacionPeriodoCalificacion.class)))
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.EVALUAR_SOLICITUD_CORRECCION_CALIFICACION})
    public Response aprobarSolicitud(SgHabilitacionPeriodoCalificacion hpc) {
        try {
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.aprobarSolicitud(hpc)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.EVALUAR_SOLICITUD_CORRECCION_CALIFICACION})
    public Response rechazarSolicitud(SgHabilitacionPeriodoCalificacion hpc) {
        try {
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.rechazarSolicitud(hpc)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
