...
import sv.gob.mined.siges.negocio.servicios.ComponentesPlanEstudioBean;
...
@RequestScoped
@Path("/v1/componentesplanestudio")
@Tag(name = "ComponentePlanEstudio API V1")
...
public class ComponentePlanEstudioRescurso {

    private static final Logger LOGGER = Logger.getLogger(ComponentePlanEstudioRescurso.class.getName());

    @Inject
    private ComponentesPlanEstudioBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgComponentePlanEstudio.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("planEstudioId") Long id) {
        try {
            SgComponentePlanEstudio entity = servicio.obtenerPorId(id);
            if (entity == null) {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgComponentePlanEstudio.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PLANES_ESTUDIO})
    public Response buscar(FiltroComponentePlanEstudio filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PLANES_ESTUDIO})
    public Response buscarTotal(FiltroComponentePlanEstudio filtro) {
        try {
    ...
    public Response obtenerHistorialPorId(@PathParam("planEstudioId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgComponentePlanEstudio.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgComponentePlanEstudio.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
     ...
     /**
     * Devuelve los componentes plan estudio numéricos que el estudiante cursó y tienen nota de APR
     *
     * @param id Long
     * @return Response SgPlanEstudio
     */
    @GET
    @Operation(summary = "Devuelve los componentes plan estudio numéricos que el estudiante cursó y tienen nota de APR.")
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgComponentePlanEstudio.class, type = SchemaType.ARRAY)))
    ...
    @Path("/buscar/cursadosestudiantenumericos/{estudianteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PLANES_ESTUDIO})
    public Response obtenerComponentesPlanEstudioNumericosCursadosEstudiante(@PathParam("estudianteId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.obtenerComponentesPlanEstudioNumericosCursadosEstudiante(id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
     ...
     */
    @GET
    @Operation(summary = " Devuelve los componentes plan estudio numéricos que el estudiante está cursando y tienen nota de ORD")
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgComponentePlanEstudio.class, type = SchemaType.ARRAY)))
    ...
    @Path("/buscar/actualesordestudiantenumericos/{estudianteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PLANES_ESTUDIO})
    public Response obtenerComponentesPlanEstudioNumericosActualesEstudiante(@PathParam("estudianteId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.obtenerComponentesPlanEstudioNumericosActualesEstudiante(id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
     ...
     /**
     * Devuelve los componentes plan estudio conceptuales que el estudiante cursó y tienen nota de APR
     *
     * @param id Long
     * @return Response SgPlanEstudio
     */
    @GET
    @Operation(summary = "Devuelve los componentes plan estudio conceptuales que el estudiante cursó y tienen nota de APR.")
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgComponentePlanEstudio.class, type = SchemaType.ARRAY)))
    ...
    @Path("/buscar/cursadosestudianteconceptuales/{estudianteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PLANES_ESTUDIO})
    public Response obtenerComponentesPlanEstudioConceptualesCursadosEstudiante(@PathParam("estudianteId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.obtenerComponentesPlanEstudioConceptualesCursadosEstudiante(id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
