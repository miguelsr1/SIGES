...
import sv.gob.mined.siges.negocio.servicios.CicloBean;
...
@RequestScoped
@Path("/v1/ciclos")
@Tag(name = "Ciclo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CICLO,
    ConstantesOperaciones.ACTUALIZAR_CICLO,
    ConstantesOperaciones.ELIMINAR_CICLO,
    ConstantesOperaciones.BUSCAR_CICLO})
public class CicloRecurso {

    private static final Logger LOGGER = Logger.getLogger(CicloBean.class.getName());

    @Inject
    private CicloBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCiclo.class)))
        ,
    ...
    @Path("/{cicloId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CICLO})
    public Response obtenerPorId(@PathParam("cicloId") Long id) {
        try {
            SgCiclo entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgCiclo
     */
    @POST
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CICLO})
    public Response buscar(FiltroCiclo filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CICLO})
    public Response buscarTotal(FiltroCiclo filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CICLO})
    public Response crear(SgCiclo entity) {
        try {
            entity.setCicPk(null);
            entity.setCicVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CICLO})
    public Response actualizar(@PathParam("cicloId") Long id, SgCiclo entity) {
        try {
            if (servicio.objetoExistePorId(id)) {
                entity.setCicPk(id);
                SgCiclo guardada = servicio.guardar(entity);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CICLO})
    public Response eliminarPorId(@PathParam("cicloId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{cicloId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("cicloId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgCiclo.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCiclo.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
