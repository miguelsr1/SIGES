...
import sv.gob.mined.siges.negocio.servicios.AsignacionPerfilBean;
 ...
 */
@RequestScoped
@Path("/v1/asignacionperfil")
@Tag(name = "Asignación perfiles API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ASIGNACION_PERFIL,
    ConstantesOperaciones.ACTUALIZAR_ASIGNACION_PERFIL,
    ConstantesOperaciones.ELIMINAR_ASIGNACION_PERFIL,
    ConstantesOperaciones.BUSCAR_ASIGNACION_PERFIL})
public class AsignacionPerfilRecurso {

    private static final Logger LOGGER = Logger.getLogger(AsignacionPerfilRecurso.class.getName());

    @Inject
    private AsignacionPerfilBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAsignacionPerfil
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAsignacionPerfil.class)))
        ,
    ...
    @Path("/{asignacionPerfilId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ASIGNACION_PERFIL})
    public Response obtenerPorId(@PathParam("asignacionPerfilId") Long id) {
        try {
            SgAsignacionPerfil ape = servicio.obtenerPorId(id);
            if (ape == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(ape).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgAsignacionPerfil
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAsignacionPerfil.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ASIGNACION_PERFIL})
    public Response buscar(FiltroAsignacionPerfil filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ASIGNACION_PERFIL})
    public Response buscarTotal(FiltroAsignacionPerfil filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ASIGNACION_PERFIL})
    public Response crear(SgAsignacionPerfil ape) {
        try {
            ape.setApePk(null);
            ape.setApeVersion(0);
            if (ape.getApeAsignacionesPerfilPersonal() != null) {
                for (SgAsignacionPerfilPersonal ap : ape.getApeAsignacionesPerfilPersonal()) {
                    ap.setAppAsignacionPerfilFk(ape);
                }
            }
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ape)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ASIGNACION_PERFIL})
    public Response actualizar(@PathParam("asignacionPerfilId") Long id, SgAsignacionPerfil ape) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ...
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ape)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ASIGNACION_PERFIL})
    public Response eliminarPorId(@PathParam("asignacionPerfilId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{asignacionPerfilId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("asignacionPerfilId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialRevisionesPorId(SgAsignacionPerfil.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgAsignacionPerfil.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
