...
import sv.gob.mined.siges.negocio.servicios.DatoContratacionBean;
...
@RequestScoped
@Path("/v1/datocontratacion")
@Tag(name = "DatoContratacion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_DATO_CONTRATACION,
    ConstantesOperaciones.ACTUALIZAR_DATO_CONTRATACION,
    ConstantesOperaciones.ELIMINAR_DATO_CONTRATACION,
    ConstantesOperaciones.BUSCAR_DATO_CONTRATACION})
public class DatoContratacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(DatoContratacionRecurso.class.getName());

    @Inject
    private DatoContratacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgDatoContratacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgDatoContratacion.class)))
        ,
    ...
    @Path("/{datoContratacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DATO_CONTRATACION})
    public Response obtenerPorId(@PathParam("datoContratacionId") Long id) {
        try {
            SgDatoContratacion entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgDatoContratacion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgDatoContratacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DATO_CONTRATACION})
    public Response buscar(FiltroDatoContratacion filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DATO_CONTRATACION})
    public Response buscarTotal(FiltroDatoContratacion filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DATO_CONTRATACION})
    public Response crear(SgDatoContratacion entity) {
        try {
            entity.setDcoPk(null);
            entity.setDcoVersion(0);

            if (entity.getDcoTipo() == null) {
                BusinessException be = new BusinessException();
                be.addError(Errores.ERROR_TIPO_VACIO);
                throw be;
            }

            switch (entity.getDcoTipo()) {
                case ADM:
                    if (!jwt.getGroups().contains(ConstantesOperaciones.CREAR_ADMINISTRATIVO)) {
                        return Response.status(HttpStatus.SC_FORBIDDEN).build();
                    }
                    break;
                case ALF:
                    if (!jwt.getGroups().contains(ConstantesOperaciones.CREAR_ALFABETIZADOR)) {
                        return Response.status(HttpStatus.SC_FORBIDDEN).build();
                    }
                    break;
                case ATPI:
                    if (!jwt.getGroups().contains(ConstantesOperaciones.CREAR_ATPI)) {
                        return Response.status(HttpStatus.SC_FORBIDDEN).build();
                    }
                    break;
                case DOC:
                    if (!jwt.getGroups().contains(ConstantesOperaciones.CREAR_DOCENTE)) {
                        return Response.status(HttpStatus.SC_FORBIDDEN).build();
                    }
                    break;
                case DOF:
                    if (!jwt.getGroups().contains(ConstantesOperaciones.CREAR_DOCENTE_OTRAS_FUENTES)) {
                        return Response.status(HttpStatus.SC_FORBIDDEN).build();
                    }
                    break;
            }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_DATO_CONTRATACION})
    public Response actualizar(@PathParam("datoContratacionId") Long id, SgDatoContratacion entity) {
        try {
            ...
            switch (entity.getDcoTipo()) {
                case ADM:
                    if (!jwt.getGroups().contains(ConstantesOperaciones.ACTUALIZAR_ADMINISTRATIVO)) {
                        return Response.status(HttpStatus.SC_FORBIDDEN).build();
                    }
                    break;
                case ALF:
                    if (!jwt.getGroups().contains(ConstantesOperaciones.ACTUALIZAR_ALFABETIZADOR)) {
                        return Response.status(HttpStatus.SC_FORBIDDEN).build();
                    }
                    break;
                case ATPI:
                    if (!jwt.getGroups().contains(ConstantesOperaciones.ACTUALIZAR_ATPI)) {
                        return Response.status(HttpStatus.SC_FORBIDDEN).build();
                    }
                    break;
                case DOC:
                    if (!jwt.getGroups().contains(ConstantesOperaciones.ACTUALIZAR_DOCENTE)) {
                        return Response.status(HttpStatus.SC_FORBIDDEN).build();
                    }
                    break;
                case DOF:
                    if (!jwt.getGroups().contains(ConstantesOperaciones.ACTUALIZAR_DOCENTE_OTRAS_FUENTES)) {
                        return Response.status(HttpStatus.SC_FORBIDDEN).build();
                    }
                    break;
            }
    ...
    public Response validar(SgDatoContratacion entity) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(DatoContratacionValidacionNegocio.validar(entity)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_DATO_CONTRATACION})
    public Response eliminarPorId(@PathParam("datoContratacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {

                FiltroDatoContratacion filtro = new FiltroDatoContratacion();
                filtro.setDcoPk(id);
                filtro.setIncluirCampos(new String[]{"dcoTipo"});
                SgDatoContratacion entity = servicio.obtenerPorFiltro(filtro).get(0);
                switch (entity.getDcoTipo()) {
                    case ADM:
                        if (!jwt.getGroups().contains(ConstantesOperaciones.ELIMINAR_ADMINISTRATIVO)) {
                            return Response.status(HttpStatus.SC_FORBIDDEN).build();
                        }
                        break;
                    case ALF:
                        if (!jwt.getGroups().contains(ConstantesOperaciones.ELIMINAR_ALFABETIZADOR)) {
                            return Response.status(HttpStatus.SC_FORBIDDEN).build();
                        }
                        break;
                    case ATPI:
                        if (!jwt.getGroups().contains(ConstantesOperaciones.ELIMINAR_ATPI)) {
                            return Response.status(HttpStatus.SC_FORBIDDEN).build();
                        }
                        break;
                    case DOC:
                        if (!jwt.getGroups().contains(ConstantesOperaciones.ELIMINAR_DOCENTE)) {
                            return Response.status(HttpStatus.SC_FORBIDDEN).build();
                        }
                        break;
                    case DOF:
                        if (!jwt.getGroups().contains(ConstantesOperaciones.ELIMINAR_DOCENTE_OTRAS_FUENTES)) {
                            return Response.status(HttpStatus.SC_FORBIDDEN).build();
                        }
                        break;
                }
    ...
    @Path("/historial/{datoContratacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("datoContratacionId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgDatoContratacion.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgDatoContratacion.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
