...
import sv.gob.mined.siges.negocio.servicios.ModuloFormacionDocenteBean;
...
@RequestScoped
@Path("/v1/modulosformaciondocente")
@Tag(name = "ModuloFormacionDocente API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MODULO_FORMACION_DOCENTE,
    ConstantesOperaciones.ACTUALIZAR_MODULO_FORMACION_DOCENTE,
    ConstantesOperaciones.ELIMINAR_MODULO_FORMACION_DOCENTE,
    ConstantesOperaciones.BUSCAR_MODULO_FORMACION_DOCENTE})
public class ModuloFormacionDocenteRecurso {

    private static final Logger LOGGER = Logger.getLogger(ModuloFormacionDocenteRecurso.class.getName());

    @Inject
    private ModuloFormacionDocenteBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgModuloFormacionDocente
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgModuloFormacionDocente.class)))
        ,
    ...
    @Path("/{moduloFormacionDocenteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MODULO_FORMACION_DOCENTE})
    public Response obtenerPorId(@PathParam("moduloFormacionDocenteId") Long id) {
        try {
            SgModuloFormacionDocente entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgModuloFormacionDocente
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgModuloFormacionDocente.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MODULO_FORMACION_DOCENTE})
    public Response buscar(FiltroModuloFormacionDocente filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MODULO_FORMACION_DOCENTE})
    public Response buscarTotal(FiltroModuloFormacionDocente filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MODULO_FORMACION_DOCENTE})
    public Response crear(SgModuloFormacionDocente entity) {
        try {
            entity.setMfdPk(null);
            entity.setMfdVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MODULO_FORMACION_DOCENTE})
    public Response actualizar(@PathParam("moduloFormacionDocenteId") Long id, SgModuloFormacionDocente entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MODULO_FORMACION_DOCENTE})
    public Response eliminarPorId(@PathParam("moduloFormacionDocenteId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{moduloFormacionDocenteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("moduloFormacionDocenteId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgModuloFormacionDocente.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgModuloFormacionDocente.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
