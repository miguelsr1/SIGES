...
import sv.gob.mined.siges.negocio.servicios.RelSedeDiplomadoBean;
 ...
 */
@RequestScoped
@Path("/v1/relsedediplomado")
@Tag(name = "Relación sede diplomado API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_SEDE_DIPLOMADO,
    ConstantesOperaciones.ACTUALIZAR_REL_SEDE_DIPLOMADO,
    ConstantesOperaciones.ELIMINAR_REL_SEDE_DIPLOMADO,
	ConstantesOperaciones.BUSCAR_REL_SEDE_DIPLOMADO})
public class RelSedeDiplomadoRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelSedeDiplomadoRecurso.class.getName());

    @Inject
    private RelSedeDiplomadoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelSedeDiplomado
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelSedeDiplomado.class)))
        ,
    ...
    @Path("/{relSedeDiplomadoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("relSedeDiplomadoId") Long id) {
        try {
            SgRelSedeDiplomado rsd = servicio.obtenerPorId(id);
            if (rsd == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(rsd).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRelSedeDiplomado
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelSedeDiplomado.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroRelSedeDiplomado filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroRelSedeDiplomado filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_SEDE_DIPLOMADO})
    public Response crear(SgRelSedeDiplomado rsd) {
        try {
            rsd.setRsdPk(null);
            rsd.setRsdVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(rsd)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_SEDE_DIPLOMADO})
    public Response actualizar(@PathParam("relSedeDiplomadoId") Long id, SgRelSedeDiplomado rsd) {
        try {
            if (servicio.objetoExistePorId(id)) {
                rsd.setRsdPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(rsd)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_SEDE_DIPLOMADO})
    public Response eliminarPorId(@PathParam("relSedeDiplomadoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relSedeDiplomadoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relSedeDiplomadoId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgRelSedeDiplomado.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelSedeDiplomado.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    /**
     * Crea y autoriza las relaciones indicadas
     *
     * @param rsd SgRegistroDiplomadosSede
     */
    @POST
    @Operation(summary = "Crea y autoriza las relaciones indicadas.")
    @APIResponses(value = {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_SEDE_DIPLOMADO, ConstantesOperaciones.ACTUALIZAR_REL_SEDE_DIPLOMADO})
    public Response autorizarRelaciones(SgRegistroDiplomadosSede rsd) {
        try {
            servicio.autorizarRelaciones(rsd);
            return Response.status(HttpStatus.SC_OK).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
     ...
     /**
     * Desautoriza las relaciones indicadas
     *
     * @param rsd SgRegistroDiplomadosSede
     */
    @POST
    @Operation(summary = "Desautoriza las relaciones indicadas.")
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.")
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_SEDE_DIPLOMADO})
    public Response desautorizarRelaciones(SgRegistroDiplomadosSede rsd) {
        try {
            servicio.desautorizarRelaciones(rsd);
            return Response.status(HttpStatus.SC_OK).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
