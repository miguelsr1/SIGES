...
import sv.gob.mined.siges.negocio.servicios.HabitosPersonalesBean;
 ...
 */
@RequestScoped
@Path("/v1/habitospersonales")
@Tag(name = "Habitos de personales API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_HABITOS_PERSONALES,
    ConstantesOperaciones.ACTUALIZAR_HABITOS_PERSONALES,
    ConstantesOperaciones.ELIMINAR_HABITOS_PERSONALES})
public class HabitosPersonalesRecurso {

    private static final Logger LOGGER = Logger.getLogger(HabitosPersonalesRecurso.class.getName());

    @Inject
    private HabitosPersonalesBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgHabitosPersonales
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgHabitosPersonales.class)))
        ,
    ...
    @Path("/{habitosPersonalesId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("habitosPersonalesId") Long id) {
        try {
            SgHabitosPersonales hap = servicio.obtenerPorId(id);
            if (hap == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(hap).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgHabitosPersonales
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgHabitosPersonales.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroHabitosPersonales filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroHabitosPersonales filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_HABITOS_PERSONALES})
    public Response crear(SgHabitosPersonales hap) {
        try {
            hap.setHapPk(null);
            hap.setHapVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(hap)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_HABITOS_PERSONALES})
    public Response actualizar(@PathParam("habitosPersonalesId") Long id, SgHabitosPersonales hap) {
        try {
            if (servicio.objetoExistePorId(id)) {
                hap.setHapPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(hap)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_HABITOS_PERSONALES})
    public Response eliminarPorId(@PathParam("habitosPersonalesId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{habitosPersonalesId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("habitosPersonalesId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgHabitosPersonales.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgHabitosPersonales.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
