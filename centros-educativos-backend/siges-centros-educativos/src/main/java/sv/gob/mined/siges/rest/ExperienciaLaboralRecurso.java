...
import sv.gob.mined.siges.negocio.servicios.ExperienciaLaboralBean;
...
@RequestScoped
@Path("/v1/experiencialaboral")
@Tag(name = "ExperienciaLaboral API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_EXPERIENCIA_LABORAL,
    ConstantesOperaciones.ACTUALIZAR_EXPERIENCIA_LABORAL,
    ConstantesOperaciones.ELIMINAR_EXPERIENCIA_LABORAL,
    ConstantesOperaciones.BUSCAR_EXPERIENCIA_LABORAL})
public class ExperienciaLaboralRecurso {

    private static final Logger LOGGER = Logger.getLogger(ExperienciaLaboralRecurso.class.getName());

    @Inject
    private ExperienciaLaboralBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgExperienciaLaboral
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgExperienciaLaboral.class)))
        ,
    ...
    @Path("/{experienciaLaboralId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_EXPERIENCIA_LABORAL})
    public Response obtenerPorId(@PathParam("experienciaLaboralId") Long id) {
        try {
            SgExperienciaLaboral entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgExperienciaLaboral
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgExperienciaLaboral.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_EXPERIENCIA_LABORAL})
    public Response buscar(FiltroExperienciaLaboral filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_EXPERIENCIA_LABORAL})
    public Response buscarTotal(FiltroExperienciaLaboral filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_EXPERIENCIA_LABORAL})
    public Response crear(SgExperienciaLaboral entity) {
        try {
            entity.setElaPk(null);
            entity.setElaVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_EXPERIENCIA_LABORAL})
    public Response actualizar(@PathParam("experienciaLaboralId") Long id, SgExperienciaLaboral entity) {
        try {
    ...
    /**
     * Valida experiencia como realizada.
     *
     * @param id Long
     * @return Response SgExperienciaLaboral
     */
    @POST
    @Operation(summary = "Valida experiencia como realizada.")
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Validado satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgExperienciaLaboral.class)))
        ,
        @APIResponse(responseCode = "404", description = "Objeto no encontrado.")
    ...
    @Path("/validarrealizado/{experienciaLaboralId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.VALIDAR_INFORMACION_FICHA_DOCENTE})
    public Response validarRealizado(@PathParam("experienciaLaboralId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
                return Response.status(HttpStatus.SC_OK).entity(servicio.validarRealizado(id)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    /**
     * Invalida experiencia como realizada.
     *
     * @param id Long
     * @return Response SgExperienciaLaboral
     */
    @POST
    @Operation(summary = "Invalida experiencia como realizada.")
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Invalidado satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgExperienciaLaboral.class)))
        ,
        @APIResponse(responseCode = "404", description = "Objeto no encontrado.")
    ...
    @Path("/invalidarrealizado/{experienciaLaboralId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.VALIDAR_INFORMACION_FICHA_DOCENTE})
    public Response invalidarRealizado(@PathParam("experienciaLaboralId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
                return Response.status(HttpStatus.SC_OK).entity(servicio.invalidarRealizado(id)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    public Response validar(SgExperienciaLaboral entity) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(ExperienciaLaboralValidacionNegocio.validar(entity)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_EXPERIENCIA_LABORAL})
    public Response eliminarPorId(@PathParam("experienciaLaboralId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{experienciaLaboralId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("experienciaLaboralId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgExperienciaLaboral.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgExperienciaLaboral.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
