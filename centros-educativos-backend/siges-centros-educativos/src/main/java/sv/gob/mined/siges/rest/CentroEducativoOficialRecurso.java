...
import sv.gob.mined.siges.negocio.servicios.CentroEducativoOficialBean;
...
@RequestScoped
@Path("/v1/centroseducativooficial")
@Tag(name = "Centro Educativo Oficial API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CENTRO_EDUCATIVO_OFICIAL,
    ConstantesOperaciones.ACTUALIZAR_CENTRO_EDUCATIVO_OFICIAL,
    ConstantesOperaciones.ELIMINAR_CENTRO_EDUCATIVO_OFICIAL,
    ConstantesOperaciones.BUSCAR_SEDES})
public class CentroEducativoOficialRecurso {

    private static final Logger LOGGER = Logger.getLogger(CentroEducativoOficialRecurso.class.getName());

    @Inject
    private CentroEducativoOficialBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCentroEducativoOficial.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("entityId") Long entityId) {
        try {
            SgCentroEducativoOficial tce = servicio.obtenerPorId(entityId);
            if (tce == null) {
     ...
     *
     * @param SgCentroEducativoOficial
     * @return Response -> SigesSedes
     */
    @POST
    ...
    public Response crear(SgCentroEducativoOficial entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CENTRO_EDUCATIVO_OFICIAL})
    public Response actualizar(@PathParam("entityId") Long id, SgCentroEducativoOficial entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CENTRO_EDUCATIVO_OFICIAL})
    ...
    public Response obtenerHistorialPorId(@PathParam("entityId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgCentroEducativoOficial.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
     ...
     *
     * @return Response SgCentroEducativoOficial
     */
    @GET
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCentroEducativoOficial.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
