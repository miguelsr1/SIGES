...
import sv.gob.mined.siges.negocio.servicios.ControlAsistenciaPersonalCabezalBean;
...
@RequestScoped
@Path("/v1/controlesasistenciapersonalcabezal")
@Tag(name = "ControlAsistenciaPersonalCabezal API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CONTROL_ASISTENCIA_PERSONAL_CABEZAL,
    ConstantesOperaciones.ACTUALIZAR_CONTROL_ASISTENCIA_PERSONAL_CABEZAL,
    ConstantesOperaciones.ELIMINAR_CONTROL_ASISTENCIA_PERSONAL_CABEZAL,
    ConstantesOperaciones.BUSCAR_CONTROL_ASISTENCIA_PERSONAL_CABEZAL})
public class ControlAsistenciaPersonalCabezalRecurso {

    private static final Logger LOGGER = Logger.getLogger(ControlAsistenciaPersonalCabezalRecurso.class.getName());

    @Inject
    private ControlAsistenciaPersonalCabezalBean servicio;
     ...
     *
     * @param controlAsistenciaPersonalCabezalId
     * @return Response -> SgControlAsistenciaPersonalCabezal
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgControlAsistenciaPersonalCabezal.class)))
        ,
    ...
    @Path("/{controlAsistenciaPersonalCabezalId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CONTROL_ASISTENCIA_PERSONAL_CABEZAL})
    public Response obtenerPorId(@PathParam("controlAsistenciaPersonalCabezalId") Long controlAsistenciaPersonalCabezalId) {
        try {
            SgControlAsistenciaPersonalCabezal entity = servicio.obtenerPorId(controlAsistenciaPersonalCabezalId);
            if (entity == null) {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgControlAsistenciaPersonalCabezal.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CONTROL_ASISTENCIA_PERSONAL_CABEZAL})
    public Response buscar(FiltroControlAsistenciaPersonalCabezal filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CONTROL_ASISTENCIA_PERSONAL_CABEZAL})
    public Response buscarTotal(FiltroControlAsistenciaPersonalCabezal filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CONTROL_ASISTENCIA_PERSONAL_CABEZAL})
    public Response crear(SgControlAsistenciaPersonalCabezal entity) {
        try {
            entity.setCpcPk(null);
            entity.setCpcVersion(0);
            for (SgAsistenciaPersonal a : entity.getCpcAsistenciaPersonal()) {
                a.setApeControl(entity);
            }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CONTROL_ASISTENCIA_PERSONAL_CABEZAL})
    public Response actualizar(@PathParam("controlAsistenciaPersonalCabezalId") Long id, SgControlAsistenciaPersonalCabezal entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CONTROL_ASISTENCIA_PERSONAL_CABEZAL})
    public Response eliminarPorId(@PathParam("controlAsistenciaPersonalCabezalId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{controlAsistenciaPersonalCabezalId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("controlAsistenciaPersonalCabezalId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgControlAsistenciaPersonalCabezal.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgControlAsistenciaPersonalCabezal.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
