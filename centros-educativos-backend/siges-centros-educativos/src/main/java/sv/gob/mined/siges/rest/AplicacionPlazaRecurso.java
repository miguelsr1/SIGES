...
import sv.gob.mined.siges.negocio.servicios.AplicacionPlazaBean;
...
@RequestScoped
@Path("/v1/aplicacionesplaza")
@Tag(name = "AplicacionPlaza API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_APLICACION_PLAZA,
    ConstantesOperaciones.ACTUALIZAR_APLICACION_PLAZA,
    ConstantesOperaciones.ELIMINAR_APLICACION_PLAZA,
    ConstantesOperaciones.BUSCAR_APLICACION_PLAZA})
public class AplicacionPlazaRecurso {

    private static final Logger LOGGER = Logger.getLogger(AplicacionPlazaRecurso.class.getName());

    @Inject
    private AplicacionPlazaBean servicio;
     ...
     *
     * @param aplicacionPlazaId
     * @return Response -> SgAplicacionPlaza
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAplicacionPlaza.class)))
        ,
    ...
    @Path("/{aplicacionPlazaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_APLICACION_PLAZA})
    public Response obtenerPorId(@PathParam("aplicacionPlazaId") Long aplicacionPlazaId) {
        try {
            SgAplicacionPlaza entity = servicio.obtenerPorId(aplicacionPlazaId);
            if (entity == null) {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAplicacionPlaza.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_APLICACION_PLAZA})
    public Response buscar(FiltroAplicacionPlaza filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_APLICACION_PLAZA})
    public Response buscarTotal(FiltroAplicacionPlaza filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_APLICACION_PLAZA})
    public Response crear(SgAplicacionPlaza entity) {
        try {
            entity.setAplPk(null);
            entity.setAplVersion(0);
            LOGGER.info("crear");
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_APLICACION_PLAZA})
    public Response actualizar(@PathParam("aplicacionPlazaId") Long id, SgAplicacionPlaza entity) {
        try {
    ...
    @Path("/seleccionadoMatriz/{aplicacionPlazaId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_APLICACION_PLAZA})
    public Response guardarSeleccionadoMatriz(@PathParam("aplicacionPlazaId") Long id, SgAplicacionPlaza entity) {
        try {
            if (servicio.objetoExistePorId(id)) {
                entity.setAplPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardarSeleccionadoMatriz(entity)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_APLICACION_PLAZA})
    public Response eliminarPorId(@PathParam("aplicacionPlazaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{aplicacionPlazaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("aplicacionPlazaId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgAplicacionPlaza.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgAplicacionPlaza.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, tracer.activeSpan().context().toString(), ex);
