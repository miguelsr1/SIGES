...
import sv.gob.mined.siges.negocio.servicios.DocenteDocumentoBean;
...
@RequestScoped
@Path("/v1/docentedocumentos")
@Tag(name = "DocenteDocumento API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_DOCENTE_DOCUMENTO,
    ConstantesOperaciones.ACTUALIZAR_DOCENTE_DOCUMENTO,
    ConstantesOperaciones.ELIMINAR_DOCENTE_DOCUMENTO})
public class DocenteDocumentoRecurso {

    private static final Logger LOGGER = Logger.getLogger(DocenteDocumentoRecurso.class.getName());

    @Inject
    private DocenteDocumentoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgDocenteDocumento
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgDocenteDocumento.class)))
        ,
    ...
    @Path("/{docenteDocumentoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("docenteDocumentoId") Long id) {
        try {
            SgDocenteDocumento entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgDocenteDocumento
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgDocenteDocumento.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroDocenteDocumento filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroDocenteDocumento filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DOCENTE_DOCUMENTO})
    public Response crear(SgDocenteDocumento entity) {
        try {
            entity.setDdoPk(null);
            entity.setDdoVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_DOCENTE_DOCUMENTO})
    public Response actualizar(@PathParam("docenteDocumentoId") Long id, SgDocenteDocumento entity) {
        try {
    ...
    /**
     * Valida documento realizado.
     *
     * @param id Long
     * @return Response SgDocenteDocumento
     */
    @POST
    @Operation(summary = "Valida documento realizado.")
    @APIResponses(value = {
    ...
    @Path("/validarrealizado/{docenteDocumentoId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.VALIDAR_INFORMACION_FICHA_DOCENTE})
    public Response validarRealizado(@PathParam("docenteDocumentoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/invalidarrealizado/{docenteDocumentoId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.VALIDAR_INFORMACION_FICHA_DOCENTE})
    public Response invalidarRealizado(@PathParam("docenteDocumentoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_DOCENTE_DOCUMENTO})
    public Response eliminarPorId(@PathParam("docenteDocumentoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{docenteDocumentoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("docenteDocumentoId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgDocenteDocumento.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgDocenteDocumento.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, tracer.activeSpan().context().toString(), ex);
