...
import sv.gob.mined.siges.negocio.servicios.DiplomaBean;
...
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_DIPLOMAS,
    ConstantesOperaciones.ACTUALIZAR_DIPLOMAS,
    ConstantesOperaciones.ELIMINAR_DIPLOMAS,
    ConstantesOperaciones.BUSCAR_DIPLOMAS})
public class DiplomaRecurso {

    private static final Logger LOGGER = Logger.getLogger(DiplomaRecurso.class.getName());

    @Inject
    private DiplomaBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgDiploma.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("diplomaId") Long id) {
        try {
            SgDiploma dil = servicio.obtenerPorId(id);
            if (dil == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(dil).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgDiploma
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgDiploma.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DIPLOMAS})
    public Response buscar(FiltroDiploma filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DIPLOMAS})
    public Response buscarTotal(FiltroDiploma filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DIPLOMAS})
    public Response crear(SgDiploma dil) {
        try {
            dil.setDilPk(null);
            dil.setDilVersion(0);
            if (dil.getDiplomaEstudiantes() != null) {
                for (SgDiplomaEstudiante des : dil.getDiplomaEstudiantes()) {
                    des.setDieDiplomaFk(dil);
                }
            }
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(dil)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_DIPLOMAS})
    public Response actualizar(@PathParam("diplomaId") Long id, SgDiploma dil) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ...
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(dil)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_DIPLOMAS})
    ...
    public Response obtenerHistorialPorId(@PathParam("diplomaId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgDiploma.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgDiploma.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
