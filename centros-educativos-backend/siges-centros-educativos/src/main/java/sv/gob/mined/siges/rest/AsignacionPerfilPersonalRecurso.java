...
import sv.gob.mined.siges.negocio.servicios.AsignacionPerfilPersonalBean;
 ...
 */
@RequestScoped
@Path("/v1/asignacionperfilpersonal")
@Tag(name = "Asignación perfiles Personal API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ASIGNACION_PERFIL_PERSONAL,
    ConstantesOperaciones.ACTUALIZAR_ASIGNACION_PERFIL_PERSONAL,
    ConstantesOperaciones.ELIMINAR_ASIGNACION_PERFIL_PERSONAL})
public class AsignacionPerfilPersonalRecurso {

    private static final Logger LOGGER = Logger.getLogger(AsignacionPerfilPersonalRecurso.class.getName());

    @Inject
    private AsignacionPerfilPersonalBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAsignacionPerfilPersonal
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAsignacionPerfilPersonal.class)))
        ,
    ...
    @Path("/{asignacionPerfilPersonalId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("asignacionPerfilPersonalId") Long id) {
        try {
            SgAsignacionPerfilPersonal app = servicio.obtenerPorId(id);
            if (app == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(app).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgAsignacionPerfilPersonal
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAsignacionPerfilPersonal.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroAsignacionPerfilPersonal filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroAsignacionPerfilPersonal filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ASIGNACION_PERFIL_PERSONAL})
    public Response crear(SgAsignacionPerfilPersonal app) {
        try {
            app.setAppPk(null);
            app.setAppVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(app)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ASIGNACION_PERFIL_PERSONAL})
    public Response actualizar(@PathParam("asignacionPerfilPersonalId") Long id, SgAsignacionPerfilPersonal app) {
        try {
            if (servicio.objetoExistePorId(id)) {
                app.setAppPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(app)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ASIGNACION_PERFIL_PERSONAL})
    public Response eliminarPorId(@PathParam("asignacionPerfilPersonalId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{asignacionPerfilPersonalId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("asignacionPerfilPersonalId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgAsignacionPerfilPersonal.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgAsignacionPerfilPersonal.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
