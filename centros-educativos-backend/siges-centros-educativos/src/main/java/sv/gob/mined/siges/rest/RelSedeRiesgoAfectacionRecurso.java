...
import sv.gob.mined.siges.negocio.servicios.RelSedeRiesgoAfectacionBean;
 ...
 */
@RequestScoped
@Path("/v1/relsederiesgoafectacion")
@Tag(name = "Rel Sede Riesgo Afectacion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_SEDE_RIESGO_AFECTACION,
    ConstantesOperaciones.ACTUALIZAR_REL_SEDE_RIESGO_AFECTACION,
    ConstantesOperaciones.ELIMINAR_REL_SEDE_RIESGO_AFECTACION,
    ConstantesOperaciones.BUSCAR_REL_SEDE_RIESGO_AFECTACION})
public class RelSedeRiesgoAfectacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelSedeRiesgoAfectacionRecurso.class.getName());

    @Inject
    private RelSedeRiesgoAfectacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelSedeRiesgoAfectacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelSedeRiesgoAfectacion.class)))
        ,
    ...
    @Path("/{relSedeRiesgoAfectacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_SEDE_RIESGO_AFECTACION})
    public Response obtenerPorId(@PathParam("relSedeRiesgoAfectacionId") Long id) {
        try {
            SgRelSedeRiesgoAfectacion rar = servicio.obtenerPorId(id);
            if (rar == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(rar).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRelSedeRiesgoAfectacion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelSedeRiesgoAfectacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_SEDE_RIESGO_AFECTACION})
    public Response buscar(FiltroRelSedeRiesgoAfectacion filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_SEDE_RIESGO_AFECTACION})
    public Response crear(SgRelSedeRiesgoAfectacion rar) {
        try {
            rar.setRarPk(null);
            rar.setRarVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(rar)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_SEDE_RIESGO_AFECTACION})
    public Response actualizar(@PathParam("relSedeRiesgoAfectacionId") Long id, SgRelSedeRiesgoAfectacion rar) {
        try {
            if (servicio.objetoExistePorId(id)) {
                rar.setRarPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(rar)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_SEDE_RIESGO_AFECTACION})
    public Response eliminarPorId(@PathParam("relSedeRiesgoAfectacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relSedeRiesgoAfectacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relSedeRiesgoAfectacionId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRelSedeRiesgoAfectacion.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelSedeRiesgoAfectacion.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
