...
import sv.gob.mined.siges.negocio.servicios.AnioLectivoBean;
...
@RequestScoped
@Path("/v1/anioslectivos")
@Tag(name = "AñoLectivo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ANIO_LECTIVO,
    ConstantesOperaciones.ACTUALIZAR_ANIO_LECTIVO,
    ConstantesOperaciones.ELIMINAR_ANIO_LECTIVO,
    ConstantesOperaciones.BUSCAR_ANIO_LECTIVO})
public class AnioLectivoRecurso {

    private static final Logger LOGGER = Logger.getLogger(AnioLectivoRecurso.class.getName());

    @Inject
    private AnioLectivoBean servicio;
    ...
    @Path("/{anioLectivoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ANIO_LECTIVO})
    public Response obtenerPorId(@PathParam("anioLectivoId") Long id) {
        try {
            SgAnioLectivo entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgAnioLectivo
     */
    @POST
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ANIO_LECTIVO})
    public Response buscar(FiltroAnioLectivo filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ANIO_LECTIVO})
    public Response buscarTotal(FiltroAnioLectivo filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ANIO_LECTIVO})
    public Response crear(SgAnioLectivo entity) {
        try {
            entity.setAlePk(null);
            entity.setAleVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ANIO_LECTIVO})
    public Response actualizar(@PathParam("anioLectivoId") Long id, SgAnioLectivo entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ANIO_LECTIVO})
    public Response eliminarPorId(@PathParam("anioLectivoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{anioLectivoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("anioLectivoId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgAnioLectivo.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgAnioLectivo.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
