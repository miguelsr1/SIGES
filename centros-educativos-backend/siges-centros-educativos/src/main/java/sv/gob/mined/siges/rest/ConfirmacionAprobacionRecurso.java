...
import sv.gob.mined.siges.negocio.servicios.ConfirmacionAprobacionBean;
 ...
 */
@RequestScoped
@Path("/v1/confirmacionesaprobaciones")
@Tag(name = "Confirmaciones de Aprobaciones API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CONFIRMAR_APROBACION,
    ConstantesOperaciones.BUSCAR_CONFIRMACION_APROBACION,
    ConstantesOperaciones.ELIMINAR_CONFIRMACION_APROBACION})
public class ConfirmacionAprobacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(ConfirmacionAprobacionRecurso.class.getName());

    @Inject
    private ConfirmacionAprobacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgConfirmacionAprobacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgConfirmacionAprobacion.class)))
        ,
    ...
    @Path("/{confirmacionPromocionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CONFIRMACION_APROBACION})
    public Response obtenerPorId(@PathParam("confirmacionPromocionId") Long id) {
        try {
            SgConfirmacionAprobacion cma = servicio.obtenerPorId(id);
            if (cma == null) {
     ...
     * @return Response Lista de SgConfirmacionAprobacion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgConfirmacionAprobacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CONFIRMACION_APROBACION})
    public Response buscar(FiltroConfirmacionAprobacion filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CONFIRMACION_APROBACION})
    public Response buscarTotal(FiltroConfirmacionAprobacion filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CONFIRMAR_APROBACION})
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CONFIRMAR_APROBACION})
    public Response preconfirmar(SgConfirmacionAprobacion cma) {
        try {
    ...
    @Path("/historial/{confirmacionPromocionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("confirmacionPromocionId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgConfirmacionAprobacion.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgConfirmacionAprobacion.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CONFIRMACION_APROBACION})
