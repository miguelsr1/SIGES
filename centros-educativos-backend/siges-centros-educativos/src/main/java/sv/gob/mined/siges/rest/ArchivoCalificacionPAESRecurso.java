...
import sv.gob.mined.siges.negocio.servicios.ArchivoCalificacionPAESBean;
 ...
 */
@RequestScoped
@Path("/v1/archivoscalificacionespaes")
@Tag(name = "ArchivoCalificacionPAES API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ARCHIVO_CALIFICACION_PAES,
    ConstantesOperaciones.ACTUALIZAR_ARCHIVO_CALIFICACION_PAES,
    ConstantesOperaciones.ELIMINAR_ARCHIVO_CALIFICACION_PAES})
public class ArchivoCalificacionPAESRecurso {

    private static final Logger LOGGER = Logger.getLogger(ArchivoCalificacionPAESRecurso.class.getName());

    @Inject
    private ArchivoCalificacionPAESBean servicio;
    ...
    @Path("/{archivoCalificacionPAESId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response obtenerPorId(@PathParam("archivoCalificacionPAESId") Long id) {
        try {
            SgArchivoCalificacionPAES cal = servicio.obtenerPorId(id);
            if (cal == null) {
    ...
    public Response buscar(FiltroArchivoCalificacionPAES filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroArchivoCalificacionPAES filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ARCHIVO_CALIFICACION_PAES})
    public Response crear(SgArchivoCalificacionPAES cal) {
        try {
            cal.setAcpPk(null);
            cal.setAcpVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ARCHIVO_CALIFICACION_PAES})
    public Response actualizar(@PathParam("archivoCalificacionPAESId") Long id, SgArchivoCalificacionPAES cal) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ARCHIVO_CALIFICACION_PAES})
    public Response eliminarPorId(@PathParam("archivoCalificacionPAESId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{archivoCalificacionPAESId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("archivoCalificacionPAESId") Long id) {
        try {
    ...
    @GET
    @Path("/calificarArchivosPAES")
    @RolesAllowed({ConstantesOperaciones.EJECUTAR_PROCESAMIENTO_CALIFICACIONES_PAES})
    public Response calificarArchivos(@QueryParam("cantidad") Long cantidad, @QueryParam("inicio") Long inicio, @QueryParam("fin") Long fin, @QueryParam("codigoexterno") String codigoExterno) {
        try {

            if (cantidad == null) {
                cantidad = 2000L;
            }
            Boolean respuesta = servicio.ejecutarEstructuraCalificacionPAES(cantidad, inicio, fin, codigoExterno);
