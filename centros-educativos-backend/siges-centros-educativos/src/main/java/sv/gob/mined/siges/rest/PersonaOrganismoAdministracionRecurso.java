...
import sv.gob.mined.siges.negocio.servicios.PersonaOrganismoAdministracionBean;
 ...
 */
@RequestScoped
@Path("/v1/personaorganismoadministracion")
@Tag(name = "Persona Organismos de Administración API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_PERSONA_ORGANISMO_ADMINISTRACION,
    ConstantesOperaciones.ACTUALIZAR_PERSONA_ORGANISMO_ADMINISTRACION,
    ConstantesOperaciones.ELIMINAR_PERSONA_ORGANISMO_ADMINISTRACION})
public class PersonaOrganismoAdministracionRecurso {

    private static final Logger LOGGER = Logger.getLogger(PersonaOrganismoAdministracionRecurso.class.getName());

    @Inject
    private PersonaOrganismoAdministracionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgPersonaOrganismoAdministracion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgPersonaOrganismoAdministracion.class)))
        ,
    ...
    @Path("/{personaOrganismoAdministracionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("personaOrganismoAdministracionId") Long id) {
        try {
            SgPersonaOrganismoAdministracion poa = servicio.obtenerPorId(id);
            if (poa == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(poa).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgPersonaOrganismoAdministracion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgPersonaOrganismoAdministracion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroPersonaOrganismoAdministrativo filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroPersonaOrganismoAdministrativo filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_PERSONA_ORGANISMO_ADMINISTRACION})
    public Response crear(SgPersonaOrganismoAdministracion poa) {
        try {
            poa.setPoaPk(null);
            poa.setPoaVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(poa)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PERSONA_ORGANISMO_ADMINISTRACION})
    public Response actualizar(@PathParam("personaOrganismoAdministracionId") Long id, SgPersonaOrganismoAdministracion poa) {
        try {
            if (servicio.objetoExistePorId(id)) {
                poa.setPoaPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(poa)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response validar(SgPersonaOrganismoAdministracion entity) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(PersonaOrganismoAdministracionValidacionNegocio.validar(entity)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PERSONA_ORGANISMO_ADMINISTRACION})
    public Response eliminarPorId(@PathParam("personaOrganismoAdministracionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{personaOrganismoAdministracionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("personaOrganismoAdministracionId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgPersonaOrganismoAdministracion.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgPersonaOrganismoAdministracion.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
