...
import sv.gob.mined.siges.negocio.servicios.EscolaridadEstudianteBean;
 ...
 */
@RequestScoped
@Path("/v1/escolaridadestudiante")
@Tag(name = "Escolaridad estudiante API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ESCOLARIDAD_ESTUDIANTE,
    ConstantesOperaciones.ACTUALIZAR_ESCOLARIDAD_ESTUDIANTE,
    ConstantesOperaciones.ELIMINAR_ESCOLARIDAD_ESTUDIANTE,
    ConstantesOperaciones.BUSCAR_ESCOLARIDAD_ESTUDIANTE})
public class EscolaridadEstudianteRecurso {

    private static final Logger LOGGER = Logger.getLogger(EscolaridadEstudianteRecurso.class.getName());

    @Inject
    private EscolaridadEstudianteBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgEscolaridadEstudiante
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEscolaridadEstudiante.class)))
        ,
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ESCOLARIDAD_ESTUDIANTE})
    public Response obtenerPorId(@PathParam("escolaridadEstudianteId") Long id) {
        try {
            SgEscolaridadEstudiante esc = servicio.obtenerPorId(id);
            if (esc == null) {
     ...
     * @return Response Lista de SgEscolaridadEstudiante
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgEscolaridadEstudiante.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ESCOLARIDAD_ESTUDIANTE})
    public Response buscar(FiltroEscolaridadEstudiante filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ESCOLARIDAD_ESTUDIANTE})
    public Response buscarTotal(FiltroEscolaridadEstudiante filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ESCOLARIDAD_ESTUDIANTE})
    public Response crear(SgEscolaridadEstudiante esc) {
        try {
            esc.setEscPk(null);
            esc.setEscVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ESCOLARIDAD_ESTUDIANTE})
    public Response actualizar(@PathParam("escolaridadEstudianteId") Long id, SgEscolaridadEstudiante esc) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ESCOLARIDAD_ESTUDIANTE})
    ...
    public Response obtenerHistorialPorId(@PathParam("escolaridadEstudianteId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialRevisionesPorId(SgEscolaridadEstudiante.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgEscolaridadEstudiante.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    @Path("/guardarEscolaridades")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ESCOLARIDAD_ESTUDIANTE})
    public Response guardarEscolaridades(SgCabezalEscolaridadEstudiante esc) {
        try {

            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardarEscolaridades(esc)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
