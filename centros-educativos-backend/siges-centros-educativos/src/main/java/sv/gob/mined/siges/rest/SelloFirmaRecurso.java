...
import sv.gob.mined.siges.negocio.servicios.SelloFirmaBean;
...
@RequestScoped
@Path("/v1/sellofirma")
@Tag(name = "Sellos Firmas API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SELLO_FIRMA,
    ConstantesOperaciones.ACTUALIZAR_SELLO_FIRMA,
    ConstantesOperaciones.ELIMINAR_SELLO_FIRMA,
    ConstantesOperaciones.BUSCAR_SELLO_FIRMA})
public class SelloFirmaRecurso {

    private static final Logger LOGGER = Logger.getLogger(SelloFirmaRecurso.class.getName());

    @Inject
    private SelloFirmaBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgSelloFirma.class)))
        ,
    ...
    @Path("/{selloFirmaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SELLO_FIRMA})
    public Response obtenerPorId(@PathParam("selloFirmaId") Long id) {
        try {
            SgSelloFirma sfi = servicio.obtenerPorId(id);
            if (sfi == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(sfi).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgSelloFirma
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgSelloFirma.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SELLO_FIRMA})
    public Response buscar(FiltroSelloFirma filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SELLO_FIRMA})
    public Response buscarTotal(FiltroSelloFirma filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SELLO_FIRMA})
    public Response crear(SgSelloFirma sfi) {
        try {
            sfi.setSfiPk(null);
            sfi.setSfiVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(sfi)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
                return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(PersistenceHelper.getSelloFirmaViolationBusinessException(ex)).build();
            } else {
                tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
                tracer.activeSpan().log(ex.getMessage());
                LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SELLO_FIRMA})
    public Response actualizar(@PathParam("selloFirmaId") Long id, SgSelloFirma sfi) {
        try {
            if (servicio.objetoExistePorId(id)) {
                sfi.setSfiPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(sfi)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_SELLO_FIRMA})
    public Response eliminarPorId(@PathParam("selloFirmaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{selloFirmaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("selloFirmaId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgSelloFirma.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgSelloFirma.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
