...
import sv.gob.mined.siges.negocio.servicios.EstudianteValoracionBean;
...
@RequestScoped
@Path("/v1/estudiantevaloracion")
@Tag(name = "Estudiante Valoracion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ESTUDIANTE_VALORACION,
    ConstantesOperaciones.ACTUALIZAR_ESTUDIANTE_VALORACION,
    ConstantesOperaciones.ELIMINAR_ESTUDIANTE_VALORACION,
    ConstantesOperaciones.BUSCAR_ESTUDIANTE_VALORACION})
public class EstudianteValoracionRecurso {

    private static final Logger LOGGER = Logger.getLogger(EstudianteRecurso.class.getName());

    @Inject
    private EstudianteValoracionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgEstudianteValoracion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEstudianteValoracion.class)))
        ,
    ...
    @Path("/{estudianteValoracionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ESTUDIANTE_VALORACION})
    public Response obtenerPorId(@PathParam("estudianteValoracionId") Long id) {
        try {
            SgEstudianteValoracion entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgEstudianteValoracion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgEstudianteValoracion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ESTUDIANTE_VALORACION})
    public Response buscar(FiltroEstudianteValoracion filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ESTUDIANTE_VALORACION})
    public Response buscarTotal(FiltroEstudianteValoracion filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ESTUDIANTE_VALORACION})
    public Response crear(SgEstudianteValoracion entity) {
        try {
            entity.setEsvPk(null);
            entity.setEsvVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ESTUDIANTE_VALORACION, ConstantesOperaciones.MODIFICAR_VALORACION_DOCENTE_PROPIA})
    public Response actualizar(@PathParam("estudianteValoracionId") Long id, SgEstudianteValoracion entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ESTUDIANTE_VALORACION, ConstantesOperaciones.MODIFICAR_VALORACION_DOCENTE_PROPIA})
    public Response eliminarPorId(@PathParam("estudianteValoracionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{estudianteValoracionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("estudianteValoracionId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgEstudianteValoracion.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgEstudianteValoracion.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
