...
import sv.gob.mined.siges.negocio.servicios.SelloFirmaTitularBean;
 ...
 */
@RequestScoped
@Path("/v1/sellofirmatitular")
@Tag(name = "Sellos Firmas Titulares API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SELLO_FIRMA_TITULAR,
    ConstantesOperaciones.ACTUALIZAR_SELLO_FIRMA_TITULAR,
    ConstantesOperaciones.ELIMINAR_SELLO_FIRMA_TITULAR,
    ConstantesOperaciones.BUSCAR_SELLO_FIRMA_TITULAR})
public class SelloFirmaTitularRecurso {

    private static final Logger LOGGER = Logger.getLogger(SelloFirmaTitularRecurso.class.getName());

    @Inject
    private SelloFirmaTitularBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgSelloFirmaTitular
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgSelloFirmaTitular.class)))
        ,
    ...
    @Path("/{selloFirmaTitularId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SELLO_FIRMA_TITULAR})
    public Response obtenerPorId(@PathParam("selloFirmaTitularId") Long id) {
        try {
            SgSelloFirmaTitular sft = servicio.obtenerPorId(id);
            if (sft == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(sft).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgSelloFirmaTitular
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgSelloFirmaTitular.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SELLO_FIRMA_TITULAR})
    public Response buscar(FiltroSelloFirmaTitular filtro) {
        try {
     ...
     *
     * @param filtro FiltroSelloFirmaTitular
     * @return Response Lista de SgSelloFirmaTitular
     */
    @POST
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SELLO_FIRMA_TITULAR})
    public Response buscarTotal(FiltroSelloFirmaTitular filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SELLO_FIRMA_TITULAR})
    public Response crear(SgSelloFirmaTitular sft) {
        try {
            sft.setSftPk(null);
            sft.setSftVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(sft)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SELLO_FIRMA_TITULAR})
    public Response actualizar(@PathParam("selloFirmaTitularId") Long id, SgSelloFirmaTitular sft) {
        try {
            if (servicio.objetoExistePorId(id)) {
                sft.setSftPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(sft)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_SELLO_FIRMA_TITULAR})
    public Response eliminarPorId(@PathParam("selloFirmaTitularId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{selloFirmaTitularId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("selloFirmaTitularId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgSelloFirmaTitular.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgSelloFirmaTitular.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
