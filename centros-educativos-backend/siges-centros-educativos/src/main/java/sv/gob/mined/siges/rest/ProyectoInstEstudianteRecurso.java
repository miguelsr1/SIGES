...
import sv.gob.mined.siges.negocio.servicios.ProyectoInstEstudianteBean;
...
@RequestScoped
@Path("/v1/proyectosinstestudiantes")
@Tag(name = "ProyectoInstEstudiante API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_PROYECTO_INST_ESTUDIANTE,
    ConstantesOperaciones.ACTUALIZAR_PROYECTO_INST_ESTUDIANTE,
    ConstantesOperaciones.ELIMINAR_PROYECTO_INST_ESTUDIANTE})
public class ProyectoInstEstudianteRecurso {

    private static final Logger LOGGER = Logger.getLogger(ProyectoInstEstudianteRecurso.class.getName());

    @Inject
    private ProyectoInstEstudianteBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgProyectoInstEstudiante
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgProyectoInstEstudiante.class)))
        ,
    ...
    @Path("/{proyectoInstEstudianteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("proyectoInstEstudianteId") Long id) {
        try {
            SgProyectoInstEstudiante entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgProyectoInstEstudiante
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgProyectoInstEstudiante.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroProyectoInstEstudiante filtro) {
        try {
     ...
     *
     * @param filtro FiltroProyectoInstEstudiante
     * @return Response Cantidad total de SgProyectoInstEstudiante
     */
    @POST
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroProyectoInstEstudiante filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_PROYECTO_INST_ESTUDIANTE})
    public Response crear(SgProyectoInstEstudiante entity) {
        try {
            entity.setPiePk(null);
            entity.setPieVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PROYECTO_INST_ESTUDIANTE})
    public Response actualizar(@PathParam("proyectoInstEstudianteId") Long id, SgProyectoInstEstudiante entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PROYECTO_INST_ESTUDIANTE})
    public Response eliminarPorId(@PathParam("proyectoInstEstudianteId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{proyectoInstEstudianteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("proyectoInstEstudianteId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgProyectoInstEstudiante.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgProyectoInstEstudiante.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
     ...
     * Crea el objeto indicado con id autogenerada
     *
     * @param entities SgProyectoInstEstudiante
     * @return Response SgProyectoInstEstudiante
     */
    @POST
    @Operation(summary = "Crea el objeto indicado con id autogenerada.")
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response guardarLista(SgProyectoInstEstudiante[] entities) {
        try {
            servicio.guardarLista(entities);
            return Response.status(HttpStatus.SC_OK).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
