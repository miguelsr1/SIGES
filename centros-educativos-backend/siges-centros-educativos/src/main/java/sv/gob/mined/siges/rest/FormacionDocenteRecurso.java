...
import sv.gob.mined.siges.negocio.servicios.FormacionDocenteBean;
...
@RequestScoped
@Path("/v1/formacionesdocente")
@Tag(name = "FormacionDocente API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_FORMACIONES_DOCENTE,
    ConstantesOperaciones.ACTUALIZAR_FORMACIONES_DOCENTE,
    ConstantesOperaciones.ELIMINAR_FORMACIONES_DOCENTE,
    ConstantesOperaciones.BUSCAR_FORMACIONES_DOCENTE})
public class FormacionDocenteRecurso {

    private static final Logger LOGGER = Logger.getLogger(FormacionDocenteRecurso.class.getName());

    @Inject
    private FormacionDocenteBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgFormacionDocente
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgFormacionDocente.class)))
        ,
    ...
    @Path("/{formacionDocenteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_FORMACIONES_DOCENTE})
    public Response obtenerPorId(@PathParam("formacionDocenteId") Long id) {
        try {
            SgFormacionDocente entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgFormacionDocente
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgFormacionDocente.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_FORMACIONES_DOCENTE})
    public Response buscar(FiltroFormacionDocente filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_FORMACIONES_DOCENTE})
    public Response crear(SgFormacionDocente entity) {
        try {
            entity.setFdoPk(null);
            entity.setFdoVersion(0);
            LOGGER.info("crear");
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_FORMACIONES_DOCENTE})
    public Response actualizar(@PathParam("formacionDocenteId") Long id, SgFormacionDocente entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_FORMACIONES_DOCENTE})
    public Response eliminarPorId(@PathParam("formacionDocenteId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{formacionDocenteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("formacionDocenteId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgFormacionDocente.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgFormacionDocente.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
