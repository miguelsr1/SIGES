...
import sv.gob.mined.siges.negocio.servicios.RelPeriodosHabilitacionPeriodoBean;
 ...
 */
@RequestScoped
@Path("/v1/relperiodoshabilitacionperiodo")
@Tag(name = "Relación entre periodo y habilitación periodo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_PERIODOS_HABILITACION_PERIODO,
    ConstantesOperaciones.ACTUALIZAR_REL_PERIODOS_HABILITACION_PERIODO,
    ConstantesOperaciones.ELIMINAR_REL_PERIODOS_HABILITACION_PERIODO,
	ConstantesOperaciones.BUSCAR_REL_PERIODOS_HABILITACION_PERIODO})
public class RelPeriodosHabilitacionPeriodoRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelPeriodosHabilitacionPeriodoRecurso.class.getName());

    @Inject
    private RelPeriodosHabilitacionPeriodoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelPeriodosHabilitacionPeriodo
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelPeriodosHabilitacionPeriodo.class)))
        ,
    ...
    @Path("/{relPeriodosHabilitacionPeriodoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_PERIODOS_HABILITACION_PERIODO})
    public Response obtenerPorId(@PathParam("relPeriodosHabilitacionPeriodoId") Long id) {
        try {
            SgRelPeriodosHabilitacionPeriodo rph = servicio.obtenerPorId(id);
            if (rph == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(rph).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRelPeriodosHabilitacionPeriodo
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelPeriodosHabilitacionPeriodo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_PERIODOS_HABILITACION_PERIODO})
    public Response buscar(FiltroRelPeriodosHabilitacionPeriodo filtro) {
        try {
    ...
    @Path("/buscarRangosConSolicitud")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_PERIODOS_HABILITACION_PERIODO})
    public Response buscarRangosConSolicitud(FiltroRelPeriodosHabilitacionPeriodo filtro) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.buscarRangosConSolicitud(filtro)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    @Path("/buscarPeriodoExtraordinarioConSolicitud")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_PERIODOS_HABILITACION_PERIODO})
    public Response buscarPeriodoExtraordinarioConSolicitud(FiltroRelPeriodosHabilitacionPeriodo filtro) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.buscarPeriodoExtraordinarioConSolicitud(filtro)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REL_PERIODOS_HABILITACION_PERIODO})
    public Response buscarTotal(FiltroRelPeriodosHabilitacionPeriodo filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_PERIODOS_HABILITACION_PERIODO})
    public Response crear(SgRelPeriodosHabilitacionPeriodo rph) {
        try {
            rph.setRphPk(null);
            rph.setRphVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(rph)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_PERIODOS_HABILITACION_PERIODO})
    public Response actualizar(@PathParam("relPeriodosHabilitacionPeriodoId") Long id, SgRelPeriodosHabilitacionPeriodo rph) {
        try {
            if (servicio.objetoExistePorId(id)) {
                rph.setRphPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(rph)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_PERIODOS_HABILITACION_PERIODO})
    public Response eliminarPorId(@PathParam("relPeriodosHabilitacionPeriodoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relPeriodosHabilitacionPeriodoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relPeriodosHabilitacionPeriodoId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgRelPeriodosHabilitacionPeriodo.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelPeriodosHabilitacionPeriodo.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
