...
import sv.gob.mined.siges.negocio.servicios.DatoEmpleadoBean;
...
@RequestScoped
@Path("/v1/datoempleado")
@Tag(name = "DatoEmpleado API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_DATO_EMPLEADO,
    ConstantesOperaciones.ACTUALIZAR_DATO_EMPLEADO,
    ConstantesOperaciones.ELIMINAR_DATO_EMPLEADO,
    ConstantesOperaciones.BUSCAR_DATO_EMPLEADO})
public class DatoEmpleadoRecurso {

    private static final Logger LOGGER = Logger.getLogger(DatoEmpleadoRecurso.class.getName());

    @Inject
    private DatoEmpleadoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgDatoEmpleado
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgDatoEmpleado.class)))
        ,
    ...
    @Path("/{datoEmpleadoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DATO_EMPLEADO})
    public Response obtenerPorId(@PathParam("datoEmpleadoId") Long id) {
        try {
            SgDatoEmpleado entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgDatoEmpleado
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgDatoEmpleado.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DATO_EMPLEADO})
    public Response buscar(FiltroDatoEmpleado filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DATO_EMPLEADO})
    public Response crear(SgDatoEmpleado entity) {
        try {
            entity.setDemPk(null);
            entity.setDemVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_DATO_EMPLEADO})
    public Response actualizar(@PathParam("datoEmpleadoId") Long id, SgDatoEmpleado entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_DATO_EMPLEADO})
    public Response eliminarPorId(@PathParam("datoEmpleadoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{datoEmpleadoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("datoEmpleadoId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgDatoEmpleado.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgDatoEmpleado.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response validar(SgDatoEmpleado entity) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(DatoEmpleadoValidacionNegocio.validar(entity)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
