...
import sv.gob.mined.siges.negocio.servicios.ReposicionTituloBean;
 ...
 */
@RequestScoped
@Path("/v1/reposiciontitulo")
@Tag(name = "Reposición Título API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REPOSICION_TITULO,
    ConstantesOperaciones.ACTUALIZAR_REPOSICION_TITULO,
    ConstantesOperaciones.ELIMINAR_REPOSICION_TITULO,
    ConstantesOperaciones.REPOSICION_TITULO})
public class ReposicionTituloRecurso {

    private static final Logger LOGGER = Logger.getLogger(ReposicionTituloRecurso.class.getName());

    @Inject
    private ReposicionTituloBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgReposicionTitulo
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgReposicionTitulo.class)))
        ,
    ...
    @Path("/{reposicionTituloId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("reposicionTituloId") Long id) {
        try {
            SgReposicionTitulo ret = servicio.obtenerPorId(id);
            if (ret == null) {
     ...
     * @return Response Lista de SgReposicionTitulo
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgReposicionTitulo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroReposicionTitulo filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroReposicionTitulo filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REPOSICION_TITULO})
    public Response crear(SgReposicionTitulo ret) {
        try {
            ret.setRetPk(null);
            ret.setRetVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REPOSICION_TITULO})
    public Response actualizar(@PathParam("reposicionTituloId") Long id, SgReposicionTitulo ret) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REPOSICION_TITULO})
    public Response eliminarPorId(@PathParam("reposicionTituloId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{reposicionTituloId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("reposicionTituloId") Long id) {
        try {

            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgReposicionTitulo.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgReposicionTitulo.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Path("/reponersolicitud")
    @RolesAllowed({ConstantesOperaciones.REPOSICION_TITULO})
    public Response reponerSolicitud(SgReposicionSolicitud ret) {
        try {

            return Response.status(HttpStatus.SC_CREATED).entity(servicio.reponerSolicitud(ret)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
