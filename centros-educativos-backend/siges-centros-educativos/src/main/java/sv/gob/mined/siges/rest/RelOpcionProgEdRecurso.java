...
import sv.gob.mined.siges.negocio.servicios.RelOpcionProgEdBean;
...
@RequestScoped
@Path("/v1/relopcionproged")
@Tag(name = "RelacionOpcionProgramaEduactivo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_RELACION_OPCION_PROGRAMA_EDUCATIVO,
    ConstantesOperaciones.ACTUALIZAR_RELACION_OPCION_PROGRAMA_EDUCATIVO,
    ConstantesOperaciones.ELIMINAR_RELACION_OPCION_PROGRAMA_EDUCATIVO,
    ConstantesOperaciones.BUSCAR_RELACION_OPCION_PROGRAMA_EDUCATIVO})
public class RelOpcionProgEdRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelOpcionProgEdRecurso.class.getName());

    @Inject
    private RelOpcionProgEdBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelOpcionProgEd
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelOpcionProgEd.class)))
        ,
    ...
    @Path("/{relOpcionProgEdId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_RELACION_OPCION_PROGRAMA_EDUCATIVO})
    public Response obtenerPorId(@PathParam("relOpcionProgEdId") Long id) {
        try {
            SgRelOpcionProgEd entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgRelOpcionProgEd
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelOpcionProgEd.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_RELACION_OPCION_PROGRAMA_EDUCATIVO})
    public Response buscar(FiltroRelOpcionProgEd filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_RELACION_OPCION_PROGRAMA_EDUCATIVO})
    public Response buscarTotal(FiltroRelOpcionProgEd filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_RELACION_OPCION_PROGRAMA_EDUCATIVO})
    public Response crear(SgRelOpcionProgEd entity) {
        try {
            entity.setRoePk(null);
            entity.setRoeVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_RELACION_OPCION_PROGRAMA_EDUCATIVO})
    public Response actualizar(@PathParam("relOpcionProgEdId") Long id, SgRelOpcionProgEd entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_RELACION_OPCION_PROGRAMA_EDUCATIVO})
    public Response eliminarPorId(@PathParam("relOpcionProgEdId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relOpcionProgEdId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relOpcionProgEdId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRelOpcionProgEd.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelOpcionProgEd.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
