...
import sv.gob.mined.siges.negocio.servicios.FichaSaludBean;
...
@RequestScoped
@Path("/v1/fichassalud")
@Tag(name = "FichaSalud API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_FICHAS_SALUD,
    ConstantesOperaciones.ACTUALIZAR_FICHAS_SALUD,
    ConstantesOperaciones.ELIMINAR_FICHAS_SALUD,
    ConstantesOperaciones.BUSCAR_FICHAS_SALUD})
public class FichaSaludRecurso {

    private static final Logger LOGGER = Logger.getLogger(FichaSaludRecurso.class.getName());

    @Inject
    private FichaSaludBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgFichaSalud.class)))
        ,
    ...
    @Path("/{fichaSaludId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("fichaSaludId") Long id) {
        try {
            SgFichaSalud entity = servicio.obtenerPorId(id);
            if (entity == null) {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgFichaSalud.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroFichaSalud filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroFichaSalud filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_FICHAS_SALUD})
    public Response crear(SgFichaSalud entity) {
        try {
            entity.setFsaPk(null);
            entity.setFsaVersion(0);
            LOGGER.info("crear");
     ...
     * @param Id
     * @param entity
     * @return Response -> SigesDireccion
     */
    @PUT
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_FICHAS_SALUD})
    public Response actualizar(@PathParam("fichaSaludId") Long Id, SgFichaSalud entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_FICHAS_SALUD})
    public Response eliminarPorId(@PathParam("fichaSaludId") Long Id) {
        try {
            if (servicio.objetoExistePorId(Id)) {
    ...
    @Path("/historial/{fichaSaludId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("fichaSaludId") Long Id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgFichaSalud.class, Id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgFichaSalud.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
