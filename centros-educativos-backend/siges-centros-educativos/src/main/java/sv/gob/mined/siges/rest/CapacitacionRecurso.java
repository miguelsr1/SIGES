...
import sv.gob.mined.siges.negocio.servicios.CapacitacionBean;
...
@RequestScoped
@Path("/v1/capacitaciones")
@Tag(name = "Capacitacion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CAPACITACIONES,
    ConstantesOperaciones.ACTUALIZAR_CAPACITACIONES,
    ConstantesOperaciones.ELIMINAR_CAPACITACIONES,
    ConstantesOperaciones.BUSCAR_CAPACITACIONES})
public class CapacitacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(CapacitacionRecurso.class.getName());

    @Inject
    private CapacitacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgCapacitacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCapacitacion.class)))
        ,
    ...
    @Path("/{capacitacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CAPACITACIONES})
    public Response obtenerPorId(@PathParam("capacitacionId") Long id) {
        try {
            SgCapacitacion entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgCapacitacion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgCapacitacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CAPACITACIONES})
    public Response crear(SgCapacitacion entity) {
        try {
            entity.setCapPk(null);
            entity.setCapVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CAPACITACIONES})
    public Response actualizar(@PathParam("capacitacionId") Long id, SgCapacitacion entity) {
        try {
    ...
    /**
     * Valida capacitacion como realizada.
     *
     * @param id Long
     * @return Response SgCapacitacion
     */
    @POST
    @Operation(summary = "Valida capacitacion como realizada.")
    @APIResponses(value = {
    ...
    @Path("/validarrealizado/{capacitacionId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.VALIDAR_INFORMACION_FICHA_DOCENTE})
    public Response validarRealizado(@PathParam("capacitacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    /**
     * Invalida capacitacion como realizada.
     *
     * @param id Long
     * @return Response SgCapacitacion
     */
    @POST
    @Operation(summary = "Invalida capacitacion como realizada.")
    @APIResponses(value = {
    ...
    @Path("/invalidarrealizado/{capacitacionId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.VALIDAR_INFORMACION_FICHA_DOCENTE})
    public Response invalidarRealizado(@PathParam("capacitacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CAPACITACIONES})
    public Response eliminarPorId(@PathParam("capacitacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{capacitacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("capacitacionId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgCapacitacion.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCapacitacion.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response validar(SgCapacitacion entity) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(CapacitacionValidacionNegocio.validar(entity)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
