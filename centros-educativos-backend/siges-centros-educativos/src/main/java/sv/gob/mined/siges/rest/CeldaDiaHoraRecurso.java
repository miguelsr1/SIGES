...
import sv.gob.mined.siges.negocio.servicios.CeldaDiaHoraBean;
 ...
 */
@RequestScoped
@Path("/v1/celdasdiahora")
@Tag(name = "Celdas Día Hora API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CELDAS_DIA_HORA,
    ConstantesOperaciones.ACTUALIZAR_CELDAS_DIA_HORA,
    ConstantesOperaciones.ELIMINAR_CELDAS_DIA_HORA,
    ConstantesOperaciones.BUSCAR_CELDAS_DIA_HORA})
public class CeldaDiaHoraRecurso {

    private static final Logger LOGGER = Logger.getLogger(CeldaDiaHoraRecurso.class.getName());

    @Inject
    private CeldaDiaHoraBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgCeldaDiaHora
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCeldaDiaHora.class)))
        ,
    ...
    @Path("/{celdaDiaHoraId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CELDAS_DIA_HORA})
    public Response obtenerPorId(@PathParam("celdaDiaHoraId") Long id) {
        try {
            SgCeldaDiaHora cdh = servicio.obtenerPorId(id);
            if (cdh == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(cdh).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgCeldaDiaHora
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgCeldaDiaHora.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CELDAS_DIA_HORA})
    public Response buscar(FiltroCeldaDiaHora filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CELDAS_DIA_HORA})
    public Response crear(SgCeldaDiaHora cdh) {
        try {
            cdh.setCdhPk(null);
            cdh.setCdhVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(cdh)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CELDAS_DIA_HORA})
    public Response actualizar(@PathParam("celdaDiaHoraId") Long id, SgCeldaDiaHora cdh) {
        try {
            if (servicio.objetoExistePorId(id)) {
                cdh.setCdhPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(cdh)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CELDAS_DIA_HORA})
    public Response eliminarPorId(@PathParam("celdaDiaHoraId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{celdaDiaHoraId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("celdaDiaHoraId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgCeldaDiaHora.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCeldaDiaHora.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
