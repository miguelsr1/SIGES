...
import sv.gob.mined.siges.negocio.servicios.DetalleMatriculaBean;
 ...
 */
@RequestScoped
@Path("/v1/detallematricula")
@Tag(name = "Detalles de matricula API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_DETALLE_MATRICULA,
    ConstantesOperaciones.ACTUALIZAR_DETALLE_MATRICULA,
    ConstantesOperaciones.ELIMINAR_DETALLE_MATRICULA,
	ConstantesOperaciones.BUSCAR_DETALLE_MATRICULA})
public class DetalleMatriculaRecurso {

    private static final Logger LOGGER = Logger.getLogger(DetalleMatriculaRecurso.class.getName());

    @Inject
    private DetalleMatriculaBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgDetalleMatricula
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgDetalleMatricula.class)))
        ,
    ...
    @Path("/{detalleMatriculaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DETALLE_MATRICULA})
    public Response obtenerPorId(@PathParam("detalleMatriculaId") Long id) {
        try {
            SgDetalleMatricula dem = servicio.obtenerPorId(id);
            if (dem == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(dem).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgDetalleMatricula
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgDetalleMatricula.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DETALLE_MATRICULA})
    public Response buscar(FiltroDetalleMatricula filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DETALLE_MATRICULA})
    public Response buscarTotal(FiltroDetalleMatricula filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DETALLE_MATRICULA})
    public Response crear(SgDetalleMatricula dem) {
        try {
            dem.setDemPk(null);
            dem.setDemVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(dem)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_DETALLE_MATRICULA})
    public Response actualizar(@PathParam("detalleMatriculaId") Long id, SgDetalleMatricula dem) {
        try {
            if (servicio.objetoExistePorId(id)) {
                dem.setDemPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(dem)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_DETALLE_MATRICULA})
    public Response eliminarPorId(@PathParam("detalleMatriculaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{detalleMatriculaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("detalleMatriculaId") Long id) {
        try {
	    return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgDetalleMatricula.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgDetalleMatricula.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
