...
import sv.gob.mined.siges.negocio.servicios.PlanEscolarAnualBean;
...
@RequestScoped
@Path("/v1/planescolaranual")
@Tag(name = "PlanEscolarAnual API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_PLAN_ESCOLAR_ANUAL,
    ConstantesOperaciones.ACTUALIZAR_PLAN_ESCOLAR_ANUAL,
    ConstantesOperaciones.ELIMINAR_PLAN_ESCOLAR_ANUAL,
    ConstantesOperaciones.BUSCAR_PLAN_ESCOLAR_ANUAL})
public class PlanEscolarAnualRecurso {

    private static final Logger LOGGER = Logger.getLogger(PlanEscolarAnualRecurso.class.getName());

    @Inject
    private PlanEscolarAnualBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgPlanEscolarAnual
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgPlanEscolarAnual.class)))
        ,
    ...
    @Path("/{planEscolarAnualId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("planEscolarAnualId") Long id) {
        try {
            SgPlanEscolarAnual entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgPlanEscolarAnual
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgPlanEscolarAnual.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PLAN_ESCOLAR_ANUAL})
    public Response buscar(FiltroPlanEscolarAnual filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PLAN_ESCOLAR_ANUAL})
    public Response buscarTotal(FiltroPlanEscolarAnual filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_PLAN_ESCOLAR_ANUAL})
    public Response crear(SgPlanEscolarAnual entity) {
        try {
            entity.setPeaPk(null);
            entity.setPeaVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PLAN_ESCOLAR_ANUAL})
    public Response actualizar(@PathParam("planEscolarAnualId") Long id, SgPlanEscolarAnual entity) {
        try {
            if (servicio.objetoExistePorId(id)) {
                entity.setPeaPk(id);
                for (SgDetallePlanEscolar det : entity.getPeaDetallePlanEscolar()) {
                    det.setDpePlanEscolarAnual(entity);
                }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PLAN_ESCOLAR_ANUAL})
    public Response eliminarPorId(@PathParam("planEscolarAnualId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{planEscolarAnualId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("planEscolarAnualId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgPlanEscolarAnual.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgPlanEscolarAnual.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
