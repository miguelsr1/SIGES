...
import sv.gob.mined.siges.negocio.servicios.DeshabilitarPersonaJuridicaBean;
...
@RequestScoped
@Path("/v1/deshabilitarPersonaJuridicaRecurso")
@Tag(name = "Deshabilitar Persona Jurídica V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.PUEDE_DESHABILITAR_PERSONA_JURIDICA})
public class DeshabilitarPersonaJuridicaRecurso {

    private static final Logger LOGGER = Logger.getLogger(DeshabilitarPersonaJuridicaRecurso.class.getName());

    @Inject
    private DeshabilitarPersonaJuridicaBean servicio;
     ...
     * @return Response Lista de SgNie
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgNie.class, type = SchemaType.ARRAY)))
        ,
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUDES_OAE})
    public Response buscar(FiltroSolicitudesOAE filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUDES_OAE})
    public Response buscarTotal(FiltroSolicitudesOAE filtro) {
        try {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgSolDeshabilitarPerJur.class)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
    ...
    public Response obtenerPorId(@PathParam("solId") Long solId) {
        try {
            SgSolDeshabilitarPerJur tce = servicio.obtenerPorId(solId);
            if (tce == null) {
     ...
     *
     * @param entity
     * @return Response -> SgSolDeshabilitarPerJur
     */
    @POST
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.PUEDE_DESHABILITAR_PERSONA_JURIDICA})
    public Response crearSolicitud(SgSolDeshabilitarPerJur entity) {
        try {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Actualizado satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgNie.class)))
        ,
        @APIResponse(responseCode = "404", description = "Objeto no encontrado.")
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.PUEDE_DESHABILITAR_PERSONA_JURIDICA})
    public Response actualizar(@PathParam("solId") Long id, SgSolDeshabilitarPerJur sol) {
        try {
            if (servicio.objetoExistePorId(id, false)) {
                sol.setDpjPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(sol)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
     ...
     *
     * @param id Long
     * @return Response Lista de SgSolDeshabilitarPerJur
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgSolDeshabilitarPerJur.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    )
    @Path("/historial/{solId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("solId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialRevisionesPorId(SgSolDeshabilitarPerJur.class, id))).build();
        } catch (Exception ex) {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAcuerdo.class)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgSolDeshabilitarPerJur.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
