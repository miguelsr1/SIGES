...
import sv.gob.mined.siges.negocio.servicios.AsistenciaBean;
...
@RequestScoped
@Path("/v1/asistencias")
@Tag(name = "Asistencia API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ASISTENCIAS,
    ConstantesOperaciones.ACTUALIZAR_ASISTENCIAS,
    ConstantesOperaciones.ELIMINAR_ASISTENCIAS,
    ConstantesOperaciones.BUSCAR_ASISTENCIAS})
public class AsistenciaRecurso {

    private static final Logger LOGGER = Logger.getLogger(AsistenciaRecurso.class.getName());

    @Inject
    private AsistenciaBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAsistencia.class)))
        ,
    ...
    @Path("/{asistenciaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ASISTENCIAS})
    public Response obtenerPorId(@PathParam("asistenciaId") Long asistenciaId) {
        try {
            SgAsistencia entity = servicio.obtenerPorId(asistenciaId);
            if (entity == null) {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAsistencia.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ASISTENCIAS})
    //@Timed
    public Response buscar(FiltroAsistencia filtro) {
        try {
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ASISTENCIAS})
    //@Timed
    public Response buscarTotal(FiltroAsistencia filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ASISTENCIAS})
    public Response crear(SgAsistencia entity) {
        try {
            entity.setAsiPk(null);
            entity.setAsiVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ASISTENCIAS})
    public Response actualizar(@PathParam("asistenciaId") Long id, SgAsistencia entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ASISTENCIAS})
    public Response eliminarPorId(@PathParam("asistenciaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{asistenciaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("asistenciaId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgAsistencia.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgAsistencia.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
