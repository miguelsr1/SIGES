...
import sv.gob.mined.siges.negocio.servicios.MedidasExamenFisicoBean;
 ...
 */
@RequestScoped
@Path("/v1/medidasexamenfisico")
@Tag(name = "Medidas antropométricas y examen físico API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MEDIDAS_EXAMEN_FISICO,
    ConstantesOperaciones.ACTUALIZAR_MEDIDAS_EXAMEN_FISICO,
    ConstantesOperaciones.ELIMINAR_MEDIDAS_EXAMEN_FISICO})
public class MedidasExamenFisicoRecurso {

    private static final Logger LOGGER = Logger.getLogger(MedidasExamenFisicoRecurso.class.getName());

    @Inject
    private MedidasExamenFisicoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgMedidasExamenFisico
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgMedidasExamenFisico.class)))
        ,
    ...
    @Path("/{medidasExamenFisicoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("medidasExamenFisicoId") Long id) {
        try {
            SgMedidasExamenFisico mef = servicio.obtenerPorId(id);
            if (mef == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(mef).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgMedidasExamenFisico
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgMedidasExamenFisico.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroMedidasExamenFisico filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroMedidasExamenFisico filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MEDIDAS_EXAMEN_FISICO})
    public Response crear(SgMedidasExamenFisico mef) {
        try {
            mef.setMefPk(null);
            mef.setMefVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(mef)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MEDIDAS_EXAMEN_FISICO})
    public Response actualizar(@PathParam("medidasExamenFisicoId") Long id, SgMedidasExamenFisico mef) {
        try {
            if (servicio.objetoExistePorId(id)) {
                mef.setMefPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(mef)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MEDIDAS_EXAMEN_FISICO})
    public Response eliminarPorId(@PathParam("medidasExamenFisicoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{medidasExamenFisicoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("medidasExamenFisicoId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgMedidasExamenFisico.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgMedidasExamenFisico.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
