...
import sv.gob.mined.siges.negocio.servicios.ArchivoCalificacionesBean;
 ...
 */
@RequestScoped
@Path("/v1/archivoCalificaciones")
@Tag(name = "Calendarios API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ARCHIVO_CALIFICACIONES,
    ConstantesOperaciones.ACTUALIZAR_ARCHIVO_CALIFICACIONES,
    ConstantesOperaciones.ELIMINAR_ARCHIVO_CALIFICACIONES})
public class ArchivoCalificacionesRecurso {

    private static final Logger LOGGER = Logger.getLogger(ArchivoCalificacionesRecurso.class.getName());

    @Inject
    private ArchivoCalificacionesBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCalendario.class)))
        ,
    ...
    @Path("/{calendarioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response obtenerPorId(@PathParam("calendarioId") Long id) {
        try {
            SgArchivoCalificaciones cal = servicio.obtenerPorId(id);
            if (cal == null) {
     ...
     *
     * @param filtro FiltroCalendario
     * @return Response Lista de SgCalendario
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgCalendario.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CALENDARIOS})
    public Response buscar(FiltroArchivoCalificaciones filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response buscarTotal(FiltroArchivoCalificaciones filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ARCHIVO_CALIFICACIONES})
    public Response crear(SgArchivoCalificaciones cal) {
        try {
            cal.setAccPk(null);
            cal.setAccVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(cal)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
                return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(PersistenceHelper.getCalendarioViolationBusinessException(ex)).build();
            } else {
                tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
                tracer.activeSpan().log(ex.getMessage());
                LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ARCHIVO_CALIFICACIONES})
    public Response actualizar(@PathParam("calendarioId") Long id, SgArchivoCalificaciones cal) {
        try {
            if (servicio.objetoExistePorId(id)) {
                cal.setAccPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(cal)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ARCHIVO_CALIFICACIONES})
    public Response eliminarPorId(@PathParam("calendarioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{calendarioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("calendarioId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgCalendario.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCalendario.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    @GET
    @Path("/calificarArchivos")
    @RolesAllowed({ConstantesOperaciones.EJECUTAR_PROCESAMIENTO_CALIFICACIONES_IMPORTADAS})
    public Response calificarArchivos(@QueryParam("cantidad") Long cantidad) {
        try {

            if (cantidad != null) {

                FiltroCodiguera fc = new FiltroCodiguera();
                fc.setCodigoExacto(Constantes.CODIGO_IMPORTAR_CALIFICACIONES_ASINC);
                List<SgConfiguracion> configList = configuracionBean.obtenerPorFiltro(fc);
                if (!configList.isEmpty()) {

                    if (configList.get(0).getConValor().equals("1")) {
                        FiltroArchivoCalificaciones fac = new FiltroArchivoCalificaciones();
                        fac.setAccEstado(EnumEstadoImportado.NO_PROCESADO);
                        fac.setMaxResults(cantidad);
                        fac.setOrderBy(new String[]{"accPk"});
                        fac.setAscending(new boolean[]{true});
                        List<SgArchivoCalificaciones> arCalList = servicio.obtenerPorFiltro(fac);
                        for (SgArchivoCalificaciones arCal : arCalList) {
                            try {
                                SgCalificacionCE cal = calificacionBean.calificarEstudiantesConArchivo(arCal);

                            } catch (Exception ex) {
                                StringWriter sw = new StringWriter();
                                PrintWriter pw = new PrintWriter(sw);
                                ex.printStackTrace(pw);
                                arCal.setAccErrorLog(sw.toString());
                                arCal.setAccEstado(EnumEstadoImportado.PROCESADO_ERROR);
                                try {
                                    servicio.guardar(arCal);
                                } catch (Exception exc) {
                                    exc.printStackTrace(pw);
                                    servicio.guardarConError(arCal, sw.toString());
                                }
                            }
                        }
                ...
                BusinessException be = new BusinessException();
                be.addError(Errores.ERROR_CONFIGURACION_IMPORTAR_CALIFICACION);
                return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();

            } else {
                BusinessException be = new BusinessException();
                be.addError(Errores.ERROR_TOKEN_INVALIDO);
                return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
            }
     ...
     */
    @GET
    @Path("/borrarArchivos/{diasAntiguedad:[1-9][0-9]*}")
    @PermitAll
    public Response eliminarArchivos(@PathParam("diasAntiguedad") Long diasAntiguedad) {
        try {
            servicio.eliminarArchivosProcesados(diasAntiguedad);
            return Response.status(HttpStatus.SC_OK).build();
