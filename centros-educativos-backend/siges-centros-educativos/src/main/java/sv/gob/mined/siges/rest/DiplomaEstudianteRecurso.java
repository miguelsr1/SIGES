...
import sv.gob.mined.siges.negocio.servicios.DiplomaEstudianteBean;
 ...
 */
@RequestScoped
@Path("/v1/diplomaestudiante")
@Tag(name = "Diploma Estudiante API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_DIPLOMAS_ESTUDIANTE,
    ConstantesOperaciones.ACTUALIZAR_DIPLOMAS_ESTUDIANTE,
    ConstantesOperaciones.ELIMINAR_DIPLOMAS_ESTUDIANTE,
	ConstantesOperaciones.BUSCAR_DIPLOMAS_ESTUDIANTE})
public class DiplomaEstudianteRecurso {

    private static final Logger LOGGER = Logger.getLogger(DiplomaEstudianteRecurso.class.getName());

    @Inject
    private DiplomaEstudianteBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgDiplomaEstudiante
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgDiplomaEstudiante.class)))
        ,
    ...
    @Path("/{diplomaEstudianteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("diplomaEstudianteId") Long id) {
        try {
            SgDiplomaEstudiante die = servicio.obtenerPorId(id);
            if (die == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(die).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgDiplomaEstudiante
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgDiplomaEstudiante.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroDiplomaEstudiante filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroDiplomaEstudiante filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DIPLOMAS_ESTUDIANTE})
    public Response crear(SgDiplomaEstudiante die) {
        try {
            die.setDiePk(null);
            die.setDieVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(die)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_DIPLOMAS_ESTUDIANTE})
    public Response actualizar(@PathParam("diplomaEstudianteId") Long id, SgDiplomaEstudiante die) {
        try {
            if (servicio.objetoExistePorId(id)) {
                die.setDiePk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(die)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_DIPLOMAS_ESTUDIANTE})
    public Response eliminarPorId(@PathParam("diplomaEstudianteId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{diplomaEstudianteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("diplomaEstudianteId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgDiplomaEstudiante.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgDiplomaEstudiante.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
