...
import sv.gob.mined.siges.negocio.servicios.EstudioRealizadoBean;
...
@RequestScoped
@Path("/v1/estudiosrealizados")
@Tag(name = "EstudioRealizado API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ESTUDIOS_REALIZADOS,
    ConstantesOperaciones.ACTUALIZAR_ESTUDIOS_REALIZADOS,
    ConstantesOperaciones.ELIMINAR_ESTUDIOS_REALIZADOS,
    ConstantesOperaciones.BUSCAR_ESTUDIOS_REALIZADOS})
public class EstudioRealizadoRecurso {

    private static final Logger LOGGER = Logger.getLogger(EstudioRealizadoRecurso.class.getName());

    @Inject
    private EstudioRealizadoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgEstudioRealizado
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEstudioRealizado.class)))
        ,
    ...
    @Path("/{estudioRealizadoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ESTUDIOS_REALIZADOS})
    public Response obtenerPorId(@PathParam("estudioRealizadoId") Long id) {
        try {
            SgEstudioRealizado ere = servicio.obtenerPorId(id);
            if (ere == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(ere).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgEstudioRealizado
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgEstudioRealizado.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ESTUDIOS_REALIZADOS})
    public Response buscar(FiltroEstudioRealizado filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ESTUDIOS_REALIZADOS})
    public Response buscarTotal(FiltroEstudioRealizado filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ESTUDIOS_REALIZADOS})
    public Response crear(SgEstudioRealizado ere) {
        try {
            ere.setErePk(null);
            ere.setEreVersion(0);

            if (ere.getEreIdiomas() != null) {
                for (SgIdiomaRealizado idioma : ere.getEreIdiomas()) {
                    idioma.setIreEstudiosRealizados(ere);
                }
            }

            if (ere.getEreEstudiosSuperiores() != null) {
                for (SgEstudioSuperior estudio : ere.getEreEstudiosSuperiores()) {
                    estudio.setEsuEstudios(ere);
                }
            }

            if (ere.getEreCapacitaciones() != null) {
                for (SgCapacitacion cap : ere.getEreCapacitaciones()) {
                    cap.setCapEstudios(ere);
                }
            }

            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ere)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ESTUDIOS_REALIZADOS})
    public Response actualizar(@PathParam("estudioRealizadoId") Long id, SgEstudioRealizado ere) {
        try {
            if (servicio.objetoExistePorId(id)) {
                ...
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ere)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ESTUDIOS_REALIZADOS})
    public Response eliminarPorId(@PathParam("estudioRealizadoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{estudioRealizadoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("estudioRealizadoId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgEstudioRealizado.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgEstudioRealizado.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
