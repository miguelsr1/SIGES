...
import sv.gob.mined.siges.negocio.servicios.AcuerdoSedeBean;
...
@RequestScoped
@Path("/v1/acuerdossedes")
@Tag(name = "Acuerdo Sede API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ACUERDO_SEDE,
    ConstantesOperaciones.ACTUALIZAR_ACUERDO_SEDE,
    ConstantesOperaciones.ELIMINAR_ACUERDO_SEDE,
    ConstantesOperaciones.BUSCAR_PERSONAL_SEDE_EDUCATIVA,
    ConstantesOperaciones.BUSCAR_ACUERDO_SEDE})
public class AcuerdoSedeRecurso {

    private static final Logger LOGGER = Logger.getLogger(AcuerdoSedeRecurso.class.getName());

    @Inject
    private AcuerdoSedeBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = RevHistorico.class)))
        ,
    ...
    @Path("/{acuerdoSedeId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PERSONAL_SEDE_EDUCATIVA})
    //@Timed
    public Response obtenerPorId(@PathParam("acuerdoSedeId") Long acuerdoSedeId, @QueryParam("dataSecurity") Boolean dataSecurity) {
        try {
            if (!jwt.getAudience().contains(Constantes.JWT_AUDIENCE_SIGES)) {
                dataSecurity = Boolean.TRUE;
            }
            SgAcuerdoSede entity = servicio.obtenerPorId(acuerdoSedeId, dataSecurity);
            if (entity == null) {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "201", description = "Entidad creada satisfactoriamente.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAcuerdoSede.class)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ACUERDO_SEDE})
    //@Timed
    public Response crear(SgAcuerdoSede entity) {
        try {
            entity.setAsePk(null);
            entity.setAseVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(entity, Boolean.FALSE)).build();
            ...
            if (PersistenceHelper.isOptimisticException(ex)) {
                ...
                return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(PersistenceHelper.getPersonalViolationBusinessException(ex)).build();
            } else {
                tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
                tracer.activeSpan().log(ex.getMessage());
                LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ACUERDO_SEDE})
    //@Timed
    public Response actualizar(@PathParam("acuerdoSedeId") Long id, SgAcuerdoSede entity, @QueryParam("dataSecurity") Boolean dataSecurity) {
        try {
            entity.setAsePk(id);
            return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(entity, dataSecurity)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isOptimisticException(ex)) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ACUERDO_SEDE,ConstantesOperaciones.ELIMINAR_ACUERDO_SEDE_TRAMITE})
    public Response eliminarPorId(@PathParam("acuerdoSedeId") Long id) {
        try {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAcuerdoSede.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    )
    @Path("/historial/{acuerdoSedeId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("acuerdoSedeId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgAcuerdoSede.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgAcuerdoSede.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ACUERDO_SEDE})
    //@Timed
    public Response buscarTotal(FiltroAcuerdoSede filtro) {
        try {
            if (!jwt.getAudience().contains(Constantes.JWT_AUDIENCE_SIGES)) {
                filtro.setSecurityOperation(ConstantesOperaciones.BUSCAR_ACUERDO_SEDE);
            }
     ...
     *
     * @param filtro FiltroAcuerdoSede
     * @return Response Lista de SgAcuerdoSede
     */
    @POST
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ACUERDO_SEDE})
    //@Timed
    public Response buscar(FiltroAcuerdoSede filtro) {
        try {
            if (!jwt.getAudience().contains(Constantes.JWT_AUDIENCE_SIGES)) {
                filtro.setSecurityOperation(ConstantesOperaciones.BUSCAR_ACUERDO_SEDE);
            }
