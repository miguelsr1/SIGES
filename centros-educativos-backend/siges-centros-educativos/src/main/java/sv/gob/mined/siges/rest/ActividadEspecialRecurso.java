...
import sv.gob.mined.siges.negocio.servicios.ActividadEspecialBean;
...
@RequestScoped
@Path("/v1/actividadesespeciales")
@Tag(name = "ActividadEspecial API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ACTIVIDADES_ESPECIALES,
    ConstantesOperaciones.ACTUALIZAR_ACTIVIDADES_ESPECIALES,
    ConstantesOperaciones.ELIMINAR_ACTIVIDADES_ESPECIALES,
    ConstantesOperaciones.BUSCAR_PLANES_ESTUDIO})
public class ActividadEspecialRecurso {

    private static final Logger LOGGER = Logger.getLogger(ActividadEspecialRecurso.class.getName());

    @Inject
    private ActividadEspecialBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgActividadEspecial.class)))
        ,
    ...
    @Path("/{actividadEspecialId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PLANES_ESTUDIO})
    public Response obtenerPorId(@PathParam("actividadEspecialId") Long actividadEspecialId) {
        try {
            SgActividadEspecial tce = servicio.obtenerPorId(actividadEspecialId);
            if (tce == null) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ACTIVIDADES_ESPECIALES})
    public Response crear(SgActividadEspecial entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ACTIVIDADES_ESPECIALES})
    public Response actualizar(@PathParam("actividadEspecialId") Long id, SgActividadEspecial entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ACTIVIDADES_ESPECIALES})
    public Response eliminarPorId(@PathParam("actividadEspecialId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{actividadEspecialId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("actividadEspecialId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgActividadEspecial.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
     ...
     *
     * @return Response SgActividadEspecial
     */
    @GET
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgActividadEspecial.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
