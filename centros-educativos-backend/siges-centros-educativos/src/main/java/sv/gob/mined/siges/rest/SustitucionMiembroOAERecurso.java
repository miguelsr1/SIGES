...
import sv.gob.mined.siges.negocio.servicios.SustitucionMiembroOAEBean;
 ...
 */
@RequestScoped
@Path("/v1/sustitucionmiembrooae")
@Tag(name = "Sustitucion de miembros OAE API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SUSTITUCION_MIEMBRO_OAE,
    ConstantesOperaciones.ACTUALIZAR_SUSTITUCION_MIEMBRO_OAE,
    ConstantesOperaciones.ELIMINAR_SUSTITUCION_MIEMBRO_OAE,
	ConstantesOperaciones.BUSCAR_SUSTITUCION_MIEMBRO_OAE})
public class SustitucionMiembroOAERecurso {

    private static final Logger LOGGER = Logger.getLogger(SustitucionMiembroOAERecurso.class.getName());

    @Inject
    private SustitucionMiembroOAEBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgSustitucionMiembroOAE
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgSustitucionMiembroOAE.class)))
        ,
    ...
    @Path("/{sustitucionMiembroOAEId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SUSTITUCION_MIEMBRO_OAE})
    public Response obtenerPorId(@PathParam("sustitucionMiembroOAEId") Long id) {
        try {
            SgSustitucionMiembroOAE smo = servicio.obtenerPorId(id);
            if (smo == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(smo).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgSustitucionMiembroOAE
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgSustitucionMiembroOAE.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SUSTITUCION_MIEMBRO_OAE})
    public Response buscar(FiltroSustitucionMiembroOAE filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SUSTITUCION_MIEMBRO_OAE})
    public Response buscarTotal(FiltroSustitucionMiembroOAE filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SUSTITUCION_MIEMBRO_OAE})
    public Response crear(SgSustitucionMiembroOAE smo) {
        try {
            smo.setSmoPk(null);
            smo.setSmoVersion(0);
            if(smo.getSmoRelSustitucionMiembroOAE()!=null){
                for(SgRelSustitucionMiembroOAE rel:smo.getSmoRelSustitucionMiembroOAE()){
                    rel.setRsmSustitucionMiembroFk(smo);
                }
            }
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(smo)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SUSTITUCION_MIEMBRO_OAE})
    public Response actualizar(@PathParam("sustitucionMiembroOAEId") Long id, SgSustitucionMiembroOAE smo) {
        try {
            if (servicio.objetoExistePorId(id)) {
                smo.setSmoPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(smo)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SUSTITUCION_MIEMBRO_OAE})
    public Response aprobar(SgSustitucionMiembroOAE smo) {
        try {
            if (servicio.objetoExistePorId(smo.getSmoPk())) {
                return Response.status(HttpStatus.SC_OK).entity(servicio.aprobar(smo)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SUSTITUCION_MIEMBRO_OAE})
    public Response rechazar(SgSustitucionMiembroOAE smo) {
        try {
            if (servicio.objetoExistePorId(smo.getSmoPk())) {
                return Response.status(HttpStatus.SC_OK).entity(servicio.rechazar(smo)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_SUSTITUCION_MIEMBRO_OAE})
    public Response eliminarPorId(@PathParam("sustitucionMiembroOAEId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{sustitucionMiembroOAEId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("sustitucionMiembroOAEId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgSustitucionMiembroOAE.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgSustitucionMiembroOAE.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
