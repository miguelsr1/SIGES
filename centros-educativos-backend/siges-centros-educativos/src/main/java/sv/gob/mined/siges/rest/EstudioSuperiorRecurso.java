...
import sv.gob.mined.siges.negocio.servicios.EstudioSuperiorBean;
...
@RequestScoped
@Path("/v1/estudiossuperiores")
@Tag(name = "EstudioSuperior API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ESTUDIOS_SUPERIORES,
    ConstantesOperaciones.ACTUALIZAR_ESTUDIOS_SUPERIORES,
    ConstantesOperaciones.ELIMINAR_ESTUDIOS_SUPERIORES,
    ConstantesOperaciones.BUSCAR_ESTUDIOS_SUPERIORES})
public class EstudioSuperiorRecurso {

    private static final Logger LOGGER = Logger.getLogger(EstudioSuperiorRecurso.class.getName());

    @Inject
    private EstudioSuperiorBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgEstudioSuperior
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgEstudioSuperior.class)))
        ,
    ...
    @Path("/{estudioSuperiorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ESTUDIOS_SUPERIORES})
    public Response obtenerPorId(@PathParam("estudioSuperiorId") Long id) {
        try {
            SgEstudioSuperior entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgEstudioSuperior
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgEstudioSuperior.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ESTUDIOS_SUPERIORES})
    public Response crear(SgEstudioSuperior entity) {
        try {
            entity.setEsuPk(null);
            entity.setEsuVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ESTUDIOS_SUPERIORES})
    public Response actualizar(@PathParam("estudioSuperiorId") Long id, SgEstudioSuperior entity) {
        try {
    ...
    /**
     * Valida estudio como realizado.
     *
     * @param id Long
     * @return Response SgEstudioSuperior
     */
    @POST
    @Operation(summary = "Valida estudio como realizado.")
    @APIResponses(value = {
    ...
    @Path("/validarrealizado/{estudioSuperiorId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.VALIDAR_INFORMACION_FICHA_DOCENTE})
    public Response validarRealizado(@PathParam("estudioSuperiorId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    /**
     * Invalida estudio como realizado.
     *
     * @param id Long
     * @return Response SgEstudioSuperior
     */
    @POST
    @Operation(summary = "Invalida estudio como realizado.")
    @APIResponses(value = {
    ...
    @Path("/invalidarrealizado/{estudioSuperiorId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.VALIDAR_INFORMACION_FICHA_DOCENTE})
    public Response invalidarRealizado(@PathParam("estudioSuperiorId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ESTUDIOS_SUPERIORES})
    public Response eliminarPorId(@PathParam("estudioSuperiorId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{estudioSuperiorId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("estudioSuperiorId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgEstudioSuperior.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgEstudioSuperior.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response validar(SgEstudioSuperior entity) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(EstudioSuperiorValidacionNegocio.validar(entity)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
