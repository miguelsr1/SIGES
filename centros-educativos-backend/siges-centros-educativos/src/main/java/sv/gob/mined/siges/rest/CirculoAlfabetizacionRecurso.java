...
import sv.gob.mined.siges.negocio.servicios.CirculoAlfabetizacionBean;
...
@RequestScoped
@Path("/v1/circulosalfabetizacion")
@Tag(name = "Círculo de Alfabetización API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SEDE_CIRCULO_ALFABETIZACION,
    ConstantesOperaciones.ACTUALIZAR_SEDE_CIRCULO_ALFABETIZACION,
    ConstantesOperaciones.ELIMINAR_SEDE_CIRCULO_ALFABETIZACION,
    ConstantesOperaciones.BUSCAR_SEDES})
public class CirculoAlfabetizacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(CirculoAlfabetizacionRecurso.class.getName());

    @Inject
    private CirculoAlfabetizacionBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCirculoAlfabetizacion.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("entityId") Long entityId) {
        try {
            SgCirculoAlfabetizacion cedPri = servicio.obtenerPorId(entityId);
            if (cedPri == null) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SEDE_CIRCULO_ALFABETIZACION})
    public Response crear(SgCirculoAlfabetizacion entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SEDE_CIRCULO_ALFABETIZACION})
    public Response actualizar(@PathParam("entityId") Long id, SgCirculoAlfabetizacion entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_SEDE_CIRCULO_ALFABETIZACION})
    ...
    public Response obtenerHistorialPorId(@PathParam("entityId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgCirculoAlfabetizacion.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
     ...
     *
     * @return Response SgCirculoAlfabetizacion
     */
    @GET
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCirculoAlfabetizacion.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
