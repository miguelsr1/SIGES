...
import sv.gob.mined.siges.negocio.servicios.ModuloDiplomaBean;
...
@RequestScoped
@Path("/v1/modulosdiploma")
@Tag(name = "ModuloDiploma API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_MODULOS_DIPLOMA,
    ConstantesOperaciones.ACTUALIZAR_MODULOS_DIPLOMA,
    ConstantesOperaciones.ELIMINAR_MODULOS_DIPLOMA,
    ConstantesOperaciones.BUSCAR_MODULOS_DIPLOMA})
public class ModuloDiplomaRecurso {

    private static final Logger LOGGER = Logger.getLogger(ModuloDiplomaRecurso.class.getName());

    @Inject
    private ModuloDiplomaBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgModuloDiplomado
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgModuloDiplomado.class)))
        ,
    ...
    @Path("/{moduloDiplomaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MODULOS_DIPLOMA})
    public Response obtenerPorId(@PathParam("moduloDiplomaId") Long id) {
        try {
            SgModuloDiplomado entity = servicio.obtenerPorId(id);
            if (entity == null) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_MODULOS_DIPLOMA})
    public Response crear(SgModuloDiplomado entity) {
        try {
            entity.setMdiPk(null);
            entity.setMdiVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_MODULOS_DIPLOMA})
    public Response actualizar(@PathParam("moduloDiplomaId") Long id, SgModuloDiplomado entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_MODULOS_DIPLOMA})
    public Response eliminarPorId(@PathParam("moduloDiplomaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
     ...
     *
     * @param id Long
     * @return Response Lista de SgModuloDiplomado
     */
    @GET
    ...
    @Path("/historial/{moduloDiplomaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("moduloDiplomaId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgModuloDiplomado.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgModuloDiplomado.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MODULOS_DIPLOMA})
    public Response buscar(FiltroModuloDiplomado filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_MODULOS_DIPLOMA})
    public Response buscarTotal(FiltroModuloDiplomado filtro) {
        try {
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgModuloDiplomado.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
