...
import sv.gob.mined.siges.negocio.servicios.ServicioEducativoBean;
...
@RequestScoped
@Path("/v1/servicioseducativos")
@Tag(name = "ServicioEducativo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SERVICIO_EDUCATIVO,
    ConstantesOperaciones.ACTUALIZAR_SERVICIO_EDUCATIVO,
    ConstantesOperaciones.ELIMINAR_SERVICIO_EDUCATIVO,
    ConstantesOperaciones.BUSCAR_SERVICIO_EDUCATIVO,
    ConstantesOperaciones.COPIAR_SERVICIOS_SEDE_PADRE})
public class ServicioEducativoRecurso {

    private static final Logger LOGGER = Logger.getLogger(ServicioEducativoRecurso.class.getName());

    @Inject
    private ServicioEducativoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgServicioEducativo
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgServicioEducativo.class)))
        ,
    ...
    @Path("/{servicioEducativoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SERVICIO_EDUCATIVO})
    //@Timed
    public Response obtenerPorId(@PathParam("servicioEducativoId") Long id) {
        try {
            SgServicioEducativo entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgServicioEducativo
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgServicioEducativo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SERVICIO_EDUCATIVO})
    //@Timed
    public Response buscar(FiltroServicioEducativo filtro) {
        try {
            if (!jwt.getAudience().contains(Constantes.JWT_AUDIENCE_SIGES)) {
                filtro.setSecurityOperation(ConstantesOperaciones.BUSCAR_SERVICIO_EDUCATIVO);
            }
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SERVICIO_EDUCATIVO})
    //@Timed
    public Response buscarTotal(FiltroServicioEducativo filtro) {
        try {
            if (!jwt.getAudience().contains(Constantes.JWT_AUDIENCE_SIGES)) {
                filtro.setSecurityOperation(ConstantesOperaciones.BUSCAR_SERVICIO_EDUCATIVO);
            }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SERVICIO_EDUCATIVO})
    public Response crear(SgServicioEducativo entity) {
        try {
            entity.setSduPk(null);
            entity.setSduVersion(0);
            ...
            if (PersistenceHelper.isConstraintViolation(ex)) {
                return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(PersistenceHelper.getServicioEducativoBusinessException(ex)).build();
            } else {
                tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
                tracer.activeSpan().log(ex.getMessage());
                LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SERVICIO_EDUCATIVO})
    public Response actualizar(@PathParam("servicioEducativoId") Long id, SgServicioEducativo entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_SERVICIO_EDUCATIVO})
    public Response eliminarPorId(@PathParam("servicioEducativoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{servicioEducativoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("servicioEducativoId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgServicioEducativo.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgServicioEducativo.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    /**
     * Copia los servicios educativos de la id indicada a la entidad enviada
     *
     *
     * @param sedeHija
     * @param sedePadre
     ...
     */
    @GET
    @Operation(summary = "Copia los servicios de la id indicada.")
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgServicioEducativo.class)))
    ...
    @Path("/copiarservicios/{sedeHija:[1-9][0-9]*}/{sedePadre:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.COPIAR_SERVICIOS_SEDE_PADRE})
    public Response copiarServicios(@PathParam("sedeHija") Long sedeHija, @PathParam("sedePadre") Long sedePadre) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.copiarServicios(sedeHija, sedePadre)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
    ...
    @Path("/copiarserviciosselectos")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SERVICIO_EDUCATIVO})
    public Response copiarServiciosSelectos(List<SgServicioEducativo> entity) {
        try {
            for (SgServicioEducativo serv : entity) {
                serv.setSduPk(null);
                serv.setSduVersion(0);
            }
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.copiarServiciosSelectos(entity)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
    ...
    @Path("/buscarserviciospadre/{sedeHija:[1-9][0-9]*}/{sedePadre:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SERVICIO_EDUCATIVO})
    public Response buscarServicioPadre(@PathParam("sedeHija") Long sedeHija, @PathParam("sedePadre") Long sedePadre) {
        try {

            return Response.status(HttpStatus.SC_OK).entity(servicio.obtenerServiciosSedePadre(sedeHija, sedePadre)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
    ...
    /**
     * Crea el conjunto de servicios ingresados de forma masiva
     *
     * @param dto SgRegistroServiciosEducativos
     */
    @POST
    @Operation(summary = "Crea el conjunto de servicios ingresados de forma masiva.")
    @APIResponses(value = {
    ...
    @Path("/habilitaciones")
    @RolesAllowed({ConstantesOperaciones.CREAR_SERVICIO_EDUCATIVO})
    public Response habilitarServiciosEducativos(SgRegistroServiciosEducativos dto) {
        try {
            servicio.habilitarServiciosEducativos(dto, Boolean.TRUE);
            return Response.status(HttpStatus.SC_OK).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
    ...
    /**
     * Deshabilita el conjunto de servicios ingresados de forma masiva
     *
     * @param dto SgRegistroServiciosEducativos
     */
    @POST
    @Operation(summary = "Deshabilita el conjunto de servicios ingresados de forma masiva.")
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.")
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Path("/deshabilitaciones")
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SERVICIO_EDUCATIVO})
    public Response deshabilitarServiciosEducativos(SgRegistroServiciosEducativos dto) {
        try {
            servicio.deshabilitarServiciosEducativos(dto, Boolean.TRUE);
            return Response.status(HttpStatus.SC_OK).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
