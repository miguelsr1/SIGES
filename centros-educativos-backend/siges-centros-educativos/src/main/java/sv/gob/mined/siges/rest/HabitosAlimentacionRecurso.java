...
import sv.gob.mined.siges.negocio.servicios.HabitosAlimentacionBean;
 ...
 */
@RequestScoped
@Path("/v1/habitosalimentacion")
@Tag(name = "Habitos de alimentación API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_HABITOS_ALIMENTACION,
    ConstantesOperaciones.ACTUALIZAR_HABITOS_ALIMENTACION,
    ConstantesOperaciones.ELIMINAR_HABITOS_ALIMENTACION})
public class HabitosAlimentacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(HabitosAlimentacionRecurso.class.getName());

    @Inject
    private HabitosAlimentacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgHabitosAlimentacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgHabitosAlimentacion.class)))
        ,
    ...
    @Path("/{habitosAlimentacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("habitosAlimentacionId") Long id) {
        try {
            SgHabitosAlimentacion hal = servicio.obtenerPorId(id);
            if (hal == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(hal).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgHabitosAlimentacion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgHabitosAlimentacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroHabitosAlimentacion filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroHabitosAlimentacion filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_HABITOS_ALIMENTACION})
    public Response crear(SgHabitosAlimentacion hal) {
        try {
            hal.setHalPk(null);
            hal.setHalVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(hal)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_HABITOS_ALIMENTACION})
    public Response actualizar(@PathParam("habitosAlimentacionId") Long id, SgHabitosAlimentacion hal) {
        try {
            if (servicio.objetoExistePorId(id)) {
                hal.setHalPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(hal)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_HABITOS_ALIMENTACION})
    public Response eliminarPorId(@PathParam("habitosAlimentacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{habitosAlimentacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("habitosAlimentacionId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgHabitosAlimentacion.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgHabitosAlimentacion.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
