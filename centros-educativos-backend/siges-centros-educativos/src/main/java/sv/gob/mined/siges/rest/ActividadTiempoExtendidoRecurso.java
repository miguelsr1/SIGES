...
import sv.gob.mined.siges.negocio.servicios.ActividadTiempoExtendidoBean;
...
@RequestScoped
@Path("/v1/actividadestiempoextendido")
@Tag(name = "ActividadTiempoExtendido API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ACTIVIDADES_TIEMPO_EXTENDIDO,
    ConstantesOperaciones.ACTUALIZAR_ACTIVIDADES_TIEMPO_EXTENDIDO,
    ConstantesOperaciones.ELIMINAR_ACTIVIDADES_TIEMPO_EXTENDIDO,
    ConstantesOperaciones.BUSCAR_PLANES_ESTUDIO})
public class ActividadTiempoExtendidoRecurso {

    private static final Logger LOGGER = Logger.getLogger(ActividadEspecialRecurso.class.getName());

    @Inject
    private ActividadTiempoExtendidoBean servicio;
     ...
     *
     * @param actividadTiempoExtendidoId
     * @return Response -> SgComponentePlanEstudio
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgActividadTiempoExtendido.class)))
        ,
    ...
    @Path("/{actividadTiempoExtendidoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PLANES_ESTUDIO})
    public Response obtenerPorId(@PathParam("actividadTiempoExtendidoId") Long actividadTiempoExtendidoId) {
        try {
            SgActividadTiempoExtendido tce = servicio.obtenerPorId(actividadTiempoExtendidoId);
            if (tce == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(tce).build();
            }
        } catch (Exception ex) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ACTIVIDADES_TIEMPO_EXTENDIDO})
    public Response crear(SgActividadTiempoExtendido entity) {
        try {
            entity.setCpePk(null);
            entity.setCpeVersion(0);
            servicio.guardar(entity);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.obtenerPorId(entity.getCpePk())).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ACTIVIDADES_TIEMPO_EXTENDIDO})
    public Response actualizar(@PathParam("actividadTiempoExtendidoId") Long id, SgActividadTiempoExtendido entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ACTIVIDADES_TIEMPO_EXTENDIDO})
    public Response eliminarPorId(@PathParam("actividadTiempoExtendidoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{actividadTiempoExtendidoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("actividadTiempoExtendidoId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgActividadTiempoExtendido.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
     ...
     *
     * @return Response SgActividadTiempoExtendido
     */
    @GET
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgActividadTiempoExtendido.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
