...
import sv.gob.mined.siges.negocio.servicios.SolicitudCursoDocenteBean;
...
@RequestScoped
@Path("/v1/solicitudescursodocente")
@Tag(name = "SolicitudCursoDocente API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SOLICITUD_CURSO_DOCENTE,
    ConstantesOperaciones.ACTUALIZAR_SOLICITUD_CURSO_DOCENTE,
    ConstantesOperaciones.ELIMINAR_SOLICITUD_CURSO_DOCENTE,
    ConstantesOperaciones.BUSCAR_SOLICITUD_CURSO_DOCENTE,
    ConstantesOperaciones.APROBAR_SOLICITUD_CURSO_DOCENTE,
    ConstantesOperaciones.RECHAZAR_SOLICITUD_CURSO_DOCENTE})
public class SolicitudCursoDocenteRecurso {

    private static final Logger LOGGER = Logger.getLogger(SolicitudCursoDocenteRecurso.class.getName());

    @Inject
    private SolicitudCursoDocenteBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgSolicitudCursoDocente
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgSolicitudCursoDocente.class)))
        ,
    ...
    @Path("/{solicitudCursoDocenteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_CURSO_DOCENTE})
    public Response obtenerPorId(@PathParam("solicitudCursoDocenteId") Long id) {
        try {
            SgSolicitudCursoDocente entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgSolicitudCursoDocente
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgSolicitudCursoDocente.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_CURSO_DOCENTE})
    public Response buscar(FiltroSolicitudCursoDocente filtro) {
        try {
            if (!jwt.getAudience().contains(Constantes.JWT_AUDIENCE_SIGES)) {
                filtro.setSecurityOperation(ConstantesOperaciones.BUSCAR_SOLICITUD_CURSO_DOCENTE);
            }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_CURSO_DOCENTE})
    public Response buscarTotal(FiltroSolicitudCursoDocente filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SOLICITUD_CURSO_DOCENTE})
    public Response crear(SgSolicitudCursoDocente entity) {
        try {
            entity.setScdPk(null);
            entity.setScdVersion(0);
            LOGGER.info("crear");
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SOLICITUD_CURSO_DOCENTE})
    public Response actualizar(@PathParam("solicitudCursoDocenteId") Long id, SgSolicitudCursoDocente entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_SOLICITUD_CURSO_DOCENTE})
    public Response eliminarPorId(@PathParam("solicitudCursoDocenteId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{solicitudCursoDocenteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("solicitudCursoDocenteId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgSolicitudCursoDocente.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgSolicitudCursoDocente.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    /**
     * Aprueba la solicitud con la id indicada.
     *
     *
     * @param id Long
     * @param entity SgSolicitudCursoDocente
    ...
    @Path("/aprobar/{solicitudCursoDocenteId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.APROBAR_SOLICITUD_CURSO_DOCENTE})
    public Response aprobarSolicitud(@PathParam("solicitudCursoDocenteId") Long id, SgSolicitudCursoDocente entity) {
        try {
            if (servicio.objetoExistePorId(id)) {
                entity.setScdPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.aprobar(entity)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    /**
     * Rechaza la solicitud con la id indicada.
     *
     *
     * @param id Long
     * @param entity SgSolicitudCursoDocente
    ...
    @Path("/rechazar/{solicitudCursoDocenteId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.APROBAR_SOLICITUD_CURSO_DOCENTE})
    public Response rechazarSolicitud(@PathParam("solicitudCursoDocenteId") Long id, SgSolicitudCursoDocente entity) {
        try {
            if (servicio.objetoExistePorId(id)) {
                entity.setScdPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.rechazar(entity)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
