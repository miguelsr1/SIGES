...
import sv.gob.mined.siges.negocio.servicios.RelSustitucionMiembroOAEBean;
 ...
 */
@RequestScoped
@Path("/v1/relsustitucionmiembrooae")
@Tag(name = "Relación sustitución miembro OAE API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_SUSTITUCION_MIEMBRO_OAE,
    ConstantesOperaciones.ACTUALIZAR_REL_SUSTITUCION_MIEMBRO_OAE,
    ConstantesOperaciones.ELIMINAR_REL_SUSTITUCION_MIEMBRO_OAE,
	ConstantesOperaciones.BUSCAR_REL_SUSTITUCION_MIEMBRO_OAE})
public class RelSustitucionMiembroOAERecurso {

    private static final Logger LOGGER = Logger.getLogger(RelSustitucionMiembroOAERecurso.class.getName());

    @Inject
    private RelSustitucionMiembroOAEBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelSustitucionMiembroOAE
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelSustitucionMiembroOAE.class)))
        ,
    ...
    @Path("/{relSustitucionMiembroOAEId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("relSustitucionMiembroOAEId") Long id) {
        try {
            SgRelSustitucionMiembroOAE rsm = servicio.obtenerPorId(id);
            if (rsm == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(rsm).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRelSustitucionMiembroOAE
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelSustitucionMiembroOAE.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroRelSustitucionMiembroOAE filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroRelSustitucionMiembroOAE filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_SUSTITUCION_MIEMBRO_OAE})
    public Response crear(SgRelSustitucionMiembroOAE rsm) {
        try {
            rsm.setRsmPk(null);
            rsm.setRsmVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(rsm)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_SUSTITUCION_MIEMBRO_OAE})
    public Response actualizar(@PathParam("relSustitucionMiembroOAEId") Long id, SgRelSustitucionMiembroOAE rsm) {
        try {
            if (servicio.objetoExistePorId(id)) {
                rsm.setRsmPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(rsm)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_SUSTITUCION_MIEMBRO_OAE})
    public Response eliminarPorId(@PathParam("relSustitucionMiembroOAEId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relSustitucionMiembroOAEId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relSustitucionMiembroOAEId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgRelSustitucionMiembroOAE.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelSustitucionMiembroOAE.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
