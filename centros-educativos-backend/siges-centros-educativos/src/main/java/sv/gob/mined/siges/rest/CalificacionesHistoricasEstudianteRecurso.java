...
import sv.gob.mined.siges.negocio.servicios.CalificacionesHistoricasEstudianteBean;
 ...
 */
@RequestScoped
@Path("/v1/calificacioneshistoricasestudiante")
@Tag(name = "Calificaciones historicas estudiante API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CALIFICACIONES_HISTORICAS_ESTUDIANTE,
    ConstantesOperaciones.ACTUALIZAR_CALIFICACIONES_HISTORICAS_ESTUDIANTE,
    ConstantesOperaciones.ELIMINAR_CALIFICACIONES_HISTORICAS_ESTUDIANTE,
	ConstantesOperaciones.BUSCAR_CALIFICACIONES_HISTORICAS_ESTUDIANTE})
public class CalificacionesHistoricasEstudianteRecurso {

    private static final Logger LOGGER = Logger.getLogger(CalificacionesHistoricasEstudianteRecurso.class.getName());

    @Inject
    private CalificacionesHistoricasEstudianteBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgCalificacionesHistoricasEstudiante
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCalificacionesHistoricasEstudiante.class)))
        ,
    ...
    @Path("/{calificacionesHistoricasEstudianteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CALIFICACIONES_HISTORICAS_ESTUDIANTE})
    public Response obtenerPorId(@PathParam("calificacionesHistoricasEstudianteId") Long id) {
        try {
            SgCalificacionesHistoricasEstudiante che = servicio.obtenerPorId(id);
            if (che == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(che).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgCalificacionesHistoricasEstudiante
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgCalificacionesHistoricasEstudiante.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CALIFICACIONES_HISTORICAS_ESTUDIANTE})
    public Response buscar(FiltroCalificacionesHistoricasEstudiante filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CALIFICACIONES_HISTORICAS_ESTUDIANTE})
    public Response buscarTotal(FiltroCalificacionesHistoricasEstudiante filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CALIFICACIONES_HISTORICAS_ESTUDIANTE})
    public Response crear(SgCalificacionesHistoricasEstudiante che) {
        try {
            che.setChePk(null);
            che.setCheVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(che)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CALIFICACIONES_HISTORICAS_ESTUDIANTE})
    public Response actualizar(@PathParam("calificacionesHistoricasEstudianteId") Long id, SgCalificacionesHistoricasEstudiante che) {
        try {
            if (servicio.objetoExistePorId(id)) {
                che.setChePk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(che)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CALIFICACIONES_HISTORICAS_ESTUDIANTE})
    //@Timed
    public Response crearCalificacionEstudiante(List<SgCalificacionesHistoricasEstudiante> cae) {
        try {

            servicio.guardarCalificaciones(cae);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CALIFICACIONES_HISTORICAS_ESTUDIANTE})
    public Response eliminarPorId(@PathParam("calificacionesHistoricasEstudianteId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{calificacionesHistoricasEstudianteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("calificacionesHistoricasEstudianteId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgCalificacionesHistoricasEstudiante.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCalificacionesHistoricasEstudiante.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CALIFICACIONES_HISTORICAS_ESTUDIANTE})
    public Response editarEstudianteCalificacion(SgCalificacionesHistoricasEstudiante che) {
        try {
            if(che.getChePk()!=null){
                servicio.editarEstudianteCalificacion(che);
                return Response.status(HttpStatus.SC_OK).build();
            }
            return Response.status(HttpStatus.SC_OK).build();
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Path("/calificarConArchivo")
    @RolesAllowed({ConstantesOperaciones.IMPORTAR_CALIFICACION_HISTORICA})
    public Response calificarConArchivo(SgArchivo che) {
        try {
            if(che!=null){
                servicio.calificarConArchivo(che);
                return Response.status(HttpStatus.SC_OK).build();
            }else{
                return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
            }
