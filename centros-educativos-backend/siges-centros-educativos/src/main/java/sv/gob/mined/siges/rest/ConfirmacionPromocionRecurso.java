...
import sv.gob.mined.siges.negocio.servicios.ConfirmacionPromocionBean;
 ...
 */
@RequestScoped
@Path("/v1/confirmacionespromociones")
@Tag(name = "Confirmaciones de Promociones API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CONFIRMAR_PROMOCION,
    ConstantesOperaciones.BUSCAR_CONFIRMACION_PROMOCION,
    ConstantesOperaciones.ELIMINAR_CONFIRMACION_PROMOCION})
public class ConfirmacionPromocionRecurso {

    private static final Logger LOGGER = Logger.getLogger(ConfirmacionPromocionRecurso.class.getName());

    @Inject
    private ConfirmacionPromocionBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgConfirmacionPromocion.class)))
        ,
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CONFIRMACION_PROMOCION})
    public Response obtenerPorId(@PathParam("confirmacionPromocionId") Long id) {
        try {
            SgConfirmacionPromocion cma = servicio.obtenerPorId(id);
            if (cma == null) {
     ...
     * @return Response Lista de SgConfirmacionPromocion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgConfirmacionPromocion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CONFIRMACION_PROMOCION})
    public Response buscar(FiltroConfirmacionPromocion filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CONFIRMACION_PROMOCION})
    public Response buscarTotal(FiltroConfirmacionPromocion filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CONFIRMAR_PROMOCION})
    public Response preconfirmar(SgConfirmacionPromocion cma) {
        try {
    ...
    public Response obtenerHistorialPorId(@PathParam("confirmacionPromocionId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgConfirmacionPromocion.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgConfirmacionPromocion.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CONFIRMACION_PROMOCION})
