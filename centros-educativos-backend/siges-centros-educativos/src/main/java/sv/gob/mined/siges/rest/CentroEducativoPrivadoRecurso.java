...
import sv.gob.mined.siges.negocio.servicios.CentroEducativoPrivadoBean;
...
import sv.gob.mined.siges.persistencia.entidades.SgRelSedeServicioInfra;
...
@RequestScoped
@Path("/v1/centroseducativoprivado")
@Tag(name = "Centro Educativo Privado API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CENTRO_EDUCATIVO_PRIVADO,
    ConstantesOperaciones.ACTUALIZAR_CENTRO_EDUCATIVO_PRIVADO,
    ConstantesOperaciones.ELIMINAR_CENTRO_EDUCATIVO_PRIVADO,
    ConstantesOperaciones.BUSCAR_SEDES})
public class CentroEducativoPrivadoRecurso {

    private static final Logger LOGGER = Logger.getLogger(CentroEducativoPrivadoRecurso.class.getName());

    @Inject
    private CentroEducativoPrivadoBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCentroEducativoPrivado.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("entityId") Long entityId) {
        try {
            SgCentroEducativoPrivado cedPri = servicio.obtenerPorId(entityId);
            if (cedPri == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(cedPri).build();
            }
        } catch (Exception ex) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CENTRO_EDUCATIVO_PRIVADO})
    public Response crear(SgCentroEducativoPrivado entity) {
        try {
            entity.setSedPk(null);
            entity.setSedVersion(0);
            if (entity.getSedRelServicioInfra() != null) {
                for (SgRelSedeServicioInfra rel : entity.getSedRelServicioInfra()) {
                    rel.setRssSede(entity);
                }
            }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CENTRO_EDUCATIVO_PRIVADO})
    public Response actualizar(@PathParam("entityId") Long id, SgCentroEducativoPrivado entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CENTRO_EDUCATIVO_PRIVADO})
     ...
     *
     * @param id Long
     * @return Response Lista de SgSedes
     */
    @GET
    ...
    public Response obtenerHistorialPorId(@PathParam("entityId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgCentroEducativoPrivado.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
     ...
     *
     * @return Response SgCentroEducativoPrivado
     */
    @GET
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCentroEducativoPrivado.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
