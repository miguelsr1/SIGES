...
import sv.gob.mined.siges.negocio.servicios.SolicitudPlazaBean;
...
@RequestScoped
@Path("/v1/solicitudesplazas")
@Tag(name = "SolicitudPlaza API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SOLICITUD_PLAZA,
    ConstantesOperaciones.ACTUALIZAR_SOLICITUD_PLAZA,
    ConstantesOperaciones.ELIMINAR_SOLICITUD_PLAZA,
    ConstantesOperaciones.BUSCAR_SOLICITUD_PLAZA})
public class SolicitudPlazaRecurso {

    private static final Logger LOGGER = Logger.getLogger(SolicitudPlazaRecurso.class.getName());

    @Inject
    private SolicitudPlazaBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgSolicitudPlaza.class)))
        ,
    ...
    @Path("/{solicitudPlazaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_PLAZA})
    public Response obtenerPorId(@PathParam("solicitudPlazaId") Long id) {
        try {
            SgSolicitudPlaza entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgPlaza
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgSolicitudPlaza.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_PLAZA})
    public Response buscar(FiltroSolicitudPlaza filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_PLAZA})
    public Response buscarTotal(FiltroSolicitudPlaza filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SOLICITUD_PLAZA})
    public Response crear(SgSolicitudPlaza entity) {
        try {
            entity.setSplPk(null);
            entity.setSplVersion(0);
            LOGGER.info("crear");
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SOLICITUD_PLAZA})
    public Response actualizar(@PathParam("solicitudPlazaId") Long id, SgSolicitudPlaza entity) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SOLICITUD_PLAZA})
    public Response publicarSolicitud(SgSolicitudPlaza entity) {
        try {          
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.publicarSolicitud(entity)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_SOLICITUD_PLAZA})
    public Response eliminarPorId(@PathParam("solicitudPlazaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{solicitudPlazaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("solicitudPlazaId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgSolicitudPlaza.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgSolicitudPlaza.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
