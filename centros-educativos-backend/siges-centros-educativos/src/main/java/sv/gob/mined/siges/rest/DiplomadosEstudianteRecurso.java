...
import sv.gob.mined.siges.negocio.servicios.DiplomadosEstudianteBean;
...
@RequestScoped
@Path("/v1/diplomadoestudiante")
@Tag(name = "Diploma API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_INSCRIPCION_DIPLOMADO,
    ConstantesOperaciones.ACTUALIZAR_INSCRIPCION_DIPLOMADO,
    ConstantesOperaciones.ELIMINAR_INSCRIPCION_DIPLOMADO,
    ConstantesOperaciones.BUSCAR_INSCRIPCION_DIPLOMADO})
public class DiplomadosEstudianteRecurso {

    private static final Logger LOGGER = Logger.getLogger(DiplomadosEstudianteRecurso.class.getName());

    @Inject
    private DiplomadosEstudianteBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgDiplomadosEstudiante
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgDiplomadosEstudiante.class)))
        ,
    ...
    @Path("/{dipEstudianteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_INSCRIPCION_DIPLOMADO})
    public Response obtenerPorId(@PathParam("dipEstudianteId") Long id) {
        try {
            SgDiplomadosEstudiante entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgDiplomadosEstudiante
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgDiplomadosEstudiante.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_INSCRIPCION_DIPLOMADO})
    public Response buscar(FiltroDiplomadosEstudiante filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_INSCRIPCION_DIPLOMADO})
    public Response buscarTotal(FiltroDiplomadosEstudiante filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_INSCRIPCION_DIPLOMADO})
    public Response crear(SgDiplomadosEstudiante entity) {
        try {
            entity.setDepPk(null);
            entity.setDepVersion(0);
            ...
            if (PersistenceHelper.isConstraintViolation(ex)) {
                return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(PersistenceHelper.getDiplomadoEstudianteViolationBusinessException(ex)).build();
            } else {
                tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
                tracer.activeSpan().log(ex.getMessage());
                LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_INSCRIPCION_DIPLOMADO})
    public Response actualizar(@PathParam("dipEstudianteId") Long id, SgDiplomadosEstudiante entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_INSCRIPCION_DIPLOMADO})
    public Response eliminarPorId(@PathParam("dipEstudianteId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{dipEstudianteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("dipEstudianteId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgDiplomadosEstudiante.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgDiplomadosEstudiante.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
