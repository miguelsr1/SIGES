...
import sv.gob.mined.siges.negocio.servicios.RelModEdModAtenBean;
...
@RequestScoped
@Path("/v1/relmodedmodaten")
@Tag(name = "RelacionModeloEducacionAtencion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_RELACION_MODALIDAD_EDUCATIVA_ATENCION,
    ConstantesOperaciones.ACTUALIZAR_RELACION_MODALIDAD_EDUCATIVA_ATENCION,
    ConstantesOperaciones.ELIMINAR_RELACION_MODALIDAD_EDUCATIVA_ATENCION,
    ConstantesOperaciones.BUSCAR_RELACION_MODALIDAD_EDUCATIVA_ATENCION})
public class RelModEdModAtenRescurso {

    private static final Logger LOGGER = Logger.getLogger(RelModEdModAtenRescurso.class.getName());

    @Inject
    private RelModEdModAtenBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelModEdModAten
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelModEdModAten.class)))
        ,
    ...
    @Path("/{relModEdModAtenId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_RELACION_MODALIDAD_EDUCATIVA_ATENCION})
    public Response obtenerPorId(@PathParam("relModEdModAtenId") Long id) {
        try {
            SgRelModEdModAten entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     *
     * @param filtro
     * @return Response Lista de SgRelModEdModAten
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelModEdModAten.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_RELACION_MODALIDAD_EDUCATIVA_ATENCION})
    public Response buscar(FiltroRelModEdModAten filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_RELACION_MODALIDAD_EDUCATIVA_ATENCION})
    public Response buscarTotal(FiltroRelModEdModAten filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_RELACION_MODALIDAD_EDUCATIVA_ATENCION})
    public Response crear(SgRelModEdModAten entity) {
        try {
            entity.setReaPk(null);
            entity.setReaVersion(0);
            LOGGER.info("crear");
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_RELACION_MODALIDAD_EDUCATIVA_ATENCION})
    public Response actualizar(@PathParam("relModEdModAtenId") Long id, SgRelModEdModAten entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_RELACION_MODALIDAD_EDUCATIVA_ATENCION})
    public Response eliminarPorId(@PathParam("relModEdModAtenId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relModEdModAtenId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relModEdModAtenId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRelModEdModAten.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelModEdModAten.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
