...
import sv.gob.mined.siges.negocio.servicios.ReglaEquivalenciaBean;
...
@RequestScoped
@Path("/v1/reglaequivalencia")
@Tag(name = "Reglas de Equivalencia API V1")
...
public class ReglaEquivalenciaRecurso {
     ...
     *
     * @param id Long
     * @return Response SgReglaEquivalencia
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgReglaEquivalencia.class)))
        ,
    ...
    @Path("/{reglaEquivalenciaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REGLA_EQUIVALENCIA})
    public Response obtenerPorId(@PathParam("reglaEquivalenciaId") Long id) {
        try {
            SgReglaEquivalencia entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgReglaEquivalencia
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgReglaEquivalencia.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REGLA_EQUIVALENCIA})
    public Response buscar(FiltroReglaEquivalencia filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_REGLA_EQUIVALENCIA})
    public Response buscarTotal(FiltroReglaEquivalencia filtro) {
        try {
    ...
    public Response crear(SgReglaEquivalencia entity) {
        try {
            entity.setReqPk(null);
            entity.setReqVersion(0);
    ...
    public Response cumpleReglaEquivalencia(SgGradoPlanOrigenDestino entity) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.cumpleReglaEquivalencia(entity)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isConstraintViolation(ex)) {
    ...
    public Response actualizar(@PathParam("reglaEquivalenciaId") Long id, SgReglaEquivalencia entity) {
        try {
    ...
    public Response eliminarPorId(@PathParam("reglaEquivalenciaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{reglaEquivalenciaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("reglaEquivalenciaId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgReglaEquivalencia.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgReglaEquivalencia.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
