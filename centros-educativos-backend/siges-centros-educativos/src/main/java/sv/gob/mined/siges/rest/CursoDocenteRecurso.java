...
import sv.gob.mined.siges.negocio.servicios.CursoDocenteBean;
...
import sv.gob.mined.siges.persistencia.entidades.SgCeldaDiaHoraCurso;
import sv.gob.mined.siges.persistencia.utilidades.RevHistorico;

@RequestScoped
@Path("/v1/cursosdocentes")
@Tag(name = "CursoDocente API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CURSOS_DOCENTES,
    ConstantesOperaciones.ACTUALIZAR_CURSOS_DOCENTES,
    ConstantesOperaciones.ELIMINAR_CURSOS_DOCENTES,
    ConstantesOperaciones.PUBLICAR_CURSOS_DOCENTES,
    ConstantesOperaciones.CERRAR_CURSOS_DOCENTES})
public class CursoDocenteRecurso {

    private static final Logger LOGGER = Logger.getLogger(CursoDocenteRecurso.class.getName());

    @Inject
    private CursoDocenteBean servicio;
     ...
     *
     * @param cursosDocentesId
     * @return Response -> SgCursoDocente
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCursoDocente.class)))
        ,
    ...
    @Path("/{cursosDocentesId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CURSOS_DOCENTES})
    public Response obtenerPorId(@PathParam("cursosDocentesId") Long cursosDocentesId) {
        try {
            SgCursoDocente entity = servicio.obtenerPorId(cursosDocentesId);
            if (entity == null) {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgCursoDocente.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CURSOS_DOCENTES})
    public Response buscar(FiltroCursoDocente filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CURSOS_DOCENTES})
    public Response buscarTotal(FiltroCursoDocente filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CURSOS_DOCENTES})
    public Response crear(SgCursoDocente entity) {
        try {
            entity.setCdsPk(null);
            entity.setCdsVersion(0);
            for (SgCeldaDiaHoraCurso celda : entity.getCdsCeldasDiaHora()) {
                celda.setCdcCurso(entity);
            }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CURSOS_DOCENTES})
    public Response actualizar(@PathParam("cursosDocentesId") Long id, SgCursoDocente entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CURSOS_DOCENTES})
    public Response eliminarPorId(@PathParam("cursosDocentesId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{cursosDocentesId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("cursosDocentesId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgCursoDocente.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCursoDocente.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    /**
     * Publica el objeto con la id indicada. Si no existe, lo crea con id
     * autogenerada.
     *
     * @param id
     * @param entity
    ...
    @Path("/publicar/{cursosDocentesId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CURSOS_DOCENTES})
    public Response publicar(@PathParam("cursosDocentesId") Long id, SgCursoDocente entity) {
        try {
            if (servicio.objetoExistePorId(id)) {
                entity.setCdsPk(id);
                for (SgCeldaDiaHoraCurso celda : entity.getCdsCeldasDiaHora()) {
                ...
                return Response.status(HttpStatus.SC_OK).entity(servicio.publicar(entity)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    /**
     * Cerrar el objeto con la id indicada. Si no existe, lo crea con id
     * autogenerada.
     *
     * @param id
     * @param entity
    ...
    @Path("/cerrar/{cursosDocentesId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CURSOS_DOCENTES})
    public Response cerrar(@PathParam("cursosDocentesId") Long id, SgCursoDocente entity) {
        try {
            if (servicio.objetoExistePorId(id)) {
                entity.setCdsPk(id);
                for (SgCeldaDiaHoraCurso celda : entity.getCdsCeldasDiaHora()) {
                ...
                return Response.status(HttpStatus.SC_OK).entity(servicio.cerrar(entity)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
