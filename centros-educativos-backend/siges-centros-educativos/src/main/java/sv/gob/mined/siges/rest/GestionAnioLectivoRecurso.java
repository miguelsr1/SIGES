...
import sv.gob.mined.siges.negocio.servicios.GestionAnioLectivoBean;
...
@RequestScoped
@Path("/v1/gestionaniolectivo")
@Tag(name = "GestionAnioLectivo API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_GESTION_ANIO_LECTIVO,
    ConstantesOperaciones.ACTUALIZAR_GESTION_ANIO_LECTIVO,
    ConstantesOperaciones.ELIMINAR_GESTION_ANIO_LECTIVO,
    ConstantesOperaciones.BUSCAR_GESTION_ANIO_LECTIVO})
public class GestionAnioLectivoRecurso {

    private static final Logger LOGGER = Logger.getLogger(GestionAnioLectivoRecurso.class.getName());

    @Inject
    private GestionAnioLectivoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgGestionAnioLectivo
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgGestionAnioLectivo.class)))
        ,
    ...
    @Path("/{gestionAnioLectivoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_GESTION_ANIO_LECTIVO})
    public Response obtenerPorId(@PathParam("gestionAnioLectivoId") Long id) {
        try {
            SgGestionAnioLectivo entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgGestionAnioLectivo
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgGestionAnioLectivo.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_GESTION_ANIO_LECTIVO})
    public Response buscar(FiltroGestionAnioLectivo filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_GESTION_ANIO_LECTIVO})
    public Response buscarTotal(FiltroGestionAnioLectivo filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_GESTION_ANIO_LECTIVO})
    public Response crear(SgGestionAnioLectivo entity) {
        try {
            entity.setGalPk(null);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_GESTION_ANIO_LECTIVO})
    public Response actualizar(@PathParam("gestionAnioLectivoId") Long id, SgGestionAnioLectivo entity) {
        try {
            if (servicio.objetoExistePorId(id)) {
                entity.setGalPk(id);
                SgGestionAnioLectivo guardada = servicio.guardar(entity);
                return Response.status(HttpStatus.SC_OK).entity(guardada).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_GESTION_ANIO_LECTIVO})
    public Response eliminarPorId(@PathParam("gestionAnioLectivoId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{gestionAnioLectivoId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("gestionAnioLectivoId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgGestionAnioLectivo.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgGestionAnioLectivo.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
