...
import sv.gob.mined.siges.negocio.servicios.ProyectoInstitucionalSedeBean;
...
@RequestScoped
@Path("/v1/proyectosinstitucionalessede")
@Tag(name = "ProyectoInstitucionalSede API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_PROYECTO_INSTITUCIONAL_SEDE,
    ConstantesOperaciones.ACTUALIZAR_PROYECTO_INSTITUCIONAL_SEDE,
    ConstantesOperaciones.ELIMINAR_PROYECTO_INSTITUCIONAL_SEDE})
public class ProyectoInstitucionalSedeRecurso {

    private static final Logger LOGGER = Logger.getLogger(ProyectoInstitucionalSedeRecurso.class.getName());

    @Inject
    private ProyectoInstitucionalSedeBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgProyectoInstitucionalSede
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgProyectoInstitucionalSede.class)))
        ,
    ...
    @Path("/{proyectoInstitucionalSedeId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("proyectoInstitucionalSedeId") Long id) {
        try {
            SgProyectoInstitucionalSede entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgProyectoInstitucionalSede
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgProyectoInstitucionalSede.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroProyectoInstitucionalSede filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroProyectoInstitucionalSede filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_PROYECTO_INSTITUCIONAL_SEDE})
    public Response crear(SgProyectoInstitucionalSede entity) {
        try {
            entity.setProPk(null);
            entity.setProVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_PROYECTO_INSTITUCIONAL_SEDE})
    public Response actualizar(@PathParam("proyectoInstitucionalSedeId") Long id, SgProyectoInstitucionalSede entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_PROYECTO_INSTITUCIONAL_SEDE})
    public Response eliminarPorId(@PathParam("proyectoInstitucionalSedeId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{proyectoInstitucionalSedeId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("proyectoInstitucionalSedeId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgProyectoInstitucionalSede.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgProyectoInstitucionalSede.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
