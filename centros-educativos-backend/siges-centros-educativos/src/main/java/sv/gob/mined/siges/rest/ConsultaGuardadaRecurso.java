...
import sv.gob.mined.siges.negocio.servicios.ConsultaGuardadaBean;
 ...
 */
@RequestScoped
@Path("/v1/consultaguardad")
@Tag(name = "Consultas Guardadas API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CONSULTA_GUARDADA,
    ConstantesOperaciones.ACTUALIZAR_CONSULTA_GUARDADA,
    ConstantesOperaciones.ELIMINAR_CONSULTA_GUARDADA})
public class ConsultaGuardadaRecurso {

    private static final Logger LOGGER = Logger.getLogger(ConsultaGuardadaRecurso.class.getName());

    @Inject
    private ConsultaGuardadaBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgConsultaGuardada
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgConsultaGuardada.class)))
        ,
    ...
    @Path("/{consultaGuardadaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("consultaGuardadaId") Long id) {
        try {
            SgConsultaGuardada cgr = servicio.obtenerPorId(id);
            if (cgr == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(cgr).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgConsultaGuardada
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgConsultaGuardada.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroConsultaGuardada filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroConsultaGuardada filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CONSULTA_GUARDADA})
    public Response crear(SgConsultaGuardada cgr) {
        try {
            cgr.setCgrPk(null);
            cgr.setCgrVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(cgr)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CONSULTA_GUARDADA})
    public Response actualizar(@PathParam("consultaGuardadaId") Long id, SgConsultaGuardada cgr) {
        try {
            if (servicio.objetoExistePorId(id)) {
                cgr.setCgrPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(cgr)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CONSULTA_GUARDADA})
    public Response eliminarPorId(@PathParam("consultaGuardadaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{consultaGuardadaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("consultaGuardadaId") Long id) {
        try {
            ...
            return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialRevisionesPorId(SgConsultaGuardada.class, id))).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgConsultaGuardada.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
