...
import sv.gob.mined.siges.negocio.servicios.DiplomadoBean;
import sv.gob.mined.siges.persistencia.utilidades.RevHistorico;

@RequestScoped
@Path("/v1/diplomados")
@Tag(name = "Diploma API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_DIPLOMADO,
    ConstantesOperaciones.ACTUALIZAR_DIPLOMADO,
    ConstantesOperaciones.ELIMINAR_DIPLOMADO,
    ConstantesOperaciones.BUSCAR_DIPLOMADO})
public class DiplomadoRecurso {

    private static final Logger LOGGER = Logger.getLogger(DiplomadoRecurso.class.getName());

    @Inject
    private DiplomadoBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgDiplomado.class)))
        ,
    ...
    @Path("/{diplomaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DIPLOMADO})
    public Response obtenerPorId(@PathParam("diplomaId") Long id) {
        try {
            SgDiplomado entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgDiplomado
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgDiplomado.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DIPLOMADO})
    public Response buscar(FiltroDiplomado filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DIPLOMADO})
    public Response buscarTotal(FiltroDiplomado filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DIPLOMADO})
    public Response crear(SgDiplomado entity) {
        try {
            entity.setDipPk(null);
            entity.setDipVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_DIPLOMADO})
    public Response actualizar(@PathParam("diplomaId") Long id, SgDiplomado entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_DIPLOMADO})
    public Response eliminarPorId(@PathParam("diplomaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{diplomaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("diplomaId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgDiplomado.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgDiplomado.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
     ...
     /**
     * Devuelve diplomados disponibles para ser autorizados en la sede indicada
     *
     * @param sedeId Long
     * @return Response List<SgDiplomado>
     */
    @GET
    @Operation(summary = "Devuelve diplomados disponibles para ser autorizados en la sede indicada.")
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgDiplomado.class, type = SchemaType.ARRAY)))
    ...
    @Path("/buscarparaautorizacionsede/{sedeId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DIPLOMAS})
    public Response buscarDiplomadosParaAutorizacionSede(@PathParam("sedeId") Long sedePk) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.obtenerDiplomadosParaAutorizacion(sedePk)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    /**
     * Devuelve diplomados autorizados en la sede indicada
     *
     * @param sedeId Long
     * @return Response List<SgDiplomado>
     */
    @GET
    @Operation(summary = "Devuelve diplomados autorizados en la sede indicada.")
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgDiplomado.class, type = SchemaType.ARRAY)))
    ...
    @Path("/buscarautorizadossede/{sedeId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarDiplomadosAutorizadosSede(@PathParam("sedeId") Long sedePk) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.obtenerDiplomadosAutorizados(sedePk)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
