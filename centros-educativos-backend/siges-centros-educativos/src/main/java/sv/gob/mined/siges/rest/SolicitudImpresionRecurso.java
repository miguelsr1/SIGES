 ...
 */
@RequestScoped
@Path("/v1/solicitudimpresion")
@Tag(name = "Solicitudes Impresion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SOLICITUD_IMPRESION,
    ConstantesOperaciones.ACTUALIZAR_SOLICITUD_IMPRESION,
    ConstantesOperaciones.ELIMINAR_SOLICITUD_IMPRESION,
    ConstantesOperaciones.BUSCAR_SOLICITUD_IMPRESION})
public class SolicitudImpresionRecurso {

    private static final Logger LOGGER = Logger.getLogger(SolicitudImpresionRecurso.class.getName());

    @Inject
    private SolicitudImpresionBean servicio;
    ...
    @Path("/{solicitudImpresionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_IMPRESION})
    public Response obtenerPorId(@PathParam("solicitudImpresionId") Long id) {
        try {
            SgSolicitudImpresion sim = servicio.obtenerPorId(id);

            if (sim == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(sim).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgSolicitudImpresion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgSolicitudImpresion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_IMPRESION})
    public Response buscar(FiltroSolicitudImpresion filtro) {
        try {
    ...
    @Path("/estudiantesHabilitados")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_IMPRESION})
    public Response buscarEstudiantesHabilitados(FiltroSolicitudImpresion filtro) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.obtenerEstudiantesHabilitados(filtro)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUD_IMPRESION})
    public Response buscarTotal(FiltroSolicitudImpresion filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SOLICITUD_IMPRESION})
    public Response crear(SgSolicitudImpresion sim) {
        try {
            sim.setSimPk(null);
            sim.setSimVersion(0);

            for (SgEstudianteImpresion estImp : sim.getSimEstudianteImpresion()) {
                estImp.setEimSolicitudImpresion(sim);
            }

            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(sim)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SOLICITUD_IMPRESION})
    public Response actualizar(@PathParam("solicitudImpresionId") Long id, SgSolicitudImpresion sim) {
        try {
            ...
            if (servicio.objetoExistePorId(id)) {
                sim.setSimPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(sim)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Path("/rechazar/{solicitudImpresionId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SOLICITUD_IMPRESION})
    public Response rechazar(@PathParam("solicitudImpresionId") Long id, SgSolicitudImpresionLite sim) {
        try {
            ...
            if (servicio.objetoExistePorId(id)) {
                sim.setSimPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.rechazar(sim)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Path("/confirmar/{solicitudImpresionId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SOLICITUD_IMPRESION})
    public Response confirmar(@PathParam("solicitudImpresionId") Long id, SgSolicitudImpresion sim) {
        try {
            ...
            if (servicio.objetoExistePorId(id)) {
                sim.setSimPk(id);
                SgSolicitudImpresion sol = servicio.confirmar(sim);
                if (sol.getSimSelloDirector() != null) {
                    sol.getSimSelloDirector().setSfiSede(null);
                }
                return Response.status(HttpStatus.SC_OK).entity(sol).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Path("/anular/{solicitudImpresionId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SOLICITUD_IMPRESION})
    public Response anular(@PathParam("solicitudImpresionId") Long id, SgSolicitudImpresionLite sim) {
        try {
            ...
            if (servicio.objetoExistePorId(id)) {
                sim.setSimPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.anular(sim)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_SOLICITUD_IMPRESION})
    public Response eliminarPorId(@PathParam("solicitudImpresionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{solicitudImpresionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("solicitudImpresionId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgSolicitudImpresion.class,
                    id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgSolicitudImpresion.class,
                    id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
    ...
    @Path("/cambioEstadoImpresa")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SOLICITUD_IMPRESION})
    public Response cambioEstadoImpresa(SgSolicitudImpresion simPk) {
        try {

            return Response.status(HttpStatus.SC_OK).entity(servicio.cambioEstadoImpresa(simPk)).build();
