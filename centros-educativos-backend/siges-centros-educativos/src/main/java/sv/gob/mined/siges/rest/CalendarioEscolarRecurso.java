...
import sv.gob.mined.siges.negocio.servicios.CalendarioEscolarBean;
 ...
 */
@RequestScoped
@Path("/v1/calendariosescolares")
@Tag(name = "Calendarios Escolares API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CALENDARIOS_ESCOLARES,
    ConstantesOperaciones.ACTUALIZAR_CALENDARIOS_ESCOLARES,
    ConstantesOperaciones.ELIMINAR_CALENDARIOS_ESCOLARES,
    ConstantesOperaciones.BUSCAR_CALENDARIOS_ESCOLARES})
public class CalendarioEscolarRecurso {

    private static final Logger LOGGER = Logger.getLogger(CalendarioEscolarRecurso.class.getName());

    @Inject
    private CalendarioEscolarBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgCalendarioEscolar
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgPeriodoCalendario.class)))
        ,
    ...
    @Path("/{calendarioEscolarId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CALENDARIOS_ESCOLARES})
    public Response obtenerPorId(@PathParam("calendarioEscolarId") Long id) {
        try {
            SgPeriodoCalendario ces = servicio.obtenerPorId(id);
            if (ces == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(ces).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgCalendarioEscolar
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgPeriodoCalendario.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CALENDARIOS_ESCOLARES})
    public Response buscar(FiltroPeriodoCalendario filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CALENDARIOS_ESCOLARES})
    public Response buscarTotal(FiltroPeriodoCalendario filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CALENDARIOS_ESCOLARES})
    public Response crear(SgPeriodoCalendario ces) {
        try {
            ces.setCesPk(null);
            ces.setCesVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(ces)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CALENDARIOS_ESCOLARES})
    public Response actualizar(@PathParam("calendarioEscolarId") Long id, SgPeriodoCalendario ces) {
        try {
            if (servicio.objetoExistePorId(id)) {
                LOGGER.log(Level.SEVERE, ces.toString());
                LOGGER.log(Level.SEVERE, "nivel:" + ces.getCesNivel().getNivNombre());
                ces.setCesPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(ces)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CALENDARIOS_ESCOLARES})
    public Response eliminarPorId(@PathParam("calendarioEscolarId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{calendarioEscolarId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("calendarioEscolarId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgPeriodoCalendario.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
     ...
     *
     * @return Response SgPeriodoCalendario
     */
    @GET
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgPeriodoCalendario.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
