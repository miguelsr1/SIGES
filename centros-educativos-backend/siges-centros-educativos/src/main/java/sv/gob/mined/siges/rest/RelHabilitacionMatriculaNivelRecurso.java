...
import sv.gob.mined.siges.negocio.servicios.RelHabilitacionMatriculaNivelBean;
 ...
 */
@RequestScoped
@Path("/v1/relhabiltacionmatriculanivel")
@Tag(name = "Rel Habilitación matrícula nivel API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_REL_HABILITACION_MATRICULA_NIVEL,
    ConstantesOperaciones.ACTUALIZAR_REL_HABILITACION_MATRICULA_NIVEL,
    ConstantesOperaciones.ELIMINAR_REL_HABILITACION_MATRICULA_NIVEL,
	ConstantesOperaciones.BUSCAR_REL_HABILITACION_MATRICULA_NIVEL})
public class RelHabilitacionMatriculaNivelRecurso {

    private static final Logger LOGGER = Logger.getLogger(RelHabilitacionMatriculaNivelRecurso.class.getName());

    @Inject
    private RelHabilitacionMatriculaNivelBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRelHabilitacionMatriculaNivel
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRelHabilitacionMatriculaNivel.class)))
        ,
    ...
    @Path("/{relHabilitacionMatriculaNivelId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("relHabilitacionMatriculaNivelId") Long id) {
        try {
            SgRelHabilitacionMatriculaNivel rhn = servicio.obtenerPorId(id);
            if (rhn == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(rhn).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgRelHabilitacionMatriculaNivel
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRelHabilitacionMatriculaNivel.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroRelHabilitacionMatriculaNivel filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroRelHabilitacionMatriculaNivel filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_REL_HABILITACION_MATRICULA_NIVEL})
    public Response crear(SgRelHabilitacionMatriculaNivel rhn) {
        try {
            rhn.setRhnPk(null);
            rhn.setRhnVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(rhn)).build();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_REL_HABILITACION_MATRICULA_NIVEL})
    public Response actualizar(@PathParam("relHabilitacionMatriculaNivelId") Long id, SgRelHabilitacionMatriculaNivel rhn) {
        try {
            if (servicio.objetoExistePorId(id)) {
                rhn.setRhnPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(rhn)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_REL_HABILITACION_MATRICULA_NIVEL})
    public Response eliminarPorId(@PathParam("relHabilitacionMatriculaNivelId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{relHabilitacionMatriculaNivelId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("relHabilitacionMatriculaNivelId") Long id) {
        try {
		...
		ObjectMapper mapper = providers.getContextResolver(ObjectMapper.class, MediaType.APPLICATION_JSON_TYPE).getContext(ConsultaHistoricoBean.class);
            	return Response.status(HttpStatus.SC_OK).entity(mapper.writeValueAsString(historico.obtenerHistorialPorId(SgRelHabilitacionMatriculaNivel.class, id))).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgRelHabilitacionMatriculaNivel.class, id, rev)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
