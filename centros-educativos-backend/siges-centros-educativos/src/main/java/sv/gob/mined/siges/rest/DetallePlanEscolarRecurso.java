...
import sv.gob.mined.siges.negocio.servicios.DetallePlanEscolarBean;
...
@RequestScoped
@Path("/v1/detalleplanescolar")
@Tag(name = "DetallePlanEscolar API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_DETALLE_PLAN_ESCOLAR,
    ConstantesOperaciones.ACTUALIZAR_DETALLE_PLAN_ESCOLAR,
    ConstantesOperaciones.ELIMINAR_DETALLE_PLAN_ESCOLAR})
public class DetallePlanEscolarRecurso {

    private static final Logger LOGGER = Logger.getLogger(DetallePlanEscolarRecurso.class.getName());

    @Inject
    private DetallePlanEscolarBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgDetallePlanEscolar.class)))
        ,
    ...
    @Path("/{detallePlanEscolarId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PERSONAL_SEDE_EDUCATIVA})
    public Response obtenerPorId(@PathParam("detallePlanEscolarId") Long detallePlanEscolarId) {
        try {
            SgDetallePlanEscolar tce = servicio.obtenerPorId(detallePlanEscolarId);
            if (tce == null) {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgDetallePlanEscolar.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_PERSONAL_SEDE_EDUCATIVA})
    public Response buscar(FiltroDetallePlanEscolar filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_DETALLE_PLAN_ESCOLAR})
    public Response crear(SgDetallePlanEscolar entity) {
        try {
            entity.setDpePk(null);
            entity.setDpeVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_DETALLE_PLAN_ESCOLAR})
    public Response actualizar(@PathParam("detallePlanEscolarId") Long id, SgDetallePlanEscolar entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_DETALLE_PLAN_ESCOLAR})
    public Response eliminarPorId(@PathParam("detallePlanEscolarId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{detallePlanEscolarId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("detallePlanEscolarId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgDetallePlanEscolar.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
     ...
     * @param id
     * @param rev
     * @return Response SgDetallePlanEscolar
     */
    @GET
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgDetallePlanEscolar.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
