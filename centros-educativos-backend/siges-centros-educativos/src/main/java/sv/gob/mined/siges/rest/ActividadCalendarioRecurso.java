...
import sv.gob.mined.siges.negocio.servicios.ActividadCalendarioBean;
 ...
 */
@RequestScoped
@Path("/v1/actividadcalendario")
@Tag(name = "Actividades Calendario API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ACTIVIDAD_CALENDARIO,
    ConstantesOperaciones.ACTUALIZAR_ACTIVIDAD_CALENDARIO,
    ConstantesOperaciones.ELIMINAR_ACTIVIDAD_CALENDARIO,
    ConstantesOperaciones.BUSCAR_ACTIVIDAD_CALENDARIO})
public class ActividadCalendarioRecurso {

    private static final Logger LOGGER = Logger.getLogger(ActividadCalendarioRecurso.class.getName());

    @Inject
    private ActividadCalendarioBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgActividadCalendario
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgActividadCalendario.class)))
        ,
    ...
    @Path("/{actividadCalendarioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ACTIVIDAD_CALENDARIO})
    public Response obtenerPorId(@PathParam("actividadCalendarioId") Long id) {
        try {
            SgActividadCalendario aca = servicio.obtenerPorId(id);
            if (aca == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(aca).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgActividadCalendario
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgActividadCalendario.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    public Response buscar(FiltroActividadCalendario filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroActividadCalendario filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ACTIVIDAD_CALENDARIO})
    public Response crear(SgActividadCalendario aca) {
        try {
            aca.setAcaPk(null);
            aca.setAcaVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(aca)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ACTIVIDAD_CALENDARIO})
    public Response actualizar(@PathParam("actividadCalendarioId") Long id, SgActividadCalendario aca) {
        try {
            if (servicio.objetoExistePorId(id)) {
                aca.setAcaPk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(aca)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ACTIVIDAD_CALENDARIO})
    public Response eliminarPorId(@PathParam("actividadCalendarioId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{actividadCalendarioId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("actividadCalendarioId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgActividadCalendario.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgActividadCalendario.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
