...
import sv.gob.mined.siges.negocio.servicios.CalendarioBean;
...
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CALENDARIOS,
    ConstantesOperaciones.ACTUALIZAR_CALENDARIOS,
    ConstantesOperaciones.ELIMINAR_CALENDARIOS,
    ConstantesOperaciones.BUSCAR_CALENDARIOS})
public class CalendarioRecurso {

    private static final Logger LOGGER = Logger.getLogger(CalendarioRecurso.class.getName());

    @Inject
    private CalendarioBean servicio;
    ...
    public Response obtenerPorId(@PathParam("calendarioId") Long id) {
        try {
            SgCalendario cal = servicio.obtenerPorId(id);
            if (cal == null) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CALENDARIOS})
    public Response buscar(FiltroCalendario filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CALENDARIOS})
    public Response buscarTotal(FiltroCalendario filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CALENDARIOS})
    public Response crear(SgCalendario cal) {
        try {
            cal.setCalPk(null);
            cal.setCalVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CALENDARIOS})
    public Response actualizar(@PathParam("calendarioId") Long id, SgCalendario cal) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CALENDARIOS})
