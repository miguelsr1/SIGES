...
import sv.gob.mined.siges.negocio.servicios.RangoFechaBean;
...
@RequestScoped
@Path("/v1/rangosfecha")
@Tag(name = "Rangos Fecha API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_RANGOS_FECHA,
    ConstantesOperaciones.ACTUALIZAR_RANGOS_FECHA,
    ConstantesOperaciones.ELIMINAR_RANGOS_FECHA,
    ConstantesOperaciones.BUSCAR_RANGOS_FECHA})
public class RangoFechaRecurso {

    private static final Logger LOGGER = Logger.getLogger(RangoFechaRecurso.class.getName());

    @Inject
    private RangoFechaBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRangoFecha.class)))
        ,
    ...
    @Path("/{rangoFechaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_RANGOS_FECHA})
    public Response obtenerPorId(@PathParam("rangoFechaId") Long id) {
        try {
            SgRangoFecha rfe = servicio.obtenerPorId(id);
            if (rfe == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(rfe).build();
            }
        } catch (Exception ex) {
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgRangoFecha.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_RANGOS_FECHA})
    public Response buscar(FiltroRangoFecha filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_RANGOS_FECHA})
    public Response buscarTotal(FiltroRangoFecha filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_RANGOS_FECHA})
    public Response crear(SgRangoFecha rfe) {
        try {
            rfe.setRfePk(null);
            rfe.setRfeVersion(0);
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardar(rfe)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_RANGOS_FECHA})
    public Response actualizar(@PathParam("rangoFechaId") Long id, SgRangoFecha rfe) {
        try {
            if (servicio.objetoExistePorId(id)) {
                rfe.setRfePk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(rfe)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_RANGOS_FECHA})
    public Response eliminarPorId(@PathParam("rangoFechaId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{rangoFechaId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("rangoFechaId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgRangoFecha.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
