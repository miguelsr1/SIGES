...
import sv.gob.mined.siges.negocio.servicios.FactorRiesgoSedeBean;
 ...
 */
@RequestScoped
@Path("/v1/factoresriesgosede")
@Tag(name = "FactorRiesgoSede API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_FACTOR_RIESGO_SEDE,
    ConstantesOperaciones.ACTUALIZAR_FACTOR_RIESGO_SEDE,
    ConstantesOperaciones.ELIMINAR_FACTOR_RIESGO_SEDE})
public class FactorRiesgoSedeRecurso {

    private static final Logger LOGGER = Logger.getLogger(FactorRiesgoSedeRecurso.class.getName());

    @Inject
    private FactorRiesgoSedeBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgFactorRiesgoSede
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgFactorRiesgoSede.class)))
        ,
    ...
    @Path("/{factorRiesgoSedeId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("factorRiesgoSedeId") Long id) {
        try {
            SgFactorRiesgoSede entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgFactorRiesgoSede
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgFactorRiesgoSede.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscar(FiltroFactorRiesgoSede filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response buscarTotal(FiltroFactorRiesgoSede filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_FACTOR_RIESGO_SEDE})
    public Response crear(SgFactorRiesgoSede entity) {
        try {
            entity.setFriPk(null);
            entity.setFriVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_FACTOR_RIESGO_SEDE})
    public Response actualizar(@PathParam("factorRiesgoSedeId") Long id, SgFactorRiesgoSede entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_FACTOR_RIESGO_SEDE})
    public Response eliminarPorId(@PathParam("factorRiesgoSedeId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{factorRiesgoSedeId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("factorRiesgoSedeId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgFactorRiesgoSede.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgFactorRiesgoSede.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
