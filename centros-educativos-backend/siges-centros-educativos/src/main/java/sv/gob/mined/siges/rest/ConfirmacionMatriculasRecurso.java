...
import sv.gob.mined.siges.negocio.servicios.ConfirmacionMatriculasBean;
 ...
 */
@RequestScoped
@Path("/v1/confirmacionesmatriculas")
@Tag(name = "Confirmaciones de Matrículas API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CONFIRMAR_MATRICULAS,
    ConstantesOperaciones.BUSCAR_CONFIRMACIONES_MATRICULAS,
    ConstantesOperaciones.ELIMINAR_CONFIRMACION_MATRICULA})
public class ConfirmacionMatriculasRecurso {

    private static final Logger LOGGER = Logger.getLogger(ConfirmacionMatriculasRecurso.class.getName());

    @Inject
    private ConfirmacionMatriculasBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgConfirmacionMatriculas
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgConfirmacionMatriculas.class)))
        ,
    ...
    @Path("/{confirmacionMatriculasId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CONFIRMACIONES_MATRICULAS})
    public Response obtenerPorId(@PathParam("confirmacionMatriculasId") Long id) {
        try {
            SgConfirmacionMatriculas cma = servicio.obtenerPorId(id);
            if (cma == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(cma).build();
            }
        } catch (Exception ex) {
     ...
     * @return Response Lista de SgConfirmacionMatriculas
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgConfirmacionMatriculas.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CONFIRMACIONES_MATRICULAS})
    public Response buscar(FiltroConfirmacionMatriculas filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_CONFIRMACIONES_MATRICULAS})
    public Response buscarTotal(FiltroConfirmacionMatriculas filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CONFIRMAR_MATRICULAS})
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CONFIRMAR_MATRICULAS})
    public Response preconfirmar(SgConfirmacionMatriculas cma) {
        try {
    ...
    @Path("/historial/{confirmacionMatriculasId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("confirmacionMatriculasId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgConfirmacionMatriculas.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgConfirmacionMatriculas.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    @Path("/{confirmacionMatriculaId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CONFIRMACION_MATRICULA})
    public Response eliminarPorId(@PathParam("confirmacionMatriculaId") Long id) {
        try {
