...
import sv.gob.mined.siges.negocio.servicios.OrganizacionCurricularBean;
...
@RequestScoped
@Path("/v1/organizacionescurricular")
@Tag(name = "OrganizacionCurricular API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ORGANIZACION_CURRICULAR,
    ConstantesOperaciones.ACTUALIZAR_ORGANIZACION_CURRICULAR,
    ConstantesOperaciones.ELIMINAR_ORGANIZACION_CURRICULAR,
    ConstantesOperaciones.BUSCAR_ORGANIZACION_CURRICULAR})
public class OrganizacionCurricularRecurso {

    private static final Logger LOGGER = Logger.getLogger(OrganizacionCurricularRecurso.class.getName());

    @Inject
    private OrganizacionCurricularBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgOrganizacionCurricular
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgOrganizacionCurricular.class)))
        ,
    ...
    @Path("/{organizacionCurricularId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ORGANIZACION_CURRICULAR})
    public Response obtenerPorId(@PathParam("organizacionCurricularId") Long id) {
        try {
            SgOrganizacionCurricular entity = servicio.obtenerPorId(id);
    ...
    public Response obtenerPorIdLazy(@PathParam("entityId") Long entityId) {
        try {
            SgOrganizacionCurricular oc = servicio.obtenerPorIdLazy(entityId);
            if (oc == null) {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            } else {
                return Response.status(HttpStatus.SC_OK).entity(oc).build();
            }
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
     ...
     * @return Response Lista de SgOrganizacionCurricular
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgOrganizacionCurricular.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ORGANIZACION_CURRICULAR})
    public Response buscar(FiltroOrganizacionCurricular filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_ORGANIZACION_CURRICULAR})
    public Response buscarTotal(FiltroOrganizacionCurricular filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ORGANIZACION_CURRICULAR})
    public Response crear(SgOrganizacionCurricular entity) {
        try {
            entity.setOcuPk(null);
            entity.setOcuVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ORGANIZACION_CURRICULAR})
    public Response actualizar(@PathParam("organizacionCurricularId") Long id, SgOrganizacionCurricular entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ORGANIZACION_CURRICULAR})
    public Response eliminarPorId(@PathParam("organizacionCurricularId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{organizacionCurricularId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("organizacionCurricularId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgOrganizacionCurricular.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgOrganizacionCurricular.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
