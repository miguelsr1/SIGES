...
import sv.gob.mined.siges.negocio.servicios.CirculoInfanciaBean;
...
@RequestScoped
@Path("/v1/circulosinfancia")
@Tag(name = "Círculo de Primera Infancia API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SEDE_CIRCULO_INFANCIA,
    ConstantesOperaciones.ACTUALIZAR_SEDE_CIRCULO_INFANCIA,
    ConstantesOperaciones.ELIMINAR_SEDE_CIRCULO_INFANCIA,
    ConstantesOperaciones.BUSCAR_SEDES})
public class CirculoInfanciaRecurso {

    private static final Logger LOGGER = Logger.getLogger(CirculoInfanciaRecurso.class.getName());

    @Inject
    private CirculoInfanciaBean servicio;
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCirculoInfancia.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("entityId") Long entityId) {
        try {
            SgCirculoInfancia cedPri = servicio.obtenerPorId(entityId);
            if (cedPri == null) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SEDE_CIRCULO_INFANCIA})
    public Response crear(SgCirculoInfancia entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SEDE_CIRCULO_INFANCIA})
    public Response actualizar(@PathParam("entityId") Long id, SgCirculoInfancia entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_SEDE_CIRCULO_INFANCIA})
    ...
    public Response obtenerHistorialPorId(@PathParam("entityId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgCirculoInfancia.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
     ...
     *
     * @return Response SgCirculoInfancia
     */
    @GET
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgCirculoInfancia.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
