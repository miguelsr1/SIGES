...
import sv.gob.mined.siges.negocio.servicios.ComponenteDocenteBean;
 ...
 */
@RequestScoped
@Path("/v1/componentesdocentes")
@Tag(name = "ComponenteDocentes API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_COMPONENTE_DOCENTE,
    ConstantesOperaciones.ACTUALIZAR_COMPONENTE_DOCENTE,
    ConstantesOperaciones.ELIMINAR_COMPONENTE_DOCENTE,
    ConstantesOperaciones.BUSCAR_COMPONENTE_DOCENTE})
public class ComponenteDocenteRecurso {

    private static final Logger LOGGER = Logger.getLogger(ComponenteDocenteRecurso.class.getName());

    @Inject
    private ComponenteDocenteBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgComponenteDocente
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgComponenteDocente.class)))
        ,
    ...
    @Path("/{componenteDocenteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("componenteDocenteId") Long id) {
        try {
            SgComponenteDocente cal = servicio.obtenerPorId(id);
            if (cal == null) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_COMPONENTE_DOCENTE})
    public Response buscar(FiltroComponenteDocente filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_COMPONENTE_DOCENTE})
    public Response buscarTotal(FiltroComponenteDocente filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_COMPONENTE_DOCENTE})
    public Response crear(SgComponenteDocente cal) {
        try {
            cal.setCdoPk(null);
            cal.setCdoVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_COMPONENTE_DOCENTE})
    public Response actualizar(@PathParam("componenteDocenteId") Long id, SgComponenteDocente cal) {
        try {
    ...
    @Path("/eliminar/{componenteDocenteId:[1-9][0-9]*}")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_COMPONENTE_DOCENTE})
    public Response eliminar(@PathParam("componenteDocenteId") Long id, SgComponenteDocente cal) {
        try {
            if (servicio.objetoExistePorId(id)) {
                cal.setCdoPk(id);
                servicio.eliminarElemento(cal);
                return Response.status(HttpStatus.SC_OK).entity(cal).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_COMPONENTE_DOCENTE})
    public Response eliminarPorId(@PathParam("componenteDocenteId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
     ...
     *
     * @param id Long
     * @return Response Lista de SgComponenteDocente
     */
    @GET
    ...
    @Path("/historial/{componenteDocenteId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("componenteDocenteId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgComponenteDocente.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    public Response obtenerEnRevision(@PathParam("entityId") Long id, @PathParam("entityRev") Long rev) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerEnRevision(SgComponenteDocente.class, id, rev)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, jwt.getSubject() + " | " + tracer.activeSpan().context().toString(), ex);
    ...
    @Path("/obtenerComponentesAsociadosDocenteEnSeccion")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerComponentesAsociadosDocenteEnSeccion(Long seccion) {
        try {
         
            return Response.status(HttpStatus.SC_OK).entity(servicio.obtenerComponentesAsociadosDocenteEnSeccion(seccion)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
