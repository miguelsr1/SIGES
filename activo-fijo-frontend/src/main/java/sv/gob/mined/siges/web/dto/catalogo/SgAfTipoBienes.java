...
@JsonIdentityInfo(generator = ObjectIdGenerators.PropertyGenerator.class,resolver = JsonIdentityResolver.class, property = "tbiPk", scope = SgAfTipoBienes.class)
public class SgAfTipoBienes implements Serializable {
    ...
    public SgAfTipoBienes() {
     ...
     public String getTbiCodigoNombreCategoria() {
        String salida = "";
        if(StringUtils.isNotBlank(this.tbiCodigo) ){
           salida  = this.tbiCodigo.toUpperCase().trim();
        }
        if(StringUtils.isNotBlank(this.tbiNombre) ){
            if(StringUtils.isNotBlank(salida)) {
               salida+="-"; 
            }
           salida += this.tbiNombre.toUpperCase().trim();
        }
        if(tbiCategoriaBienes != null && StringUtils.isNotBlank(this.tbiCategoriaBienes.getCabNombre())) {
            if(StringUtils.isNotBlank(salida)) {
               salida+="-"; 
            }
            salida += this.tbiCategoriaBienes.getCabNombre().toUpperCase().trim();
        }
        return salida;
    }
