...
public class UnidadesActivoFijoRestClient implements Serializable {
   ...
   @Inject
    ...
    @Retry(maxRetries = 2, retryOn = {HttpServerUnavailableException.class, HttpServerException.class, TimeoutException.class}, delay = 250)
    public List<SgAfUnidadesActivoFijo> buscarSinPermisos(FiltroUnidadesActivoFijo filtro) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (filtro == null) {
            return new ArrayList<>();
        }
        WebTarget webTarget = RestClient.getWebTarget(client, ConstantesServiciosRest.SERVICIO_CATALOGO, "v1/unidadesAF/buscar");
        SgAfUnidadesActivoFijo[] unidades = restClient.invokePost(webTarget, filtro, SgAfUnidadesActivoFijo[].class);
        return Arrays.asList(unidades);
    }
}