...
public class MenuViewBean implements Serializable {
    ...
    public void init() {
        try {
            ...
            String etiqueta=Etiquetas.getValue("hinfraestructura", locale);
                
            if(sessionBean.getOperaciones().contains(ConstantesOperaciones.TIENE_ACCESO_LEGALIZACION_INMUEBLE) && !sessionBean.getOperaciones().contains(ConstantesOperaciones.TIENE_ACCESO_INFRAESTRUCTURA)){
               
                
                submenu = new DefaultSubMenu(Etiquetas.getValue("legalizacion", locale));
