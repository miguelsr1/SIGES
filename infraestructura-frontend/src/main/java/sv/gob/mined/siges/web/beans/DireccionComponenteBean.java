...
public class DireccionComponenteBean implements Serializable {
    ...
    public void changeTab(TabChangeEvent event) { 
        ...
        tabActiveIndex = ((TabView)event.getSource()).getChildren().indexOf(activeTab);
        if (this.tabActiveIndex == 1) {
            String updateId = event.getComponent().getNamingContainer().getParent().getNamingContainer().getClientId() + ":tabDireccion:fila_mapa";
