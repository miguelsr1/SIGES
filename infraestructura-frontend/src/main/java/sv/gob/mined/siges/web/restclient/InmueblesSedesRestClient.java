...
public class InmueblesSedesRestClient implements Serializable {
   ...
   public SgInmueblesSedes guardarListaOtroServiciosBasicos(SgInmueblesSedes inmueble) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (inmueble == null) {
            return null;
        }
       if (inmueble.getTisPk()!= null){
            WebTarget webTarget = RestClient.getWebTarget(client, ConstantesServiciosRest.INFRAESTRUCTURA, "v1/inmueblesede/actualizarListaOtroServicio");
            webTarget = webTarget.path(inmueble.getTisPk().toString());
            return restClient.invokePut(webTarget, inmueble, SgInmueblesSedes.class);
        }
        return inmueble;
    ...
    public SgInmueblesSedes guardarListaObraExterior(SgInmueblesSedes inmueble) throws HttpServerException, HttpClientException, HttpServerUnavailableException, BusinessException {
        if (inmueble == null) {
            return null;
        }
       if (inmueble.getTisPk()!= null){
            WebTarget webTarget = RestClient.getWebTarget(client, ConstantesServiciosRest.INFRAESTRUCTURA, "v1/inmueblesede/actualizarListaRelObraExterior");
            webTarget = webTarget.path(inmueble.getTisPk().toString());
            return restClient.invokePut(webTarget, inmueble, SgInmueblesSedes.class);
        }
        return inmueble;
