...
public class JSFUtils {
    ...
    public static void agregarMensajes(String globalId, FacesMessage.Severity sev, BusinessException be) {
        limpiarMensajesError();
        for (int i = 0; i < be.getErrores().size(); i++) {
            BusinessError error = be.getErrores().get(i);
        ...
        for (int i = 0; i < be.getErroresTextoPlano().size(); i++) {
            BusinessError error = be.getErroresTextoPlano().get(i);
            String mensaje = error.getMensaje();
            FacesMessage msg = new FacesMessage(sev, mensaje, "");
            FacesContext.getCurrentInstance().addMessage(globalId, msg);
    ...
    public static void agregarMensajesDirecto(String globalId, FacesMessage.Severity sev, List<BusinessError> errores) {
        limpiarMensajesError();
        for (int i = 0; i < errores.size(); i++) {
            BusinessError error = errores.get(i);
            String mensaje = error.getMensaje();     
