...
@Named
@RequestScoped
public class CartaCongelaFondosCIEGenerator {
    private static final Logger LOGGER = Logger.getLogger(CartaCongelaFondosCIEGenerator.class.getName());
    ...
    public MasterReport generarReporte(Long sedeId) throws Exception {
       try {
            ...
            if (BooleanUtils.isTrue(utilizarPlantillasExternas)) {
                SgPlantilla plantilla = plantillaClient.obtenerPorCodigo(Constantes.PLANTILLA_CARTA_CONGELA_FONDOS_CIE);
                if (plantilla == null || plantilla.getPlaArchivo() == null) {
                    be.addError("Plantilla " + Constantes.PLANTILLA_CARTA_CONGELA_FONDOS_CIE + " inexistente.");
                    throw be;
                }
                ...
                reportDefinitionURL = classloader.getResource("CARTA_CONGELA_FONDOS_CIE.prpt");
            }
    ...
    private DataFactory getDataFactory(List<SgCuentasBancarias> cuentas, SgSede sede,List<SgPersonaOrganismoAdministracion> listaOrganismoEscolar) throws Exception {
        ...
        for(SgPersonaOrganismoAdministracion poa : listaOrganismoEscolar) {
                if(poa.getPoaCargo() != null && poa.getPoaCargo().getCoaCodigo() != null) {
                   ...
                   if(poa.getPoaCargo().getCoaCodigo().trim().equals(Constantes.CODIGO_PRESIDENTE_CIE)) {
                        if(preregistrado) {
                        ...
                        count ++;
                   } else if(poa.getPoaCargo().getCoaCodigo().trim().equals(Constantes.CODIGO_TESORERO_CIE)) {
                       if(preregistrado) {
                        ...
                        count ++;
                   } else if(poa.getPoaCargo().getCoaCodigo().trim().equals(Constantes.CODIGO_CONSEJAL_CIE)) {
                        if(preregistrado) {
