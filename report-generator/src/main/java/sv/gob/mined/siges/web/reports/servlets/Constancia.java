...
import sv.gob.mined.siges.web.reports.generator.ConstanciaGenerator;
import sv.gob.mined.siges.web.utilidades.JSFUtils;
import sv.gob.mined.siges.web.utilidades.ServletUtils;

@WebServlet(name = "constanciaServlet ", urlPatterns = "/wa/constancia")
public class Constancia extends HttpServlet {

    private static final long serialVersionUID = 1L;

    private static final Logger LOGGER = Logger.getLogger(Constancia.class.getName());

    @Inject
    private SessionBean sessionBean;

    @Inject
    private ConstanciaGenerator reportGenerator;
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (sessionBean.getUserCode() == null || !sessionBean.getOperaciones().contains(ConstantesOperaciones.GENERAR_REPORTE_CONSTANCIA)) {
                sessionBean.setUserIp(JSFUtils.getRemoteAddress(request));
                sessionBean.setUser(request.getUserPrincipal());
            }
            
            if (!sessionBean.getOperaciones().contains(ConstantesOperaciones.GENERAR_REPORTE_CONSTANCIA)) {
                ServletUtils.printMessage("Constancia", "Permisos insuficientes", response);
                return;   
            }
            ...
            ServletUtils.printMessage("Constancia", be, response);
        } catch (Exception e) {
