...
@WebServlet(name = "confirmacionMatriculaSedeServlet ", urlPatterns = "/wa/confirmacionMatriculaSede")
public class ConfirmacionMatriculaSede extends HttpServlet {

    private static final long serialVersionUID = 1L;

    private static final Logger LOGGER = Logger.getLogger(ConfirmacionMatriculaSede.class.getName());
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (sessionBean.getUserCode() == null || !sessionBean.getOperaciones().contains(ConstantesOperaciones.GENERAR_CONFIRMACION_MATRICULA_SEDE)) {
                sessionBean.setUserIp(JSFUtils.getRemoteAddress(request));
                sessionBean.setUser(request.getUserPrincipal());
            }

            if (!sessionBean.getOperaciones().contains(ConstantesOperaciones.GENERAR_CONFIRMACION_MATRICULA_SEDE)) {
