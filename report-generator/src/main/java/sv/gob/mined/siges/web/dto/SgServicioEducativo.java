...
public class SgServicioEducativo implements Serializable {
    ...
    public String getSduNombrePromocion() {
        StringBuilder s = new StringBuilder();
        s.append(this.sduGrado.getGraNombre()).append(" - ");
        s.append(this.sduGrado.getGraRelacionModalidad().getReaModalidadEducativa().getModNombre()).append(" - ");
        s.append(this.sduGrado.getGraRelacionModalidad().getReaModalidadEducativa().getModCiclo().getCicNivel().getNivNombre());
        if (this.sduGrado.getGraRelacionModalidad().getReaModalidadAtencion() != null){
            s.append(" - ").append(this.sduGrado.getGraRelacionModalidad().getReaModalidadAtencion().getMatNombre());
        }
    ...
    public String getSduNombre() {
        ...
        if (this.sduProgramaEducativo != null){
            s.append(" - ").append(this.sduProgramaEducativo.getPedNombre());
        }
        if (this.sduOpcion != null){
            s.append(" - ").append(this.sduOpcion.getOpcNombre());
        }
        if (this.sduGrado.getGraRelacionModalidad().getReaModalidadAtencion() != null){
            s.append(" - ").append(this.sduGrado.getGraRelacionModalidad().getReaModalidadAtencion().getMatNombre());
        }
        if (this.sduGrado.getGraRelacionModalidad().getReaSubModalidadAtencion() != null){
            s.append(" - ").append(this.sduGrado.getGraRelacionModalidad().getReaSubModalidadAtencion().getSmoNombre());
        }
        return SofisStringUtils.normalizarString(s.toString());
    }
    
    public String getSduNombre(Long cantidadCiclo) {
        StringBuilder s = new StringBuilder();
        s.append(this.sduGrado.getGraRelacionModalidad().getReaModalidadEducativa().getModCiclo().getCicNivel().getNivNombre()).append(" - ");
        if(cantidadCiclo>1){
            s.append(this.sduGrado.getGraRelacionModalidad().getReaModalidadEducativa().getModCiclo().getCicNombre()).append(" - ");
