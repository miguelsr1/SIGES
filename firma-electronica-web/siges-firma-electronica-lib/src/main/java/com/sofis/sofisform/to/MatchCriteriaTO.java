 ...
 */
public class MatchCriteriaTO extends ValueCriteriaTO {
    ...
    public String getStringQuery(HashMap col, HashMap leftJoin) {
        ...
        if (this.getValue() != null && !this.getValue().toString().equalsIgnoreCase("")) {
            ...
            //MB. 28-12-2017 - Se agrega if para que no discrimine mayúsculas de minúsculas. Trabaja junto con modificación en JPADaoImp (También comentario MB. 28-12-2017).
            //Anteriormente el if comenzaba en el -> else if
            if ((value instanceof String) && matchType.equals(MatchCriteriaTO.types.EQUALS.CONTAINS.type)) {
                toReturn = "( " + "upper(" + index + "." + property + ") " + matchTypeToQuery();
            }else if ((value instanceof Date) && matchType.equals(MatchCriteriaTO.types.EQUALS.CONTAINS.type)) {
