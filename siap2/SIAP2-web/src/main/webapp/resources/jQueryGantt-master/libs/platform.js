...
jQuery.fn.createErrorAlert = function(errorCode, message) {
  this.each(function() {
    ...
    if ($("#" + el.attr("id") + "error").size() <= 0) {
      ...
      err=$(err);
      err.attr("title",errMess).attr("src","../javax.faces.resource/jQueryGantt-master/res/alert.gif.xhtml");
      el.after(err);
    }
  });
  return this;
...
function openBlackPopup(url,width,height,onCloseCallBack,iframeId){
  ...
  bdiv.append("<img src=\"../javax.faces.resource/jQueryGantt-master/res/closeSmall.png.xhtml\" style='cursor:pointer;position:absolute;right:0px;top:30px;'>");
...
function createBlackPage(width,height,onCloseCallBack){
  ...
  var bg=$("<div>").attr("id","__blackpopup__");
  bg.css({position:'fixed',top:"0px",paddingTop:"50px",left:0,width:'100%',height:'100%',backgroundColor:'rgba(0, 0, 0, 0.5)'});
  bg.append("<div id='bwinPopupd' name='bwinPopupd'></div>");
  bg.bringToFront();

  var ret=bg.find("#bwinPopupd");
  ret.css({width:width, height:height,top:10,overflow:'auto',backgroundColor:"#fff",margin:"auto","-webkit-box-shadow":'0 5px 15px rgba(0,0,0,.5)',"box-shadow":'0 5px 15px rgba(0,0,0,.5)',"border-radius":'4px',"padding":'15px'});

  var bdiv= $("<div>").css({width:width,position:"relative",height:"0px", textAlign:"right", margin:"auto" });
  var img=$("<img src='../javax.faces.resource/jQueryGantt-master/res/closeSmall.png.xhtml' style='cursor:pointer;position:absolute;right:10px;top:10px;' title='close'>");
  bdiv.append(img);
  img.click( function(){
    bg.trigger("close");
  });
