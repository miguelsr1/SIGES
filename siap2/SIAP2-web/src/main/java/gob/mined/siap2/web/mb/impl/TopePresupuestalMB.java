...
@ViewScoped
@Named(value = "topePresupuestalMB")
public class TopePresupuestalMB implements Serializable {
   ...
   // private SofisComboG<Cuentas> comboCuentas = new SofisComboG<>();
    //private SofisComboG<Cuentas> comboCuentasHijas = new SofisComboG<>();
    ...
    public void init() {
        try {
            ...
            flagCreate = (buscarIdentificador("create") != null);
            topePresupuestal = new TopePresupuestal();
            cargarCombos();
            ...
            if (!TextUtils.isEmpty(id)) {
                findTopePresupuestalById(Integer.parseInt(id));
            }
            
            
            llenarCombosDeTopeSeleccionado();
        } catch (Exception ex){
    ...
    /**
     * Este método se utiliza para asignar el valor del tope seleccionado, a los combos de edicion/creacion de registro
     */
    public void llenarCombosDeTopeSeleccionado(){
        try {
            if (topePresupuestal != null && topePresupuestal.getId() != null && topePresupuestal.getId() != 0){
                this.comboAnioFiscal.setSelectedT(topePresupuestal.getAnioFiscal());
                cargarCuentas();
                this.comboEstadoTope.setSelectedT(topePresupuestal.getEstado());
                if (topePresupuestal.getSubCuenta() != null) {
                    unidadPresupuestariaSelected = topePresupuestal.getSubCuenta().getCuentaPadre();
                    cargarSubCuentas();
                    lineaPresupuestariaSelected = topePresupuestal.getSubCuenta();
                }
                if (topePresupuestal.getGesPresEs() != null) {
                    this.componenteSelected = topePresupuestal.getGesPresEs().getCategoriaPresupuestoEscolar();
                    cargarComponentesPorCategoria();
                    this.subComponenteSelected = topePresupuestal.getGesPresEs();
                }
                this.codSede = topePresupuestal.getSede().getCodigo();
            }
            else{
                if(flagCreate){
                    this.comboEstadoTope.setSelectedT(EstadoTopePresupuestal.EN_PROCESO);
                }
            }
        } catch (Exception ex) {
            logger.log(Level.SEVERE, null, ex);
    ...
    /**
     * Metodo utilizado para buscar un registro de TopePresupuestal, filtrado
     * por ID
     *
     * @param id
     */
    public void findTopePresupuestalById(Integer id) {
        try {
            if (id != null) {
                setTopePresupuestal(presupuestalBean.getTopePresupuestalById(id));
            }
        } catch (Exception ex) {
            logger.log(Level.SEVERE, null, ex);
            jSFUtils.mostrarErrorByPropertieCode(ConstantesErrores.ERROR_GENERAL);
    ...
    /**
     * Metodo utilizado para asignar un componente
     */
    public void asignarComponente() {
        try {
            getTopePresupuestal().setGesPresEs(this.subComponenteSelected);
        } catch (Exception ex) {
            logger.log(Level.SEVERE, null, ex);
            jSFUtils.mostrarErrorByPropertieCode(ConstantesErrores.ERROR_GENERAL);
            RequestContext.getCurrentInstance().execute("$('html,body').animate({scrollTop: 0})");
    ...
    public void guardarActualizar() {
        try {
            ...
            SgSede sede = findSede();
            if(sede == null){
                jSFUtils.mostrarErrorByPropertieCode(ConstantesErrores.ERR_CODIGO_DE_CENTRO_EDUCATIVO_NO_VALIDO);
                return ;
            }
            
            getTopePresupuestal().setAnioFiscal(this.comboAnioFiscal.getSelectedT());
            getTopePresupuestal().setSubCuenta(this.lineaPresupuestariaSelected);
            getTopePresupuestal().setSede(sede);
            getTopePresupuestal().setEstado(this.comboEstadoTope.getSelectedT());
            getTopePresupuestal().setGesPresEs(this.subComponenteSelected);
           
            verificarMovimiento();
            
            if(this.getTopePresupuestal().getId() == null && !verificarTechoExistente(this.getTopePresupuestal())){
                jSFUtils.mostrarErrorByPropertieCode(ConstantesErrores.ERR_TECHO_EXISTENTE_ENCONTRADO);
                return ;
            } 
            if(getTopePresupuestal() != null) {
                if(getTopePresupuestal().getPresupuestoEscolarMovimiento() != null && getTopePresupuestal().getPresupuestoEscolarMovimiento().getPresupuestoEscolar() != null) {
                   /** if(getTopePresupuestal().getPresupuestoEscolarMovimiento().getPresupuestoEscolar().getEstado().equals(EstadoPresupuestoEscolar.FORMULADO)) {
                        jSFUtils.mostrarErrorByPropertieCode(ConstantesErrores.ERR_PRESUPUESTO_FORMULADO);
                        return ;
                    } else **/if(getTopePresupuestal().getPresupuestoEscolarMovimiento().getPresupuestoEscolar().getEstado().equals(EstadoPresupuestoEscolar.CERRADO)) {
                        jSFUtils.mostrarErrorByPropertieCode(ConstantesErrores.ERR_PRESUPUESTO_CERRADO);
                        return ;
                    } else {
                        if(!getTopePresupuestal().getPresupuestoEscolarMovimiento().getPresupuestoEscolar().getEstado().equals(EstadoPresupuestoEscolar.APROBADO) && 
                                !getTopePresupuestal().getPresupuestoEscolarMovimiento().getPresupuestoEscolar().getEstado().equals(EstadoPresupuestoEscolar.CERRADO)) {

                            if(getTopePresupuestal().getEstado().equals(EstadoTopePresupuestal.APROBACION)) {
                                getTopePresupuestal().getPresupuestoEscolarMovimiento().getPresupuestoEscolar().setEstado(EstadoPresupuestoEscolar.EN_AJUSTE);
                            } else if(getTopePresupuestal().getEstado().equals(EstadoTopePresupuestal.EN_PROCESO)) {
                                getTopePresupuestal().getPresupuestoEscolarMovimiento().getPresupuestoEscolar().setEstado(EstadoPresupuestoEscolar.EN_PROCESO);
                            } else if(getTopePresupuestal().getEstado().equals(EstadoTopePresupuestal.FORMULACION)) {
                                getTopePresupuestal().getPresupuestoEscolarMovimiento().getPresupuestoEscolar().setEstado(EstadoPresupuestoEscolar.FORMULADO);
                            } else if(getTopePresupuestal().getEstado().equals(EstadoTopePresupuestal.CERRADO)) {
                                getTopePresupuestal().getPresupuestoEscolarMovimiento().getPresupuestoEscolar().setEstado(EstadoPresupuestoEscolar.CERRADO);
                            }
                        }
                    }
            ...
            this.topePresupuestal = presupuestalBean.crearActualizar(getTopePresupuestal());
            filterTable();
    ...
    public void filterTable() {
        try {
            ...
            if (this.componenteSelected != null) {
                MatchCriteriaTO criterio = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.EQUALS, "gesPresEs.categoriaPresupuestoEscolar.id", this.componenteSelected.getId());
                criterios.add(criterio);
            }
            if (this.subComponenteSelected != null) {
                MatchCriteriaTO criterio = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.EQUALS, "gesPresEs.id", this.subComponenteSelected.getId());
                criterios.add(criterio);
            }
            if (this.fuenteFinanciamientoSelected != null) {
                MatchCriteriaTO criterio = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.EQUALS, "fuenteFinanciamiento.id", this.fuenteFinanciamientoSelected.getId());
                criterios.add(criterio);
            }
            
            if (this.fuenteRecursosSelected != null) {
                MatchCriteriaTO criterio = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.EQUALS, "fuenteRecursos.id", this.fuenteRecursosSelected.getId());
                criterios.add(criterio);
            }
            
            if (getComboTipoCredito().getSelectedT() != null) {
                MatchCriteriaTO criterio = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.EQUALS, "gesPresEs.tipoCredito.id", getComboTipoCredito().getSelectedT().getId());
                criterios.add(criterio);
            }
            if (this.unidadPresupuestariaSelected!= null) {
                MatchCriteriaTO criterio = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.EQUALS, "subCuenta.cuentaPadre.id", this.unidadPresupuestariaSelected.getId());
                criterios.add(criterio);
            }
            if (this.lineaPresupuestariaSelected != null) {
                MatchCriteriaTO criterio = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.EQUALS, "subCuenta.id", this.lineaPresupuestariaSelected.getId());
                criterios.add(criterio);
            }
            if (getComboEstadoTope() != null && getComboEstadoTope().getSelectedT() != null) {
                MatchCriteriaTO criterio = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.EQUALS, "estado", getComboEstadoTope().getSelectedT());
                criterios.add(criterio);
            }
            if (getCodSede() != null && !getCodSede().isEmpty()) {
                MatchCriteriaTO criterio = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.EQUALS, "sede.codigo", getCodSede());
                criterios.add(criterio);
            }
            if (getComboTipoPresupuestoAnio().getSelectedT() != null) {
                MatchCriteriaTO criterio = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.EQUALS, "gesPresEs.relacionGesPresEsAnioFiscals.tipo", getComboTipoPresupuestoAnio().getSelectedT());
                criterios.add(criterio);
            }
            
            if(rel != null && rel.getId() != null) {
                MatchCriteriaTO criterio = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.EQUALS, "relGesPres.id", rel.getId());
                criterios.add(criterio);
            }
            ArrayList<CriteriaTO> listaMontos = new ArrayList();
            MatchCriteriaTO criterioMonto = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.GREATER, "monto", 0);
            listaMontos.add(criterioMonto);
                
            MatchCriteriaTO criterioMontoAprobado = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.GREATER, "montoAprobado", 0);
...
//            }
            ...
            String[] propiedades = {"id", "monto", "montoAprobado", "anioFiscal.anio", "estado", "subCuenta.codigo", "gesPresEs.nombre", "gesPresEs.categoriaPresupuestoEscolar.nombre",
                "gesPresEs.nombre", "gesPresEs.tipoCredito.nombre", "subCuenta.cuentaPadre.nombre","subCuenta.cuentaPadre.codigo", "subCuenta.nombre", "sede.codigo","sede.nombre","sede.direccion.departamento.nombre","cantidadMatricula"};
            String className = TopePresupuestal.class.getName();
            String[] orderBy = {"id"};
            boolean[] asc = {true};
     ...
     * @return
     */
    public boolean verificarEstadoAprobado() {
        if (getComboEstadoTope() != null && getComboEstadoTope().getSelectedT() != null) {
            return getComboEstadoTope().getSelectedT().compareTo(EstadoTopePresupuestal.APROBACION) == 0;
        }
        return false;
    }
    ...
    //********** CREACION DE MOVIMIENTOS Y PRESUPUESTO **********
    
    /**
     * Este método verifica las condiciones para la creacion de un movimiento
     */
    public void verificarMovimiento(){
        try {
            estadoCambiado();
            if(getTopePresupuestal().getPresupuestoEscolarMovimiento() != null){
                getTopePresupuestal().getPresupuestoEscolarMovimiento().setMonto(getTopePresupuestal().getMonto());
                getTopePresupuestal().getPresupuestoEscolarMovimiento().setMontoAprobado((getTopePresupuestal().getMontoAprobado() != null ? getTopePresupuestal().getMontoAprobado() : BigDecimal.ZERO));
                getTopePresupuestal().getPresupuestoEscolarMovimiento().getPresupuestoEscolar().setSede(getTopePresupuestal().getSede());
            }
        } catch (Exception ex) {
            logger.log(Level.SEVERE, null, ex);
            jSFUtils.mostrarErrorByPropertieCode(ConstantesErrores.ERROR_GENERAL);
    ...
    /**
     * Este método verifica la existencia de un movimiento o el cambio de estado de un TopePresupuestal
     */
    public void estadoCambiado(){
        try {
            if(getTopePresupuestal().getMonto() == null || getTopePresupuestal().getMonto().equals(BigDecimal.ZERO) || getTopePresupuestal().getMontoAprobado() == null || getTopePresupuestal().getMontoAprobado().equals(BigDecimal.ZERO)){
                return;
            }
            if((getTopePresupuestal().getId() == null || getTopePresupuestal().getId() == 0) && getComboEstadoTope().getSelectedT().equals(EstadoTopePresupuestal.FORMULACION)){
                getTopePresupuestal().setPresupuestoEscolarMovimiento(obtenerNuevoMovimiento());
                
            }else if(getTopePresupuestal().getId() != null && comboEstadoTope.getSelectedT().equals(EstadoTopePresupuestal.APROBACION)){
                
                if(getTopePresupuestal().getPresupuestoEscolarMovimiento() == null || getTopePresupuestal().getPresupuestoEscolarMovimiento().getId() == null){

                    getTopePresupuestal().setPresupuestoEscolarMovimiento(obtenerNuevoMovimiento());
                }
                
            }else if(getTopePresupuestal().getId() != null && comboEstadoTope.getSelectedT().equals(EstadoTopePresupuestal.FORMULACION)){
                if(getTopePresupuestal().getPresupuestoEscolarMovimiento() == null || getTopePresupuestal().getPresupuestoEscolarMovimiento().getId() == 0){
                    getTopePresupuestal().setPresupuestoEscolarMovimiento(obtenerNuevoMovimiento());
                }
            }
        } catch (Exception ex) {
     ...
     * @return 
     */
    public SgPresupuestoEscolarMovimiento obtenerNuevoMovimiento(){
        try {
            ...
            movi.setMonto(getTopePresupuestal().getMonto());
            movi.setMontoAprobado(getTopePresupuestal().getMontoAprobado());
            movi.setTopePresupuestal(getTopePresupuestal());
            SgPresupuestoEscolar pe = presupuestoEscolarBean.getPresupuestoEscolarByAnioYSede(getTopePresupuestal().getAnioFiscal().getAnio(), getTopePresupuestal().getSede().getId());
            if(pe == null){
                pe = crearPresupuestoEscolar();
                movi.setNumMovimiento(1);
            }else{
                Integer movimientoActual = presupuestalBean.getMaxMovimiento(pe.getPk());
     ...
     * @return 
     */
    public SgPresupuestoEscolar crearPresupuestoEscolar(){
        try {
            SgPresupuestoEscolar pe = new SgPresupuestoEscolar();
            pe.setAnioFiscal(getTopePresupuestal().getAnioFiscal());
            pe.setNombre(getTopePresupuestal().getAnioFiscal().getAnio()+"-"+getTopePresupuestal().getSede().getCodigo());
            pe.setSede(getTopePresupuestal().getSede());
    ...
    //********** CREACION DE MOVIMIENTOS Y PRESUPUESTO, PARA MULTIPLES  **********
    
    /**
     * Metodo que actualiza los estados de los registros de topeSeleccionados
     */
    public void cambiarEstadoTopes(){
        try {
            int contadorActualizados = 0;
            int contadorNoActualizados = 0;
            if(this.listaSeleccionTopes != null && !this.listaSeleccionTopes.isEmpty()){
                if(this.comboEstadoMultiples != null && this.comboEstadoMultiples.getSelectedT() != null){
                    
                    //SI EL ESTADO SELECCIONADO ES APROBACION, QUITARA LOS QUE NO TENGAN MONTO APROBADO
                    if(comboEstadoMultiples.getSelectedT().equals(EstadoTopePresupuestal.APROBACION)){
                        Iterator<TopePresupuestal> iteradorEvaluacionMontos = listaSeleccionTopes.listIterator();
                        TopePresupuestal tp;
                        while(iteradorEvaluacionMontos.hasNext()){
                            tp = iteradorEvaluacionMontos.next();
                            if(tp.getMontoAprobado() == null || tp.getMontoAprobado().compareTo(BigDecimal.ZERO) <= 0){
                                iteradorEvaluacionMontos.remove();
                                contadorNoActualizados ++;
                            }
                        }
                    }
                   
                    
                    List<TopePresupuestal> listaTopeMultiple = new ArrayList<TopePresupuestal>();
                    for(TopePresupuestal iteTope : listaSeleccionTopes){
                        iteTope = verificarMovimientoMultiple(iteTope);
                        if(iteTope != null){
                            listaTopeMultiple.add(iteTope);
                            contadorActualizados++;
                        }
                    }
                    presupuestalBean.actualizarEstadoTopes(listaTopeMultiple, this.comboEstadoMultiples.getSelectedT());
                }else{
                    jSFUtils.mostrarErrorByPropertieCode(ConstantesErrores.ERR_NINGUN_ESTADO_SELECCIONADO);
                }
            }else{
                jSFUtils.mostrarErrorByPropertieCode(ConstantesErrores.ERR_NINGUN_REGISTRO_SELECCIONADO);
            }
            jSFUtils.mostrarMensaje("Registros actualizados correctamente: "+contadorActualizados, FacesMessage.SEVERITY_INFO);
            if(contadorNoActualizados > 0){
                jSFUtils.mostrarMensaje("Registros no actualizados: "+contadorNoActualizados, FacesMessage.SEVERITY_ERROR);
                jSFUtils.mostrarErrorByPropertieCode(ConstantesErrores.ERR_MONTO_APROBADO_NO_INGRESADO);
            }
        } catch (Exception ex) {
            logger.log(Level.SEVERE, null, ex);
            jSFUtils.mostrarErrorByPropertieCode(ConstantesErrores.ERROR_GENERAL);
     ...
     * @return 
     */
    public TopePresupuestal verificarMovimientoMultiple(TopePresupuestal tope){
        try {
            return estadoCambiadoMultiple(tope);
        } catch (Exception ex) {
            logger.log(Level.SEVERE, null, ex);
            jSFUtils.mostrarErrorByPropertieCode(ConstantesErrores.ERROR_GENERAL);
            RequestContext.getCurrentInstance().execute("$(window).scrollTop(0);");
     ...
     * @return 
     */
    public TopePresupuestal estadoCambiadoMultiple(TopePresupuestal tope){
        try {
            //SI EL ESTADO SELECCIONADO ES APROBACION O FORMULACION, Y NO TIENE UN MOVIMIENTO CREADO; CREA UN MOVIMIENTO NUEVO.
            if((this.comboEstadoMultiples.getSelectedT().compareTo(EstadoTopePresupuestal.APROBACION) == 0 || this.comboEstadoMultiples.getSelectedT().compareTo(EstadoTopePresupuestal.FORMULACION) == 0) 
                    && (tope.getPresupuestoEscolarMovimiento() == null || tope.getPresupuestoEscolarMovimiento().getId() == null)){
                
                tope.setPresupuestoEscolarMovimiento(obtenerNuevoMovimientoMultiple(tope));
            }
            return tope;
        } catch (Exception ex) {
            logger.log(Level.SEVERE, null, ex);
     ...
     * @return 
     */
    public SgPresupuestoEscolarMovimiento obtenerNuevoMovimientoMultiple(TopePresupuestal tope){
        try {
            SgPresupuestoEscolarMovimiento movi = new SgPresupuestoEscolarMovimiento();
            movi.setOrigen(EstadoMovimientos.Values.TRANSFERENCIA);
            movi.setTipo(EstadoMovimientos.Values.INGRESO);
            movi.setMonto(tope.getMonto());
            movi.setMontoAprobado(tope.getMontoAprobado() != null ? tope.getMontoAprobado() : BigDecimal.ZERO);
            movi.setTopePresupuestal(tope);
            SgPresupuestoEscolar pe = presupuestoEscolarBean.getPresupuestoEscolarByAnioYSede(tope.getAnioFiscal().getAnio(), tope.getSede().getId());
            if(pe == null){
                pe = crearPresupuestoEscolarMultiple(tope);
            }else{
                Integer movimientoActual = presupuestalBean.getMaxMovimiento(pe.getPk());
                movi.setNumMovimiento(movimientoActual == null ? 1 : (movimientoActual+1));
            }
     ...
     * @return 
     */
    public SgPresupuestoEscolar crearPresupuestoEscolarMultiple(TopePresupuestal tope){
        try {
            SgPresupuestoEscolar pe = new SgPresupuestoEscolar();
