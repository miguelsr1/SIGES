...
import sv.gob.mined.siges.negocio.servicios.ReglaContextoBean;

@RequestScoped
@Path("/v1/reglascontexto")
@Tag(name = "Regla Contexto API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO})
public class ReglaContextoRecurso {

    private static final Logger LOGGER = Logger.getLogger(ReglaContextoRecurso.class.getName());

    @Inject
    private ReglaContextoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgReglaContexto
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgReglaContexto.class)))
        ,
    ...
    public Response obtenerPorId(@PathParam("recPk") Long id) {
        try {
            SgReglaContexto entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgReglaContexto
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgReglaContexto.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response crear(SgReglaContexto entity) {
        try {
            entity.setRecPk(null);
            entity.setRecVersion(0);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response actualizar(@PathParam("recPk") Long id, SgReglaContexto entity) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response eliminarPorId(@PathParam("recPk") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{recPk:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("recPk") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialPorId(SgReglaContexto.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
