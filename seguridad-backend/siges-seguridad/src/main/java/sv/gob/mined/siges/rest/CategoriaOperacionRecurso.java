...
import sv.gob.mined.siges.negocio.servicios.CategoriaOperacionBean;
...
@RequestScoped
@Path("/v1/categoriasoperacion")
@Tag(name = "CategoriaOperacion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_CATEGORIA_OPERACION,
    ConstantesOperaciones.ACTUALIZAR_CATEGORIA_OPERACION,
    ConstantesOperaciones.ELIMINAR_CATEGORIA_OPERACION})
public class CategoriaOperacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(CategoriaOperacionRecurso.class.getName());

    @Inject
    private CategoriaOperacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgCategoriaOperacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgCategoriaOperacion.class)))
        ,
    ...
    @Path("/{categoriaOperacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("categoriaOperacionId") Long id) {
        try {
            SgCategoriaOperacion entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgCategoriaOperacion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgCategoriaOperacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_CATEGORIA_OPERACION})
    public Response crear(SgCategoriaOperacion entity) {
        try {
            entity.setCopPk(null);
            entity.setCopVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_CATEGORIA_OPERACION})
    public Response actualizar(@PathParam("categoriaOperacionId") Long id, SgCategoriaOperacion entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_CATEGORIA_OPERACION})
    public Response eliminarPorId(@PathParam("categoriaOperacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{categoriaOperacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("categoriaOperacionId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialPorId(SgCategoriaOperacion.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
