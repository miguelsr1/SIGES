...
public class SgUsuario implements Serializable {
    ...
    @OneToMany(cascade = CascadeType.ALL, mappedBy = "purUsuario")
    @AtributoInicializarColeccion
    private List<SgUsuarioRol> pusPersonaUsuarioRol;
    ...
    @Column(name = "usu_enviado_pentaho")
    private Boolean usuEnviadoPentaho;
    
    @Column(name = "usu_es_admin_pentaho")
    private Boolean usuEsAdminPentaho;
    
    @Column(name = "usu_pass_expired")
    private Integer usuPassExpired; //0 = false, 1 = true
    
    @Column(name = "usu_update_pass")
    private LocalDateTime usuUpdatePass;

    public SgUsuario() {
        this.usuSuperUsuario = Boolean.FALSE;
    }
    
    @PrePersist
    @PreUpdate
    public void preSave() {
        if (usuPassExpired == null){
            usuPassExpired = 1;
        }
    }
    ...
    public List<SgUsuarioRol> getPusPersonaUsuarioRol() {
        return pusPersonaUsuarioRol;
    }

    public void setPusPersonaUsuarioRol(List<SgUsuarioRol> pusPersonaUsuarioRol) {
        this.pusPersonaUsuarioRol = pusPersonaUsuarioRol;
    }
