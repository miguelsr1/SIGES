...
import sv.gob.mined.siges.negocio.servicios.RolOperacionBean;
...
@RequestScoped
@Path("/v1/rolesoperacion")
@Tag(name = "RolOperacion API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_ROL_OPERACION,
    ConstantesOperaciones.ACTUALIZAR_ROL_OPERACION,
    ConstantesOperaciones.ELIMINAR_ROL_OPERACION})
public class RolOperacionRecurso {

    private static final Logger LOGGER = Logger.getLogger(RolOperacionRecurso.class.getName());

    @Inject
    private RolOperacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgRolOperacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description  = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgRolOperacion.class)))
        ,
    ...
    @Path("/{rolOperacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerPorId(@PathParam("rolOperacionId") Long id) {
        try {
            SgRolOperacion entity = servicio.obtenerPorId(id);
            if (entity == null) {
     ...
     * @return Response Lista de SgRolOperacion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON, 
                schema = @Schema(implementation = SgRolOperacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_ROL_OPERACION})
    public Response crear(SgRolOperacion entity) {
        try {
            entity.setRopPk(null);
            entity.setRopVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_ROL_OPERACION})
    public Response actualizar(@PathParam("rolOperacionId") Long id, SgRolOperacion entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_ROL_OPERACION})
    public Response eliminarPorId(@PathParam("rolOperacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id)) {
    ...
    @Path("/historial/{rolOperacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("rolOperacionId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialPorId(SgRolOperacion.class, id)).build();
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, ex.getMessage(), ex);
            return Response.status(HttpStatus.SC_INTERNAL_SERVER_ERROR).entity(Errores.ERROR_GENERAL).build();
        }
