...
import sv.gob.mined.siges.negocio.servicios.DepreciacionSubcuentasContablesBean;

@RequestScoped
@Path("/v1/reportesdepreciacion")
@Tag(name = "Bienes Depreciables API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.BUSCAR_DEPRECIACIONES
})
public class DepreciacionRecurso {
    private static final Logger LOGGER = Logger.getLogger(DepreciacionRecurso.class.getName());

    @Inject
    private DepreciacionSubcuentasContablesBean servicio;
     ...
     * @return Response Lista de DepreciacionContable
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = DepreciacionContable.class, type = SchemaType.ARRAY)))
        ,
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_DEPRECIACIONES})
    public Response buscar(FiltroBienesDepreciables filtro) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.obtenerCategoriasPorFiltro(filtro)).build();
        } catch (Exception ex) {
