...
@RequestScoped
@Path("/v1/depreciacion")
@Tag(name = "Depreciacion de bienes API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_DEPRECIACION_BIEN,
    ConstantesOperaciones.ACTUALIZAR_DEPRECIACION_BIEN,
    ConstantesOperaciones.ELIMINAR_DEPRECIACION_BIEN,
    ConstantesOperaciones.BUSCAR_DEPRECIACION_BIENES
})
public class ProcesoDepreciacionRecurso {
    private static final Logger LOGGER = Logger.getLogger(ProcesoDepreciacionRecurso.class.getName());

    @Inject
    private DepreciacionBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAfDepreciacion
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAfDepreciacion.class)))
        ,
    ...
    @Path("/{depreciacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLICITUDES_DESCARGO})
    public Response obtenerPorId(@PathParam("depreciacionId") Long id, @QueryParam("dataSecurity") Boolean dataSecurity) {
        try {
            SgAfDepreciacion entity = servicio.obtenerPorId(id, dataSecurity);
            if (entity == null) {
     ...
     * @return Response Lista de SgAfDepreciacion
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAfDepreciacion.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Path("/historial/{depreciacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("depreciacionId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgAfDepreciacion.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, tracer.activeSpan().context().toString(), ex);
