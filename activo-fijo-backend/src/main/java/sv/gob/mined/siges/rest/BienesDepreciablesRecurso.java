...
@RequestScoped
@Path("/v1/bienesdepreciables")
@Tag(name = "Bienes Depreciables API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_BIEN_DEPRECIABLE,
    ConstantesOperaciones.ACTUALIZAR_BIEN_DEPRECIABLE,
    ConstantesOperaciones.ELIMINAR_BIEN_DEPRECIABLE,
    ConstantesOperaciones.BUSCAR_BIENES_DEPRECIABLES
})
public class BienesDepreciablesRecurso {
    private static final Logger LOGGER = Logger.getLogger(BienesDepreciablesRecurso.class.getName());

    @Inject
    private BienesDepreciablesBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAfBienDepreciable
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAfBienDepreciable.class)))
        ,
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_BIENES_DEPRECIABLES})
    public Response obtenerPorId(@PathParam("bienId") Long id, @QueryParam("dataSecurity") Boolean dataSecurity) {
        try {
            SgAfBienDepreciable entity = servicio.obtenerPorId(id, dataSecurity);
            if (entity == null) {
     ...
     * @return Response Lista de SgAfBienDepreciable
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAfBienDepreciable.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Path("/buscar/ultimocorrelativo")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_BIENES_DEPRECIABLES})
    public Response buscarUltimoCorrelativo(FiltroBienesDepreciables filtro) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(servicio.obtenerUltimoCorrelativo(filtro,Boolean.TRUE)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, tracer.activeSpan().context().toString(), ex);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_BIEN_DEPRECIABLE})
    public Response crear(SgAfBienDepreciable entity) {
        try {
            entity.setBdePk(null);
            entity.setBdeVersion(0);
     ...
     * Crea el objeto indicado con id autogenerada
     *
     * @param entity SgAfBienDepreciableInmDTO
     * @return Response SgAfBienDepreciableInmDTO
     */
    @POST
    @Operation(summary = "Crea el objeto indicado con id autogenerada.")
    @APIResponses(value = {
        @APIResponse(responseCode = "201", description = "Entidad creada satisfactoriamente.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAfBienDepreciableInmDTO.class)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "422", description = "Entidad no se puede procesar. Datos incorrectos.")
    ...
    @RolesAllowed({ConstantesOperaciones.CREAR_BIEN_DEPRECIABLE})
    @Path("/infra")
    public Response guardarBienDepreciableDeInfraestructura(SgAfBienDepreciableInmDTO dto) {
        try {
            return Response.status(HttpStatus.SC_CREATED).entity(servicio.guardarBienDepreciableDeInfraestructura(dto)).build();
        } catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        } catch (Exception ex) {
            if (PersistenceHelper.isOptimisticException(ex)) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_BIEN_DEPRECIABLE})
    public Response actualizar(@PathParam("bienId") Long id, SgAfBienDepreciable entity) {
        try {
            if (servicio.objetoExistePorId(id, Boolean.TRUE)) {
                entity.setBdePk(id);
                return Response.status(HttpStatus.SC_OK).entity(servicio.guardar(entity, Boolean.TRUE, Boolean.FALSE)).build();
            } else {
                return Response.status(HttpStatus.SC_NOT_FOUND).build();
            }
        } catch (BusinessException be) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_BIEN_DEPRECIABLE, ConstantesOperaciones.ELIMINAR_BIEN_VALOR_ADQUISICION_MENOR_LIMITE_AF})
    public Response eliminarPorId(@PathParam("bienId") Long id) {
        try {
            if (servicio.objetoExistePorId(id, Boolean.TRUE)) {
    ...
    /**
     * Elimina una lista de objetos
     *
     * @param List<SgAfBienesDepreciables> lista
     * @return Response
     */
    @POST
    @Operation(summary = "Elimina el objeto con la id indicada.")
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_BIEN_DEPRECIABLE, ConstantesOperaciones.ELIMINAR_BIEN_VALOR_ADQUISICION_MAYOR_LIMITE_AF})
    public Response eliminarLista(List<SgAfBienDepreciable> lista) {
        try {
            servicio.eliminarLista(lista);
            return Response.status(HttpStatus.SC_OK).build();
        }  catch (BusinessException be) {
            return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(new BusinessReturnedException(be.getErrores())).build();
        }  catch (Exception ex) {
    ...
    @Path("/historial/{bienId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("bienId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgAfBienDepreciable.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, tracer.activeSpan().context().toString(), ex);
