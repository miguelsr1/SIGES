...
import sv.gob.mined.siges.negocio.servicios.NotificacionCumplimientoBean;
...
@RequestScoped
@Path("/v1/notificacionescumplimiento")
@Tag(name = "Notificaciones de cumplimiento API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_NOTIFICACION_CUMPLIMIENTO,
    ConstantesOperaciones.ACTUALIZAR_NOTIFICACION_CUMPLIMIENTO,
    ConstantesOperaciones.ELIMINAR_NOTIFICACION_CUMPLIMIENTO,
    ConstantesOperaciones.BUSCAR_NOTIFICACIONES_CUMPLIMIENTO
})
public class NotificacionCumplimientoRecurso {
    private static final Logger LOGGER = Logger.getLogger(NotificacionCumplimientoRecurso.class.getName());

    @Inject
    private NotificacionCumplimientoBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAfNotificacionCumplimiento
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAfNotificacionCumplimiento.class)))
        ,
    ...
    @Path("/{notificacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_NOTIFICACIONES_CUMPLIMIENTO})
    public Response obtenerPorId(@PathParam("notificacionId") Long id, @QueryParam("dataSecurity") Boolean dataSecurity) {
        try {
            SgAfNotificacionCumplimiento entity = servicio.obtenerPorId(id, dataSecurity);
            if (entity == null) {
     ...
     * @return Response Lista de SgAfNotificacionCumplimiento
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAfNotificacionCumplimiento.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_NOTIFICACIONES_CUMPLIMIENTO})
    public Response buscar(FiltroNotificacionCumplimiento filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_NOTIFICACIONES_CUMPLIMIENTO})
    public Response buscarTotal(FiltroNotificacionCumplimiento filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_NOTIFICACION_CUMPLIMIENTO})
    public Response crear(SgAfNotificacionCumplimiento entity) {
        try {
            entity.setNcuId(null);
            entity.setNcuVersion(0);
            ...
            if (PersistenceHelper.isOptimisticException(ex)) {
                ...
                return Response.status(HttpStatus.SC_UNPROCESSABLE_ENTITY).entity(PersistenceHelper.getCodigoInventarioViolationBusinessException(ex)).build();
            } else {
                tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
                tracer.activeSpan().log(ex.getMessage());
                LOGGER.log(Level.SEVERE, tracer.activeSpan().context().toString(), ex);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_NOTIFICACION_CUMPLIMIENTO})
    public Response actualizar(@PathParam("notificacionId") Long id, SgAfNotificacionCumplimiento entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_NOTIFICACION_CUMPLIMIENTO})
    public Response eliminarPorId(@PathParam("notificacionId") Long id) {
        try {
            if (servicio.objetoExistePorId(id, Boolean.TRUE)) {
                servicio.eliminar(id, Boolean.TRUE);
    ...
    @Path("/historial/{notificacionId:[1-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.AUTENTICADO})
    public Response obtenerHistorialPorId(@PathParam("notificacionId") Long id) {
        try {
            return Response.status(HttpStatus.SC_OK).entity(historico.obtenerHistorialRevisionesPorId(SgAfNotificacionCumplimiento.class, id)).build();
        } catch (Exception ex) {
            tracer.activeSpan().setTag(Constantes.OPENTRACING_ERROR_TAG, true);
            tracer.activeSpan().log(ex.getMessage());
            LOGGER.log(Level.SEVERE, tracer.activeSpan().context().toString(), ex);
