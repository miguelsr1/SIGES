...
import sv.gob.mined.siges.negocio.servicios.SolvenciasBean;
...
@RequestScoped
@Path("/v1/solvencias")
@Tag(name = "Solvencias de Unidades API V1")
@DeclareRoles({ConstantesOperaciones.AUTENTICADO,
    ConstantesOperaciones.CREAR_SOLVENCIA_UNIDAD,
    ConstantesOperaciones.ACTUALIZAR_SOLVENCIA_UNIDAD,
    ConstantesOperaciones.ELIMINAR_SOLVENCIA_UNIDAD,
    ConstantesOperaciones.BUSCAR_SOLVENCIAS_UNIDADES
})
public class SolvenciasRecurso {
     private static final Logger LOGGER = Logger.getLogger(SolvenciasRecurso.class.getName());

    @Inject
    private SolvenciasBean servicio;
     ...
     *
     * @param id Long
     * @return Response SgAfSolvencias
     */
    @GET
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                ...
                schema = @Schema(implementation = SgAfSolvencias.class)))
        ,
    ...
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLVENCIAS_UNIDADES})
    public Response obtenerPorId(@PathParam("solid") Long id, @QueryParam("dataSecurity") Boolean dataSecurity) {
        try {
            SgAfSolvencias entity = servicio.obtenerPorId(id, dataSecurity);
            if (entity == null) {
     ...
     * @return Response Lista de SgAfSolvencias
     */
    @POST
    ...
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "Consumo satisfactorio.", content = @Content(
                mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = SgAfSolvencias.class, type = SchemaType.ARRAY)))
        ,
        @APIResponse(responseCode = "400", description = "Solicitud con sintaxis incorrecta.")
        ,
        @APIResponse(responseCode = "500", description = "Error interno del servidor.")}
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLVENCIAS_UNIDADES})
    public Response buscar(FiltroSolvencias filtro) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.BUSCAR_SOLVENCIAS_UNIDADES})
    public Response buscarTotal(FiltroSolvencias filtro) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.CREAR_SOLVENCIA_UNIDAD})
    public Response crear(SgAfSolvencias entity) {
        try {
            entity.setSolPk(null);
            entity.setSolVersion(0);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ACTUALIZAR_SOLVENCIA_UNIDAD})
    public Response actualizar(@PathParam("solid") Long id, SgAfSolvencias entity) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ConstantesOperaciones.ELIMINAR_SOLVENCIA_UNIDAD})
    public Response eliminarPorId(@PathParam("solid") Long id) {
        try {
            if (servicio.objetoExistePorId(id, Boolean.TRUE)) {
